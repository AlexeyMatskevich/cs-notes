...
Пусть программа должна открыть `salaries.txt`, прочитать первую строку и превратить её в [число](variables-and-types.md). Здесь могут сломаться три разные вещи: файла нет, строка не прочиталась, текст оказался не числом.
...
Сильная сторона исключений в том, что обработчик не обязан стоять прямо рядом с местом сбоя. Ошибка может подняться по цепочке [вызовов функций](functions.md) — тем самым кадрам [стека](memory.md) из [заметки о памяти](memory.md):
...
Следующий вопрос уже не про сбой во время работы, а про запуск программы вообще: почему `ruby file.rb` и `gcc file.c` выглядят так по-разному? Это вопрос [компиляции и интерпретации](compilation-and-interpretation.md).
