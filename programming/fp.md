...
Обратите внимание: внутри блока используется `tax_rate`, хотя параметр у блока один — `total`. Блок запоминает переменные из окружения, где он создан, — это называется замыканием (closure). Механика в Ruby разобрана в [Блоках](../ruby/internal/blocks.md).
