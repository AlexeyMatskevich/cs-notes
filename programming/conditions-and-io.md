...
Данные пришли через ввод, условие выбрало нужную ветку, результат ушёл на вывод. Осталась другая проблема: если одинаковое действие нужно повторить десять, сто или тысячу раз, писать один и тот же `if` вручную слишком дорого. Для этого нужны [циклы](loops.md).
