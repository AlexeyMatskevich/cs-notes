...
**Дробные числа (float)** — числа с точкой: налоговая ставка, температура, координаты. Почему `0.1 + 0.2` не равно `0.3` — [числа с плавающей точкой](../foundations/03-floating-point.md).
...
**Строки (string)** — текст, заключённый в кавычки. Не число, хотя может содержать цифры.
