...
Люди решили: пусть каждая числовая инструкция получит короткое читаемое имя. `0010` стало `ADD`, `0001` стало `MOV`, переход к другой инструкции — `JMP` (от английского jump — прыжок). Набор таких имён и правил их записи называется ассемблером (assembler — буквально «сборщик»: он собирает программу из мнемоник обратно в числа, понятные процессору).
...
Одна ячейка хранит число от 0 до 255 — ровно 256 разных значений (почему именно 256 и откуда берётся +4 — [двоичная система и байты](../foundations/00-binary-and-bytes.md), [целые числа](../foundations/01-integers.md)). Четыре ячейки вместе дают 256 × 256 × 256 × 256 ≈ 4 миллиарда вариантов — этого хватает для большинства обычных чисел. Поэтому если первое число лежит по адресу 200, следующее начинается не с 201, а с 204 — через 4 ячейки.

**Регистры.** Процессор не может складывать числа прямо в памяти — он сначала копирует их к себе, во внутреннее хранилище. Это хранилище состоит из нескольких ячеек, которые называются регистрами. Регистров мало (в простых процессорах — от 4 до 16), зато доступ к ним мгновенный. Типичные имена: `EAX`, `EBX`, `ECX`, `EDX`.
...
```asm
 ...
 7    ADD EBX, 4      ; сдвинуть адрес на 4 ячейки (следующее число)
...
- знать, что одно целое число занимает 4 ячейки памяти, поэтому адрес сдвигается на 4.
...
Реальные наборы команд процессоров (ARM, x86) и почему у разных архитектур разный ассемблер — в [ISA](../computer/programmer-model/00-isa.md).
