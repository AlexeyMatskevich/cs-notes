...
`.each do |delivery| ... end` — конструкция, уже знакомая по [коллекции объектов](08-oop.md): `.each` обходит массив, вызывая блок `do ... end` для каждого элемента; `|delivery|` — имя, которое блок даёт текущему элементу. `.to_s` внутри `summary` превращает число в строку, чтобы его можно было соединить с текстом.
