...
<- [Что такое программирование](what-is-programming.md) | [Переменные и типы](variables-and-types.md) ->
...
Люди решили: пусть каждая числовая инструкция получит короткое читаемое имя. `0010` стало `ADD`, `0001` стало `MOV`, переход к другой инструкции — `JMP` (от английского jump — прыжок). Набор таких имён и правил называется языком ассемблера, а ассемблер (assembler) переводит этот текст обратно в числа для процессора. Позже этот приём — «дать имя числу» — развился в идею [переменных](variables-and-types.md): имя получает уже не машинная команда, а хранилище для данных.
...
Раньше компьютер был представлен тремя частями: процессор, память, ввод/вывод. Ближе к уровню машины у памяти и процессора обнаруживаются ещё две детали.
...
Единица такой ячейки — байт: минимальный кусок памяти, у которого есть собственный адрес; на современных процессорах он хранит 8 бит. Одно целое число обычно не помещается в один байт, поэтому занимает несколько соседних. В этой заметке для удобства примем: одно целое значение занимает 4 байта. Тогда между соседними элементами массива будет разница 4 (200 -> 204).

Почему память считают байтами и почему размер числа в других случаях может отличаться, см. [двоичная система и байты](../foundations/binary-and-bytes.md) и [целые числа](../foundations/integers.md).
...
В этих примерах уже видны три способа записать операнд. Число без скобок (`200`, `5`) — это сам литерал, значение как оно есть. Число в квадратных скобках (`[200]`) — прямой адрес: «ячейка памяти с номером 200». Регистр в квадратных скобках (`[EBX]`) — косвенный адрес: «ячейка по тому номеру, который сейчас лежит в EBX». Скобки — это и есть «посмотри в память по этому адресу».
...
> [!info]- Что будет, если заменить ADD EBX, 4 на ADD EBX, 1?
...
**Регистров мало, а задач много.** Четыре регистра при сложении пяти чисел — терпимо. Но при расчёте зарплаты (оклад, налог, бонус, стаж, надбавка) регистров не хватает, приходится постоянно сохранять значения в [память](memory.md) и загружать обратно. Программист должен помнить, что в какой ячейке лежит.
...
Люди задались вопросом: что, если машина сама будет следить за адресами, а программист будет работать с именами? Вместо `MOV EAX, [200]` написать `salary = 1500` — и пусть специальная программа сама решит, в какую ячейку положить число и какой регистр использовать. Это привело к идее [переменных](variables-and-types.md).

## Sources

Реальные наборы команд процессоров (ARM, x86) и почему у разных архитектур (наборов инструкций конкретного процессора) разный ассемблер — в [ISA](../computer/programmer-model/isa.md).
