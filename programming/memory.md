...
Ссылки и мутация объясняют, почему `copy[3] = 1800` из начала заметки затронул `orders`. Но они же создают следующий вопрос: если одни и те же данные разделяют несколько частей программы, кто следит за тем, чтобы их не сломали? Это вопрос организации кода — ответ даёт [объектно-ориентированное программирование](oop.md).
