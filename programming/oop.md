...
Класс окупается тогда, когда у сущности есть данные **и** правила работы с ними. Если данные просто передаются из [функции](functions.md) в [функцию](functions.md) без собственного поведения — хеша достаточно.
...
Класс `Seller` работает, пока все продавцы подчиняются одним правилам. Но что делать, если в системе появляются способы доставки — самовывоз, курьер, экспресс — каждый со своей ценой и сроком? Для этого нужны [наследование и полиморфизм](inheritance-and-polymorphism.md).
