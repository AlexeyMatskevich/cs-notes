...
**Предпосылки:** [оперативная память](03-ram.md) (DRAM, row buffer, bandwidth vs latency).

← [Оперативная память](03-ram.md) | [Шины и DMA](05-buses-and-dma.md) →
...
Серверный диск обычно содержит 2–5 пластин (4–10 рабочих поверхностей), вращающихся на одном шпинделе. Скорость вращения определяет класс диска: 5400 RPM для десктопов, 7200 RPM для серверов общего назначения, 10000–15000 RPM для высокопроизводительных серверных массивов. Чем быстрее вращение — тем меньше rotational latency, но больше энергопотребление, нагрев и шум. Диски 15000 RPM потребляют 15–25 Вт и требуют активного охлаждения; 5400 RPM — 3–5 Вт.
...
Вернёмся к нашему сценарию. База данных обрабатывает запрос пользователя, и планировщик решил прочитать 1000 случайных страниц по 8 КБ (index scan по некластеризованному индексу). На HDD 7200 RPM это 1000 / 100 IOPS = 10 секунд чистого ожидания I/O. Один пользовательский запрос — 10 секунд. Если в систему приходит 100 таких запросов в секунду, одного диска не хватит даже на один запрос за секунду.
...
Для SSD переупорядочивание запросов бессмысленно — нет головки и нет seek. Поэтому Linux для NVMe-дисков по умолчанию использует планировщик `none`: запросы передаются устройству напрямую, без посредника. Любые попытки «оптимизировать» порядок только добавляют задержку. Проверить текущий планировщик: `cat /sys/block/nvme0n1/queue/scheduler`. Если на NVMe-диске стоит `mq-deadline` вместо `none` — это конфигурационная ошибка, стоящая 5–10% IOPS.

## SSD: убираем механику

Твердотельный накопитель (SSD, Solid State Drive) не имеет движущихся частей. Данные хранятся в NAND flash — полупроводниковой памяти на основе транзисторов с плавающим затвором (floating gate transistors). Электроны, запертые в плавающем затворе, сохраняются без питания годами — это и обеспечивает энергонезависимость.
...
Последовательное чтение тоже радикально быстрее: SATA SSD — до 550 МБ/с (упирается в интерфейс), NVMe SSD — 3–7 ГБ/с, топовые модели с PCIe 5.0 — до 14 ГБ/с. Но главное отличие от HDD: разница между последовательным и случайным доступом на SSD — в 2–5 раз, а не в 400 раз. Случайное чтение больше не катастрофа.

Это изменило правила проектирования баз данных. На HDD планировщик PostgreSQL предпочитал sequential scan даже при чтении 10–20% таблицы, потому что случайные IOPS стоили в 400 раз дороже. На SSD index scan выгоден вплоть до 30–50% таблицы. Конфигурационный параметр `random_page_cost` в PostgreSQL по умолчанию = 4.0 (предполагает HDD: случайное чтение в 4 раза дороже последовательного с учётом OS page cache). Для SSD рекомендуется 1.1–1.5. Неправильное значение приводит к тому, что планировщик выбирает sequential scan вместо index scan — и запрос вместо 5 мс занимает 500 мс.

## NAND flash: устройство ячейки

Ячейка NAND flash — это MOSFET-транзистор с дополнительным слоем: плавающим затвором (floating gate), изолированным диэлектриком со всех сторон. Чтобы записать «0», на управляющий затвор подаётся высокое напряжение (~20 В), электроны туннелируют через тонкий слой оксида и застревают в плавающем затворе. Заряженный плавающий затвор повышает пороговое напряжение транзистора — при чтении это детектируется как «0» (или конкретный уровень заряда). Чтобы стереть — подаётся напряжение обратной полярности, электроны выталкиваются обратно.

Этот оксидный слой — ключевое место. Каждый проход электронов через него повреждает структуру оксида: электроны застревают в дефектах, смещая пороговое напряжение. После определённого числа циклов записи-стирания оксид деградирует настолько, что ячейка перестаёт надёжно удерживать заряд — пороговые напряжения «расплываются» и соседние уровни начинают перекрываться, вызывая ошибки чтения. Контроллер SSD использует коды коррекции ошибок (ECC, Error Correction Code) — для TLC это LDPC-коды (Low-Density Parity-Check), способные исправить десятки бит на страницу. По мере износа число ошибок растёт, ECC справляется всё хуже, и в какой-то момент блок помечается как неиспользуемый. Это определяет ресурс ячейки.

Типы ячеек отличаются количеством бит на транзистор:

SLC (Single-Level Cell) — 1 бит на ячейку. Два уровня заряда: «заряжен» / «не заряжен». Максимальная надёжность и скорость, минимальная плотность. Ресурс: ~100 000 циклов записи-стирания (P/E cycles). Используется в серверных SSD с экстремальными требованиями к записи.
...
Ещё одна особенность NAND — ячейки соединены последовательно в цепочки (NAND string, отсюда и название «NAND» — от логического элемента NOT-AND). Для чтения одной ячейки контроллер должен «открыть» все остальные ячейки в цепочке, пропустив через них ток, и измерить напряжение на целевой ячейке. Это проще и компактнее, чем адресовать каждую ячейку отдельно (как в NOR flash), но медленнее для произвольного доступа к отдельным байтам. NAND оптимизирован для постраничного доступа — прочитать 4 КБ целиком быстрее, чем 4 КБ по одному байту.
...
Посчитаем на конкретном примере. SSD на 1 ТБ TLC с ресурсом 3000 P/E cycles. Полная ёмкость с учётом over-provisioning — допустим, 1.1 ТБ NAND. Максимальный объём записи за жизнь: 1.1 ТБ × 3000 = 3300 ТБ. Если WAF = 3 (типичная случайная нагрузка), то полезная запись от ОС — 3300 / 3 = 1100 ТБ, или ~1100 TBW (Terabytes Written). При записи 50 ГБ в день (активный сервер) диск проживёт 1100 ТБ / 50 ГБ = ~22 000 дней ≈ 60 лет. При записи 500 ГБ/день (интенсивная база) — 6 лет. Спецификации производителей указывают TBW с учётом типичного WAF; серверные модели гарантируют 1–3 DWPD (Drive Writes Per Day — полных перезаписей в день) в течение 5 лет.

## TRIM: помощь от ОС

Когда ОС удаляет файл, она просто помечает блоки файловой системы как свободные в своих метаданных. SSD об этом не знает — с его точки зрения, по этим LBA всё ещё хранятся данные, которые нужно беречь и перекопировать при GC. Команда **TRIM** (в протоколе ATA; в NVMe — Deallocate) позволяет ОС сообщить SSD: «данные по этим LBA больше не нужны». FTL помечает соответствующие физические страницы как невалидные, и при следующем GC их не нужно копировать — блок освобождается эффективнее, write amplification снижается.
...
Первые SSD подключались через интерфейс SATA (Serial ATA), который был спроектирован для HDD. SATA — последовательный интерфейс с пропускной способностью 600 МБ/с (SATA III) и протоколом AHCI, который поддерживает одну очередь команд глубиной 32. Для HDD с его 100 IOPS 32 команды в очереди — с запасом. Для SSD, способного выполнить 500 000 операций в секунду — узкое горлышко.

**NVMe** (Non-Volatile Memory Express) — протокол, разработанный специально для flash-хранилищ. NVMe работает поверх шины PCIe напрямую, без посредников. Ключевые отличия:

Пропускная способность: PCIe 4.0 x4 — до 8 ГБ/с, PCIe 5.0 x4 — до 16 ГБ/с. Против 600 МБ/с у SATA — разница в 13–26 раз.

Очереди: NVMe поддерживает до 65 535 очередей по 65 535 команд в каждой. Это позволяет каждому ядру CPU иметь собственную submission queue и completion queue, без блокировок и конкуренции за общий ресурс. На сервере с 64 ядрами каждое ядро отправляет команды в свою очередь, и контроллер SSD обрабатывает их параллельно. SATA с одной очередью на 32 команды вынуждает ядра конкурировать за единственную точку входа — при 64 ядрах это превращается в spinlock contention, и производительность не масштабируется.

Латентность: SATA-команда проходит через контроллер AHCI, который добавляет ~6 мкс накладных расходов. NVMe обращается к контроллеру SSD через memory-mapped I/O, накладные расходы — ~2.5 мкс. При латентности самого NAND в 50–100 мкс это не критично, но при дальнейшем ускорении flash (Intel Optane c ~10 мкс латентности) протокольные накладные расходы SATA становились бы доминирующими.
...
Форм-фактор NVMe-дисков — обычно M.2 (компактная плата 22×80 мм, подключается напрямую к разъёму на материнской плате) или U.2 (2.5-дюймовый корпус для серверов, с отдельным кабелем). В серверах высокой плотности используют также формат EDSFF (E1.S, E3.S) — длинные узкие платы, оптимизированные для охлаждения в серверных шасси. Все эти форм-факторы используют один и тот же протокол NVMe поверх PCIe. Важно не путать форм-фактор с протоколом: существуют M.2-диски с SATA-интерфейсом (ограниченные 550 МБ/с) и M.2-диски с NVMe (3–7 ГБ/с). Одинаковый разъём — радикально разная производительность.
...
**Tail latency.** Средняя латентность чтения SSD — 75 мкс. Но если чтение совпало с GC, который стирает блок (2–5 мс), операция ждёт. 99.9-й перцентиль латентности может оказаться в 20–50 раз выше среднего. Для баз данных, где P99.9 латентности важнее средней — это существенно. Серверные SSD ограничивают продолжительность GC-пауз и распределяют стирание мелкими порциями. Некоторые серверные контроллеры реализуют deterministic GC: стирание выполняется только в заранее выделенные временные окна или равномерно распределяется, чтобы tail latency оставалась предсказуемой. В спецификациях серверных SSD обычно указывают не только средний IOPS, но и 99.99-й перцентиль — для Samsung PM9A3, например, это ~500 мкс при среднем ~100 мкс.
...
В продакшен-системах HDD и SSD часто работают вместе.

SSD для горячих данных: индексы базы данных, активные таблицы, WAL-журнал, метаданные файловой системы. Здесь критичен random IOPS и низкая латентность. NVMe SSD выдаёт 500 000 IOPS — достаточно для базы данных, обрабатывающей десятки тысяч транзакций в секунду.

HDD для холодных данных: архивы, бэкапы, логи, данные которые читаются последовательно и редко. Последовательная скорость HDD (150–200 МБ/с) достаточна для потокового чтения бэкапов, а стоимость за терабайт — в 5–10 раз ниже, чем у SSD. При объёмах в сотни терабайт (data lakes, видеохранилища, научные данные) разница в стоимости между SSD и HDD составляет десятки тысяч долларов — HDD остаётся единственным экономически разумным вариантом для массового хранения.

Типичная конфигурация сервера баз данных: 2–4 NVMe SSD в RAID для активной базы, массив HDD для pg_basebackup и WAL-архивов. Или в распределённых системах: горячий слой на SSD (последние 7 дней данных), тёплый/холодный слой на HDD (история за годы).

Вернёмся к исходному сценарию и посчитаем полную картину. База данных получает 10 000 запросов в секунду. Каждый запрос порождает в среднем 5 случайных чтений (index lookup + heap fetch) при условии, что большая часть данных не попала в буферный кеш. Итого 50 000 random read IOPS. На HDD 7200 RPM: 50 000 / 100 = нужно 500 дисков только для чтения. На одном NVMe SSD с 500 000 IOPS — 10% нагрузки. Разница в инфраструктуре: стойка серверов с HDD-массивами или один NVMe-диск ценой в $150.

На практике буферный кеш базы данных и page cache ОС обслуживают 90–99% чтений из RAM, и до диска доходит лишь малая доля запросов. Но даже оставшиеся 1% при 10 000 QPS — это 500 random IOPS, что на одном HDD означает задержку в секунды для части запросов. SSD обслуживает эти «промахи кеша» за микросекунды, делая хвост распределения латентности предсказуемым.
...
Вся архитектура компьютера построена вокруг этого разрыва: кеши прячут латентность DRAM, буферный кеш базы данных прячет латентность SSD, readahead ОС прячет случайные обращения за последовательным чтением. Каждый уровень — это кеш над более медленным уровнем ниже.
...
HDD превосходит SSD только в двух параметрах: стоимость за терабайт и максимальная ёмкость одного устройства. Во всём остальном SSD выигрывает от одного до четырёх порядков. HDD также более уязвим к физическим воздействиям: вибрация или удар во время работы может привести к контакту головки с поверхностью пластины (head crash), что означает необратимую потерю данных.

## Что дальше

NVMe SSD выдаёт 500 000 IOPS и 7 ГБ/с, но эти данные должны физически попасть с контроллера SSD в оперативную память. Если CPU будет копировать каждый байт сам — выполняя load и store для каждых 8 байт — передача 4 КБ страницы займёт 512 инструкций, а при 7 ГБ/с потока CPU не будет успевать делать ничего другого. При 500 000 IOPS это 256 миллионов load/store инструкций в секунду только на перекладывание данных — целое ядро занято копированием. Нужен механизм, который позволит устройствам писать в RAM напрямую, без участия процессора.
