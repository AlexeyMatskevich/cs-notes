...
**Exclusive (E)** — линия присутствует только в этом кеше и совпадает с RAM. Ни одно другое ядро её не кеширует. Отличие от Modified: данные чистые, при вытеснении записывать не нужно. Ключевое свойство: переход E -> M происходит мгновенно — ядро уже единственный владелец, оповещать другие ядра не нужно.
...
Обратите внимание: RAM не обновлена. В состоянии Modified свежие данные существуют только в L1 ядра 0. RAM будет обновлена позже — при вытеснении линии из кеша ([write-back](./00-memory-hierarchy.md#запись-в-кеш-write-back-и-write-allocate)) или когда другое ядро запросит эту линию (как увидим в следующем шаге).

### Шаг 4: Core 1 читает counter

Core 1 выполняет `load [0x7f00]`. Его копия — Invalid. Core 1 посылает bus read. Core 0 видит запрос через snooping и обнаруживает, что линия в состоянии Modified — значит, в RAM устаревшие данные, и нужно отдать свою копию. Core 0 отвечает данными напрямую — данные идут из кеша одного ядра в кеш другого, минуя RAM (cache-to-cache transfer). Обе копии переходят в **Shared**. RAM тоже обновляется.
...
Важный момент в этой последовательности: интерконнект нужен не для каждого чтения, а только в точках смены владения или когда другой кеш уже держит более свежую копию, чем RAM. Пока линия локальна и не оспаривается, чтения и записи идут по цене обычного L1 hit.
...
```text
  ...
  E -> M :  store (без когерентного запроса — уже единственный владелец)
  ...
  M -> I :  другое ядро хочет писать (cache-to-cache transfer + invalidate)
```

## Сериализация когерентных операций

Два ядра не могут одновременно послать invalidate по одному адресу — интерконнект сериализует все когерентные транзакции. В каждый момент времени только одно ядро может выиграть арбитраж и отправить своё сообщение. Остальные ждут.

На практике топология соединений между ядрами зависит от процессора, но принцип тот же: протокол обеспечивает глобальный порядок когерентных транзакций. Два ядра не могут одновременно получить состояние Modified для одной линии — протокол физически это запрещает.
...
С ростом числа ядер ситуация ухудшается: каждый invalidate должен быть подтверждён всеми ядрами, а арбитраж интерконнекта становится узким местом.

## Устранение false sharing: выравнивание по кеш-линии

Решение — разнести переменные в разные кеш-линии. Если каждая переменная начинается с границы 64 байт, она гарантированно не делит линию ни с чем. Механизм [выравнивания](../programmer-model/01-abi-and-data-layout.md) (alignment) уже знаком из ABI — здесь тот же принцип, но цель не корректность доступа, а производительность.
...
<details>
<summary>Как обнаружить false sharing: perf c2c и HITM</summary>
