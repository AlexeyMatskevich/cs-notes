...
Когда двум адресам приходится делить одну позицию, один вытесняет другой — это [конфликтный промах](#три-типа-промахов). Сколько линий помещается в одном наборе — определяет, насколько остра эта проблема. Это число называется **ассоциативностью** (associativity, буквально — степень группировки).
...
**Второй механизм — `B` не доживает до следующего внешнего обхода.** Наивный цикл устроен так: для `C[0][0]` внутренний цикл прочитал `B[0..1023][0]`, для `C[0][1]` — `B[0..1023][1]`, ..., для `C[0][1023]` — `B[0..1023][1023]`. За один проход внешнего цикла по `j` через кеш прошла вся матрица `B` — 8 МБ. Теперь внешний цикл по `i` переходит к `C[1][0]`, и снова нужен `B[0..1023][0]`. Чтобы эти линии дожили, L3 должен всё это время удерживать всю `B` (8 МБ) плюс активную строку `A` плюс растущий кусок `C`. На десктопном L3 в 25 МБ одна `B` уже занимает треть, и с учётом кода, других потоков и других ядер она не гарантированно выживает между двумя проходами. Когда не выживает — повторное обращение оказывается L3-промахом, идёт в RAM и стоит в десятки раз дороже L1-попадания. В терминах [трёх типов промахов](#три-типа-промахов) это capacity miss, и на 1024 × 1024 он становится доминантой. На серверных процессорах с огромным L3 (например, 384 МБ у EPYC 9654) сценарий выживает: `B` стоит в L3 с запасом, и кросс-итерационный реюз работает штатно.
