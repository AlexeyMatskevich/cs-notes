...
Что это значит на практике? Возьмём перемножение двух матриц с записью результата в третью, все элементы — `double` (8 байт). При размере 256 × 256 каждая матрица занимает 512 КБ, три вместе — 1.5 МБ. Всё это целиком помещается в кеш процессора, IPC держится на уровне 2.5, конвейер загружен. При размере 1024 × 1024 каждая матрица уже 8 МБ, три — 24 МБ, и в кеш (у типичного процессора — единицы-десятки мегабайт) они не помещаются. IPC проседает до 0.3–0.5: процессор тот же, инструкции те же, но большую часть времени он ждёт данные из RAM.
...
Механизм работает, потому что промахи редки. У типичного кода hit rate (доля попаданий) L1 держится на уровне 95–97%, у L2 — 80–95%. Даже если L3 перехватывает всего 70–80% оставшихся запросов, до RAM доходит 1–5% обращений. Средняя задержка для кода с хорошей локальностью — порядка 2–4 нс вместо 60–80 нс.
...
Диагностика: счётчики производительности процессора (hardware performance counters) через [`perf stat`](../../linux/infrastructure/tracing.md) в Linux показывают количество L1/L2/L3 промахов. Команда `perf stat -e L1-dcache-load-misses,LLC-load-misses ./program` выдаёт абсолютные числа промахов по уровням. Для более детального анализа `perf record -e mem_load_retired.l3_miss` записывает адреса промахов — можно найти конкретные строки кода, вызывающие промахи L3.
...
Кеш работает по тому же принципу, что и [хеш-таблица с цепочками](../../algorithms-and-data-structures/linear/hash-table.md) (chaining): хеш от ключа выбирает бакет, в бакете лежит несколько пар «ключ-значение», при поиске сравниваем ключ с каждой парой. В кеше роль ключа играет физический адрес, бакет — это набор линий (set), а сравнение ключа — сверка тегов (tag).
...
Смещение: log2(64) = 6 бит (2⁶ = 64, поэтому 6 бит достаточно для адресации 64 позиций) (позиция внутри линии). Индекс: log2(64 набора) = 6 бит. Тег: оставшиеся 64 - 6 - 6 = 52 бита (на практике физический адрес может быть 48 бит, тогда тег = 36 бит).
...
При шаге больше 4096 байт каждое обращение попадает на новую страницу, промахи TLB накладываются на промахи кеша, и пропускная способность падает ещё в 2–3 раза.
...
Принцип «расстояние = время» работает и внутри самой RAM. В серверах с несколькими процессорными сокетами каждый сокет имеет свой контроллер памяти, и обращение к «своей» памяти в 1.5–2 раза дешевле, чем к памяти соседнего сокета. Эта архитектура — NUMA (Non-Uniform Memory Access) — подробно разобрана в [оперативной памяти](ram.md).
