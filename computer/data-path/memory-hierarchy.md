...
**Предпосылки:** [процессор](../cpu.md) (конвейер, суперскалярность, внеочерёдное исполнение, IPC, ядро); [хеш-таблица](../../algorithms-and-data-structures/linear/hash-table.md) (хеш-функция, коллизия, бакет).
...
Что это значит на практике? Возьмём перемножение двух матриц — обозначим их `A` и `B` — с записью результата в третью, `C`. Все элементы — `double` (8 байт). Перемножение выполняется наивным тройным циклом по индексам `i`, `j`, `k`, внутренний цикл накапливает `C[i][j] += A[i][k] * B[k][j]`. При размере 256 × 256 каждая матрица занимает 512 КБ, три вместе — 1.5 МБ. Всё это целиком помещается в кеш процессора, IPC держится на уровне 2.5, конвейер загружен. При размере 1024 × 1024 каждая матрица уже 8 МБ — и в типичный десктопный кеш три такие матрицы вместе не помещаются. IPC проседает до 0.3–0.5: процессор тот же, инструкции те же, но большую часть времени он ждёт данные из RAM.
...
Числа зависят от конкретного процессора, но порядок величин устойчив уже двадцать лет. L1 на практике разделён на кеш данных (L1d) и кеш инструкций (L1i) — почему именно так, разберём ниже; пока достаточно знать, что размеры в спецификациях указывают L1d. У Intel Core i7-12700 (2022): L1d = 48 КБ на ядро, L2 = 1.25 МБ на ядро, L3 = 25 МБ общий. У Apple M2 (2022): L1d = 128 КБ на ядро, L2 = 16 МБ общий. У AMD EPYC 9654 (2022): L3 = 384 МБ (распределённый по чиплетам).
...
Механизм работает, потому что промахи редки. У типичного кода hit rate (доля попаданий) L1 держится на уровне 95–97%, у L2 — 80–95%. Даже если L3 перехватывает всего 70–80% оставшихся запросов, до RAM доходит 1–5% обращений. Средняя задержка для кода, где обращения концентрируются вокруг недавних и соседних адресов, — порядка 2–4 нс вместо 60–80 нс. Почему обращения так концентрируются, разберём в следующей секции.

## Локальность данных: почему кеш вообще помогает

Кеш полезен, только если программа обращается к одним и тем же данным повторно или к данным рядом друг с другом. Это свойство называют **локальностью данных**, точнее локальностью обращений к данным (locality of reference). Два её вида работают в паре.
...
Практический смысл один: важна не только асимптотика, но и layout данных. Если один доступ приносит сразу полезный кусок соседних данных, а следующий шаг переиспользует уже загруженную линию, кеш работает на тебя. Если каждый шаг прыгает в новое место памяти, процессор быстро упирается в латентность RAM.
...
Слишком большая линия (скажем, 256 байт) загружала бы много данных за раз, но у этого три проблемы. Первая — время загрузки: при пропускной способности соединения между L1 и L2 порядка 64 байт за такт, загрузка 256 байт занимает 4 такта вместо 1. Вторая — впустую потраченный объём: если программа обращается к одному `int32` и больше не трогает окрестность — остальные 252 байта загружены зря, а из кеша вытеснена потенциально полезная линия. Третья — при записи в любой байт линии нужно инвалидировать (обесценить) всю линию на других ядрах — чем больше линия, тем чаще возникает ложный конфликт ([[computer/data-path/cache-coherency#Устранение false sharing: выравнивание по кеш-линии|false sharing]]), об этом подробнее в [[computer/data-path/cache-coherency|когерентности кешей]].

64 байта — точка баланса, найденная эмпирически и устоявшаяся в индустрии с середины 2000-х: хватает для 16 элементов `int32` или 8 элементов `double`, соединение не перегружается, [[computer/data-path/cache-coherency#Устранение false sharing: выравнивание по кеш-линии|false sharing]] терпимый.
...
Обязательный промах, cold miss — первое обращение к линии. Данные никогда не были в кеше, промах неизбежен. Единственный способ уменьшить — prefetch: загрузить данные заранее, до первого обращения.

### Capacity miss

Промах ёмкости — рабочий набор данных (working set) не помещается в кеш. Линии вытесняются не из-за конфликтов, а потому что кеш физически меньше, чем данные. Обход массива 40 МБ при L3 = 8 МБ неизбежно вытесняет ранее загруженные линии. Способ борьбы — уменьшить рабочий набор: компактные структуры данных, блочные алгоритмы (tiling), отложенная обработка. Перемножение матриц 1024 × 1024 — канонический пример доминирования capacity miss; подробный разбор — в [[#блочный-обход-данных|блочном обходе данных]].
...
Диагностика: счётчики производительности процессора (hardware performance counters) через [[linux/infrastructure/tracing#perf-stat-аппаратные-счётчики-производительности|`perf stat`]] в Linux показывают количество L1/L2/L3 промахов. Команда `perf stat -e L1-dcache-load-misses,LLC-load-misses ./program` выдаёт абсолютные числа промахов по уровням. Для более детального анализа `perf record -e mem_load_retired.l3_miss` записывает адреса промахов — можно найти конкретные строки кода, вызывающие промахи L3.
...
Кеш работает по тому же принципу, что и [[algorithms-and-data-structures/linear/hash-table#стратегия-1-chaining-цепочки|хеш-таблица с цепочками]] (chaining): хеш от ключа выбирает бакет, в бакете лежит несколько пар «ключ-значение», при поиске сравниваем ключ с каждой парой. В кеше роль ключа играет физический адрес, бакет — это набор линий (set), а сравнение ключа — сверка тегов (tag).
...
Когда двум адресам приходится делить одну позицию, один вытесняет другой — это [[#conflict-miss|конфликтный промах]]. Сколько линий помещается в одном наборе — определяет, насколько остра эта проблема. Это число называется **ассоциативностью** (associativity, буквально — степень группировки).

В терминах хеш-таблицы: ассоциативность — это сколько линий умещается в одном бакете. Больше линий в бакете — меньше коллизий, но больше параллельных сравнений. Три типа ниже — три точки на этом спектре.

**Кеш прямого отображения** (direct-mapped, ассоциативность 1). В бакете ровно одна позиция, и любая коллизия — это немедленное вытеснение: два адреса с одинаковым индексом не могут сосуществовать. Зато поиск предельно простой — сверяется один тег за такт. Расплата за простоту — катастрофа на неудачных паттернах доступа: если цикл чередует обращения к двум адресам с одинаковым индексом, hit rate падает до 0%, каждое обращение выбрасывает то, что понадобится следующему.

Классический пример: два массива по 32 КБ, выровненные по 32 КБ — в direct-mapped кеше на 32 КБ элементы `A[i]` и `B[i]` всегда попадают в один набор и вытесняют друг друга.

**Полностью ассоциативный кеш** (fully associative). Другая крайность: один бакет на весь кеш, индексации нет — любая линия может занять любую позицию. Структурных коллизий не бывает вовсе, два адреса никогда не мешают друг другу. Но цена — аппаратная: тег запроса сравнивается со всеми тегами одновременно. При 512 линиях это 512 параллельных сравнений за такт — железо такого объёма не масштабируется, поэтому fully associative остаётся выбором только для маленьких специализированных структур на 64–128 записей.

**Множественно-ассоциативный кеш** (set-associative). Компромисс между двумя крайностями: бакет фиксированного размера N — 4, 8, 16 позиций. Биты адреса, как и в direct-mapped, детерминированно указывают на один конкретный набор, но внутри набора линия свободно занимает любую из N позиций — как в крошечном fully associative. Поиск проверяет N тегов параллельно — посильная нагрузка для железа. Коллизия превращается в конфликтный промах только когда в один набор одновременно претендует N+1 разных адресов, а это на порядок реже, чем у direct-mapped. У современных L1 кешей ассоциативность обычно 8-way, у Intel P-ядер начиная с Sunny Cove (Ice Lake, 2019) — 12-way.
...
## Запись в кеш: write-through/write-back и write-allocate

До сих пор речь шла о чтении. Но при матричном умножении процессор ещё и пишет результат на каждой итерации. У записи есть два независимых вопроса. Первый: когда изменённые данные должны уйти в нижний уровень? Это выбор между **write-through** и **write-back**. Второй: что делать при промахе на запись — загружать ли линию в кеш или записать мимо него? Это выбор между **write-allocate** и **no-write-allocate**.
...
При промахе на запись кеш обычно применяет **write-allocate** (запись с размещением): сначала загружает в кеш всю 64-байтную линию, содержащую нужный адрес, а затем изменяет в ней нужные байты. Причина в том, что кеш хранит и вытесняет данные линиями целиком, тогда как обычная store-инструкция меняет лишь часть линии, например 4–8 байт. Если создать линию в кеше без чтения старого содержимого, незатронутые байты останутся неизвестными: повторное чтение из этой линии вернёт мусор, а при **write-back** этот мусор может ещё и уйти в нижний уровень. Альтернатива — **no-write-allocate** (запись мимо кеша): запись идёт напрямую в следующий уровень, без загрузки в кеш. No-write-allocate используется для потоковых записей (streaming stores), когда данные точно не будут перечитаны — например, при заполнении большого буфера, который сразу отправляется по сети. На x86-64 для этого есть инструкции `movntps`/`movntdq` (non-temporal store — запись без временно́й локальности: процессор знает, что данные не будут перечитаны, и не загружает линию в кеш).

Для L1d стандартна комбинация **write-back + write-allocate**. У неё есть неочевидное следствие: обнуление большого массива через `memset` (функция стандартной библиотеки C, заполняющая блок памяти заданным байтом: `memset(arr, 0, size)`) сначала вынужденно читает старую линию, а уже потом пишет нули. Если массив не помещается в кеш, это превращается в чтение из RAM для каждой линии, хотя программа всё равно собирается перезаписать её целиком. Для массивов, не помещающихся в кеш, `memset` со streaming stores (через `movntps`) может быть в 2 раза быстрее обычного, потому что не тратит bandwidth на чтение старых данных. [Glibc](../../linux/infrastructure/elf-and-linking.md) (стандартная библиотека C в Linux) на x86-64 автоматически переключается на non-temporal stores для `memset` при размере, превышающем определённую долю L3 (порог зависит от версии glibc и микроархитектуры).

## Store buffer: как запись не останавливает конвейер

Запись в кеш устроена сложнее, чем чтение. При чтении процессор может выдать адрес и ждать данных (или, чаще, выполнять другие инструкции из-за [[computer/cpu#внеочерёдное-исполнение|out-of-order execution]]). В типичном для L1d случае **write-back + write-allocate** нужно не только поместить данные в линию и пометить её dirty, но и при промахе сначала загрузить всю линию. Если каждая запись ждала бы завершения этих операций, конвейер простаивал бы на каждом `mov [addr], rax`.

Задержку скрывает [[computer/cpu#Store buffer|store buffer]] — тот же буфер записей, который уже появлялся в заметке о процессоре. Там он был нужен, чтобы store не стала необратимой до коммита. Здесь важен другой эффект того же механизма: буфер позволяет конвейеру продолжать выполнение, пока запись в фоне добирается до L1d и, при промахе, ждёт write-allocate. Инструкция записи помещает в буфер адрес и значение, после чего конвейер идёт дальше. Буфер «сливает» (drain) записи в L1d по мере освобождения портов кеша. Если следующая инструкция читает только что записанный адрес, данные берутся напрямую из store buffer ([[computer/cpu#Store-to-load forwarding|store-to-load forwarding]] — пересылка из записи в чтение), без обращения к кешу.
...
Если код упирается в bandwidth (а не в latency), увеличение кеша не поможет — данные просто не успевают приходить. Типичный пример: суммирование большого массива `double` (8 байт), не помещающегося в кеш — каждый элемент читается один раз, все данные идут из RAM. Одно ядро при одном сложении за такт потребляет 3 ГГц × 8 байт = 24 ГБ/с — RAM (40 ГБ/с) справляется. Но четыре ядра с той же задачей потребляют 4 × 24 = 96 ГБ/с, а RAM по-прежнему выдаёт 40. Ядра простаивают, ожидая данных. Операция ограничена памятью (memory-bound): добавление пятого ядра не ускорит её, потому что узкое место — доставка данных, а не вычисления. Для диагностики: на Intel можно использовать счётчик [[linux/infrastructure/tracing#perf-stat-аппаратные-счётчики-производительности|`perf stat`]] `-e offcore_response.demand_data_rd.l3_miss.any_response`, показывающий количество запросов, дошедших до RAM (имена счётчиков зависят от микроархитектуры).

## Блочный обход данных

Латентность и пропускная способность — два разных способа «упереться в память». Оба эффекта видны на наивном перемножении матриц: при 256 × 256 IPC держится на пике ~2.5, при 1024 × 1024 — проседает до 0.3–0.5. Разберём, что происходит в каждом случае и как это чинится.

У 256 × 256 горячий набор внутреннего цикла компактный и полностью умещается в L1d. Строка `A` — 256 элементов `double` подряд, это 32 кеш-линии = 2 КБ. Столбец `B` читается с шагом 256 × 8 = 2048 байт, и каждый элемент сидит в своей собственной кеш-линии: 256 линий × 64 байта = 16 КБ, которые L1 вынужден удерживать одновременно (данных в столбце всего 2 КБ, но кеш живёт линиями, а не байтами). Итого ~18 КБ горячих данных — помещается в L1d 32 КБ с запасом. На уровне всей задачи три матрицы по 512 КБ = 1.5 МБ целиком живут в L3 (25 МБ на i7-12700); первый раз данные подтягиваются из RAM в кеш ([[#compulsory-miss|compulsory misses]]), дальше работа идёт целиком внутри кеша. Отсюда и высокий IPC: конвейер кормится из L1, изредка из L2/L3, к RAM почти не дёргается.

При 1024 × 1024 каждая матрица — уже 8 МБ, и наивный алгоритм ломается сразу двумя способами, которые работают в паре.

**Первый механизм — L1 уже не удерживает столбец `B`.** Внутренний цикл по `k` читает 1024 элемента `B[k][j]` с шагом 1024 × 8 = 8192 байта. Каждый элемент сидит в своей кеш-линии, итого 1024 линии × 64 байта = 64 КБ, которые L1 должен удерживать одновременно. L1d — 32 КБ. То есть в L1 не помещается даже один столбец внутреннего цикла, не то что столбец плюс активная строка `A` (ещё 8 КБ = 128 линий). В 256 × 256 этот же показатель был 16 КБ — половина L1 — и столбец жил там целиком; в 1024 × 1024 он выбивает L1 вдвое, и данные спускаются в L2 ещё по ходу самого прохода. Hardware prefetch тоже не спасает: шаг 8192 Б в сотни раз больше длины линии, stride-prefetcher такой паттерн как полезный не распознаёт.

**Второй механизм — `B` не доживает до следующего внешнего обхода.** Наивный цикл устроен так: для `C[0][0]` внутренний цикл прочитал `B[0..1023][0]`, для `C[0][1]` — `B[0..1023][1]`, ..., для `C[0][1023]` — `B[0..1023][1023]`. За один проход внешнего цикла по `j` через кеш прошла вся матрица `B` — 8 МБ. Теперь внешний цикл по `i` переходит к `C[1][0]`, и снова нужен `B[0..1023][0]`. Чтобы эти линии дожили, L3 должен всё это время удерживать всю `B` (8 МБ) плюс активную строку `A` плюс растущий кусок `C`. На десктопном L3 в 25 МБ одна `B` уже занимает треть, и с учётом кода, других потоков и других ядер она не гарантированно выживает между двумя проходами. Когда не выживает — повторное обращение оказывается L3-промахом, идёт в RAM и стоит в десятки раз дороже L1-попадания. Это [[#capacity-miss|capacity miss]], и на 1024 × 1024 он становится доминантой. На серверных процессорах с огромным L3 (например, 384 МБ у EPYC 9654) сценарий выживает: `B` стоит в L3 с запасом, и кросс-итерационный реюз работает штатно.

Сложение двух механизмов — L1 не удерживает столбец `B` внутри прохода, L3 не удерживает всю `B` между проходами — и есть причина того разрыва IPC от 2.5 до 0.3–0.5 при одном и том же коде и одной и той же микроархитектуре.
...
Внутри блока обход столбца `B` укладывается в 32 элемента — 32 кеш-линии × 64 байта = 2 КБ, помещающиеся в L1 вместе с блоками `A` и `C`. Первый механизм снят. Блок `B` переиспользуется подряд всеми итерациями одного блока `C` и отпускается, как только этот `C`-блок посчитан; удерживать всю матрицу `B` в L3 между внешними итерациями больше не нужно. Второй механизм снят. Prefetch восстанавливается на внутри-блочных обходах. Разница между наивной и блочной реализацией для 1024 × 1024 — 5–10 раз, IPC возвращается к 2–3.
...
Принцип «расстояние = время» работает и внутри самой RAM. В серверах с несколькими процессорными сокетами каждый сокет имеет свой контроллер памяти, и обращение к «своей» памяти в 1.5–2 раза дешевле, чем к памяти соседнего сокета. Эта архитектура — NUMA (Non-Uniform Memory Access) — подробно разобрана в [[computer/data-path/ram#numa-когда-у-каждого-процессора-своя-ram|оперативной памяти]].
