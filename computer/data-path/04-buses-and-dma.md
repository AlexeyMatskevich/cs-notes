...
Простейший — и исторически первый — способ передать данные от устройства в память: **programmed I/O (программный ввод-вывод, PIO)**. Процессор выполняет инструкции порта ввода-вывода — `inb` (один байт), `insw` (два), `insd` (четыре) — и копирует данные в память порция за порцией. Каждая порция — отдельная инструкция, отдельное обращение к шине.

Чтение блока 4 КБ через PIO: 4096 инструкций `inb`, каждая занимает порядка 100 нс — итого ~400 мкс только на копирование. Для сравнения, задержка чтения страницы NAND flash внутри SSD — порядка 75 мкс. При использовании PIO процессор тратит в 5 раз больше времени на перетаскивание данных, чем устройство — на их подготовку. Суммарная задержка — ~475 мкс, из которых более 80% — бессмысленная работа процессора.
...
С `insd` (4 байта за инструкцию) блок 4 КБ занимает 1024 инструкции вместо 4096. Но каждая по-прежнему стоит ~100 нс, итого ~100 мкс. Выигрыш в 4 раза не меняет картину принципиально: процессор по-прежнему занят копированием на всё время передачи.
...
Новые ARM-процессоры серверного класса (AWS Graviton3, Ampere Altra) решают проблему когерентности аппаратно — через когерентный интерконнект, в котором устройства ввода-вывода подключены к когерентному домену наравне с ядрами. DMA-транзакции проходят через тот же snoop-протокол, что и обращения между ядрами — аналогично x86. SMMU (System MMU) — ARM-аналог IOMMU — решает отдельную задачу: трансляцию адресов и изоляцию устройств (подробнее — в следующем разделе). Но старые и встроенные ARM-чипы по-прежнему требуют ручного управления кешем.
...
---------------------------------------------------
4 КБ чтение с NVMe SSD     ~475 мкс          ~103 мкс
