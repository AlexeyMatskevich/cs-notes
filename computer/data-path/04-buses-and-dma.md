...
В ядре Linux эта гибридная схема реализована с версии 4.18 (2018) — `io_poll` в блочном слое. Драйвер NVMe регистрирует polling-очереди наряду с interrupt-очередями. При подаче I/O-запроса с флагом `RWF_HIPRI` (high priority — высокий приоритет) ядро помещает его в polling-очередь и вместо ожидания прерывания опрашивает очередь завершений контроллера. Для приложений с требованиями к задержке (базы данных, key-value хранилища) это снижает P99 latency (задержку, которую превышают лишь 1% запросов) с ~20 мкс до ~10 мкс за счёт выделения ядра процессора под polling.
...
Устройство обращается не к физическому адресу напрямую, а к виртуальному адресу устройства — DVA (Device Virtual Address). IOMMU транслирует DVA в физический адрес, проверяя права доступа:
...
IOMMU добавляет небольшую задержку к DMA-транзакции: трансляция адреса занимает ~100–200 нс (с учётом кеширования в IOTLB (I/O Translation Lookaside Buffer) — аналог TLB для устройств). На фоне типичной DMA-передачи (~100 мкс для NVMe) это незаметно.
...
- Jonathan Corbet, 2016, *Block-layer I/O polling* — LWN.net (io_poll, RWF_HIPRI, blk-mq polling)
- NVIDIA, 2024, *GPUDirect Storage Design Guide* — docs.nvidia.com/gpudirect-storage/design-guide/ (peer-to-peer DMA SSD→GPU)
- Arm, 2021, *Arm CoreLink CMN-600 Coherent Mesh Network Technical Reference Manual* — (когерентный интерконнект для серверных ARM)
