...
Есть три способа переместить 4 КБ с SSD в RAM: процессор скопирует его вручную, побайтно — это займёт 400 мкс и процессор будет заблокирован. Или устройство само напишет в память, не занимая процессор — это займёт полмикросекунды. Или использовать общую шину с контролем доступа — определяет скорость и количество устройств одновременно. Каждый подход решает свой компромисс между простотой, скоростью и ресурсами.
...
С точки зрения процессора MMIO выглядит как обычные инструкции `mov` по определённым адресам. Но процессор по диапазону адреса определяет, куда направить обращение: доступы к RAM идут в сторону DRAM через контроллер памяти (компонент внутри процессора, отвечающий за чтение и запись в DRAM; подробнее — [в секции ниже](#контроллер-памяти-ram--не-pcie)), а MMIO-диапазоны уходят через PCIe Root Complex (буквально «корневой узел» — PCIe-контроллер внутри процессора; подробнее — [в разделе Топология](#топология-как-всё-соединено) ниже). Обращение к MMIO стоит дороже, чем к RAM: ~100–500 нс вместо ~70 нс, потому что проходит через PCIe.
...
Шаг 1: процессор записывает в управляющие регистры контроллера NVMe описание запроса — физический адрес буфера в RAM, куда положить данные, размер блока и номер логического блока (LBA) на диске. Адрес должен быть физическим, а не виртуальным: устройство не знает о [виртуальной памяти](../../linux/foundations/05-virtual-memory.md) процесса и работает напрямую с физическими адресами (если нет [IOMMU](#iommu-защита-dma-от-устройств) — о нём ниже). Настройка занимает несколько обращений к MMIO (memory-mapped I/O) — порядка 0.5–1 мкс.
...
Для понимания прерываний достаточно различать два режима работы процессора. В [пользовательском режиме](../../linux/foundations/01-cpu-modes-and-syscalls.md) работают обычные программы — им запрещено обращаться к устройствам напрямую. В [режиме ядра](../../linux/foundations/01-cpu-modes-and-syscalls.md) (kernel mode) работает операционная система с полным доступом к оборудованию. При прерывании процессор переключается из пользовательского режима в режим ядра и сохраняет состояние на стек — область памяти, куда процессор складывает точку возврата, чтобы потом продолжить прерванную работу. У ядра ОС для этого отдельный стек, не связанный со стеком программы.
...
4. Процессор переключается в режим ядра и переходит по адресу из таблицы прерываний (**IDT — Interrupt Descriptor Table, таблица дескрипторов прерываний**) на соответствующий обработчик. IDT — массив из 256 записей, каждая указывает на функцию-обработчик для своего номера прерывания
5. Обработчик делает нужную работу: помечает I/O-запрос как завершённый, будит [процесс](../../linux/foundations/02-processes.md), ждущий данных, или ставит задачу в очередь отложенной обработки
...
Шины, DMA-контроллеры и прерывания — аппаратный фундамент, на котором строится операционная система. Ядро ОС управляет этими механизмами: настраивает DMA-буферы, регистрирует [обработчики прерываний](../../linux/kernel/01-interrupts.md), распределяет линии PCIe. Без понимания аппаратного уровня невозможно понять, почему ядро устроено определённым образом: зачем нужен [режим ядра](../../linux/foundations/01-cpu-modes-and-syscalls.md), как работают [системные вызовы](../../linux/kernel/00-syscall-internals.md), почему [драйверы устройств](../../linux/kernel/02-devices-and-drivers.md) — самая сложная часть ОС.
