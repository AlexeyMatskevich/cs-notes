...
Но для атомарных инструкций [[computer/cpu#store-buffer|store buffer]] не помогает — процессор обязан дождаться завершения когерентной транзакции, чтобы гарантировать неделимость. Поэтому атомарные операции над общей ячейкой, за которую одновременно борются несколько ядер, особенно хорошо проявляют цену когерентности; подробно этот случай разобран в [атомарных инструкциях](../atomic-instructions.md).
