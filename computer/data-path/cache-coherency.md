...
Когерентность не определяет порядок между записями в **разные** адреса — это задача [модели памяти](../../linux/concurrency/memory-ordering.md).
...
Если бы ядро останавливалось на каждой записи и ждало подтверждения invalidate, производительность была бы ещё хуже. Для обычных (не атомарных) записей процессор использует **store buffer** (буфер записи, рассмотренный в [иерархии памяти](memory-hierarchy.md), секция «Store buffer») — здесь он выполняет аналогичную роль, но скрывает задержку когерентного протокола, а не записи в кеш. Ядро помещает запись в буфер (порядка 56 записей на Intel Skylake), не дожидаясь когерентного протокола.
...
- [Когерентность прикладных кешей](../../system-design/caching.md) — аналогичная проблема согласованности на уровне архитектуры: как поддерживать консистентность между локальным кешем приложения и источником данных
