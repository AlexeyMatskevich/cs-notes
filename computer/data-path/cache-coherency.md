...
**Предпосылки:** [иерархия памяти и кеш](memory-hierarchy.md) (кеш-линия, L1/L2 на ядро, L3 общий, hit/miss).

← [Устройство кеша](cache-internals.md) | [Оперативная память](ram.md) →
...
Core 1 выполняет `load [0x7f00]`. Адреса в его кеше нет. Core 1 посылает bus read. Core 0 видит этот запрос на интерконнекте (механизм **snooping**, буквально «подслушивание»: каждое ядро отслеживает адреса всех транзакций на интерконнекте и сверяет их со своими кеш-линиями — если адрес совпадает, ядро реагирует по правилам MESI). Core 0 обнаруживает, что у него есть эта линия в состоянии E. Оба ядра переводят свои копии в **Shared**.
...
Обратите внимание: RAM не обновлена. В состоянии Modified свежие данные существуют только в L1 ядра 0 — RAM содержит устаревшую копию, пока ядро не отдаст линию ([[computer/data-path/cache-internals#Политики записи: write-through/write-back и write-allocate|write-back]]).
...
Переход `M -> S` в диаграмме выше включает [[#Cache-to-cache transfer|cache-to-cache transfer]] и [[computer/data-path/cache-internals#Политики записи: write-through/write-back и write-allocate|write-back]].

## Когерентность — не атомарность

Когерентность гарантирует видимость каждой отдельной записи. Но операция `counter++` — это не одна запись, а три шага: load (прочитать текущее значение), add (прибавить 1), store (записать результат). [Атомарная операция](../atomic-instructions.md) (atomic operation) — операция, которую процессор выполняет целиком, не позволяя другому ядру вмешаться между её шагами. Когерентность не даёт атомарности: она гарантирует, что каждый отдельный store станет виден, но не запрещает другому ядру вклиниться между load и store.
...
Запись в линию Shared из таблицы выше — это тот же обмен, что в [[#Шаг 3: Core 0 записывает counter = 1 (invalidate и подтверждение инвалидации)|шаге 3]]: ядро посылает invalidate, ждёт подтверждения инвалидации от всех кешей, где есть копия, и только потом получает линию в Modified.

Эти десятки наносекунд — цена передачи владения кеш-линией между ядрами одного **сокета** (физического процессора с группой ядер и общим L3). Общее правило: как только операция требует [[#Шаг 3: Core 0 записывает counter = 1 (invalidate и подтверждение инвалидации)|invalidate и подтверждения инвалидации]], локальный доступ уровня L1 превращается в когерентную транзакцию на десятки наносекунд. Подробный пример с атомарным инкрементом общего счётчика — в [[computer/atomic-instructions|атомарных инструкциях]].

> [!info]- Чиплеты: когда цена растёт до сотен наносекунд
> На процессорах с чиплетной компоновкой ядра сгруппированы в **CCX** (Core Complex) — кластеры по 4–8 ядер со своим L3 (AMD Ryzen и EPYC). Когерентная транзакция между CCX идёт через межчиплетную шину (Infinity Fabric) и стоит 80–200 нс — эффект, похожий на [[computer/data-path/ram#NUMA: когда у каждого процессора «своя» RAM|NUMA]], но для кешей: локально на CCX — десятки наносекунд, между CCX — сотни.

## Store buffer: когерентность не платится синхронно

Таблица выше показывает полную цену когерентной транзакции. Но обычная запись не всегда тормозит ядро на эту цену. Store-инструкции попадают не в кеш напрямую, а сначала в [[computer/data-path/cache-internals#Store buffer: запись не останавливает конвейер|буфер ожидающих записей]], который позволяет ядру продолжать выполнение, пока invalidate и подтверждение инвалидации идут в фоне. Когерентная транзакция начинается, когда запись покидает буфер.

Для [[computer/atomic-instructions|атомарных инструкций]] эта уловка не работает: read-modify-write должен дождаться захвата линии в Modified, иначе неделимость была бы фикцией. Поэтому атомарные операции над оспариваемой линией платят полную цену когерентности, а обычные записи — лишь амортизированную: ядро не ждёт каждую запись, её цена уходит в фоновые когерентные транзакции.

Побочный эффект буфера — программный порядок записей не всегда совпадает с тем порядком, в котором их увидят другие ядра. Когда и почему это происходит, определяет [модель памяти](../../linux/concurrency/memory-ordering.md).

## False sharing: ловушка скрытого разделения

Когда два ядра по очереди пишут в одну линию, владение ею прыгает туда-обратно — каждый store отбирает линию у соседа. На общем счётчике этот **пинг-понг** легко заметить: оба ядра действительно записывают в один адрес. Но существует ситуация, когда два потока (каждый исполняется на своём ядре) работают с **разными** переменными, и производительность всё равно деградирует в десятки раз. Это [[#Устранение false sharing: выравнивание по кеш-линии|false sharing]] (ложное разделение) — одна из самых коварных проблем многоядерного программирования.
...
> ```
>
> Высокое значение Remote HITM на одном адресе — верный признак [[#Устранение false sharing: выравнивание по кеш-линии|false sharing]] или истинного разделения. Дальше нужно смотреть, какие поля структуры по этому адресу модифицируют разные потоки.
...
Любая операция, требующая когерентной транзакции с другим ядром, стоит 20–70 нс (а между CCX/сокетами — до сотен) — сопоставимо с промахом в RAM. Кеш помогает только когда данные локальны для ядра. Как только два ядра начинают писать в одну линию — настоящее разделение или [[#Устранение false sharing: выравнивание по кеш-линии|false sharing]] — кеш превращается из ускорителя в накладной расход на передачу владения. Разница в два порядка между L1-hit и cross-socket обращением — самый дорогой компонент [[computer/atomic-instructions|атомарных инструкций]].

Цена, которую здесь зафиксировала таблица, — это цена передачи владения. Вторая часть задержки памяти — физика самого DRAM: строки, столбцы, банки, тайминги. Без неё число «~100 нс из RAM» повисает как данность; разберём его устройство в [[computer/data-path/ram|оперативной памяти]].
