...
60–80 нс — это не время пролёта сигнала (электрический сигнал проходит модуль DIMM (Dual Inline Memory Module — плата с чипами памяти) за ~1 нс). Задержка складывается из внутренней логики DRAM: активация строки (~13 нс) — чтение конденсаторов в усилители считывания, выбор столбца (~13 нс), передача данных по шине данных (data bus — набор проводников между модулем памяти и контроллером в CPU, ~5 нс), плюс накладные расходы контроллера памяти. Суммарная задержка даёт те самые ~60–80 нс. Производители маркируют модули скоростью передачи — DDR5-5600 (DDR — Double Data Rate, двойная скорость передачи) означает 5600 мегатрансферов в секунду, — но задержка за 20 лет почти не изменилась: росла только пропускная способность. Подробная арифметика таймингов — в [оперативной памяти](02-ram.md).
...
До сих пор кеш был «чёрным ящиком». При матричном умножении процессор читает очередной элемент — и за один такт кеш определяет, есть ли нужная линия, или нужен запрос в L2. Как это работает?

Кеш работает по тому же принципу, что и [хеш-таблица с цепочками](../../algorithms-and-data-structures/linear/05-hash-table.md) (chaining): хеш от ключа выбирает бакет, в бакете лежит несколько пар «ключ-значение», при поиске сравниваем ключ с каждой парой. В кеше роль ключа играет физический адрес, бакет — это набор линий (set), а сравнение ключа — сверка тегов (tag).
...
Вычислять хеш не нужно: роль хеш-функции играют биты самого адреса. Программа обращается к виртуальным адресам; прежде чем разложить адрес на tag/index/offset, процессор сопоставляет виртуальный адрес с физическим — как именно, разберём ниже.
...
Как работает обращение. CPU выполняет `mov rax, [0x7FFF00012340]`. Последние три hex-цифры адреса — `340`, в двоичной записи `0011 0100 0000` — содержат и смещение, и индекс:
...
Помимо L1i и L1d, у каждого ядра есть **TLB** (Translation Lookaside Buffer — буфер трансляции адресов). Программа никогда не работает с реальными адресами RAM напрямую: каждый процесс видит собственное адресное пространство — виртуальные адреса. Прежде чем обратиться к кешу или RAM, процессор должен преобразовать виртуальный адрес в физический — реальную позицию в микросхемах памяти. Это преобразование происходит на каждом обращении, и ОС организует его блоками фиксированного размера — **страницами** (page), обычно 4 КБ (подробнее — в заметке о [виртуальной памяти](../../linux/foundations/05-virtual-memory.md)).

TLB кеширует результаты трансляции «виртуальная страница → физический фрейм» — без него каждое обращение требовало бы нескольких дополнительных чтений из памяти для прохода по таблице трансляции. Промах TLB стоит 10–50 нс — сопоставимо с промахом кеша данных. При случайном доступе к массиву, занимающему тысячи страниц, промахи TLB добавляются поверх промахов кеша данных. TLB добавляет ещё один порог в stride-тесте:
...
Принцип «расстояние = время» работает и внутри самой RAM. В серверах с несколькими процессорными сокетами каждый сокет имеет свой контроллер памяти, и обращение к «своей» памяти в 1.5–2 раза дешевле, чем к памяти соседнего сокета. Эта архитектура — NUMA (Non-Uniform Memory Access) — подробно разобрана в [оперативной памяти](02-ram.md).
