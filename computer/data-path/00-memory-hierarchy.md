...
4 Б        16 из 16             1                    вся линия полезна
64 Б       1 из 16              16                   каждое обращение — новая линия
128+ Б     1 из 16              16                   + prefetcher теряет паттерн
```
...
Кеш работает по тому же принципу, что и [хеш-таблица](../../algorithms-and-data-structures/linear/05-hash-table.md) в программе. Ключ — физический адрес памяти, «бакет» — набор линий (set, набор), «сравнение ключа» — сверка тегов (tag).

```text
Хеш-таблица (программа)            Кеш (аппаратура)
------------------------------------------------------
хеш(ключ) -> номер бакета          биты адреса -> номер набора
сравнение ключа в бакете            сравнение тега в наборе
