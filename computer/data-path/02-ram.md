...
Если обходить массив последовательно, даже без кеша RAM сама по себе отдаёт данные в 5–10 раз быстрее, чем при случайном доступе. Разница не в кеше — данные в него не помещаются. Причина — во внутреннем устройстве DRAM-чипа: при последовательном чтении чип переиспользует уже подготовленные данные, а при случайном каждый запрос требует полного цикла подготовки заново.
...
Добавление второго модуля RAM в определённый слот удваивает пропускную способность. В другой слот — не удваивает. Разница между 25 ГБ/с и 50 ГБ/с определяется тем, разнесены ли модули так, чтобы контроллер мог читать их параллельно.
...
Таблицу задержек и размеров по всей иерархии (от регистров до RAM) можно найти в [иерархии памяти](00-memory-hierarchy.md). Устройство DRAM добавляет к ней одно важное различие: пропускная способность RAM зависит от паттерна доступа.

```text
Паттерн доступа     Латентность     Эффективный bandwidth
──────────────────────────────────────────────────────────
