...
На первый взгляд, программист не управляет RAM напрямую — за неё отвечает контроллер памяти в CPU. Но знание внутренней механики RAM объясняет три реальных эффекта, с которыми сталкивается любой, кто работает с данными, не помещающимися в кеш.

Если обходить массив последовательно, даже без кеша RAM сама по себе отдаёт данные в 5–10 раз быстрее, чем при случайном доступе. Разница не в кеше — данные в него не помещаются. Причина — в row buffer внутри DRAM-чипа: буфере, который хранит целую строку матрицы и отдаёт последовательные столбцы быстро, без повторной активации.

На сервере с двумя сокетами (два физических процессора) доступ к «своей» памяти занимает ~80 нс, а к памяти соседнего процессора — ~140 нс. Почти вдвое дольше. Если база данных выделяет буфер при старте и половина потоков оказывается на «чужом» сокете — запросы замедляются без видимой причины в профилировщике. Без понимания архитектуры памяти невозможно объяснить, откуда берётся эта разница.

Добавление второго модуля RAM в определённый слот удваивает пропускную способность. В другой слот — не удваивает. Разница между 25 ГБ/с и 50 ГБ/с определяется тем, попали ли модули в разные каналы или в один.
...
Суммарная задержка первого чтения: tRCD + CL + накладные расходы на передачу составляют примерно 40-60 нс только на уровне микросхемы. Плюс задержка передачи по шине от модуля до контроллера памяти CPU — итого те самые ~60-100 нс. Для DDR5-5600 типичный CAS latency — 40 тактов шины; при тактовой частоте 2800 МГц это 40 / 2800 × 10^6 ≈ 14.3 нс. Суммарная задержка (tRCD + CAS + контроллер) — ~70-90 нс. Напряжение, считанное с конденсатора, настолько мало (десятки милливольт), что его нужно усилить специальной схемой — **sense amplifier** (усилитель считывания), которая сравнивает заряд с эталонным уровнем и определяет «0» или «1». Именно sense amplifier формирует данные в row buffer, и его работа — основной вклад в tRCD.
...
```mermaid
flowchart TB
    REQ["Read address"] --> OPEN{"В банке уже<br/>открыта строка?"}
    ...
    CAS_H --> HIT["Row buffer hit:<br/>~13 нс"]
    CAS_M --> MISS["Row buffer miss:<br/>~40 нс на чипе"]
```

Разница — тройная. И именно она объясняет эксперимент из начала.

Row buffer hit выигрывает потому, что банк уже держит нужную строку в SRAM-подобном row buffer. При miss приходится либо открыть строку с нуля, либо сначала закрыть чужую, а потом открыть нужную — именно эти дополнительные шаги и съедают десятки наносекунд.
...
```mermaid
flowchart TB
    MC["Memory controller"] --> CHA["Channel A<br/>отдельная 64-битная шина"]
    MC --> CHB["Channel B<br/>отдельная 64-битная шина"]
    CHA --> RA["Типичный rank<br/>8 чипов отвечают вместе = 64 бита"]
    CHB --> RB["Ещё один channel / rank<br/>параллельная передача"]
    RA --> CHIP["Один DRAM chip<br/>выдаёт 8 бит"]
    CHIP --> BANKS["Banks 0..31<br/>независимые матрицы<br/>со своими row buffers"]
```

Эта иерархия отвечает на три разных вопроса. Channel даёт дополнительную полосу пропускания. Rank набирает ширину шины из узких чипов. Banks дают параллелизм внутри каждого чипа, чтобы скрывать `activate` / `precharge` задержки.
...
На сервере с двумя или более физическими процессорами картина меняется. У каждого CPU свой контроллер памяти и своя «локальная» RAM. Если ядру на CPU 0 нужны данные из RAM, подключённой к CPU 1, запрос идёт через межпроцессорную шину: Intel QPI (QuickPath Interconnect) / UPI (Ultra Path Interconnect) или AMD Infinity Fabric. Это дополнительные ~30-60 нс поверх обычной латентности.

```mermaid
flowchart LR
    CPU0["CPU 0<br/>ядра 0-7"] -- "~80 нс" --> RAM0["RAM 0<br/>128 ГБ"]
    CPU1["CPU 1<br/>ядра 8-15"] -- "~80 нс" --> RAM1["RAM 1<br/>128 ГБ"]
    CPU0 <-- "межпроцессорная шина<br/>QPI / UPI / Infinity Fabric" --> CPU1
    CPU0 -. "~110-140 нс (удалённая)" .-> RAM1
```
