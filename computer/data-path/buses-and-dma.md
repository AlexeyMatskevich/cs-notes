...
Если бы быстрый накопитель отдавал блок 4 КБ через PIO, [процессору](../cpu.md) пришлось бы выполнить 4096 отдельных чтений по байту, каждое занимает порядка 100 нс — итого ~400 мкс только на копирование. Для сравнения, задержка чтения страницы NAND flash внутри SSD — порядка 75 мкс. При использовании PIO [процессор](../cpu.md) тратит в 5 раз больше времени на перетаскивание данных, чем устройство — на их подготовку. Суммарная задержка — ~475 мкс, из которых более 80% — бессмысленная работа [процессора](../cpu.md).
...
Шаг 1: [процессор](../cpu.md) записывает в управляющие регистры контроллера NVMe описание запроса — физический адрес буфера в RAM, куда положить данные, размер блока и номер логического блока (LBA) на диске. Адрес должен быть физическим, а не виртуальным: устройство не знает о [виртуальной памяти](../../linux/foundations/virtual-memory.md) процесса и работает напрямую с физическими адресами (если нет [IOMMU](#iommu-защита-dma-от-устройств) — о нём ниже). Настройка занимает несколько обращений к MMIO (memory-mapped I/O) — порядка 0.5–1 мкс.
...
## Прерывания: устройство зовёт [процессор](../cpu.md)
...
Для понимания прерываний достаточно различать два режима работы [процессора](../cpu.md). В [пользовательском режиме](../../linux/foundations/cpu-modes-and-syscalls.md) работают обычные программы — им запрещено обращаться к устройствам напрямую. В [режиме ядра](../../linux/foundations/cpu-modes-and-syscalls.md) (kernel mode) работает операционная система с полным доступом к оборудованию. При прерывании [процессор](../cpu.md) переключается из пользовательского режима в режим ядра и сохраняет состояние на стек — область памяти, куда [процессор](../cpu.md) складывает точку возврата, чтобы потом продолжить прерванную работу. У ядра ОС для этого отдельный стек, не связанный со стеком программы.
...
2. [Процессор](../cpu.md) завершает текущую инструкцию — не бросает её на полпути
3. [Процессор](../cpu.md) сохраняет состояние: указатель инструкции (RIP), регистр флагов (RFLAGS), указатель стека — на стек ядра
4. [Процессор](../cpu.md) переключается в режим ядра и переходит по адресу из таблицы прерываний (**IDT — Interrupt Descriptor Table, таблица дескрипторов прерываний**) на соответствующий обработчик. IDT — массив из 256 записей, каждая указывает на функцию-обработчик для своего номера прерывания
5. Обработчик делает нужную работу: помечает I/O-запрос как завершённый, будит [процесс](../../linux/foundations/processes.md), ждущий данных, или ставит задачу в очередь отложенной обработки
6. [Процессор](../cpu.md) восстанавливает состояние инструкцией `iret` (interrupt return) и продолжает прерванную работу с точки, где остановился
...
На **x86-64** эту проблему решает аппаратура. DMA-транзакции проходят через тот же механизм [когерентности](cache-coherency.md), что и обычные обращения к памяти. Когда данные пишутся в RAM через DMA, контроллер памяти отправляет **snoop-запрос** (тот же [MESI-протокол](cache-coherency.md), что и между ядрами). Если какое-то ядро держит эту [кеш-линию](memory-hierarchy.md) в состоянии Modified или Exclusive, линия инвалидируется. При следующем чтении ядро получит свежие данные из RAM.
...
Для устройства IOMMU играет ту же роль, что обычный MMU для [процессора](../cpu.md): переводит адрес и одновременно проверяет границы доступа. Без него DMA было бы быстрым, но слишком опасным для виртуализации и изоляции устройств.

IOMMU добавляет небольшую задержку к DMA-транзакции: трансляция адреса занимает ~100–200 нс (с учётом [кеширования](memory-hierarchy.md) в IOTLB). На фоне типичной DMA-передачи (~100 мкс для NVMe) это незаметно.
