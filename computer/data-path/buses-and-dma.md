...
Для понимания прерываний достаточно различать два режима работы [процессора](../cpu.md). В [пользовательском режиме](../../linux/foundations/cpu-modes-and-syscalls.md#кольца-привилегий) работают обычные программы — им запрещено обращаться к устройствам напрямую. В [режиме ядра](../../linux/foundations/cpu-modes-and-syscalls.md#кольца-привилегий) (kernel mode) работает операционная система с полным доступом к оборудованию.
...
Мы разобрали, как устройства подключены к процессору через PCIe. Но у CPU есть ещё один путь — к RAM — и он принципиально другой.
...
У Intel этот механизм называется **VT-d (Virtualization Technology for Directed I/O — технология виртуализации направленного ввода-вывода)**, у AMD — **AMD I/O Virtualization Technology** (часто пишут **AMD-Vi**). Без IOMMU безопасная виртуализация с прямым проброском устройств (PCIe passthrough) невозможна: гостевая ОС получила бы через DMA доступ к памяти хоста и других виртуальных машин.
...
Устройство обращается не к физическому адресу напрямую, а к виртуальному адресу устройства — DVA (Device Virtual Address). IOMMU транслирует DVA в физический адрес, проверяя права доступа. Недавние трансляции кешируются в **IOTLB (I/O Translation Lookaside Buffer)** — аналоге [TLB](../../linux/foundations/virtual-memory.md#tlb-кеш-трансляций) для устройств:
