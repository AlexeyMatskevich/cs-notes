...
Для понимания прерываний достаточно различать два режима работы процессора. В [пользовательском режиме](../../linux/foundations/cpu-modes-and-syscalls.md) работают обычные программы — им запрещено обращаться к устройствам напрямую. В [режиме ядра](../../linux/foundations/cpu-modes-and-syscalls.md) (kernel mode) работает операционная система с полным доступом к оборудованию. При прерывании процессор переключается из пользовательского режима в режим ядра и сохраняет состояние на стек — область памяти, куда процессор складывает точку возврата, чтобы потом продолжить прерванную работу. У ядра ОС для этого отдельный стек, не связанный со стеком программы.
...
На **x86-64** эту проблему решает аппаратура. DMA-транзакции проходят через тот же механизм когерентности, что и обычные обращения к памяти. Когда данные пишутся в RAM через DMA, контроллер памяти отправляет **snoop-запрос** (тот же [MESI-протокол](cache-coherency.md), что и между ядрами). Если какое-то ядро держит эту кеш-линию в состоянии Modified или Exclusive, линия инвалидируется. При следующем чтении ядро получит свежие данные из RAM.
