...
**Предпосылки:** [оперативная память](02-ram.md) (DRAM, row buffer, bandwidth vs latency).

← [Оперативная память](02-ram.md) | [Шины и DMA](04-buses-and-dma.md) →
...
<details>
<summary>Сводная таблица: HDD vs SSD</summary>
...
NVMe SSD выдаёт 500 000 IOPS и 7 ГБ/с, но эти данные должны физически попасть с контроллера SSD в оперативную память. Если CPU будет копировать каждый байт сам — выполняя load и store для каждых 8 байт — передача 4 КБ страницы займёт 512 инструкций, а при 7 ГБ/с потока CPU не будет успевать делать ничего другого. При 500 000 IOPS это 256 миллионов load/store инструкций в секунду только на перекладывание данных — целое ядро занято копированием. Нужен механизм, который позволит устройствам писать в RAM напрямую, без участия процессора. Это решение — DMA (Direct Memory Access, прямой доступ к памяти). Как он устроен — в [следующей заметке](04-buses-and-dma.md).
