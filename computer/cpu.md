...
**Состояние конвейера.** Store к исполнению не готов: `rdi` сам вычисляется из результата предыдущей загрузки, а та идёт в оперативную память, и до её завершения ещё сотня тактов. `rsi` готов, `ebx` готов. Store висит в reservation station; load готов исполниться; `add` ждёт `eax` от load'а.
