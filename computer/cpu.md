...
Чтобы это выражение стало конкретным номером ячейки, процессор его **считает**: умножает `rcx` на 4 и прибавляет к `rbx`. Арифметикой адресов занимается отдельный исполнительный блок — AGU (Address Generation Unit, блок вычисления адресов). Сама операция простая, один такт, но она возможна только когда **все регистры в выражении уже имеют значения**. В нашем цикле это почти всегда мгновенно: `rcx` обновляется быстрым `inc`. Но в более сложном коде задержка может быть существенной. Например, в `arr[indices[i]]` индекс сам читается из другой таблицы: адрес нижнего `mov`'а зависит от результата верхнего. Если верхний `mov` попал в кеш-промах — нижний будет ждать сотни тактов в reservation station, пока верхний не вернёт значение: один из регистров в его адресном выражении ещё не произведён, и считать нечего. Store-инструкции ведут себя ровно так же — если в их адресном выражении есть регистр, который пока формирует какая-то инструкция выше по программе, store висит и не знает своего адреса.

Когда store наконец получает операнды и считает свой адрес, у него на руках и адрес, и значение — всё, что нужно для записи в память. Но писать сразу он всё равно не будет. В OoO-процессоре store-инструкции после исполнения уходят в отдельное хранилище — **store buffer** (буфер записей), — и лежат там до коммита. Причина — та же, что у ROB: до коммита всё сделанное считается спекулятивным, а запись в память **необратима**, её нельзя откатить. Если позже выяснится, что store оказался на неверной ветке предсказания перехода, процессор должен иметь возможность выбросить его как не случившийся — а это возможно только пока в память он ещё не дошёл. Поэтому сам момент записи откладывается до коммита: когда store добирается до головы ROB, его запись в store buffer считается зафиксированной и только тогда уходит наружу, в память. До этого момента «записанное» значение живёт только внутри процессора, в store buffer, и снаружи его как будто не существует.
...
Разберём конкретный проигрыш по тактам. Допустим, в программе идут три инструкции подряд — store, за ним load, за ним использование загруженного значения:

```asm
    mov  [rdi], 42    ; store: записать 42 по адресу из rdi
    mov  eax, [rsi]   ; load:  прочитать из памяти по адресу из rsi в eax
    add  ebx, eax     ; ebx = ebx + eax
```

Пусть в этом вызове `rdi` и `rsi` оказались равны одному и тому же адресу — 0x1000 (p и q — псевдонимы одной переменной), в памяти по 0x1000 сейчас лежит значение 10, а `ebx` на входе держит 5. Другие регистры нас не интересуют.

**Правильный ответ.** По программному порядку должно случиться: store пишет 42 в 0x1000 → load читает 0x1000 и получает 42 → `add ebx, eax` даёт `5 + 42 = 47`. Правильный финал — `ebx = 47`. Это та цифра, с которой будем сверять реальное поведение.

**Состояние конвейера.** Store к исполнению не готов: `rdi` сам вычисляется из результата предыдущей загрузки, которая попала в кеш-промах, и до её завершения ещё сотня тактов. `rsi` готов, `ebx` готов. Store висит в reservation station; load готов исполниться; `add` ждёт `eax` от load'а.

**Ставка.** Memory disambiguation предполагает, что адреса store и load разные, и запускает load прямо сейчас. Load идёт в память по `rsi = 0x1000`, читает **10** (старое значение — store ведь ещё ничего не записал) и кладёт в `eax`. Следом исполняется `add ebx, eax`: `ebx = 5 + 10 = 15`. Процессор считает, что вся тройка уже отработала.

**Разоблачение.** Сотню тактов спустя `rdi` наконец вычислен — 0x1000. Store готов писать. Прежде чем это сделать, процессор проверяет: в спекулятивном окне уже был load, читавший по 0x1000? Был — тот самый, вернувший 10. А теперь, когда адрес store известен и равен адресу load'а, становится видно, что между ними **есть** зависимость по данным: load должен был вернуть не 10, а 42. Ставка проиграна.

**Откат.** Выбрасывается всё, что пошло от ошибочного load'а: `eax = 10` выбрасывается, `add ebx, eax` (итог `ebx = 15`) выбрасывается, любые инструкции за ним, успевшие подхватить новый `ebx`, — тоже.

**Переигровка.** Load переисполняется. Теперь оба адреса известны и совпадение видно сразу, поэтому load не идёт в память — берёт свежее значение напрямую из store buffer через forwarding: `eax = 42`. `add ebx, eax` пересчитывается: `ebx = 5 + 42 = 47`. Это та самая цифра, которую мы зафиксировали в начале как правильный ответ: программа в итоге получила верный результат — но на откат и переигровку ушли десятки тактов впустую, и плюс сотня тактов, в течение которых процессор шёл не по тому следу.

Окупаются эти потери ровно потому, что адресное совпадение у соседних store и load в реальном коде — случай редкий: на каждый такой проигрыш приходится десятки пар, где ставка сыграла и load стартовал без единого такта ожидания. Если бы совпадения были частыми, ставка перестала бы быть выгодной — проигрыши съели бы весь выигрыш, и процессору было бы дешевле просто ждать.

Но сам принцип спекулятивного исполнения — выполнить инструкции до того, как известно, можно ли было, а потом откатить регистры при ошибке — стал причиной двух крупнейших аппаратных уязвимостей, Spectre и Meltdown, обнаруженных в январе 2018 года. Речь здесь уже не про memory disambiguation как частный случай, а про спекуляцию в целом: и про предсказание переходов, и про спекулятивный доступ к памяти в обход проверки прав. Общий принцип обеих атак: при спекулятивном выполнении процессор обращается к данным, к которым у программы нет доступа. Регистры потом откатываются, но побочный эффект — изменение состояния кеша (промежуточной памяти между процессором и RAM — подробно в [следующей заметке](data-path/memory-hierarchy.md)) — остаётся. Кеш откату не подлежит: это отдельная структура, которую спекулятивный код успевает «испачкать» до того, как ошибка заметится. Атакующий измеряет тайминг доступа к кешу и по тому, какая кеш-линия «нагрета», восстанавливает секретные данные.
