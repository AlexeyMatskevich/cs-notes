...
← [Процессор](00-cpu.md) | [Когерентность кешей](02-cache-coherency.md) →
...
**SRAM (Static RAM — статическая память)** хранит бит в схеме из шести транзисторов (six-transistor cell). Шесть транзисторов образуют защёлку (latch), которая сохраняет состояние без подзарядки. Это даёт задержку доступа порядка 1–2 нс, но каждый бит стоит дорого: шесть транзисторов занимают примерно в 30 раз больше площади на кристалле, чем один бит DRAM. На весь чип помещается максимум десятки мегабайт SRAM.
...
60–100 нс — это не время пролёта сигнала (электрический сигнал проходит модуль DIMM за ~1 нс). Задержка складывается из внутренней логики DRAM: активация строки (row activation, ~13 нс) — чтение конденсаторов в sense amplifiers, выбор столбца (CAS latency, Column Address Strobe — ~13 нс), передача данных по шине (~5 нс), плюс накладные расходы контроллера памяти. Для DDR5-5600 типичный CAS latency — 40 тактов шины, при тактовой частоте 2800 МГц это 40 / 2800 × 10^6 ≈ 14.3 нс. Суммарная задержка (tRCD + CAS + контроллер) даёт те самые ~60–80 нс. Производители маркируют модули тактовой частотой (DDR5-5600), но задержка за 20 лет почти не изменилась — росла только пропускная способность.
...
```text
              Объём         Задержка       Технология
            ─────────────────────────────────────────
Регистры      ~128 Б        ~0.25 нс       триггеры (flip-flops)
L1 кеш       32–64 КБ       ~1 нс          SRAM
L2 кеш      256 КБ – 1 МБ   4–7 нс         SRAM
L3 кеш       8–64 МБ        10–30 нс        SRAM
RAM          8–128 ГБ        60–100 нс       DRAM
```
...
Кеш не умеет загружать один байт или одно четырёхбайтовое слово. Минимальная единица, которую кеш читает из памяти и хранит у себя — **кеш-линия** (cache line), она же строка кеша. На x86-64 и ARM размер кеш-линии — 64 байта. Это можно проверить:
...
Восемь параллельных сравнений реализуются как восемь аппаратных компараторов (comparator — схема побитового сравнения), работающих одновременно. Это фиксированная стоимость: увеличение ассоциативности до 16 потребовало бы 16 компараторов, увеличения площади и потенциально более длинного критического пути. Поэтому 8-way — устоявшийся компромисс для L1.
...
L1 кеш разделён на два независимых кеша: **L1i** (instruction cache — кеш инструкций) и **L1d** (data cache — кеш данных). На Intel Skylake: L1i = 32 КБ, L1d = 32 КБ, оба 8-way.
...
**Exclusive** (исключающий): линия хранится ровно на одном уровне. Вытеснение из L1 помещает линию в L2, а не удаляет. Плюс: суммарная ёмкость = L1 + L2 + L3 без дублирования. Минус: при промахе L1, если данные найдены в L2, их нужно переместить (swap): линия из L2 уходит в L1, а вытесненная из L1 — в L2. AMD использовала exclusive L2/L3 в архитектурах Zen 1–3.
...
Когда процессор записывает данные, кеш может вести себя двумя способами.
...
Процессор не просто реагирует на промахи — он пытается их предотвратить. **Hardware prefetcher** (аппаратный предвыборщик) отслеживает паттерн обращений к памяти и загружает линии до того, как программа их запросит.
...
Задержка (latency) — не единственное ограничение. Даже если промахи неизбежны (рабочий набор больше L3), важно, сколько байт в секунду система может доставить из RAM в кеш. Это **пропускная способность** (bandwidth, memory bandwidth).
...
Пропускная способность ограничивает не только RAM. Каждый уровень кеша имеет свой bandwidth. L1d Intel Skylake обеспечивает ~200 ГБ/с (два чтения по 64 байта за такт при ~3 ГГц). L2 — ~50–70 ГБ/с. L3 — ~30–40 ГБ/с на всё кольцо (ring bus). RAM — ~40–50 ГБ/с.

Если код упирается в bandwidth (а не в latency), увеличение кеша не поможет — данные просто не успевают приходить. Типичный пример: суммирование массива `double` (8 байт). Одно сложение за такт, 3 ГГц = 3 × 10^9 сложений/с × 8 байт = 24 ГБ/с потребности. Если массив в L1 — bandwidth 200 ГБ/с, узкое место — ALU. Если массив в RAM — bandwidth 40 ГБ/с, операция ограничена памятью (memory-bound), и более быстрый процессор не ускорит её. Для диагностики: счётчик `perf stat -e offcore_response.demand_data_rd.l3_miss.any_snoop` показывает количество запросов, дошедших до RAM.

## Возвращаемся к матрице
...
На серверах с несколькими процессорными сокетами не вся RAM одинаково близка: обращение к памяти «своего» сокета стоит ~80 нс, а к памяти соседнего — ~130 нс (в 1.5–2 раза дороже). Эта разница — следствие архитектуры NUMA (Non-Uniform Memory Access), где каждый сокет имеет свой контроллер памяти. Подробная архитектура NUMA, диаграмма топологии, политика first-touch и инструменты (`numactl`, `numastat`) — в [оперативной памяти](03-ram.md).

## Когерентность: проблема общих данных

Кеш решает проблему доступа к данным для одного ядра. Но у каждого ядра свой L1 и L2, а данные — общие. Когда два ядра записывают по одному адресу, каждое видит свою копию в своём L1 — и эти копии расходятся. Чтобы система оставалась корректной, нужен механизм, гарантирующий, что все ядра видят согласованное состояние памяти. Как устроен протокол когерентности — в [следующей заметке](02-cache-coherency.md).
