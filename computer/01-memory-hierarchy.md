...
60–100 нс — это не время пролёта сигнала (электрический сигнал проходит модуль DIMM за ~1 нс). Задержка складывается из внутренней логики DRAM: активация строки (~13 нс) — чтение конденсаторов в усилители считывания, выбор столбца (~13 нс), передача данных по шине (~5 нс), плюс накладные расходы контроллера памяти. Суммарная задержка даёт те самые ~60–80 нс. Производители маркируют модули тактовой частотой (DDR5-5600), но задержка за 20 лет почти не изменилась — росла только пропускная способность. Подробная арифметика таймингов — в [оперативной памяти](03-ram.md).
