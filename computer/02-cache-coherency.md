...
**Предпосылки:** [иерархия памяти и кеш](01-memory-hierarchy.md) (cache line, L1/L2 per core, L3 shared).

← [Иерархия памяти и кеш](01-memory-hierarchy.md) | [Оперативная память](03-ram.md) →
...
## Сценарий: атомарный счётчик на двух потоках
...
**Когерентность кешей (cache coherence)** — аппаратный протокол, который гарантирует: если одно ядро записало значение по адресу X, все остальные ядра при чтении адреса X увидят это новое значение. Кеш не врёт — каждое ядро видит актуальные данные, даже если физически они хранятся в L1 другого ядра.
...
Основной протокол когерентности в процессорах Intel и AMD — **MESI**. Название — аббревиатура четырёх состояний, в которых может находиться кеш-линия.
...
Core 1 выполняет `load [0x7f00]`. Адреса в его кеше нет. Core 1 посылает bus read. Core 0 видит этот запрос на шине (механизм **snooping** — каждое ядро «подслушивает» все транзакции шины). Core 0 обнаруживает, что у него есть эта линия в состоянии E. Оба ядра переводят свои копии в **Shared**.
...
До сих пор мы говорили о когерентности между L1 разных ядер. В реальности иерархия сложнее: L1 -> L2 -> L3 (shared) -> RAM. Как когерентность работает на нескольких уровнях?

L3 в большинстве современных процессоров (Intel начиная с Nehalem, AMD начиная с Zen) — **inclusive** или **non-inclusive**. В inclusive-дизайне L3 содержит копии всех линий из L1 и L2. Когда одно ядро инвалидирует линию у другого, L3 знает, в каком ядре эта линия находится — не нужно рассылать запрос всем. Это снижает трафик на интерконнекте. В non-inclusive дизайне (Intel начиная с Skylake-SP) L3 — victim cache, и используется snoop filter — отдельная структура, отслеживающая, какое ядро кеширует какую линию.
...
Атомарная RMW (без конкуренции)     ~6 нс
