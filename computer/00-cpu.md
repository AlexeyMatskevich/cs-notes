...
[Путь данных: Иерархия памяти](data-path/00-memory-hierarchy.md) | [Программная модель: CISC и RISC](programmer-model/00-isa.md) →
...
Между регистрами и оперативной памятью необходим промежуточный слой — кеш-память (cache), которая хранит копии часто используемых данных ближе к процессору. Стена памяти ведёт к [иерархии кешей](data-path/00-memory-hierarchy.md). Но есть и другой вопрос: какой набор команд процессор предоставляет программе и как это влияет на энергоэффективность — [CISC и RISC](programmer-model/00-isa.md).
