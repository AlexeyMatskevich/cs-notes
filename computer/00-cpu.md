...
**Предпосылки:** [базовое программирование](../programming/index.md) (переменные, циклы, функции).
...
Исправления на трёх уровнях. В аппаратуре — обновления микрокода с барьерами спекуляции. В операционных системах — KPTI (kernel page table isolation): ядро и пользовательское пространство используют разные таблицы страниц, при каждом системном вызове процессор переключает таблицу. На процессорах без PCID (механизм, позволяющий TLB хранить записи нескольких процессов без полного сброса) это означает полный сброс [TLB](../linux/foundations/05-virtual-memory.md#tlb-кеш-трансляций) (translation lookaside buffer — кеш трансляции виртуальных адресов в физические). В компиляторах — retpoline: замена косвенных переходов на последовательности, не допускающие спекулятивного выполнения по произвольному адресу.
...
Всё вышеописанное — конвейер, суперскалярность, OoO, спекулятивное исполнение — это архитектура одного **ядра** (core). Ядро — независимый вычислительный блок: собственный конвейер, регистры, кеши первого уровня. Современные процессоры помещают на один кристалл несколько таких ядер: 8–16 в десктопных, 64–128 в серверных. Каждое ядро исполняет свой поток инструкций независимо от остальных.

Но если ядра работают с общими данными, возникают два вопроса: где именно лежат эти данные (в чьём кеше?) и как гарантировать, что все ядра видят одну и ту же версию? Ответы — в [иерархии памяти](data-path/00-memory-hierarchy.md) и [когерентности кешей](data-path/01-cache-coherency.md).
