...
**Предпосылки:** [процессор](00-cpu.md) (регистры, ISA, конвейер).

← [Шины и DMA](05-buses-and-dma.md)

ABI отвечает на четыре вопроса: где лежат аргументы функции, где адрес возврата, как расположены поля в структуре, в каком порядке лежат байты числа. Каждый ответ — компромисс между скоростью и универсальностью.
...
System V AMD64 ABI требует, чтобы перед инструкцией `call` значение `rsp` было выровнено по 16 байтам. Причина — инструкции SSE/AVX (`movaps`, `movdqa`) ожидают 16-байтовое выравнивание данных в стеке. Если `rsp` не делится на 16, эти инструкции генерируют аппаратное исключение. Сама `call` кладёт 8 байт адреса возврата, сдвигая `rsp` на 8, поэтому на входе в функцию `rsp` всегда смещён на 8 относительно 16. Функция восстанавливает выравнивание, вычитая из `rsp` нечётное количество 8-байтовых слов.
...
Структура `Order` хранится в памяти — но как именно записан `int id` со значением `0x00000001`? Ответ зависит от порядка байтов.

**Little-endian** (буквально: «младший конец первый») — младший байт лежит по младшему адресу. Число `0x01020304` хранится как `04 03 02 01`. x86 и ARM (в стандартном режиме) используют little-endian.

**Big-endian** («старший конец первый») — старший байт по младшему адресу. То же число хранится как `01 02 03 04`. Сетевые протоколы (TCP/IP), форматы Java `.class` файлов, PNG и JPEG — big-endian. Исторически big-endian использовали SPARC, PowerPC, старые MIPS.
...
Термины big-endian и little-endian ввёл Дэнни Коэн в статье 1980 года, ссылаясь на «Путешествия Гулливера» Джонатана Свифта: два королевства воевали из-за того, с какого конца разбивать варёное яйцо. Аналогия точная — вопрос технически непринципиальный, но несовместимость между двумя вариантами создаёт реальные проблемы.

Когда порядок байтов имеет значение: при передаче данных между машинами с разной endianness, при записи на диск (файл, прочитанный на другой платформе), при разборе сетевых пакетов. Функции `htonl` (host to network long) и `ntohl` (network to host long) конвертируют между порядком байтов машины и сетевым (big-endian). На big-endian машине эти функции ничего не делают — просто возвращают значение как есть. На little-endian — переставляют байты.

Внутри одной программы на одной машине endianness незаметен: процессор читает и пишет в своём порядке, и пока данные не покидают машину, разницы нет. Проблема возникает на границе: отправка по сети, запись в файл, межъязыковая сериализация. Классический баг: программист на x86 записывает `int32_t` в файл через `fwrite(&value, 4, 1, f)`. На той же машине файл читается корректно. На big-endian машине (или при разборе сетевого пакета) числа оказываются перевёрнутыми: `0x00000001` становится `0x01000000` — число 16 777 216 вместо 1.

Современные форматы сериализации (Protocol Buffers, MessagePack, JSON) абстрагируют endianness: кодирование и декодирование выполняются библиотекой, и программисту не нужно думать о порядке байтов. Проблема остаётся актуальной при работе с низкоуровневыми протоколами (TCP/IP-заголовки, бинарные форматы файлов) и при прямой записи структур в память или на диск.
