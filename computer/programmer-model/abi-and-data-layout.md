...
Причина — в аппаратуре. [Процессор](../cpu.md) читает данные из памяти не побайтово, а [кеш-линиями](../data-path/memory-hierarchy.md#кеш-линия-минимальная-единица-обмена) по 64 байта. Если 8-байтовый `double` лежит невыровненно, но целиком внутри одной [кеш-линии](../data-path/memory-hierarchy.md#кеш-линия-минимальная-единица-обмена) — современные процессоры (Intel Sandy Bridge+, AMD Zen+) обрабатывают это аппаратно без значимого штрафа. Реальная проблема — **split load (расщеплённое чтение)** при пересечении границы между двумя [кеш-линиями](../data-path/memory-hierarchy.md#кеш-линия-минимальная-единица-обмена): если адрес 0x3C, а данные заходят на вторую кеш-линию с адреса 0x40, нужны два обращения к L1, и штраф составляет ~5–20 тактов в зависимости от микроархитектуры. Ещё хуже — пересечение границы страницы памяти (4 КБ): штраф достигает сотен тактов.

На x86-64 невыровненный доступ допустим — [процессор](../cpu.md) справится, но медленнее. На AArch64 (ARMv8, 64-битный режим) ситуация аналогична: невыровненный доступ к обычной памяти обрабатывается аппаратно, со штрафом при пересечении [кеш-линии](../data-path/memory-hierarchy.md#кеш-линия-минимальная-единица-обмена). Обязательное выравнивание сохраняется только для [атомарных операций](../atomic-instructions.md) и обращений к регистрам устройств. На ранних ARM (до ARMv6) невыровненный доступ действительно вызывал аппаратное исключение.
...
- [Ruby VALUE tagged pointers](../../ruby/internal/object-model/objects-and-classes.md#объекты-без-структуры-immediate-values) — 64-bit word: нижний бит отличает immediate integer от heap pointer (возможно благодаря выравниванию pointer'ов по 8 байтам), ещё биты — symbol, true/false/nil
