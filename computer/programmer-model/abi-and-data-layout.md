...
**Выравнивание стека перед вызовом.** System V AMD64 ABI дополнительно требует, чтобы перед инструкцией `call` значение `rsp` было кратно 16. Причина — 16-байтовые SSE-инструкции ([Streaming SIMD Extensions](simd.md)) на невыровненном адресе генерируют аппаратное исключение. Выровненные AVX-записи требуют уже 32 байт, но ABI гарантирует только 16. Сама `call` кладёт на стек 8 байт адреса возврата, поэтому на входе в функцию `rsp` смещён на 8 относительно 16; функция восстанавливает выравнивание, вычитая из `rsp` нечётное количество 8-байтовых слов.
