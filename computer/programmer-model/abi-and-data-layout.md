...
**Предпосылки:** [базовое программирование](../../programming/programming.md) (структуры, указатели, компилятор), [порядок байтов](../../foundations/endianness.md) (little-endian, big-endian), [процессор](../cpu.md) (регистры, такт, конвейер), [CISC и RISC](isa.md) (ISA, micro-ops, CISC vs RISC).

← [CISC и RISC](isa.md) | [SIMD и векторные расширения](simd.md) →

[ISA](isa.md) определяет, какие инструкции и регистры [процессор](../cpu.md) предоставляет. Но как именно компилятор использует эти регистры — куда кладёт аргументы, как выравнивает данные — определяет ABI.

[Процессор](../cpu.md) знает, как складывать регистры, загружать данные из памяти и переходить по адресу. Но программа — это не один поток инструкций: функция вызывает другую функцию, та — третью, каждая принимает аргументы и возвращает результат. Где лежат аргументы? Куда записать адрес возврата? Как расположить поля структуры, чтобы [процессор](../cpu.md) не тратил лишние такты на чтение? Ответы на эти вопросы не зашиты в железо — они определяются соглашениями, которые компилятор, ОС и процессор соблюдают совместно. Совокупность таких соглашений — ABI (Application Binary Interface, двоичный интерфейс приложений, буквально: интерфейс на уровне двоичного кода).

Функция `process_order` принимает структуру заказа, вызывает `calculate_total`, получает результат и возвращает его вызвавшему коду. Компилятор должен решить: куда передать аргументы, куда положить результат, как вернуть управление.

```c
// struct — запись из именованных полей разных типов; int занимает 4 байта, double — 8
struct Order {
    int     id;        // 4 байта
    double  price;     // 8 байт
...
[Процессор](../cpu.md) x86-64 имеет регистр `rsp` (stack pointer — указатель стека), который указывает на вершину **стека вызовов** — области памяти, куда процессор при каждом вызове функции складывает адрес возврата и локальные данные. Стек растёт сверху вниз, от старших адресов к младшим, и решает задачу, которую регистры решить не могут: хранить данные произвольной глубины вложенности. Функция `main` вызывает `process_order`, та вызывает `calculate_total` — три уровня. Если `calculate_total` рекурсивно вызывает себя, уровней может быть тысячи. Регистров 16, а глубина вложенности неизвестна заранее.

Один регистр не способен хранить цепочку адресов возврата. Даже на RISC-архитектурах, где вызов изначально кладёт адрес в отдельный регистр, компилятор при вложенном вызове сохраняет этот регистр на стек — иначе адрес возврата старшей функции будет затёрт. Стек для адресов возврата — обязательная часть контракта любой функции, которая сама может вызвать другую. На x86-64 инструкции `call` и `ret` делают эту работу автоматически, прямо через [ISA](isa.md).
...
Структура `Order` — 24 байта, слишком много для передачи через регистры (порог — 16 байт; причина — число регистровых слотов, заложенное в [[#Соглашение о вызовах: System V AMD64 ABI|calling convention]]). Поэтому `main` копирует `order` на стек перед вызовом. Для вызова `process_order(order)` стек концептуально выглядит так (в неоптимизированном коде, с сохранением rbp):
...
Стек хранит копию структуры-аргумента, адреса возврата и локальные данные. Скалярные аргументы (`price`, `quantity`) и результат `calculate_total` передаются через регистры — без обращения к памяти. Сочетание стека для крупных данных и регистров для «горячих» значений делает вызов функции одновременно вложенным и относительно дешёвым. ABI ещё требует, чтобы перед `call` значение `rsp` было выровнено по 16 байтам; причина и механика — в [[#Выравнивание|разделе о выравнивании]].

## Соглашение о вызовах: System V AMD64 ABI

Стек медленнее регистров: запись в стек — это запись в память (пусть и попадающая в [[computer/data-path/memory-hierarchy#Иерархия: от регистров до оперативной памяти|L1-кеш]], это ~1 нс против ~0.3 нс для регистра). Передавать все аргументы через стек расточительно, если большинство функций принимают 1–3 параметра. Calling convention (соглашение о вызовах) определяет, какие аргументы передаются в регистрах, какие — на стеке.

На Linux x86-64 действует **System V AMD64 ABI**. Первые шесть целочисленных аргументов (и указателей) передаются в регистрах `rdi`, `rsi`, `rdx`, `rcx`, `r8`, `r9` — именно в этом порядке. Первые восемь аргументов с плавающей точкой — в `xmm0`–`xmm7` (`xmm` — отдельное семейство 128-битных регистров для операций с числами с плавающей точкой и SIMD, в отличие от 16 регистров общего назначения). Если аргументов больше — остаток уходит на стек.
...
Причина — в аппаратуре. [Процессор](../cpu.md) читает данные из памяти не побайтово, а [[computer/data-path/memory-hierarchy#Кеш-линия: минимальная единица обмена|кеш-линиями]] по 64 байта. Если 8-байтовый `double` лежит невыровненно, но целиком внутри одной [[computer/data-path/memory-hierarchy#Кеш-линия: минимальная единица обмена|кеш-линии]] — современные процессоры (Intel Sandy Bridge+, AMD Zen+) обрабатывают это аппаратно без значимого штрафа. Реальная проблема — **split load (расщеплённое чтение)** при пересечении границы между двумя [[computer/data-path/memory-hierarchy#Кеш-линия: минимальная единица обмена|кеш-линиями]]: если адрес 0x3C, а данные заходят на вторую кеш-линию с адреса 0x40, нужны два обращения к L1, и штраф составляет ~5–20 тактов в зависимости от микроархитектуры. Ещё хуже — пересечение границы страницы памяти (4 КБ — единицы, которой операционная система раздаёт память): штраф достигает сотен тактов.

На x86-64 невыровненный доступ допустим — [процессор](../cpu.md) справится, но медленнее. На AArch64 (ARMv8, 64-битный режим) ситуация аналогична: невыровненный доступ к обычной памяти обрабатывается аппаратно, со штрафом при пересечении [[computer/data-path/memory-hierarchy#Кеш-линия: минимальная единица обмена|кеш-линии]]. Обязательное выравнивание сохраняется только для [атомарных операций](../atomic-instructions.md) и обращений к регистрам устройств. На ранних ARM (до ARMv6) невыровненный доступ действительно вызывал аппаратное исключение.

**Выравнивание стека перед вызовом.** System V AMD64 ABI дополнительно требует, чтобы перед инструкцией `call` значение `rsp` было кратно 16. Причина — 16-байтовые SSE-инструкции ([Streaming SIMD Extensions](simd.md)) на невыровненном адресе генерируют аппаратное исключение. Выровненные [AVX](simd.md)-записи (более широкие, 256/512-битные векторные расширения) требуют уже 32 байт, но ABI гарантирует только 16. Сама `call` кладёт на стек 8 байт адреса возврата, поэтому на входе в функцию `rsp` смещён на 8 относительно 16, и функция первым делом сама довыравнивает его.
...
`price` (выравнивание 8) стоит первой, начинается с 0. `id` (выравнивание 4) начинается с 8 — делится на 4. `quantity` — с 12 — делится на 4. Trailing padding не нужен: 16 делится на 8. Итого 16 вместо 24 — экономия 33%. При массиве из миллиона заказов это 8 МБ разницы: больше [[computer/data-path/memory-hierarchy#Три типа промахов|кеш-промахов]], больше обращений к памяти, больше тактов ожидания.
...
Правила выравнивания и padding определяют, как данные живут в памяти работающего процесса. Но когда данные пересекают границу процесса — уходят в сетевой пакет или бинарный файл — возникает ещё один вопрос: каким порядком расположены байты внутри 8-байтового `double`?

Подробно о порядке байтов, двух соглашениях (little-endian и big-endian), причинах выбора x86 и практических последствиях — в [порядок байтов](../../foundations/endianness.md). Здесь важно одно: внутри одной программы на одной машине endianness незаметен. Проблема возникает на границе: отправка по сети, запись в файл, межъязыковая сериализация. Современные x86-64 и AArch64 используют little-endian, сетевые протоколы (TCP/IP) — big-endian.

## ABI как контракт

Каждое соглашение, описанное выше, — фрагмент одного контракта. Calling convention определяет, как функция принимает аргументы и возвращает результат. Выравнивание определяет, по каким адресам лежат данные. Размещение структур определяет, как поля расположены относительно друг друга. Порядок байтов определяет, как многобайтовые значения записаны в памяти. Вместе они образуют **ABI** — набор правил, которым следуют все участники: компилятор, ОС, динамический линкер (программа, которая при запуске подключает разделяемые библиотеки к исполняемому файлу) и любая библиотека.

На Linux x86-64 действует **System V AMD64 ABI**, описанный в документе ~130 страниц. Он охватывает не только вызовы функций, но и формат [[linux/infrastructure/elf-and-linking#exec() и загрузка ELF|ELF]]-файлов (Executable and Linkable Format), способ передачи структур (маленькие структуры в регистрах, крупные — через копирование на стек), размещение thread-local storage (локальное хранилище потока) и даже способ обработки исключений C++.

Windows x64 использует другой ABI: первые четыре целочисленных аргумента идут в `rcx`, `rdx`, `r8`, `r9` (не `rdi`, `rsi`, как в System V). Вызывающая функция обязана зарезервировать 32 байта на стеке для «shadow space» — области, куда вызываемая функция может записать регистровые аргументы при необходимости. Набор callee-saved регистров тоже отличается. Код, скомпилированный для Linux, нельзя вызвать из Windows-библиотеки — даже на том же [процессоре](../cpu.md) с тем же набором [инструкций](isa.md).

Это объясняет, почему ABI-стабильность критична для разделяемых библиотек (`.so`, `.dll`). Библиотека libc.so скомпилирована отдельно от программы. Если компилятор решит изменить порядок аргументов в регистрах или выравнивание структуры в новой версии ABI, каждая программа, слинкованная с libc.so, перестанет работать. Поэтому ABI меняется крайне редко: System V AMD64 ABI используется без принципиальных изменений с начала 2000-х. Переход от x86 32-бит, где конкурировали несколько соглашений о вызовах, к x86-64 был одним из немногих моментов, когда индустрия согласовала единый ABI для платформы.

Для системных вызовов Linux использует тот же набор регистров, но с другим распределением: номер вызова в `rax`, аргументы в `rdi`, `rsi`, `rdx`, `r10`, `r8`, `r9` (обратите внимание: `r10` вместо `rcx`, потому что инструкция `syscall` затирает `rcx` адресом возврата). Подробности — в [механизме системных вызовов](../../linux/kernel/syscall-internals.md). Формат ELF, через который ядро загружает программу и разрешает зависимости, описан в [[linux/infrastructure/elf-and-linking#exec() и загрузка ELF|ELF и линковке]].
...
- [[ruby/internal/object-model/objects-and-classes#Объекты без структуры: immediate values|Ruby VALUE tagged pointers]] — 64-bit word: нижний бит отличает immediate integer от heap pointer (возможно благодаря выравниванию pointer'ов по 8 байтам), ещё биты — symbol, true/false/nil
