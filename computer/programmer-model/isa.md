...
**Предпосылки:** [базовое программирование](../../programming/programming.md) (компилятор), [процессор](../cpu.md) ([[computer/cpu#Конвейер|конвейер]], fetch, decode, регистры, [[computer/cpu#Суперскалярное исполнение|суперскалярность]], [[computer/cpu#Внеочерёдное исполнение|внеочерёдное исполнение]], [[computer/cpu#Спекулятивное исполнение|спекулятивное исполнение]]).
...
> [!info]- CISC vs RISC на [ассемблере](../../programming/assembler.md): a = a + b
> ```
> x86 (CISC):
...
Современные x86-процессоры внутри работают как RISC: сложные CISC-инструкции на стадии decode разбиваются на [[computer/cpu#Суперскалярное исполнение|микрооперации (μops)]] — те самые простые внутренние команды фиксированной длины, с которыми работает суперскалярный конвейер. [[computer/cpu#Конвейер|Конвейер]], [[computer/cpu#Суперскалярное исполнение|суперскалярный движок]] и [[computer/cpu#Внеочерёдное исполнение|блок внеочерёдного исполнения]] работают уже с μops. Та самая операция `a += b`, записанная двумя x86-инструкциями, внутри процессора превращается в те же загрузку, сложение и запись — но с накладными расходами на трансляцию. Снаружи — CISC ради совместимости с 45+ годами экосистемы. Внутри — RISC ради эффективности [[computer/cpu#Конвейер|конвейера]].
...
Но ISA — не единственное соглашение между процессором и программой. ISA говорит «вот эти команды я понимаю». Программе нужно знать и другое: куда кладутся аргументы функций, где лежит результат, как выравниваются данные в памяти. Эти правила определяют **ABI** — бинарный интерфейс приложений, который связывает компилятор, процессор и операционную систему в единый контракт.
