...
**Предпосылки:** [ABI и размещение данных](01-abi-and-data-layout.md) (xmm-регистры, выравнивание).

← [ABI и размещение данных](01-abi-and-data-layout.md)

В ABI аргументы с плавающей точкой передаются через xmm0–xmm7 — 128-битные регистры. Но зачем регистру 128 бит, если `double` занимает 64? Ответ: каждый xmm-регистр вмещает **два** `double`, и существуют инструкции, которые обрабатывают оба за один такт. Это SIMD (Single Instruction, Multiple Data — одна инструкция, много данных): способ обрабатывать несколько одинаковых элементов одной инструкцией.
...
Суперскалярность запускает **несколько разных инструкций** параллельно (load + mul + store на разных блоках). SIMD — другой вид параллелизма: **одна инструкция** обрабатывает **несколько одинаковых данных**. Это не многопоточность (разные потоки с разным кодом) и не суперскалярность (разные инструкции). Это расширение самого пути данных: вместо 64-битного регистра и 64-битного умножителя — 256-битный регистр и 256-битный умножитель, обрабатывающий 4 `double` за один такт.
...
Первые x86-SIMD-расширения — MMX (1997): 64-битные регистры, только целочисленные, делили регистровый файл с FPU (нельзя использовать одновременно с вещественной арифметикой). MMX быстро устарел.
...
AVX (Advanced Vector Extensions, 2011) удвоил ширину до 256 бит: 16 регистров YMM, каждый вмещает 4 `double` или 8 `float`. Инструкция `vmulpd ymm0, ymm1, ymm2` умножает 4 `double` за один такт — наш цикл ускоряется до ×4. AVX использует трёхоперандный формат (два источника + результат), в отличие от SSE, где результат перезаписывал один из источников.
...
AVX-512 появился в серверных Xeon (Skylake-SP, 2017), затем в клиентских (Ice Lake, 2019), но Intel убрал его из E-cores гибридных процессоров (Alder Lake, 2021). На процессорах с гибридной архитектурой использование AVX-512 возможно только если все E-cores отключены.

<details>
<summary>x86-специфичные подводные камни: throttling и transition penalty</summary>

**AVX-512 frequency throttling:** при активации тяжёлых AVX-512 инструкций процессор снижает частоту. Широкие 512-битные ALU потребляют больше энергии, и thermal budget не позволяет держать номинальную частоту. Если SIMD-секция кода короткая, потеря от снижения частоты может перевесить выигрыш от широкого вектора.
...
В [ABI](01-abi-and-data-layout.md) стек выровнен по 16 байтам, и `movaps` (aligned load) требует этого выравнивания. Для 256-битных AVX-загрузок данные выигрывают от 32-байтового выравнивания: `vmovapd` (aligned) требует его, а `vmovupd` (unaligned) на современных процессорах (Haswell+) не штрафуется для обращений внутри одной [кеш-линии](../data-path/00-memory-hierarchy.md). Штраф возникает, когда 32-байтовая загрузка пересекает границу 64-байтовой кеш-линии — в этом случае нужны два обращения к кешу вместо одного.

Для динамически выделяемых массивов: `malloc` на большинстве платформ гарантирует 16-байтовое выравнивание. Для 32-байтового — `aligned_alloc(32, size)` в C11, `_mm_malloc(size, 32)` через Intel intrinsics.
...
Если данные лежат подряд и выровнены, компилятор часто может векторизовать цикл сам, без intrinsics. Наш `c[i] = a[i] * b[i]` с `-O2 -march=native` компилируется в AVX-инструкции без единого intrinsic в исходном коде. GCC включает автовекторизацию на `-O2` (`-ftree-vectorize`), Clang — тоже. Флаг `-march=native` разрешает использовать все расширения текущего процессора.

Автовекторизация работает, когда компилятор может доказать, что итерации независимы (нет зависимости `c[i]` от `c[i-1]`), указатели не пересекаются (`a`, `b`, `c` — разные массивы; `restrict` помогает) и цикл достаточно простой — без вызовов функций и сложных условий внутри.

Когда автовекторизация не справляется — зависимые итерации, pointer aliasing, условная логика внутри цикла — нужен ручной контроль.
...
<details>
<summary>FMA: умножение и сложение за одну инструкцию</summary>

FMA (Fused Multiply-Add) — отдельное расширение, не связанное с шириной вектора. Инструкция `vfmadd213pd` вычисляет `a * b + c` за один такт вместо двух (mul + add). FMA доступен с Haswell (2013) на x86 и в NEON на ARM. Для задач вида SAXPY (`y[i] = a * x[i] + y[i]`) или матричного умножения FMA удваивает арифметическую пропускную способность. На 3 ГГц с двумя FMA-блоками и AVX2: 2 блока × 4 double × 2 операции (mul+add) × 3 ГГц = 48 GFLOPS на одном ядре.
...
На массивах, не помещающихся в кеш, к ограничению портов добавляется [пропускная способность памяти](../data-path/00-memory-hierarchy.md): DDR5-5600 в двух каналах даёт ~70 ГБ/с, и этот потолок не зависит от ширины SIMD-регистров. Для нашего цикла с 3 обращениями по 8 байт на элемент (24 байта на итерацию) и 1024 элементами теоретический compute ceiling с AVX = 256 итераций × 1 такт = 256 тактов. Но если массив в RAM, bandwidth ceiling = 24 КБ / 70 ГБ/с ≈ 0.34 мкс — и ширина SIMD уже не определяет скорость.

## Когда SIMD не помогает

SIMD ускоряет чистые циклы с независимыми итерациями. Не всё подходит под это описание.

**Зависимые итерации.** Если `c[i]` зависит от `c[i-1]` (как `sum += a[i]` — loop-carried dependency), векторизация невозможна без алгоритмической перестройки (например, несколько аккумуляторов). Скалярная редукция 1024 элементов занимает ~1024 такта из-за цепочки зависимостей; векторная с 4 аккумуляторами — ~256.
...
**Портируемость.** Код с AVX intrinsics не запустится на CPU без AVX. Runtime dispatch (проверка `cpuid` → выбор кодовой ветки) решает проблему, но увеличивает сложность. Библиотеки (Highway, Abseil) инкапсулируют dispatch.
...
- Intel, *Intel Intrinsics Guide* — API intrinsics: https://www.intel.com/content/www/us/en/docs/intrinsics-guide/index.html
- ARM, 2023, *ARM Architecture Reference Manual for A-profile* — NEON, SVE: https://developer.arm.com/documentation/ddi0487/latest
- GCC, *Auto-vectorization in GCC* — флаги и условия: https://gcc.gnu.org/projects/tree-ssa/vectorization.html
