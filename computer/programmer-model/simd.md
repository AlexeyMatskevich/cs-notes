...
**Предпосылки:** [базовое программирование](../../programming/programming.md) (циклы, массивы, указатели, компилятор), [бит, байт](../../foundations/binary-and-bytes.md), [числа с плавающей точкой](../../foundations/floating-point.md) (`float` — 32 бита, `double` — 64 бита), [[computer/cpu#Суперскалярное исполнение|суперскалярное исполнение]] (несколько разных инструкций за такт), [ABI и размещение данных](abi-and-data-layout.md) (xmm-регистры, выравнивание, структуры с полями), [[computer/data-path/memory-hierarchy#Кеш-линия: минимальная единица обмена|кеш-линия]] (64 байта — единица передачи между кешем и памятью).
...
> [!info]- Как собрана мнемоника `vmulpd`
> Имя инструкции складывается из частей, и каждая кодирует уже знакомое различие:
> - `v` — вектор: операция над тем самым широким (256-битным) регистром, а не над 64-битным скаляром;
> - `mul` — multiply, умножение;
> - `p` — packed, «упакованные»: одна команда работает сразу над несколькими элементами регистра;
> - `d` — double, двойная точность (64-битный `double`).
>
> Скалярная версия `mulsd` собрана так же: `mul` + `s` (scalar — один элемент) + `d` (double). Вся разница между «по одному» и «пачкой» — в одной букве: `s` -> `p`. По тому же правилу читаются `vaddpd` (сложение) и `vsubpd` (вычитание).

Для цикла выше это означает: вместо `N` скалярных итераций — `N / 4` широких. Идеальный потолок по ширине вектора — 4× с AVX (256 бит / 64 бита) или 8× с AVX-512 (512 бит). Но потолок — не обещание, и первое же, что встаёт между ним и реальным выигрышем, — сам компилятор: прежде чем сгенерировать `vmulpd`, он должен убедиться, что итерации действительно независимы.
...
> [!info]- Задача: где здесь перекрытие?
> Функция объявлена как `mul_arrays(double *a, double *b, double *c, int n)` — без `restrict`. Что произойдёт при вызове `mul_arrays(arr, brr, arr + 1, n)`: даст ли векторная версия тот же результат, что скалярная?
>
> **Частая ошибка:** раз указатели разные (`arr` и `arr + 1`), перекрытия нет.
>
> Здесь `a == arr`, а `c == arr + 1` — значит `c[i]` и `a[i+1]` это одна и та же ячейка (`arr[i+1]`). Запись `c[i]` меняет `a[i+1]`, который следующая итерация обязана прочитать. Скалярный цикл прочитает уже изменённое значение, векторный (читает четыре элемента разом, до записей) — старое. Результаты разойдутся — поэтому без `restrict` компилятор и оставляет скалярный fallback.
...
Потолок 4× или 8× из прошлого раздела зависит от того, какое расширение набора команд (ISA — Instruction Set Architecture) доступно на целевой машине, — а доступная ширина росла поколениями. Каждое поколение добавляло ширину или типы данных. Ключевые точки:
...
AVX-512 появился в серверных Xeon (2017), затем ненадолго в клиентских. Но гибридные процессоры (мощные P-ядра рядом с энергоэффективными E-ядрами) принесли проблему: 512-битные блоки оказались не на всех ядрах, и Intel в итоге убрал AVX-512 из клиентских CPU по дизайну, оставив его в серверных Xeon и AMD EPYC. На замену анонсирована унифицированная **AVX10**, рассчитанная на гибридные ядра, но её путь к клиентам пока не зафиксирован. Для сегодняшнего кода вывод практический: писать под AVX-512 на клиентский CPU бессмысленно — на клиентах потолок остаётся AVX2 с 256-битными YMM.

> [!info]- x86-специфичные подводные камни: throttling и gather
> **Frequency throttling.** Активация тяжёлых 512-битных инструкций заставляет процессор снижать частоту: широкие ALU (арифметико-логические блоки, выполняющие умножение и сложение) потребляют больше энергии, и тепловые ограничения не позволяют держать номинальные гигагерцы. Если векторизованная секция коротка, потеря частоты может съесть выигрыш от широкого вектора. На Ice Lake и новее throttling ослаблен, но не исчез.
>
...
**Выравнивание** — про адрес начала. `vmovapd` (aligned load) требует адрес, кратный 32 байтам. `vmovupd` (unaligned) работает по любому адресу и на Haswell и новее почти не штрафуется — пока загрузка не пересекает границу [[computer/data-path/memory-hierarchy#Кеш-линия: минимальная единица обмена|кеш-линии]] (64 байта). Пересечение превращает одну загрузку в два обращения к [[computer/data-path/memory-hierarchy#Иерархия: от регистров до оперативной памяти|L1]]. Обычный `malloc` гарантирует 16-байтовое выравнивание; для 32-байтового (AVX) используется `aligned_alloc(32, size)` из C11 или `_mm_malloc(size, 32)` из Intel-заголовков.
