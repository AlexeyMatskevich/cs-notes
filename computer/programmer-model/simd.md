...
**Предпосылки:** [бит, байт](../../foundations/binary-and-bytes.md), [числа с плавающей точкой](../../foundations/floating-point.md) (`float` — 32 бита, `double` — 64 бита), [ABI и размещение данных](abi-and-data-layout.md) (xmm-регистры, выравнивание), [кеш-линия](../data-path/memory-hierarchy.md#кеш-линия-минимальная-единица-обмена) (64 байта — единица передачи между кешем и памятью).
...
В [ABI](abi-and-data-layout.md) стек выровнен по 16 байтам, и `movaps` (aligned load) требует этого выравнивания. Для 256-битных AVX-загрузок данные выигрывают от 32-байтового выравнивания: `vmovapd` (aligned) требует его, а `vmovupd` (unaligned) на современных [процессорах](../cpu.md) (Haswell+) не штрафуется для обращений внутри одной [кеш-линии](../data-path/memory-hierarchy.md#кеш-линия-минимальная-единица-обмена). Штраф возникает, когда 32-байтовая загрузка пересекает границу 64-байтовой [кеш-линии](../data-path/memory-hierarchy.md#кеш-линия-минимальная-единица-обмена) — в этом случае нужны два обращения к кешу вместо одного.
