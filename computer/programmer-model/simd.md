...
В [ABI](abi-and-data-layout.md) стек выровнен по 16 байтам, и `movaps` (aligned load) требует этого выравнивания. Для 256-битных AVX-загрузок данные выигрывают от 32-байтового выравнивания: `vmovapd` (aligned) требует его, а `vmovupd` (unaligned) на современных процессорах (Haswell+) не штрафуется для обращений внутри одной [кеш-линии](../data-path/memory-hierarchy.md). Штраф возникает, когда 32-байтовая загрузка пересекает границу 64-байтовой кеш-линии — в этом случае нужны два обращения к кешу вместо одного.
