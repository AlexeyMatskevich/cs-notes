...
**Предпосылки:** [бит, байт](../../foundations/binary-and-bytes.md), [числа с плавающей точкой](../../foundations/floating-point.md) (`float` — 32 бита, `double` — 64 бита), [ABI и размещение данных](abi-and-data-layout.md) (xmm-регистры, выравнивание), [[computer/data-path/memory-hierarchy#кеш-линия-минимальная-единица-обмена|кеш-линия]] (64 байта — единица передачи между кешем и памятью).

← [ABI и размещение данных](abi-and-data-layout.md) | [Атомарные инструкции](../atomic-instructions.md) →

Программы постоянно обрабатывают длинные одинаковые данные: умножить координаты тысяч вершин, применить фильтр к миллиону пикселей, просуммировать столбец чисел. Скалярный цикл делает это по одному элементу за инструкцию — и процессор тратит такт на каждое значение, даже если все элементы обрабатываются одинаково.
...
[[computer/cpu#суперскалярное-исполнение|Суперскалярность]] помогает: процессор выполняет load, mul и store параллельно на разных исполнительных блоках. Но фундаментально: каждая `mulsd` обрабатывает один `double`. Что если расширить регистр, чтобы одна инструкция умножала сразу несколько элементов?

## SIMD: расширяем путь данных

[[computer/cpu#суперскалярное-исполнение|Суперскалярность]] запускает **несколько разных инструкций** параллельно (load + mul + store на разных блоках). SIMD — другой вид параллелизма: **одна инструкция** обрабатывает **несколько одинаковых данных**. Это не многопоточность (разные потоки с разным кодом) и не суперскалярность (разные инструкции). Это расширение самого пути данных: вместо 64-битного регистра, где лежит один `double`, — 256-битный регистр, где лежат четыре `double` подряд, и одна инструкция `vmulpd` умножает все четыре пары за раз.
...
AVX дал 4 `double` за инструкцию, но циклы с ветвлениями и «хвостами» (когда длина массива не кратна 4) всё ещё плохо ложились на SIMD. AVX-512 (2017) удвоил ширину до 512 бит: 32 регистра ZMM, 8 `double` за инструкцию. И добавил mask-регистры (k0–k7): биты маски управляют, какие элементы вектора обрабатываются, а какие пропускаются. Это позволяет обрабатывать «хвосты» массивов и условные операции без ветвлений.
...
>
> **gather/scatter:** инструкции загрузки/записи по массиву индексов (`vgatherdpd`, `vscatterdpd`) медленнее, чем последовательные загрузки. На Skylake gather из 4 элементов исполняется как серия отдельных обращений к памяти и по пропускной способности сопоставим с 4 скалярными load — реального выигрыша не даёт. Ощутимый эффект gather начинает давать только на AVX-512 с 8+ элементами, когда амортизированная стоимость одной инструкции перестаёт доминировать.

## ARM: NEON и SVE

x86 расширяла ширину поколениями: 128 → 256 → 512 бит, каждый раз добавляя новые инструкции и регистры. Цена — перекомпиляция под каждое поколение и ветвление кода на runtime-dispatch. ARM столкнулся с той же задачей (переносимость SIMD-кода между поколениями), но решил её иначе.
...
В [ABI](abi-and-data-layout.md) стек выровнен по 16 байтам, и `movaps` (aligned load) требует этого выравнивания. Для 256-битных AVX-загрузок данные выигрывают от 32-байтового выравнивания: `vmovapd` (aligned) требует его, а `vmovupd` (unaligned) на современных [процессорах](../cpu.md) (Haswell+) не штрафуется для обращений внутри одной [[computer/data-path/memory-hierarchy#кеш-линия-минимальная-единица-обмена|кеш-линии]]. Штраф возникает, когда 32-байтовая загрузка пересекает границу 64-байтовой [[computer/data-path/memory-hierarchy#кеш-линия-минимальная-единица-обмена|кеш-линии]] — в этом случае нужны два обращения к кешу вместо одного.

Для динамически выделяемых массивов обычный `malloc` на большинстве платформ гарантирует только 16-байтовое выравнивание. Чтобы гарантировать 32-байтовое (для AVX) или 64-байтовое (для AVX-512), используют специализированные функции выделения памяти, которые возвращают указатель, кратный запрошенной границе: `aligned_alloc(32, size)` из стандарта C11 или `_mm_malloc(size, 32)` из заголовков Intel.

### AoS vs SoA

Массив структур (AoS, Array of Structures) плохо подходит для SIMD. Если 3D-точка хранится как структура из трёх полей `x`, `y`, `z` типа `double` (24 байта на точку), то в массиве таких точек координаты `x` разных точек разделены этими 24 байтами — между ними лежат `y` и `z` предыдущей точки. Одна SIMD-загрузка из четырёх `double` захватит `x`, `y`, `z` одной точки и `x` следующей — бесполезная смесь из разных полей.
...
Если данные лежат подряд и выровнены, компилятор часто может векторизовать цикл сам. Наш `c[i] = a[i] * b[i]` с `-O2 -march=native` компилируется в SIMD-инструкции целевого процессора без специальных функций в исходном коде. GCC включает автовекторизацию на `-O2` (`-ftree-vectorize`) начиная с GCC 12 (до GCC 12 — только на `-O3`), Clang — тоже. Флаг `-march=native` разрешает использовать все расширения текущего процессора.

Автовекторизация работает, когда компилятор может доказать, что итерации независимы (нет зависимости `c[i]` от `c[i-1]`), указатели не пересекаются — то есть массивы `a`, `b`, `c` действительно разные области памяти, а не разные указатели на один и тот же буфер (квалификатор `restrict` в C — это обещание программиста компилятору: «через этот указатель никто не прикоснётся к тем же данным, что и через другие»), — и цикл достаточно простой, без вызовов функций и сложных условий внутри.
...
На массивах, не помещающихся в кеш, к ограничению портов добавляется [пропускная способность памяти](../data-path/memory-hierarchy.md): DDR5-5600 в двух каналах даёт около 90 ГБ/с пиковой пропускной способности, и этот потолок не зависит от ширины SIMD-регистров. Для нашего цикла с 3 обращениями по 8 байт на элемент (24 байта на итерацию) и 1024 элементами вычислительный потолок (compute ceiling) с AVX при одной `vmulpd` за такт — 256 тактов; на архитектурах с двумя FMA-блоками потолок вдвое ниже. Но если массив в RAM, потолок пропускной способности (bandwidth ceiling) = 24 КБ / 90 ГБ/с ≈ 0.27 мкс — и ширина SIMD уже не определяет скорость.
...
**Непрямой доступ.** `c[index[i]] = a[i] * b[index[i]]` — scatter/gather. Адреса зависят от данных, и SIMD не может загрузить элементы одной быстрой инструкцией: на Skylake gather из 4 элементов внутри процессора разворачивается в серию отдельных обращений к памяти, так что по пропускной способности он сопоставим с 4 скалярными load — выигрыша над обычным циклом почти нет.
