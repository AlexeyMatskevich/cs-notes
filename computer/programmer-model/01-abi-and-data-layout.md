...
Внутри одной программы на одной машине endianness незаметен: процессор читает и пишет в своём порядке. Проблема возникает на границе: отправка по сети, запись в файл, межъязыковая сериализация. Современные x86-64 и AArch64-системы используют little-endian (младший байт по младшему адресу), сетевые протоколы (TCP/IP) — big-endian. Современные форматы сериализации (Protocol Buffers, MessagePack) абстрагируют endianness, но при работе с низкоуровневыми протоколами и бинарными форматами проблема остаётся актуальной.
