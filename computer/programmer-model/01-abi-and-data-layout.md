...
Структура `Order` — 24 байта, слишком много для передачи через регистры (точный порог и причину разберём в разделе о соглашении о вызовах). Поэтому `main` копирует `order` на стек перед вызовом. Для вызова `process_order(order)` стек выглядит так:
...
Причина — в аппаратуре. Процессор читает данные из памяти не побайтово, а кеш-линиями по 64 байта. Внутри кеш-линии контроллер памяти выдаёт данные выровненными порциями. Если 8-байтовый `double` лежит по адресу 0x04, он пересекает две 8-байтовые группы: байты 0x04–0x07 в одной, 0x08–0x0B — в другой. Процессор должен прочитать обе группы и склеить результат — это **split load (расщеплённое чтение)**. На x86-64 split load внутри одной кеш-линии стоит ~2–3 дополнительных такта. Если данные пересекают границу между двумя кеш-линиями (адрес 0x3C, данные заходят на кеш-линию с адреса 0x40), штраф составляет ~5–20 тактов в зависимости от микроархитектуры, потому что нужны два обращения к L1. Ещё хуже — пересечение границы страницы (4 КБ): процессору приходится транслировать два виртуальных адреса вместо одного, и штраф может достигать сотен тактов.
