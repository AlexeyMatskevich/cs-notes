...
На x86 атомарность обычной инструкции достигается добавлением префикса `LOCK`. Процессор захватывает кеш-линию в эксклюзивное владение (состояние Modified в [протоколе MESI](data-path/cache-coherency.md)), выполняет всю инструкцию и только после этого позволяет другим ядрам обращаться к этой линии.
...
Атомарная инструкция — это [когерентная](data-path/cache-coherency.md) транзакция. Чтобы выполнить неделимый read-modify-write, [процессор](cpu.md) должен получить эксклюзивный доступ к [кеш-линии](data-path/memory-hierarchy.md) на время операции.
...
Если линию держит другое ядро, [процессор](cpu.md) должен выполнить [когерентную](data-path/cache-coherency.md) транзакцию: послать запрос, дождаться передачи линии, получить её в Modified. Это стоит ~40-70 нс — те же десятки наносекунд, что описаны в [стоимости когерентности](data-path/cache-coherency.md). Когда два ядра непрерывно обращаются к одной линии атомарными инструкциями, возникает [ping-pong](data-path/cache-coherency.md) — линия мечется между кешами, и каждая операция стоит столько же, сколько промах в кеше.
