...
Есть и особый случай — двуместный CAS (double-width CAS): на x86 это `LOCK CMPXCHG16B`, работающий над двумя смежными 8-байтными словами. Он позволяет атомарно менять пару «указатель + счётчик» за одну операцию — так в неблокирующих (lock-free) структурах борются с ABA: значение по адресу успело смениться и вернуться к прежнему между двумя чтениями, а одиночный CAS подмены не замечает.
