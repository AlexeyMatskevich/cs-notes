...
На x86 атомарность обычной инструкции достигается добавлением префикса `LOCK`. Процессор захватывает кеш-линию в эксклюзивное владение (состояние Exclusive в [[computer/data-path/cache-coherency#протокол-mesi-четыре-состояния-кеш-линии|протоколе MESI]]), выполняет всю инструкцию — в ходе записи линия переходит в Modified — и только после этого позволяет другим ядрам обращаться к этой линии.
...
CAS-цикл выглядит дёшево — несколько инструкций и условный повтор. Но любая атомарная инструкция — это [[computer/data-path/cache-coherency|когерентная]] транзакция, и у неё есть цена. Чтобы выполнить неделимый read-modify-write, [процессор](cpu.md) должен получить эксклюзивный доступ к [[computer/data-path/memory-hierarchy|кеш-линии]] на время операции.
...
Если линию держит другое ядро, [процессор](cpu.md) должен выполнить [[computer/data-path/cache-coherency|когерентную]] транзакцию: послать запрос, дождаться передачи линии, получить её в Modified. Это стоит ~40-70 нс — те же десятки наносекунд, что описаны в [[computer/data-path/cache-coherency#цена-когерентности-в-наносекундах|стоимости когерентности]]. Когда два ядра непрерывно обращаются к одной линии атомарными инструкциями, возникает [[computer/data-path/cache-coherency#цена-когерентности-в-наносекундах|ping-pong]] — линия мечется между кешами, и каждая операция стоит столько же, сколько промах в кеше.
...
- [Упорядочение памяти](../linux/concurrency/memory-ordering.md) — какие гарантии порядка дают атомарные инструкции помимо неделимости
- [Примитивы синхронизации](../linux/concurrency/synchronization.md) — мьютексы, семафоры и другие абстракции, построенные поверх CAS
- [Lock-free структуры данных: проблема ABA](../linux/concurrency/lock-free.md#проблема-aba) — ситуация, когда CAS видит «то же» значение, а контекст изменился; центральная проблема lock-free программирования
- [Атомарность команд Redis](../databases/redis/atomicity/single-command.md) — атомарность на другом уровне: однопоточный event loop гарантирует неделимость целой команды без аппаратных примитивов
