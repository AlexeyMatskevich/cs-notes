...
Если линию держит другое ядро, процессор должен выполнить когерентную транзакцию: послать запрос, дождаться передачи линии, получить её в Modified. Это стоит ~40-70 нс — те же десятки наносекунд, что описаны в [стоимости когерентности](data-path/cache-coherency.md). Когда два ядра непрерывно обращаются к одной линии атомарными инструкциями, возникает [ping-pong](data-path/cache-coherency.md) — линия мечется между кешами, и каждая операция стоит столько же, сколько промах в кеше.
