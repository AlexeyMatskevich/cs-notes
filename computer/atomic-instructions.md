...
На x86 атомарность обычной инструкции достигается добавлением префикса `LOCK`. Процессор захватывает кеш-линию в эксклюзивное владение (состояние Modified в [[computer/data-path/cache-coherency|протоколе MESI]]), выполняет всю инструкцию и только после этого позволяет другим ядрам обращаться к этой линии.
...
Атомарная инструкция — это [[computer/data-path/cache-coherency|когерентная]] транзакция. Чтобы выполнить неделимый read-modify-write, [процессор](cpu.md) должен получить эксклюзивный доступ к [[computer/data-path/memory-hierarchy|кеш-линии]] на время операции.
...
Если линию держит другое ядро, [процессор](cpu.md) должен выполнить [[computer/data-path/cache-coherency|когерентную]] транзакцию: послать запрос, дождаться передачи линии, получить её в Modified. Это стоит ~40-70 нс — те же десятки наносекунд, что описаны в [[computer/data-path/cache-coherency|стоимости когерентности]]. Когда два ядра непрерывно обращаются к одной линии атомарными инструкциями, возникает [[computer/data-path/cache-coherency|ping-pong]] — линия мечется между кешами, и каждая операция стоит столько же, сколько промах в кеше.
