...
[WAL](../durability/00-wal.md) живёт в `pg_wal/`, CLOG (статусы транзакций для [MVCC](../concurrency/00-mvcc.md)) — в `pg_xact/`. При аварийном восстановлении PostgreSQL читает WAL и применяет изменения к страницам в main fork, используя `pd_lsn` каждой страницы для определения, какие WAL-записи уже применены.
