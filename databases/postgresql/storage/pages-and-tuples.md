...
← [ACID в PostgreSQL](acid.md) | [TOAST](toast.md) →
...
**HDD (жёсткий диск, [подробнее](../../../computer/data-path/storage.md)):** вращающиеся пластины и механическая головка. Для случайного чтения/записи устройству нужно «дойти» до нужного места, поэтому задержка одной операции измеряется миллисекундами.
...
Именно поэтому PostgreSQL много внимания уделяет двум вещам: читать меньше страниц (например, через индексы) и удерживать горячие страницы в памяти (см. [буферный кеш](../durability/buffer-cache.md)).
...
PostgreSQL избегает необратимых повреждений за счёт [WAL](../durability/wal.md). Правило простое: прежде чем изменённая страница попадёт на диск, на диск должны попасть соответствующие записи WAL (WAL before data). А чтобы torn page можно было восстановить даже после контрольной точки, PostgreSQL при первом изменении страницы после неё пишет в WAL полный образ страницы (Full Page Writes). Контрольная точка (checkpoint) и Full Page Writes подробнее описаны в [WAL](../durability/wal.md) и [буферном кеше](../durability/buffer-cache.md).
...
**Почему «куча»?** Потому что данные «свалены» без порядка, как вещи в куче. Это НЕ [heap как структура данных](../../../algorithms-and-data-structures/non-linear/heap.md) (двоичная куча для приоритетной очереди) и НЕ heap как область памяти в языках программирования.
...
- Заголовок (23 байта): xmin, xmax (идентификаторы транзакций, подробнее в [MVCC](../concurrency/mvcc.md)), ctid, флаги, информация о NULL
- Данные полей
...
Страницы стандартизируют I/O, но не гарантируют, что COMMIT переживёт сбой. Для этого нужен журнал упреждающей записи — [WAL](../durability/wal.md). А когда данные не помещаются в одну страницу, PostgreSQL использует [TOAST](toast.md).
