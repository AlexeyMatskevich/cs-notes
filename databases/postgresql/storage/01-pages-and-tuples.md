...
PostgreSQL избегает необратимых повреждений за счёт [WAL](../durability/00-wal.md). Правило простое: прежде чем изменённая страница попадёт на диск, на диск должны попасть соответствующие записи WAL (WAL before data). А чтобы torn page можно было восстановить даже после контрольной точки, PostgreSQL при первом изменении страницы после неё пишет в WAL полный образ страницы (Full Page Writes). Контрольная точка (checkpoint) и Full Page Writes подробнее описаны в [WAL](../durability/00-wal.md) и [буферном кеше](../durability/01-buffer-cache.md).
