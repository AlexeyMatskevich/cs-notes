...
← [EXPLAIN](explain.md) | [Prepared statements](prepared-statements.md) →
...
Если сортировка появляется из‑за `ORDER BY`, первое, что стоит проверить: может ли индекс отдать строки в нужном порядке. Для типового случая это [B-tree](../indexes/btree.md) по ключу сортировки или составной индекс, который покрывает фильтр и порядок.
