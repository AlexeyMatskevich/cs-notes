...
SQL — декларативный язык. Запрос описывает *что* получить, а не *как*. Один и тот же запрос можно выполнить десятками способов, которые дадут одинаковый результат, но время выполнения может отличаться в тысячи раз.

Планировщик решает задачу: из множества возможных планов выполнения выбрать тот, который потребует минимум ресурсов. Масштаб задачи виден на простом примере: запрос с тремя таблицами и JOIN между ними даёт 3! = 6 вариантов порядка соединения, для каждого — три алгоритма (nested loop, hash, merge), плюс выбор метода доступа к каждой таблице (seq scan, index scan, bitmap scan). Даже такой простой запрос может иметь сотни возможных планов.
