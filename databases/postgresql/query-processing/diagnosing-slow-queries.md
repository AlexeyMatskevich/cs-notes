...
← [Prepared statements](prepared-statements.md) | [Репликация](../distribution/replication.md) →
...
- параметризованный запрос получил generic plan и перестал учитывать конкретное значение (см. [prepared statements](prepared-statements.md)).
...
Когда проблема в порядке соединения, помогает понимание [join ordering](join-order.md) и поведения [подзапросов/CTE](subqueries-and-cte.md).
...
Про индексы под форму запроса — в [B-tree](../indexes/btree.md) и в заметках по типам индексов.
...
Подробности того, как это выглядит и почему так происходит — в [память и spill](memory-and-spill.md).
...
Признак — один и тот же plan shape в приложении при принципиально разных входах. Разбор причин и терминов — в [prepared statements](prepared-statements.md).
