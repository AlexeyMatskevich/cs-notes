...
Длинные транзакции мешают не только VACUUM — они держат [табличные блокировки](locks.md), которые каскадно блокируют DDL и все последующие запросы. Дежурный инженер запускает миграцию, а на таблице работает длинный аналитический SELECT.
