...
Транзакции не блокируют друг друга до момента записи. Цена — при конфликте вся работа транзакции теряется и повторяется. Retry с exponential backoff — общий [паттерн надёжности](../../../system-design/06-reliability-patterns.md); здесь он применяется к ошибкам сериализации.
...
Паттерны описывают правильные подходы. [Распространённые ошибки](05-common-mistakes.md) показывают, что бывает, когда эти подходы игнорируются или применяются неверно.
