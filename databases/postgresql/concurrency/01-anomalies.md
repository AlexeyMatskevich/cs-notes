...
**Предпосылки:** [MVCC](00-mvcc.md). Примеры используют Ruby/ActiveRecord — синтаксис интуитивно понятен, но для полного понимания полезно знание [основ SQL](../../sql/index.md).
...
PostgreSQL даёт более сильные гарантии на некоторых уровнях. REPEATABLE READ в PostgreSQL использует Snapshot Isolation с фиксированным snapshot на всю транзакцию — это предотвращает и phantom read, хотя стандарт его допускает. Как PostgreSQL реализует каждый уровень через политику snapshot и реакцию на конфликт записи — в [уровнях изоляции](02-isolation-levels.md).
