...
← [MVCC](00-mvcc.md) | [Уровни изоляции](02-isolation-levels.md) →
...
SQL-стандарт не упоминает lost update явно — эта аномалия возникает из-за паттерна в приложении, а не из-за самой СУБД. Защита от lost update на практике — через `SELECT ... FOR UPDATE` или `REPEATABLE READ` с retry-логикой — рассматривается в [паттернах параллельного доступа](04-patterns.md).
...
Стандарт не упоминает lost update и write skew — эти аномалии не укладываются в таблицу, потому что зависят от паттернов в приложении (read-modify-write) и структуры данных (пересекающиеся чтения).

Стандарт определяет **минимальные** требования. СУБД может давать более сильные гарантии: PostgreSQL на уровне REPEATABLE READ использует Snapshot Isolation, которая предотвращает и phantom read — это строже стандарта. Как PostgreSQL реализует каждый уровень через политику snapshot и реакцию на конфликт записи — в [уровнях изоляции](02-isolation-levels.md).
