...
Без координации чтение между шардами становится eventually consistent — [модели консистентности](../../../system-design/consistency-models.md) описывают спектр промежуточных гарантий.
...
[VACUUM](../maintenance/vacuum.md), бэкапы, мониторинг pg_stat, обновление версии PostgreSQL — всё умножается на число шардов. Один «забытый» шард с распухшими таблицами может неделями деградировать, пока не заполнит диск.
...
Consistent hashing минимизирует объём миграции: при добавлении узла перемещается ~1/N данных вместо большинства. Механизм подробнее описан в контексте [распределённого кэша](../../../system-design/caching.md).
