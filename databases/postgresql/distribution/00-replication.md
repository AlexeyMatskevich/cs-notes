...
## Replication slots: WAL не удаляется, пока нужен

Replication slot — маркер, который говорит primary: «эти WAL-данные ещё не дошли до реплики, не удаляй их». Это защищает от ситуации «реплика отстала, а WAL уже удалён, и нужен полный resync».

Обратная сторона: если реплика умерла навсегда, а слот остался — WAL копится на primary. В production это регулярная причина «диск заполнен на 100%»: 500 ГБ WAL накопились за выходные, пока никто не заметил мёртвую реплику. Слоты нужно мониторить (`pg_replication_slots`) и удалять, когда они больше не нужны.
