...
До PostgreSQL 10 существовало и третье ограничение: hash index не писался в [WAL](../durability/wal.md). При crash индекс повреждался и требовал `REINDEX`. Это делало hash непригодным для production. С PostgreSQL 10 WAL полностью поддерживается — ограничение снято.
