...
При росте данных bucket'ы переполняются и обрастают длинными цепочками overflow — поиск замедляется. В обычной хеш-таблице в памяти эту проблему решает [rehashing](../../../algorithms-and-data-structures/linear/05-hash-table.md): массив удваивается, все элементы перехешируются за O(n). На диске полная перестройка означала бы перезапись всех страниц индекса — для нашего 1.9 ГБ hash index это неприемлемо.
...
До PostgreSQL 10 существовало и третье ограничение: hash index не писался в [WAL](../durability/00-wal.md). При crash индекс повреждался и требовал `REINDEX`. Это делало hash непригодным для production. С PostgreSQL 10 WAL полностью поддерживается — ограничение снято.
...
Hash — нишевый инструмент для точечных запросов по длинным ключам. Для огромных append-only таблиц с естественной корреляцией данных существует самый компактный тип индекса — [BRIN](04-brin.md).
