...
Consumer group (группа потребителей) позволяет нескольким обработчикам читать один поток параллельно: каждое сообщение доставляется ровно одному обработчику в группе. Это аналог consumer group в Kafka.
