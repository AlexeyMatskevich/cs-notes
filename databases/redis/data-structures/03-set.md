...
**listpack** (Redis 7.2+) используется для небольших множеств со строковыми элементами — по тому же принципу, что и в [Hash](01-hash.md) и [Sorted Set](04-sorted-set.md). Элементы хранятся последовательно в одном блоке памяти без указателей. Пороги переключения: `set-max-listpack-entries` (по умолчанию 128) и `set-max-listpack-value` (по умолчанию 64 байта).
