...
`HGETALL` на хеше с десятками тысяч полей возвращает мегабайты данных за одну команду. Пока Redis формирует ответ, однопоточный [event loop](../architecture/01-event-loop.md) не обрабатывает другие команды. На хеше из 10 000 полей это может занять несколько миллисекунд — незаметно для одного клиента, но блокирует все остальные.
...
STRING-ключи истекают по отдельности: `user:123:session` живёт 30 минут, `user:123:name` — бессрочно. Поля хеша лишены этой гибкости. Redis позволяет установить TTL только на весь ключ целиком — отдельные поля хеша не могут иметь собственный TTL.

Это не ограничение реализации, а следствие дизайна: поля хеша — части одного объекта, и Redis управляет временем жизни на уровне объектов. На практике это означает выбор из нескольких стратегий. Если каждому полю нужен свой TTL (например, кеш с разными сроками) — приходится возвращаться к отдельным STRING-ключам, принимая overhead. Если нужна периодическая очистка полей — Lua-скрипт, который обходит поля и удаляет устаревшие по метке времени. Если TTL нужен одинаковый на весь объект — `EXPIRE` на ключ хеша.

## Две кодировки: listpack и hashtable

Маленькие хеши в Redis компактнее, чем можно ожидать. При малом количестве полей Redis использует listpack (`src/listpack.c`) — последовательный блок памяти, где поля и значения записаны друг за другом без указателей и заголовков. Поиск по полю — линейный обход (O(n)), но при нескольких десятках полей данные лежат в нескольких кеш-линиях процессора, и линейный обход оказывается быстрее хеш-таблицы с её разбросанными по памяти узлами. Хеш из 50 полей в listpack занимает примерно в 3 раза меньше памяти, чем в hashtable.

Когда количество полей превышает `hash-max-listpack-entries` (по умолчанию 128) или размер любого поля/значения превышает `hash-max-listpack-value` (по умолчанию 64 байта), Redis переключает кодировку на полноценную хеш-таблицу (`src/dict.c`) с O(1) доступом по полю. Переключение одностороннее: даже если потом удалить поля и хеш снова станет маленьким, обратной конвертации в listpack не произойдёт. Если удалить все поля — ключ удаляется автоматически: пустых хешей в Redis не бывает.
