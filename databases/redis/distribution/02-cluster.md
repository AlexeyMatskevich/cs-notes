...
```text
ключ ─→ слот ─→ мастер (владелец слота) ─→ реплика(и)
           ↑
         клиент (кэширует карту «слот → узел», умеет перенаправления)

узлы обмениваются служебными сообщениями
```
...
Redis Cluster делит пространство ключей на 16 384 слота. Принадлежность ключа к слоту определяется хешированием: Cluster использует хеш-функцию CRC16 и берёт остаток от деления на 16 384.
...
`SELECT` — команда одиночного Redis, которая переключает «логическую базу» (db `0`, `1`, ...). Иногда базы используют как простой способ разделить данные в одном Redis: например, приложение пишет в db 0, а тесты/временные ключи — в db 1.

```bash
# Одиночный Redis (не Cluster)
...
В Redis Cluster баз нет: всегда используется db 0, а `SELECT` запрещён. Это важно, если у вас в конфигурации клиента указан `db != 0` (или вы привыкли к `redis-cli -n`): такой подход в Cluster не работает, и данные придётся разделять иначе.

Обычно разделение делают префиксами в ключах (например, `app1:user:123`) или разными экземплярами/кластерами.

Pub/Sub в классическом виде рассылает каждое сообщение на все узлы. При большом числе узлов и высокой частоте публикаций это создаёт много сетевого трафика. [Sharded Pub/Sub](../data-structures/08-pub-sub.md) (Redis 7.0+) решает эту проблему, привязывая каналы к слотам.
...
Этот параметр — прямой выбор между CP и AP при partition: `yes` останавливает весь кластер ради консистентности, `no` продолжает обслуживать доступные слоты ценой неполных данных — подробнее в [CAP-теореме](../../../system-design/00-cap-theorem.md).

## Когда нужен Cluster

Если данных достаточно для одного узла, но хочется автоматического failover, обычно проще начать с репликации + [Sentinel](01-sentinel.md): клиентам не нужна поддержка Cluster на уровне библиотек, а ограничения Cluster не мешают.
...
Redis Cluster использует фиксированное число слотов (16 384) и простой хеш (`CRC16(key) mod 16384`). Альтернативный подход — [consistent hashing](../../../system-design/07-caching.md#consistent-hashing) — минимизирует перемещение ключей при добавлении узлов, но сложнее в реализации.
