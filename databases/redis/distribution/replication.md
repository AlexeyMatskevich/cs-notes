...
← [Проектирование ключей](../memory/key-design.md) | [Sentinel](sentinel.md) →

## В Redis: master и replicas

Один Redis-сервер — единая точка отказа: при его падении приложение теряет и данные, и доступ к кешу. Репликация создаёт копии данных на других серверах. В Redis мастер принимает запись, а реплики получают snapshot + поток команд. Это даёт две вещи: «горячую» копию для переключения при сбое (автоматически — через [Sentinel](sentinel.md)) и возможность разгрузить мастер чтениями. Общая теория и компромиссы репликации — в [отдельной заметке](../../../system-design/replication.md).

```mermaid
flowchart LR
    CW["clients (writes)"] --> M["master"]
    CR["clients (reads)"] -.->|"опционально"| R["replicas"]
    M -->|"snapshot + поток команд"| R
```
...
Создание snapshot — самая затратная часть. Мастер запускает фоновое сохранение (`BGSAVE`), которое создаёт дочерний процесс через `fork()` с [copy-on-write](../../../linux/foundations/processes.md) — физические страницы не копируются, пока родитель не изменит их. Чем активнее запись в мастер во время `BGSAVE`, тем больше страниц копируется: растёт потребление RAM пропорционально объёму изменений. Помимо памяти, передача snapshot нагружает сеть и (в disk-based варианте) диск.

Если диск медленный, а сеть быстрая, Redis может обойти запись на диск: diskless-репликация (`repl-diskless-sync yes`) стримит [RDB](../persistence/rdb.md)-данные напрямую в сокет реплики.
