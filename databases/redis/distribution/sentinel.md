...
← [Репликация](replication.md) | [Cluster](cluster.md) →
...
В production Sentinel запускают несколькими экземплярами — минимум 3, и Redis официально рекомендует нечётное число. Почему именно нечётное — объяснено в [[system-design/replication#кворум-как-узлы-определяют-кто-изолирован|репликации]]: чётное число (например, 4) не увеличивает отказоустойчивость, а при разделении пополам ни одна сторона не набирает большинства и failover невозможен. Один Sentinel — единая точка отказа: если он недоступен, клиентам не у кого спросить «кто сейчас мастер», а автоматическое голосование не состоится.
...
Sentinel решает задачу высокой доступности (HA): автоматическое обнаружение сбоя и failover. Но он не делит данные между узлами — все данные по-прежнему на каждом сервере. Для горизонтального масштабирования нужен [Cluster](cluster.md).

Потеря данных при failover остаётся возможной — это следствие [[databases/redis/distribution/replication#асинхронность-устаревшие-чтения-и-окно-потери|асинхронной репликации]]: команды, не дошедшие до реплик до момента падения мастера, теряются при промоутировании. Масштаб потери ограничен окном асинхронности, но для критичных операций стоит использовать `WAIT` и/или настройки кворума реплик на запись.
