...
**Предпосылки:** [распределение данных](../../distribution.md), [RDB](../persistence/00-rdb.md), [AOF](../persistence/01-aof.md), сеть (клиент/сервер).

## В Redis: master и replicas

Общая теория репликации и её компромиссов — в [распределении данных](../../distribution.md). Здесь — особенности реализации в Redis.
...
В Redis репликация по умолчанию асинхронная: мастер отвечает клиенту, не дожидаясь подтверждения от реплик. Общие последствия (lag и окно потери вокруг failover) описаны в [распределении данных](../../distribution.md).
