...
Создание snapshot — самая затратная часть. Мастер запускает фоновое сохранение (`BGSAVE`), которое создаёт дочерний процесс через `fork()` с [copy-on-write](../../../linux/foundations/02-processes.md) — физические страницы не копируются, пока родитель не изменит их. Чем активнее запись в мастер во время `BGSAVE`, тем больше страниц копируется: растёт потребление RAM пропорционально объёму изменений. Помимо памяти, передача snapshot нагружает сеть и (в disk-based варианте) диск.
