...
На миллионе ключей это блокирует [event loop](../architecture/01-event-loop.md) на десятки миллисекунд. На десяти миллионах — на сотни. Пока `KEYS` работает, все остальные клиенты ждут: Redis не может обработать ни одну команду, пока полное сканирование не завершится.
...
При отладке расхода памяти или неожиданного поведения eviction полезно понять, что происходит с конкретным ключом: какую кодировку Redis выбрал, сколько памяти ключ занимает, и как часто к нему обращаются.
