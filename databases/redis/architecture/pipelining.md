...
← [Event loop](event-loop.md) | [Логические базы](logical-databases.md) →
...
Pipelining — это отправка нескольких команд подряд, не дожидаясь ответа на каждую. Клиент записывает N команд в TCP-соединение, затем читает N ответов. Redis получает команды и выполняет их последовательно — в зависимости от объёма данных это может занять один или несколько оборотов [event loop](event-loop.md).
...
Если нужна гарантия, что набор команд выполнится без вклинивания, используется [MULTI/EXEC](../atomicity/multi-exec.md) — все команды между MULTI и EXEC выполняются как единое целое. MULTI/EXEC можно комбинировать с pipelining: отправить MULTI, команды и EXEC одним пакетом, получив и атомарность, и экономию на сетевых задержках.
...
Pipelining работает, когда команды независимы — их можно отправить все сразу, не глядя на результаты. Если каждая следующая команда зависит от результата предыдущей (прочитать значение, посчитать что-то на его основе, записать результат), отправить их одним пакетом невозможно. В таких случаях нужен либо [Lua-скрипт](../atomicity/lua-scripting.md), который выполнит всю логику на стороне Redis, либо несколько последовательных round-trip'ов.
