...
Отдельная категория — [Lua-скрипты](../atomicity/lua-scripting.md) (`EVAL`): Redis позволяет отправить серверу программу на встроенном языке Lua, которая выполнится атомарно. Пока скрипт работает, никакие другие команды не обрабатываются. Длинный скрипт — длинная блокировка для всех клиентов.
