...
Кэш ускоряет чтение, но добавляет задачу поддержания согласованности с основным хранилищем. [Архитектура кэширования](../../../system-design/07-caching.md) описывает общую модель: слои кэша, инвалидацию, trade-off между свежестью и скоростью. Здесь — конкретные паттерны реализации в Redis: как обрабатывать промахи, как избежать stampede, как обновлять кэш при записи.
