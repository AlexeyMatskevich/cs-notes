...
-- если обработчик упал — сообщение остаётся в processing-списке
-- отдельный процесс (monitor) периодически проверяет:
LRANGE queue:emails:processing 0 -1
-- элементы, висящие дольше таймаута, возвращаются в основную очередь

Чтобы понимать «висящие дольше таймаута», нужен источник времени начала обработки (например, ZSET `queue:emails:processing:ts` со score = timestamp начала, или timestamp внутри payload).
```
...
- [Практика в Ruby/Rails: LIST](../../../rails/redis/02-data-structures-in-practice.md#list) — LPUSH/BRPOP, очереди и capped lists
