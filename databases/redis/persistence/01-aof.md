...
Подход напоминает журнал упреждающей записи ([WAL в PostgreSQL](../../postgresql/durability/00-wal.md)), но с важным отличием: Redis пишет в AOF после выполнения команды, а не до. Если Redis упал между выполнением команды и записью в лог — команда выполнена в памяти, но в AOF не попала. При перезапуске эта команда будет потеряна. Именно поэтому даже `appendfsync always` не даёт абсолютной гарантии — окно в одну команду остаётся.
