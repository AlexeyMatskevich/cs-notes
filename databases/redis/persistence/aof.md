...
`write()` помещает данные в page cache, а `fsync()` сбрасывает их на диск ([механика write/fsync](../../../linux/foundations/filesystems.md#fsync-явная-гарантия-durability)). Redis предоставляет три режима, определяющих, как часто вызывается `fsync`.
