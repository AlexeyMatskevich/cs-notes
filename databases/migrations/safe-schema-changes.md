...
> [!info]- Предпосылки
> [Таблицы и типы](../sql/schema/tables-and-types.md) (ALTER TABLE, DEFAULT), [ограничения](../sql/schema/constraints.md) (NOT NULL, FK, CHECK), [индексы](../sql/schema/indexes.md) (CREATE INDEX), [транзакции](../sql/modification/transactions.md) (BEGIN/COMMIT/ROLLBACK), [индексы в production](../sql/postgresql/index-operations.md) (CONCURRENTLY, INVALID), [блокировки](../postgresql/concurrency/locks.md) (уровни блокировок, очередь, lock_timeout).

← [Обзор серии](migrations.md) | [Эволюция схемы](schema-evolution.md) →
...
Для FK — VALIDATE сканирует обе таблицы (дочернюю и родительскую). На больших таблицах VALIDATE конфликтует с [VACUUM](../postgresql/maintenance/vacuum.md) (фоновая очистка мёртвых версий строк) и другим DDL — долгий VALIDATE может задержать очистку.
...
Компромисс — build в период низкого трафика без приостановки writes. Окно для дубликатов короче, но не закрыто: если дубликат проскочит, PostgreSQL может начать отклонять часть новых writes ещё до завершения команды, затем build упадёт и оставит INVALID-индекс. Cleanup rule тот же самый: сначала `DROP INDEX CONCURRENTLY` сломанного индекса, только потом разбирать данные, повторять build или возвращать обычный write traffic.
...
Версии PG, ограничения паттернов (plain B-tree для USING INDEX, партиционированные таблицы) — в таблице стоимости выше и в [обзоре серии](migrations.md).

Этого файла достаточно, чтобы оценить риск отдельной DDL-команды. Но реальная [миграция](migrations.md) редко состоит из одной команды: между `ADD COLUMN`, backfill и `SET NOT NULL` продолжает работать старый код. Следующая заметка — про этот уже не DDL-, а workflow-уровень: как согласовать схему, данные и rollout приложения между шагами.
