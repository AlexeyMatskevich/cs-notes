...
Миграции — дисциплина безопасного изменения схемы на рабочей базе данных: с данными, трафиком и несколькими версиями кода одновременно. DDL (Data Definition Language — ALTER TABLE, CREATE INDEX, DROP TABLE) описывает *что* менять. Миграции — *как* менять безопасно, не блокируя DML (Data Manipulation Language — INSERT, UPDATE, DELETE).
...
Безопасные паттерны из серии предполагают:

1. **Прямое подключение к PostgreSQL**, не через PgBouncer (connection pooler — промежуточный слой между приложением и PostgreSQL, переиспользующий соединения) в transaction mode. Session-level `SET` (lock_timeout, statement_timeout) и [advisory locks](../postgresql/concurrency/03-locks.md) привязаны к backend-соединению (серверный процесс PostgreSQL, обслуживающий клиента); transaction pooling переназначает соединения между транзакциями, и настройки теряются.

2. **DDL вне транзакции.** [CREATE INDEX CONCURRENTLY](../sql/postgresql/04-index-operations.md) и REINDEX CONCURRENTLY не работают внутри BEGIN/COMMIT. Migration runner не должен оборачивать такие операции в транзакцию.
...
Если среда не удовлетворяет этим условиям, безопасные паттерны не гарантируют безопасность.
