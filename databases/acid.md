...
**Предпосылки:** SQL ([BEGIN, COMMIT, ROLLBACK](sql/modification/01-transactions.md)), понятие транзакции как группы операций.
...
Consistency в ACID не следует путать с consistency в [CAP-теореме](../system-design/00-cap-theorem.md) и [моделях консистентности](../system-design/01-consistency-models.md): там речь о согласованности копий данных между узлами распределённой системы, здесь — о валидности состояния внутри одного узла. Совпадение термина — историческая случайность.
...
Не все системы хранения предоставляют полный ACID. [Redis](redis/index.md), Memcached, многие NoSQL-системы осознанно жертвуют частью гарантий ради производительности и простоты. Выбор между ACID и [ослабленными гарантиями (BASE)](../system-design/10-storage-selection.md) определяется ценой ошибки: двойное списание денег — инцидент, устаревший счётчик просмотров — допустимо.
