...
PostgreSQL автоматически создаёт уникальный [индекс](04-indexes.md) для PRIMARY KEY. Почти каждая таблица должна иметь PRIMARY KEY.
...
UNIQUE запрещает дубликаты в столбце (или комбинации столбцов). PostgreSQL реализует UNIQUE через уникальный [индекс](04-indexes.md).
