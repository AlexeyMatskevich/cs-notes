...
Нормализация возвращается к [реляционной модели](foundations/00-relational-model.md) и отвечает на вопрос «как далеко разделять таблицы». Она не зависит от выражений — её можно читать сразу после реляционной модели.
...
Безопасное изменение схемы на рабочей базе данных с данными и трафиком. Требует знания DDL, транзакций и [PG-специфики](postgresql/index.md) (блокировки, CONCURRENTLY).
