...
UNION (англ. «объединение») — SQL-реализация операции объединения из реляционной алгебры (см. [[databases/sql/foundations/relational-model#реляционная-алгебра|реляционная модель]]). Объединяет результаты двух запросов, **удаляя дубликаты**:
...
Как и в DISTINCT, операции над множествами считают два NULL **одинаковыми** — [[databases/sql/foundations/types-and-null#null-в-разных-контекстах|то же поведение]], что и в GROUP BY. UNION удалит дубликат из двух строк (NULL), INTERSECT их сопоставит, EXCEPT вычтет.
