...
← [Сортировка и ограничение](sorting-and-limiting.md) | [Соединения](joins.md) →
...
Все агрегатные функции **игнорируют [[databases/sql/foundations/types-and-null#null--отсутствие-значения|NULL]]** (кроме `COUNT(*)`): NULL означает «значение неизвестно», и включать неизвестное в сумму бессмысленно. SUM сложил 90000+60000+85000+70000+55000 = 360000. AVG (от average, «среднее») поделил на 5, не на 6 — Дина пропущена. Это важно: если бы AVG делил на 6, средняя была бы 60000 вместо 72000.
...
AVG для `department_id=1` = (90000 + 85000) / 2 = 87500. Дина (NULL) игнорируется — делим на 2, не на 3. NULL-значения `department_id` образуют **одну группу**, [[databases/sql/foundations/types-and-null#null-в-разных-контекстах|аналогично DISTINCT]].
...
Подробнее о режиме 2 и чисто оконных функциях — в [оконных функциях](window-functions.md). В PostgreSQL при больших объёмах данных агрегация может не поместиться в память — подробнее в [memory и spill](../../postgresql/query-processing/memory-and-spill.md).
