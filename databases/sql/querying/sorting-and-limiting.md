...
← [SELECT и фильтрация](select-and-filtering.md) | [Агрегация](aggregation.md) →

Данные выбраны и отфильтрованы — теперь менеджер хочет их **презентовать**: показать топ по зарплате, разбить на страницы, убрать дубликаты. Результат запроса без ORDER BY — неупорядоченная коллекция строк: в [[databases/sql/foundations/relational-model#свойства-отношений|реляционной модели]] отношение не имеет порядка, и без явной инструкции СУБД ничего не гарантирует. Порядок вывода произволен и может меняться. Подробнее о пагинации см. в отдельной заметке [пагинация](pagination.md).
...
Все предыдущие шаги выполняются полностью, прежде чем LIMIT/OFFSET отбросит лишнее. Сортировка больших объёмов данных требует памяти — в PostgreSQL механизм выделения и сброса на диск описан в [memory и spill](../../postgresql/query-processing/memory-and-spill.md). Это объясняет проблему **пагинации** через OFFSET: страница 1 — `LIMIT 10 OFFSET 0`, страница 2 — `LIMIT 10 OFFSET 10`. Чтобы показать страницу 1000, PostgreSQL должен пройти шаги 1–4 целиком — отсортировать все строки — и лишь затем пропустить 9990. Чем глубже страница, тем дороже запрос. Для больших таблиц существует keyset pagination — см. [пагинация](pagination.md).
...
Задача «первая строка в каждой группе» (например, самый высокооплачиваемый в каждом отделе) потребует инструментов, которые рассмотрены в [оконных функциях](window-functions.md).
