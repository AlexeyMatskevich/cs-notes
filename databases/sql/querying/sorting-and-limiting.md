...
Данные выбраны и отфильтрованы — теперь менеджер хочет их **презентовать**: показать топ по зарплате, разбить на страницы, убрать дубликаты. Результат запроса без ORDER BY — неупорядоченная коллекция строк: в [реляционной модели](../foundations/relational-model.md#свойства-отношений) отношение не имеет порядка, и без явной инструкции СУБД ничего не гарантирует. Порядок вывода произволен и может меняться.
