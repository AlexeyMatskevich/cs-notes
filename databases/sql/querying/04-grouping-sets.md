...
NULL в столбцах `department_id` и `year` означает «по всем значениям этого измерения». Но как отличить NULL-итог от настоящего NULL в данных?
...
GROUPING SETS, ROLLUP и CUBE — расширения GROUP BY на шаге 3. Они генерируют несколько уровней группировки в одном проходе:

```
1. FROM + JOIN     -- источник строк
2. WHERE           -- фильтрация строк
3. GROUP BY        -- группировка (GROUPING SETS генерирует
   GROUPING SETS      несколько уровней)
...
GROUPING() доступна в SELECT и HAVING для различения итогов и реальных NULL.
