...
Все предыдущие шаги выполняются полностью, прежде чем LIMIT/OFFSET отбросит лишнее. Это объясняет проблему **пагинации** через OFFSET: страница 1 — `LIMIT 10 OFFSET 0`, страница 2 — `LIMIT 10 OFFSET 10`. Чтобы показать страницу 1000, PostgreSQL должен пройти шаги 1–4 целиком — отсортировать все строки — и лишь затем пропустить 9990. Чем глубже страница, тем дороже запрос. Для больших таблиц существует keyset pagination — см. [пагинация](../../postgresql/query-processing/07-pagination.md).
