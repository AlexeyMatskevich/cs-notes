...
Данные выбраны и отфильтрованы — теперь менеджер хочет их **презентовать**: показать топ по зарплате, разбить на страницы, убрать дубликаты. Результат запроса без ORDER BY — неупорядоченная коллекция строк. Порядок вывода произволен и может меняться.
...
Три уникальных значения: 1, 2 и NULL. DISTINCT считает два NULL **одинаковыми** (дубликатами). Это отличается от поведения `=`, где `NULL = NULL` даёт NULL. DISTINCT использует специальный механизм «IS NOT DISTINCT FROM».
