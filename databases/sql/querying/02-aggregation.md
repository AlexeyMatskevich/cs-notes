...
Все агрегатные функции **игнорируют NULL** (кроме `COUNT(*)`). SUM сложил 90000+60000+85000+70000+55000 = 360000. AVG поделил на 5, не на 6 — Дина пропущена. Это важно: если бы AVG делил на 6, средняя была бы 60000 вместо 72000.
...
AVG для `department_id=1` = (90000 + 85000) / 2 = 87500. Дина (NULL) игнорируется — делим на 2, не на 3. NULL-значения `department_id` образуют **одну группу**, аналогично DISTINCT.
...
GROUP BY вставляется между WHERE и SELECT — сначала фильтрация строк, потом схлопывание в группы:
...
«Покажи только отделы, где средняя зарплата больше 70000». Попробуем `WHERE AVG(salary) > 70000` — ошибка: `aggregate functions are not allowed in WHERE`. Почему? WHERE выполняется на шаге 2, а групп на этом шаге ещё не существует — агрегировать нечего.
...
«Покажи в отчёте одну строку на отдел с именами сотрудников через запятую». `string_agg(expression, delimiter)` собирает значения в строку через разделитель:
...
Если результат нужен не для отчёта, а для API — `jsonb_agg(expression)` собирает значения в JSON-массив: `jsonb_agg(name)` --> `["Анна", "Вера", "Дина"]`.
