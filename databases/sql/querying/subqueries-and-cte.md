...
Подзапрос в FROM выполняется на шаге 1 [порядка выполнения](aggregation.md#полный-порядок-выполнения-запроса) — он становится источником строк, как обычная таблица. Подзапрос в FROM обязательно имеет псевдоним (AS dept_stats).
...
Подзапрос возвращает `{1, 2, NULL}`. Для любого `department_id = X` условие `X NOT IN (1, 2, NULL)` раскрывается как `X <> 1 AND X <> 2 AND X <> NULL`. Последнее сравнение `X <> NULL` --> NULL ([трёхзначная логика](../foundations/types-and-null.md#трёхзначная-логика)), и всё выражение --> NULL. Результат: **ноль строк**.
