...
Это следствие [порядка выполнения](aggregation.md#полный-порядок-выполнения-запроса): ON выполняется на шаге 1 вместе с FROM, а WHERE — на шаге 2, когда LEFT JOIN уже подставил NULL-строки.
