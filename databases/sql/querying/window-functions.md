...
FILTER (описан в [агрегации](aggregation.md)) работает и с оконными агрегатами:
...
В PostgreSQL top-1 в группе решается короче — [DISTINCT ON](../postgresql/distinct-on.md). Альтернативный подход к top-N через коррелированный подзапрос в FROM — [[databases/sql/querying/subqueries-and-cte#lateral--подзапрос-с-доступом-к-внешним-строкам|LATERAL]].
...
В PostgreSQL оконные функции с большими секциями потребляют значительную память — подробнее в [memory и spill](../../postgresql/query-processing/memory-and-spill.md).
