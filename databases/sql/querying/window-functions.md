...
В PostgreSQL top-1 в группе решается короче — [DISTINCT ON](../postgresql/distinct-on.md). Альтернативный подход к top-N через коррелированный подзапрос в FROM — [LATERAL](subqueries-and-cte.md#lateral--подзапрос-с-доступом-к-внешним-строкам).
