...
`=` (равно), `<>` или `!=` (не равно), `<`, `>`, `<=`, `>=`. Равенство — `=`, не `==`. Условия комбинируются через `AND`, `OR`, `NOT` — их поведение с NULL описано в [[databases/sql/foundations/types-and-null#логические-операторы-и-null|типах и NULL]].
