...
UNION (англ. «объединение») — SQL-реализация операции объединения из [реляционной алгебры](../foundations/00-relational-model.md#реляционная-алгебра). Объединяет результаты двух запросов, **удаляя дубликаты**:
...
Как и в DISTINCT, операции над множествами считают два NULL **одинаковыми** — [то же поведение](../foundations/01-types-and-null.md#null-в-разных-контекстах), что и в GROUP BY. UNION удалит дубликат из двух строк (NULL), INTERSECT их сопоставит, EXCEPT вычтет.
