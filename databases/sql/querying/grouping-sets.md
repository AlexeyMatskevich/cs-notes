...
NULL в столбцах `department_id` и `year` означает «по всем значениям этого измерения». Но [NULL уже означает «значение неизвестно»](../foundations/types-and-null.md#null--отсутствие-значения). Пока это не проблема — мы отфильтровали NULL в WHERE. А что если не фильтровать?
