...
**Предпосылки:** [соединения](03-joins.md) (JOIN), [агрегация](02-aggregation.md) (GROUP BY, агрегатные функции).
...
Подзапрос в FROM выполняется на шаге 1 [порядка выполнения](02-aggregation.md#полный-порядок-выполнения-запроса) — он становится источником строк, как обычная таблица. Подзапрос в FROM обязательно имеет псевдоним (AS dept_stats).
...
При пустом подзапросе: `ANY` --> FALSE (не нашлось ни одного подходящего), `ALL` --> TRUE (условие выполняется для всех нуля элементов — вакуумная, или вакантная, истина).
