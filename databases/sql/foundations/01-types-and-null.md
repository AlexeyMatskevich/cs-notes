...
`text` — строка произвольной длины. Основной строковый тип в PostgreSQL. `varchar(n)` — строка с ограничением длины (максимум `n` символов). `char(n)` — строка фиксированной длины, добивается пробелами; на практике используется редко.
