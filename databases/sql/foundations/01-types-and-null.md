...
В PostgreSQL `text` и `varchar` без указания длины работают практически одинаково — `text` полноценный тип, а не «тип второго сорта».

## Дата и время

`date` — только дата: `2025-03-15`. `timestamp` — дата и время без часового пояса. `timestamptz` — дата и время **с учётом часового пояса**: PostgreSQL хранит значение в UTC и конвертирует при выводе.
