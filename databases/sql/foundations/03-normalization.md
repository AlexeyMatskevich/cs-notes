...
**Предпосылки:** [реляционная модель](00-relational-model.md) — аномалии, первичный ключ, внешний ключ.

← [Реляционная модель](00-relational-model.md) | [SELECT и фильтрация](../querying/00-select-and-filtering.md) →

В [реляционной модели](00-relational-model.md) мы увидели, что плоская таблица порождает аномалии, и разделение на несколько связанных таблиц их устраняет. Но как далеко разделять? Какой принцип определяет, какие столбцы должны быть вместе, а какие — в отдельной таблице? Ответ даёт теория нормальных форм. Её фундамент — понятие функциональной зависимости.
...
Три JOIN вместо нуля. Каждый JOIN — поиск соответствия между таблицами. На малых масштабах разница незаметна: при наличии [индексов](../schema/04-indexes.md) на внешних ключах каждый JOIN — быстрый поиск по индексу, несколько микросекунд. Проблема проявляется в двух ситуациях.
...
- [Профили чтения и записи](../../../system-design/04-read-write-profiles.md) определяют, какая сторона tradeoff важнее: если запись редка, оптимизация чтения через денормализацию оправдана
- [Выбор хранилища](../../../system-design/10-storage-selection.md) — разные хранилища для разных паттернов доступа: OLTP-база для транзакций, аналитическое хранилище с денормализованными таблицами для отчётов
- [CQRS](../../../system-design/13-event-driven-architecture.md#cqrs) — нормализованная модель для записи, денормализованная проекция для чтения, ценой eventual consistency
