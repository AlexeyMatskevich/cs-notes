...
Три JOIN вместо нуля. Каждый JOIN — поиск соответствия между таблицами. На малых масштабах разница незаметна: при наличии [индексов](../schema/04-indexes.md) на внешних ключах каждый JOIN — поиск по B-tree, несколько микросекунд. Проблема проявляется в двух ситуациях.
...
- [CQRS](../../../system-design/13-event-driven-architecture.md) — нормализованная модель для записи, денормализованная проекция для чтения, ценой eventual consistency
