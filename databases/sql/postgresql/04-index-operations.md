...
Проблема усугубляется: ожидающий `SHARE` lock блокирует все последующие запросы — SELECT встают в очередь за ним. Подробнее о механизме блокировок — в [блокировках PostgreSQL](../../postgresql/concurrency/03-locks.md#table-level-locks--защита-структуры-таблицы).
