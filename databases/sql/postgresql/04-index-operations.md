...
PostgreSQL делает два прохода по таблице. Первый сканирует существующие строки. Второй собирает изменения, сделанные во время первого прохода. Цена: работает в 2–3 раза дольше, требует больше ресурсов, **нельзя использовать внутри транзакции** (поэтому транзакционный DDL здесь не работает — см. [транзакционный DDL](03-functions-and-procedures.md#транзакционный-ddl)).
