...
← [JSONB](jsonb.md) | [Полнотекстовый поиск](full-text-search.md) →
...
[GIN индекс](../../postgresql/indexes/gin.md) для операторов `@>`, `&&`:
...
Для `room_id WITH =` необходимо расширение `btree_gist` — подробнее в [EXCLUSION](exclusion-constraints.md).

`room_id WITH =` — ограничение на одну и ту же комнату. `during WITH &&` — диапазоны не должны пересекаться. [GiST индекс](../../postgresql/indexes/gist.md) обеспечивает эффективную проверку — PostgreSQL не сканирует все бронирования, а проверяет только потенциальные конфликты через дерево.
...
Подробнее — в [ограничениях](../schema/constraints.md).

### Индексирование диапазонов

[GiST индекс](../../postgresql/indexes/gist.md) для операторов `&&`, `@>`, `<@`:
