...
**Предпосылки:** [ограничения](../schema/01-constraints.md) (CHECK, UNIQUE), [индексы](../schema/04-indexes.md) (CREATE INDEX).

← [Индексы в production](04-index-operations.md) | [Партиционирование](06-partitioning.md) →
...
«Для одинакового `room_id` диапазоны `during` не могут пересекаться (`&&`)». Без `btree_gist` PostgreSQL не сможет создать GiST-индекс по `room_id WITH =` и вернёт ошибку. Структура GiST — в [GiST](../../postgresql/indexes/02-gist.md).
