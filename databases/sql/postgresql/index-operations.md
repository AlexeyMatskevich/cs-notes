...
Проблема усугубляется: ожидающий `SHARE` lock блокирует все последующие запросы — SELECT встают в очередь за ним. Подробнее о механизме блокировок — в [[databases/postgresql/concurrency/locks#table-level-locks--защита-структуры-таблицы|блокировках PostgreSQL]].
