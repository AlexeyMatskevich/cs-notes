...
> [!info]- Предпосылки
> [DML](../modification/dml.md) (INSERT, UPDATE), [выражения](../foundations/expressions.md) (CASE), [агрегация](../querying/aggregation.md).

← [Полнотекстовый поиск](full-text-search.md) | [Триггеры](triggers.md) · [Индексы в production](index-operations.md) →
...
Исключения: некоторые операции не транзакционны даже в PostgreSQL — `CREATE DATABASE`, `CREATE INDEX CONCURRENTLY` (подробнее в [индексах в production](index-operations.md)). Но основные DDL-команды (CREATE TABLE, ALTER TABLE, DROP TABLE) полностью транзакционны.
