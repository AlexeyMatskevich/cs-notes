...
← [EXCLUSION](exclusion-constraints.md) | [Материализованные представления](materialized-views.md) →

В [партиционировании](../schema/partitioning.md) рассмотрена идея: логическая таблица из физических частей, DROP вместо DELETE. PostgreSQL реализует её через декларативный DDL — PARTITION BY.
