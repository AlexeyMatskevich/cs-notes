...
[DML](00-dml.md) показывает базовый INSERT...SELECT (архивирование заказов). Здесь добавлено: подзапрос для фильтрации дубликатов, вычисление `CURRENT_TIMESTAMP` на лету.
...
Вставка новых товаров — только первый шаг. Существующие товары тоже нужно обновить: цена могла измениться. [INSERT ON CONFLICT](00-dml.md) решает upsert, но привязан к уникальному индексу и не поддерживает условные ветки.
