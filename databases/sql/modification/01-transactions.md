...
В PostgreSQL конкретный механизм — [MVCC](../../postgresql/concurrency/00-mvcc.md#цена-mvcc-dead-tuples): открытая транзакция не позволяет СУБД освободить старые версии строк, и таблица может вырасти на гигабайты. Защита — `idle_in_transaction_session_timeout`.
