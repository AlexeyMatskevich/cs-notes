...
← [DML — изменение данных](dml.md) | [Составные DML-операции](compound-dml.md) →
...
Savepoint'ы не бесплатны: СУБД отслеживает состояние каждого, и на масштабе сотен тысяч overhead становится заметным. В PostgreSQL каждый SAVEPOINT создаёт subtransaction state — подробнее в [паттернах параллельного доступа](../../postgresql/concurrency/patterns.md).
