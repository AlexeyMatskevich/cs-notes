...
<- [Графемы и нормализация](unicode-normalization-and-graphemes.md)
...
Big-endian — стандарт для сетевых протоколов. Он зафиксирован как **network byte order** (сетевой порядок байтов) в RFC 791 (Internet Protocol, 1981), Appendix B. TCP/IP-заголовки, номера портов, IP-адреса — всё записывается big-endian.
...
**Сетевые протоколы.** [TCP/IP](../networking/transport/tcp.md) передаёт многобайтовые поля в big-endian. x86-машина, отправляя TCP-пакет, должна переставить байты номера порта и IP-адреса из своего little-endian в сетевой big-endian. Принимая пакет — обратно. Для этого существуют стандартные функции преобразования:
...
Вопрос «какой порядок правильный?» не имеет ответа — оба работают, пока обе стороны согласны. Настоящая проблема возникает не от выбора, а от молчаливого предположения о нём: любой бинарный протокол, который не фиксирует порядок байтов явно, в какой-то момент встретит партнёра, думающего наоборот. Именно поэтому `htonl`/`ntohl` вызывают всегда, BOM пишут в начало UTF-16, а в ELF-заголовок закладывают флаг endianness — лучше один явный байт соглашения, чем отлаженная на одной машине и рушащаяся на другой программа.

## Sources

- Cohen, D., 1980, *On Holy Wars and a Plea for Peace*. USC/ISI. https://www.ietf.org/rfc/ien/ien137.txt
- RFC 791, 1981, *Internet Protocol*, Appendix B (network byte order). https://www.rfc-editor.org/rfc/rfc791
- RFC 1700, 1994, *Assigned Numbers* (IANA reference). https://www.rfc-editor.org/rfc/rfc1700
