...
<- [Двоичная система и байты](binary-and-bytes.md) | [Побитовые операции](bitwise-operations.md) ->
...
Сами биты не несут знака — это соглашение поверх них. Прежде чем вводить такое соглашение, стоит разобрать сложение беззнаковых чисел подробнее на 8-битных числах: тот же механизм ляжет в основу знаковой арифметики.
...
Девятый бит (перенос) выходит за пределы 8 бит и отбрасывается. Оставшиеся 8 бит — `00000000` = 0. Сумматор, ничего не знающий о знаках, дал правильный результат.
...
Практическое следствие: выражение `−x` не всегда помещается в тот же тип. Для i8: −(−128) = 128, но максимум i8 — 127. Смена знака у минимального значения приводит к переполнению.
...
Девятый бит отбрасывается — остаётся `00000000` = 0. Счётчик совершил полный оборот. 255 + 1 = 0. Это не ошибка процессора — он просто отбрасывает бит, который не влезает. Результат всегда равен (a + b) mod 2ⁿ — остатку от деления на 2ⁿ, тому самому «обороту счётчика».
...
Переполнение — причина реальных сбоев, и тот же механизм обёртки виден в классическом примере — «kill screen» аркады Pac-Man (1980). Номер уровня хранится в одном байте; на 256-м уровне он равен 255 (отсчёт идёт с нуля). Чтобы нарисовать внизу экрана фрукты-бонусы, подпрограмма берёт копию этого номера и прибавляет к ней 1 — и ровно как 255 + 1 выше, результат не помещается в байт: счётчик отрисовки оборачивается в 0. Цикл, рассчитанный пройти столько шагов, прокручивается из-за этого 256 раз и заливает правую половину экрана мусором. Уровень становится непроходимым — игра упёрлась в собственную 8-битную границу.

## Стандартные ширины

Когда брать i8, а когда — i32 или i64, определяет диапазон значений и совместимость с памятью и форматами данных. Чем шире тип, тем дальше граница переполнения, но и тем больше места на каждое число. Современные процессоры предлагают фиксированный набор ширин:
...
Дополнительный код превратил знак из отдельной сущности в удачно размещённую комбинацию. Процессор по-прежнему не различает знаковое и беззнаковое число — эту разницу делают инструкции, которые над битами работают. А что, если интересен не результат арифметики, а отдельный бит внутри числа: признак ошибки, разрешение на запись, четвёртый флаг в упакованной структуре? Тогда нужны [побитовые операции](bitwise-operations.md) — инструменты, которые смотрят не на число, а на биты, из которых оно сложено.
...
- Pittman, J., 2009, *The Pac-Man Dossier*. https://pacman.holenet.info/
