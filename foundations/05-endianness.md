...
<- [Кодирование текста](04-text-encoding.md)
...
На этом серия `foundations/` завершается. Путь от переключателя с двумя состояниями оказался длинным: [бит и байт](00-binary-and-bytes.md) — минимальные единицы, из которых строится всё. [Целые числа](01-integers.md) — как из байтов получить отрицательные числа и не сломать сложение. [Побитовые операции](02-bitwise-operations.md) — прямая работа с отдельными битами. [Числа с плавающей точкой](03-floating-point.md) — компромисс между диапазоном и точностью для дробей. [Кодирование текста](04-text-encoding.md) — как дать номер каждому символу каждой письменности. И наконец, порядок байтов — соглашение о том, как многобайтовые значения раскладываются в памяти.

Всё это — фундамент, на который опираются [процессоры](../computer/00-cpu.md), [сетевые протоколы](../networking/foundations/00-ethernet-and-switching.md), [языки программирования](../programming/index.md) и [базы данных](../databases/postgresql/index.md).
