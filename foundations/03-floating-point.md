...
**Предпосылки:** [Двоичная система и байты](00-binary-and-bytes.md), [Целые числа](01-integers.md).

<- [Побитовые операции](02-bitwise-operations.md) | [Кодирование текста](04-text-encoding.md) ->
...
Пример: 0.101 в двоичной = 1 * 0.5 + 0 * 0.25 + 1 * 0.125 = 0.625.

Ещё один: 1.11 в двоичной = 1 + 0.5 + 0.25 = 1.75.

Принцип тот же, что и для целой части: каждая позиция — степень двойки, только с отрицательным показателем.

Как перевести десятичную дробь в двоичную? Последовательно умножаем дробную часть на 2 и записываем целые части:
...
Результат: 0.625 = 0.101 в двоичной
```

Проверка: 0.101 = 1 * 0.5 + 0 * 0.25 + 1 * 0.125 = 0.625. Совпадает.
...
Показатель степени может быть отрицательным: 1.01 * 2^(-3) для числа 0.15625. Но хранить его в дополнительном коде (как для целых чисел со знаком) неудобно — это ломает быстрое сравнение.

При сравнении двух чисел с плавающей точкой (какое больше?) хочется сравнивать их так же быстро, как целые числа — побитово, от старших разрядов к младшим. Если экспоненты хранить в дополнительном коде, отрицательная экспонента начинается с единиц в старших битах, и число с маленькой экспонентой (-3) будет выглядеть «больше», чем число с большой (+5), если сравнивать как беззнаковое целое. Сортировка сломается.

Решение — смещение (bias): к настоящему значению экспоненты прибавляется фиксированная величина, чтобы хранимое значение всегда было неотрицательным.
