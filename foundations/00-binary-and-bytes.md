...
**Степень двойки.** 8 = 2³. Память компьютера — пронумерованный ряд ячеек; номер ячейки называется **адресом**. Когда размер ячейки — степень двойки, процессору проще вычислять адреса: вместо умножения достаточно сдвига битов (что это за операция — будет в [побитовых операциях](02-bitwise-operations.md)).
...
Буква 'A' — потому что в таблице **ASCII** (American Standard Code for Information Interchange — «американский стандартный код для обмена информацией», 1963) буква A имеет код 65. ASCII задаёт соответствие между числами 0–127 и символами: 65 = 'A', 66 = 'B', 48 = '0', 32 = пробел. Подробнее о кодировках символов — в [отдельной заметке](04-text-encoding.md).
