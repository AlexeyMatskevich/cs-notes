...
У всех этих поворотов общий корень: формат жертвует точностью ради широкого диапазона, и там, где число подходит к краю диапазона, к самому нулю или проходит через тысячи операций подряд, эта незаметная в обычной арифметике жертва выходит наружу — то отдельным правилом, то тихой ошибкой.
...
Цена платится временем. Многие процессоры обрабатывают денормализованные числа отдельным, медленным путём — операция над такими операндами может быть в десятки раз медленнее обычной. В аудио-DSP и других горячих циклах это проявляется как внезапная просадка производительности, когда затухающий сигнал уходит в область денормализованных амплитуд. Стандартный обходной путь — режим самого процессора (управляющий бит в регистре арифметического блока), который включают на горячий цикл: FTZ (flush to zero) обнуляет денормализованный результат, DAZ (denormals are zero) — денормализованный операнд. Это быстрее, но ломает свойство плавного исчезновения, и применять его можно только там, где потеря крошечных значений безопасна.
...
Ассоциативность сложения тоже нарушается из-за похожего механизма. Если `a = 10²⁰`, `b = −10²⁰`, `c = 1.0`, то `(a + b) + c = 0 + 1 = 1.0`, но `a + (c + b)` даёт 0: сначала `c + b = 1.0 + (−10²⁰)` округляется до −10²⁰ (единица меньше одного шага мантиссы при таком масштабе и теряется на фоне 10²⁰), затем `a + (−10²⁰) = 0`. Порядок операций для float имеет значение: `(a + b) + c ≠ a + (c + b)`.
...
Наивная сумма миллиона дробных чисел — скажем, по 0.1 — в float64 заметно расходится с точным ответом. Каждое 0.1 уже хранится с крошечной погрешностью, и эти погрешности складываются. Хуже того: чем больше накопленная сумма, тем крупнее её шаг — расстояние до соседнего представимого числа, — и тем большая часть младших битов очередного слагаемого пропадает при округлении. Чем длиннее сумма, тем больше накопленная погрешность — в худшем случае линейно от количества слагаемых, O(N).
...
Три лишние операции на шаг — плата за то, что накопленная ошибка практически перестаёт расти с длиной суммы: её главная часть от количества слагаемых не зависит. Научные библиотеки редко используют именно Кэхана, но часто применяют более дешёвую родственную схему — попарное суммирование (pairwise): массив складывается не подряд, а деревом, половинами, и накопленная ошибка растёт гораздо медленнее, чем при сложении по одному. Поэтому встроенная в библиотеку функция суммы на большом массиве обычно точнее ручного цикла со сложением слагаемых одно за другим.
...
**Деньги храните в целых.** Сумма `0.10 + 0.20` в float даёт `0.30000000000000004`. Умноженная на миллион транзакций, эта прибавка превращается в расхождение баланса. Стандартные решения: хранить суммы в наименьших целых единицах (центах); либо использовать десятичные типы — DECIMAL/NUMERIC в базах данных, money-библиотеки в языках (`decimal` в Python, `BigDecimal` в Java). Все они считают в основании 10, поэтому множитель 5, из-за которого [0.1 и 0.3 не укладываются в двоичную дробь](floating-point.md), им уже не страшен — эти дроби представляются точно.

**JSON и целочисленные ID опасны после 2⁵³.** Спецификация JSON не указывает тип чисел, но большинство парсеров (JavaScript в первую очередь) читают их в float64. Мантисса float64 — 53 бита, и все целые от 0 до 2⁵³ = 9 007 199 254 740 992 представляются точно. За этой границей шаг между соседними представимыми числами становится больше 1, и, например, 2⁵³ + 1 округляется обратно до 2⁵³. Большие целочисленные идентификаторы — многие распределённые генераторы ID 64-[[foundations/binary-and-bytes#Бит|битны]] — в JSON безопасны только если передаются строками. Twitter столкнулся с этим в 2011, когда клиенты начали терять младшие биты ID твитов — с тех пор API отдаёт `id_str` рядом с `id`.

**Осторожно с катастрофической потерей.** Если формула содержит разность близких чисел, поищите алгебраически эквивалентную запись без этого вычитания. Например, при малом x выражение `log(1+x)` теряет точность (прибавка x тонет при округлении 1+x), и для него есть устойчивая функция `log1p(x)`, вычисляющая то же значение напрямую. Большинство математических библиотек содержат такие «численно устойчивые» варианты для опасных случаев.
