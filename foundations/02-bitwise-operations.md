...
<- [Целые числа](01-integers.md) | [Числа с плавающей точкой](03-floating-point.md) ->
...
Побитовые операции (bitwise operations, от bit + wise — «побитно») делают именно это: берут одно или два числа и применяют логическое действие к каждой паре бит на одной и той же позиции, независимо от остальных позиций. Операций всего шесть: четыре логические (NOT, AND, OR, XOR) и два сдвига (влево и вправо). Из них собираются все остальные приёмы.
...
Если вспомнить обратный код из [целых чисел](01-integers.md), это ровно та операция, которая давала обратный код (ones' complement): NOT превращает число в его побитовую инверсию. Для 8-битного беззнакового числа NOT(x) = 255 - x, потому что 11111111 минус любой набор бит даёт зеркальный набор.
...
OR с нулём не меняет бит (0 OR x = x). OR с единицей всегда даёт единицу (1 OR x = 1). Поэтому маска «включает» только те позиции, где стоит 1, а остальные оставляет как были.
...
Каждая из шести операций по отдельности делает что-то простое. Но вместе они образуют набор инструментов, из которого собираются стандартные приёмы для работы с конкретным битом на позиции N (позиции нумеруются с 0 от младшего бита).
