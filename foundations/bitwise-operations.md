...
Число обычно говорит о величине: сколько заказов, какой по счёту элемент. Но тот же байт можно использовать иначе — как восемь независимых переключателей «да/нет», по одному на каждый бит: включён ли каждый из восьми признаков. Допустим, нужно переключить ровно один из них, не задев остальные семь. Обычная арифметика этого не умеет: прибавление или умножение меняет число целиком, и перенос задевает соседние разряды сразу. Нужны операции, которые работают с каждой позицией по отдельности — смотрят не на число как величину, а на отдельные биты, из которых оно сложено.
...
Сдвиг вправо на N — целочисленное деление на 2^N (дробная часть отбрасывается). Логика та же, что у сдвига влево, только обратная: убрать младший разряд — это как в десятичной системе стереть последнюю цифру, то есть разделить на основание и отбросить остаток. Потерянный младший бит и есть та самая отброшенная дробная часть.
...
Четыре логические операции и два сдвига — это базовый набор, из которого собирается остальное. Каждая по отдельности делает что-то элементарное: инвертирует, фильтрует, включает, переключает, двигает. Но на практике задача звучит иначе: «проверить бит номер 5» или «сбросить бит номер 3, не трогая остальные». Для этого нужны готовые формулы, собранные из базовых операций.
