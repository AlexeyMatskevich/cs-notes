...
Если вспомнить обратный код из [целых чисел](integers.md), это ровно та операция, которая давала обратный код (ones' complement): NOT превращает число в его побитовую инверсию. Для 8-битного беззнакового числа NOT(x) = 255 - x, потому что 11111111 минус любой набор бит даёт зеркальный набор.
