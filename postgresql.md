...
Атомарность записи страницы. Современные диски гарантируют: запись сектора либо происходит целиком, либо не происходит. Страница 8 КБ = 16 секторов по 512 байт, но PostgreSQL использует дополнительные механизмы — журнал упреждающей записи (WAL) и запись полного образа страницы (full page writes) — для защиты от частичной записи.
...
**Важно:** ctid — это адрес конкретной *версии* строки, не самой строки. При UPDATE создаётся новая версия с новым ctid. Индексы хранят ctid и обычно должны обновляться. Есть оптимизация HOT (Heap-Only Tuple): если UPDATE не меняет индексируемые поля, индекс может не обновляться.
