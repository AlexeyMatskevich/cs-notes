...
- [Broker, topic, partition, offset](architecture/00-what-is-kafka.md) — фундаментальная модель: почему Redis Streams не справляется при росте, как Kafka распределяет лог по серверам, тройная роль партиции, несколько producer'ов в одну партицию, consumer groups с независимыми offset'ами
...
- [Transactions](architecture/04-transactions.md) — transactional producer, exactly-once для consume-transform-produce, LSO, read_committed isolation
