...
Idempotent producer гарантирует **exactly-once запись в одну партицию от одного живого процесса producer'а**. Sequence number привязан к паре (PID, partition). За этими рамками — три класса проблем, которые idempotent producer не решает.

**Перезапуск producer'а.** PID назначается при подключении. Процесс Orders-сервиса упал — буфер producer'а в памяти потерян. При рестарте producer получает **новый PID**, sequence number'ы начинаются с 0. Broker не может связать старый и новый PID — для него это два разных producer'а.

С точки зрения Kafka producer'а как библиотеки дубликат при перезапуске невозможен: буфер потерян, переотправлять нечего. Дубликат возникает, если **приложение** само хранит данные и повторяет отправку — например, через outbox-таблицу в PostgreSQL. Orders-сервис записал событие в outbox, отправил в Kafka, Kafka записала, но сервис упал до `UPDATE outbox SET sent = true`. Рестартнул, прочитал outbox — запись не помечена. Создал нового producer'а (PID=12), отправил то же событие. Broker видит PID=12, seq=0 — новый producer, записывает. Дубликат. Но это уровень приложения, не Kafka producer'а.
...
**Дубликаты на стороне consumer'а.** Consumer прочитал событие, обработал, упал до commit'а offset'а. Рестартнул — прочитал то же событие снова. Idempotent producer — гарантия на стороне записи, не на стороне чтения.

Для защиты от дубликатов при перезапуске и для атомарной записи в несколько партиций существует **transactional producer**, использующий стабильный `transactional.id`, который переживает рестарты.
