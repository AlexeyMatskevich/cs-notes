...
Enrichment-сервис — [consumer](consumer-internals.md) в группе `enrichment-group`. Если посреди открытой [транзакции](transactions.md) произошёл [rebalancing](consumer-internals.md) (например, добавился новый экземпляр сервиса), [consumer](consumer-internals.md) теряет часть [партиций](../../../system-design/message-queues.md). Offset'ы для отозванных [партиций](../../../system-design/message-queues.md) уже включены в `send_offsets_to_transaction` — commit таких offset'ов некорректен, потому что [consumer](consumer-internals.md) больше не владеет этими [партициями](../../../system-design/message-queues.md). [Транзакция](transactions.md) должна быть abort'нута до передачи [партиций](../../../system-design/message-queues.md). На практике это реализуется через `ConsumerRebalanceListener`: в callback'е `onPartitionsRevoked` вызывается `producer.abort_transaction()`, а после получения новых [партиций](../../../system-design/message-queues.md) — начинается новая [транзакция](transactions.md) с чистого состояния.
