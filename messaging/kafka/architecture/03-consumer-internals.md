...
Для `order_events` с платёжными данными потеря неприемлема — значит, commit после обработки и at-least-once. Но при at-least-once повторная обработка записей 15–19 не должна сломать данные. Если обработчик [идемпотентен](../../../system-design/06-reliability-patterns.md) — `UPDATE orders SET status = 'paid' WHERE id = 42` выполни хоть десять раз, результат один — дубликаты безвредны. При at-least-once идемпотентная обработка не опциональна, а обязательна.
