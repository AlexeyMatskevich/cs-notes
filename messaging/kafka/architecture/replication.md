...
Нужны копии. Каждая [партиция](../../../system-design/message-queues.md) должна существовать на нескольких broker'ах, чтобы при потере одного сервера данные и доступность сохранялись.

## Единица репликации — партиция, а не broker

В [Redis Sentinel](../../../databases/redis/distribution/sentinel.md) реплицируется master целиком: все данные копируются на replica-сервер. В [PostgreSQL](../../../databases/postgresql/distribution/replication.md) — аналогично: WAL-поток переносит все изменения primary на standby. Одна реплика = полная копия всего сервера.
...
Каждый broker — leader для двух [партиций](../../../system-design/message-queues.md) и follower для четырёх. Запись равномерна: ~50 events/sec на каждый broker.

Broker 0 умирает. [Партиция](../../../system-design/message-queues.md) 0 получает нового leader'а — Broker 1. [Партиция](../../../system-design/message-queues.md) 3 — Broker 2. Теперь каждый из двух выживших — leader для трёх [партиций](../../../system-design/message-queues.md): 75 events/sec вместо 50. Нагрузка распределилась поровну, ни один broker не захлебнулся.

Per-partition [репликация](replication.md) даёт независимый failover для каждой [партиции](../../../system-design/message-queues.md) и равномерное распределение leader'ов после сбоя. Каждый broker одновременно и leader, и follower — простаивающих реплик нет.

## Leader принимает всё: и записи, и чтения

Leader принимает записи от [producer'а](producer-reliability.md) — но кто обслуживает чтения? В PostgreSQL standby-реплика обслуживает read-запросы (hot standby). В Redis — `READONLY` на реплике. В Kafka follower **не обслуживает чтений** — [consumer](consumer-internals.md) всегда читает с leader'а.

Причина — в природе данных. Consumer отслеживает позицию в партиции через [offset](what-is-kafka.md): «обработал до offset 7, дай с offset 8». Follower может отставать от leader'а — leader записал offset'ы 0–10, а follower скопировал только 0–7. Consumer читает с follower'а, получает записи до 7, коммитит offset = 7. На самом деле leader уже ушёл на 10. Consumer думает, что дочитал до конца, а 3 события пропущены.
...
[Consumer'ы](consumer-internals.md) видят только записи ниже high watermark. Если leader умрёт, новый leader (любая реплика из ISR) будет иметь все записи, которые [consumer](consumer-internals.md) уже видел. Записи выше HW — «в процессе [репликации](replication.md)», [consumer](consumer-internals.md) их не получит до тех пор, пока все ISR-реплики не подтвердят.

High watermark — ключ к пониманию того, почему при выборе нового leader'а не нужно сравнивать offset'ы реплик (как это делает [Redis Sentinel](../../../databases/redis/distribution/sentinel.md), выбирая реплику с наибольшим replication offset). При `acks=all` [producer](producer-reliability.md) получает подтверждение только после того, как все ISR-реплики записали событие — HW продвинулся. Всё, что ниже HW, одинаково на всех репликах в ISR — любая годится в leader'ы.
...
В отличие от [PostgreSQL](../../../databases/postgresql/distribution/replication.md), где режим [репликации](replication.md) (синхронная/асинхронная) — в первую очередь настройка сервера (хотя `synchronous_commit` можно менять и на уровне отдельной транзакции), в Kafka гарантию durability выбирает **каждый [producer](producer-reliability.md) для себя**.
...
[Producer](producer-reliability.md) ждёт подтверждения от leader'а. Leader записал событие в свой локальный лог — ответил «ok». Follower'ы ещё не получили это событие.
...
Leader подтвердил, через 200ms Broker 0 умирает. Follower'ы не успели скопировать последнее событие. Новый leader выбирается из follower'ов — событие потеряно. Тот же сценарий, что при [асинхронной репликации в Redis](../../../databases/redis/distribution/sentinel.md): master подтвердил, упал до [репликации](replication.md).
...
[Producer](producer-reliability.md) ждёт, пока leader получит подтверждение от **всех реплик в ISR**. Leader записал к себе, дождался, пока follower'ы тоже записали, и только тогда ответил «ok».
...
Broker 0 умирает — оба follower'а уже имеют событие. Потеря данных исключена. Аналог [синхронной репликации в PostgreSQL](../../../databases/postgresql/distribution/replication.md), где standby записывает в WAL до того, как primary ответит клиенту.
...
Partition leader election не требует [консенсуса](../../../system-design/consensus.md) между broker'ами. Controller просто берёт **первую живую реплику из ISR** и назначает её leader'ом. Одно решение одного узла, без голосования.

Почему не нужно сравнивать offset'ы реплик, как в [Redis Sentinel](../../../databases/redis/distribution/sentinel.md)? При `acks=all` все реплики в ISR имеют данные до high watermark — любая годится. При `acks=1` записи выше HW всё равно будут потеряны — сравнивать нечего.
...
KRaft появился в Kafka 2.8 (2021) как ранний доступ и стал production-ready в версии 3.3 (2022). Controller'ы — выделенные broker'ы, которые выбирают leader'а **между собой по протоколу [Raft](../../../system-design/consensus.md)**: term'ы, голосование большинства, randomized election timeout. Метаданные хранятся в internal topic `__cluster_metadata`, реплицируемом по [Raft](../../../system-design/consensus.md). Внешних зависимостей нет.

Итого два уровня leader election в Kafka:

```
Controller election:       [Raft](../../../system-design/consensus.md) (голосование, кворум, term'ы)
