...
Кластер из трёх broker'ов с `replication.factor=3`, `acks=all`, `min.insync.replicas=2` обеспечивает durability для `order_events`. [Producer](producer-reliability.md) отправляет событие, leader ждёт подтверждения от всех ISR-реплик и только тогда отвечает «ok». Данные переживают смерть одного broker'а.

Но мы смотрели на это со стороны кластера. Теперь — со стороны producer'а.

## Потерянный ответ

Orders-сервис обработал платёж за заказ #42 и публикует `order_paid` в `order_events`. [Producer](producer-reliability.md) отправляет событие с `acks=all`. Leader на Broker 0 записывает, follower'ы на Broker 1 и Broker 2 забирают через fetch, все ISR-реплики подтвердили. Leader формирует ответ «ok» — и в этот момент сетевое соединение между [producer'ом](producer-reliability.md) и Broker 0 обрывается. Ответ не доходит. [Producer](producer-reliability.md) ждёт — timeout.

```
[Producer](producer-reliability.md)                          Leader (Broker 0)
   │                                   │
   │── event: order_paid #42 ────────> │
   │                                   │ записал в лог
...
Событие **записано** — на трёх broker'ах, committed, [consumer'ы](consumer-internals.md) его увидят. Но [producer](producer-reliability.md) об этом не знает. С его точки зрения запись могла не пройти: timeout мог случиться до того, как leader получил событие, или после получения, но до [репликации](replication.md). [Producer](producer-reliability.md) не может отличить «записано, ответ потерялся» от «не записано вообще».

Два варианта. Не повторять отправку — событие `order_paid` потенциально потеряно. Seller read DB не обновится, fraud detection не увидит платёж. Это [at-most-once](../../../system-design/delivery-guarantees.md): максимум одна доставка, но без гарантии, что она произошла. Повторить отправку — событие `order_paid` может оказаться в партиции дважды. Consumer обработает оплату два раза: двойное списание, дублирование в ClickHouse, ложный сигнал fraud detection. Это [at-least-once](../../../system-design/delivery-guarantees.md): хотя бы одна доставка, но возможны дубликаты.
...
[Producer](producer-reliability.md) не отправляет событие и ждёт вечно. Три параметра управляют повторами:
...
В нашем сценарии: timeout на первую попытку — [producer](producer-reliability.md) автоматически шлёт `order_paid #42` повторно. Если Broker 0 снова недоступен — ещё раз. И ещё. До «ok» или до истечения `delivery.timeout.ms`.

Но [producer](producer-reliability.md) не отправляет события по одному, дожидаясь ответа на каждое. Он работает **асинхронно**: отправил событие A, не дожидаясь ответа — отправляет B, потом C. Это контролирует параметр **`max.in.flight.requests.per.connection`** — сколько неподтверждённых запросов [producer](producer-reliability.md) держит одновременно на одном соединении с broker'ом. Дефолт — 5.
...
Три запроса в полёте одновременно. Все идят в одну [партицию](../../../system-design/message-queues.md) (хеши ключей совпали). Request 1 **падает с ошибкой** — timeout, leader temporarily unavailable. Requests 2 и 3 **успешно записаны**. [Producer](producer-reliability.md) ставит request 1 на retry.
...
[Producer](producer-reliability.md) retries request 1:
...
[Producer](producer-reliability.md) отправил события в порядке: \#42 paid → \#77 shipped → \#99 created. В [партиции](../../../system-design/message-queues.md) порядок стал: \#77 shipped → \#99 created → \#42 paid. Retry переупорядочил записи.
...
Цена — throughput. Вместо 5 запросов в полёте — один. [Producer](producer-reliability.md) простаивает, ожидая каждый round-trip (~5–30 ms при `acks=all`). Для наших 150 events/sec это может быть терпимо, но при тысячах events/sec — bottleneck.
...
При первом подключении к кластеру [producer](producer-reliability.md) получает от broker'а уникальный числовой идентификатор — **Producer ID (PID)**. PID назначается кластером, не приложением (это не [partition](../../../system-design/message-queues.md) key и не client.id).

Далее [producer](producer-reliability.md) для каждой [партиции](../../../system-design/message-queues.md), в которую пишет, ведёт **sequence number** — монотонно возрастающий счётчик, начиная с 0. Каждый запрос к [партиции](../../../system-design/message-queues.md) содержит PID, sequence number и данные.
...
**seq ≤ last** — дубликат. Broker уже видел этот sequence number от этого producer'а в эту партицию. Запись **отклоняется без ошибки**: broker возвращает «ok», но ничего не записывает. [Producer](producer-reliability.md) думает, что retry удался, а запись уже была на месте.
...
[Producer](producer-reliability.md)-клиент (библиотека) получает `OutOfOrderSequenceException` и переотправляет батчи в правильном порядке. Когда retry request 1 приходит с seq=0 — broker записывает его первым, затем seq=1, затем seq=2. Порядок сохранён.
...
При пропуске в sequence number'ах broker буферизирует запросы с более высоким seq в памяти, ожидая пропущенный. При `max.in.flight=5` — максимум 4 запроса в буфере на каждую комбинацию (PID, [partition](../../../system-design/message-queues.md)). При `max.in.flight=100` — до 99 запросов. Умножить на тысячи [producer'ов](producer-reliability.md) и десятки [партиций](../../../system-design/message-queues.md) — серьёзный расход памяти broker'а. Пять — эмпирический компромисс: достаточно для высокого throughput (5 запросов в полёте покрывают latency pipeline'а), и буфер на broker'е остаётся маленьким.

## Границы idempotent producer

Idempotent [producer](producer-reliability.md) гарантирует **exactly-once на уровне транспорта** — retry одного и того же `send()` не создаст дубликат. Scope: одна [партиция](../../../system-design/message-queues.md), один экземпляр [producer'а](producer-reliability.md). Sequence number привязан к паре (PID, [partition](../../../system-design/message-queues.md)). За этими рамками — четыре класса проблем, которые idempotent [producer](producer-reliability.md) не решает.
...
**Атомарность записи в несколько [партиций](../../../system-design/message-queues.md).** Orders-сервис хочет записать два события в один topic, но [partition](../../../system-design/message-queues.md) key'и разные — события попадают в разные [партиции](../../../system-design/message-queues.md). Sequence number'ы ведутся per [partition](../../../system-design/message-queues.md). Первое событие записано, второе нет (broker упал) — частичная запись. Idempotent [producer](producer-reliability.md) не обеспечивает «всё или ничего» для нескольких [партиций](../../../system-design/message-queues.md).

**Дубликаты на стороне [consumer'а](consumer-internals.md).** [Consumer](consumer-internals.md) прочитал событие, обработал, упал до commit'а offset'а. Рестартнул — прочитал то же событие снова. [Producer](producer-reliability.md) ничего не знает о [consumer'е](consumer-internals.md) и никак не влияет на его гарантии — idempotent [producer](producer-reliability.md) работает на стороне записи, не на стороне чтения.
