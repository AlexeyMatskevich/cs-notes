...
Каталог — типичная read-heavy система: товары обновляются десятки раз в день, а просматриваются миллионы раз. URL shortener, Wikipedia, лента Twitter/X, [CDN](../network.md) — всё это примеры read-heavy: данные создаются однажды, а потом читаются на порядки чаще.
...
Реплики распределяют нагрузку между узлами, но каждый запрос всё равно обращается к диску. [Кэш](07-caching.md) (Redis, Memcached, [CDN](../network.md)) хранит результат в памяти, чтобы не повторять обращение к диску при следующем запросе.
