...
**Предпосылки:** [паттерны надёжности](06-reliability-patterns.md) (retry, idempotency, cascading failure), [кэширование](07-caching.md) (Redis как внешний сервис).
...
**Без подтверждения** брокер считает сообщение доставленным в момент отправки consumer. Consumer упал до завершения — сообщение потеряно. Это **at-most-once** delivery: сообщение обрабатывается не более одного раза (возможно, ноль). Для записи в аналитику at-most-once может быть приемлемым — потеря одного события из миллиона незаметна в статистике. Для резервирования товара — нет: потерянное сообщение означает, что покупатель заплатил, а товар не зарезервирован.

**С подтверждением (acknowledgment)** consumer явно сообщает брокеру: «обработка завершена». Брокер удаляет сообщение только после ACK. Если ACK не пришёл — сообщение возвращается в очередь для повторной обработки. Это **at-least-once** delivery: сообщение обрабатывается минимум один раз, возможно несколько.
...
At-least-once создаёт проблему дубликатов. Consumer (warehouse) успел зарезервировать товар, но упал до ACK. Сообщение вернулось, другой consumer резервирует тот же товар второй раз.

**Exactly-once** (ровно один раз) кажется идеальной гарантией, но на уровне брокера она фундаментально невозможна. Между моментами «consumer выполнил работу» и «брокер получил ACK» есть окно, в котором может произойти сбой. Брокер не знает, что случилось на стороне consumer. Даже Kafka, которая маркетирует «exactly-once semantics», реализует at-least-once доставку плюс дедупликацию на уровне producer/consumer.

**Exactly-once = at-least-once + idempotency.** Consumer должен быть написан так, чтобы повторная обработка того же сообщения не ломала систему. Warehouse-сервис при резервировании проверяет: «заказ #42 уже зарезервирован?» — и не резервирует повторно. Подробнее об идемпотентности — в [паттернах надёжности](06-reliability-patterns.md#idempotency-безопасность-повторных-запросов).
...
Это различие определяет два архетипа брокеров.

**Традиционная очередь** (RabbitMQ, AWS SQS, Sidekiq+Redis) удаляет сообщение после подтверждения. Очередь — буфер для необработанных задач. Сообщение — это **задача**: выполнить и забыть.
...
**Распределённый лог** (Kafka, Redis Streams) записывает сообщения в append-only лог и не удаляет их после обработки. Consumer отслеживает свою позицию (offset) в логе. Разные consumers читают один и тот же лог с разных позиций. Сообщение — это **факт** (событие), которое произошло. Факт не исчезает после того, как кто-то о нём узнал — он хранится до истечения настроенного срока хранения (retention period), обычно дни или недели, после чего старые сегменты удаляются.
...
Ключевое свойство лога — **replay**: возможность перечитать поток событий с любой точки. Команда data science подключает consumer, ставит offset на начало лога и проходит все 30 дней заказов. С традиционной очередью эти сообщения давно удалены — перечитать невозможно.

Выбор между архетипами определяется тем, нужна ли история. Отправка email после регистрации — задача, которую нужно выполнить и забыть: традиционная очередь. Поток бизнес-событий, к которым могут подключаться новые consumers (аналитика, модели прогнозирования, аудит) — лог. На практике сервис часто использует оба: Sidekiq для фоновых задач (отправить email, обработать изображение), Kafka для потока бизнес-событий (заказ создан, оплата прошла, товар отгружен).
...
Аналогия с [шардингом](../databases/distribution.md#шардинг) прямая: партиция — шард, partition key — shard key, хеш-функция распределяет данные по узлам.
...
**Backpressure** (обратное давление) — механизм, который сообщает producer: «притормози, consumer не справляется». Три основные стратегии.

**Drop** — новые сообщения отклоняются, когда очередь полна. Producer получает ошибку и решает сам: retry, сохранить локально, потерять. Подходит, когда потеря допустима — метрики, логи, необязательные уведомления. Redis при достижении `maxmemory` с дефолтной политикой `noeviction` возвращает ошибку на команды записи — producer (Rails-приложение) получит исключение.

**Block** — producer блокируется на операции `send()`, пока в очереди не появится место. Producer автоматически замедляется до скорости consumer. Опасно, если producer — web-сервер: заблокированный `send()` = зависший HTTP-запрос, треды копятся, начинается [cascading failure](06-reliability-patterns.md#cascading-failure-механизм).

**Overflow to disk** — горячие сообщения в памяти, остальные сбрасываются на диск. Kafka работает так по умолчанию: данные пишутся на диск, page cache операционной системы используется для горячих сегментов. Throughput падает при чтении холодных данных, но ничего не теряется и ничего не блокируется.
...
**Queue depth** — количество сообщений, ожидающих обработки. Растущий тренд означает, что consumers не справляются. Это leading indicator: показывает проблему до того, как пользователи заметят задержки в доставке email или обновлении склада.

**Consumer lag** — для log-based систем (Kafka, Streams) это разница между последним записанным offset и текущей позицией consumer. Lag в 1 000 000 сообщений при скорости consumer 1 000 msg/sec — отставание ~17 минут. Если lag растёт — consumer не успевает за producer.

**Processing time** — время обработки одного сообщения. Рост указывает на деградацию зависимостей (медленный внешний API, тяжёлые запросы к БД) или усложнение самих сообщений.

**Error rate** — доля сообщений, уходящих в retry или DLQ. Рост error rate при стабильном queue depth — появились poison messages или деградировал внешний сервис. Рост вместе с queue depth — consumers захлёбываются.
...
Брокер — дополнительный компонент: деплой, мониторинг, бэкапы, масштабирование. Код усложняется: вместо синхронного вызова — producer, consumer, idempotency, DLQ, мониторинг лага. Добавлять эту сложность стоит только при конкретной проблеме.
...
- [Sidekiq](../rails/sidekiq.md) — традиционная очередь задач: архитектура, retry, graceful shutdown
