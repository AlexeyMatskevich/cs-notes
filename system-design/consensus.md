...
Все три проблемы ведут к одному: узлы расходятся в представлении о текущем состоянии. Самый опасный вариант — [[system-design/replication#split-brain-два-primary-одновременно|split brain]]: два узла одновременно считают себя лидером и принимают записи, данные расходятся необратимо. Алгоритм консенсуса должен гарантировать, что в любой момент максимум один узел считает себя лидером.
...
[[system-design/replication#кворум-как-узлы-определяют-кто-изолирован|Кворум]] — правило большинства, описанное в репликации: узел работает, только если связан с большинством кластера. Raft добавляет к нему два механизма.
...
Primary держит lease в etcd — ключ с TTL, который нужно продлевать каждые N секунд, иначе он истекает и лидерство считается свободным. Если primary не обновил lease (упал), lease истекает. Replicas обнаруживают свободный lease, начинают выборы, и побеждает replica с наиболее актуальными данными. Победитель захватывает lease и выполняет `pg_promote()`, становясь новым primary. Что происходит на уровне PostgreSQL при промоутировании — выход из recovery, переключение WAL timeline — описано в [репликации PostgreSQL](../databases/postgresql/distribution/replication.md).
