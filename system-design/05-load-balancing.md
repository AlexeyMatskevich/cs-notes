...
**Предпосылки:** клиент-серверная архитектура, DNS (домен → IP), [TCP](../networking/transport/01-tcp.md)/[HTTP](../networking/application/01-http.md), горизонтальное масштабирование (несколько одинаковых серверов), понятие отказа узла.
...
**Sticky sessions** (session affinity) — LB запоминает связь «клиент → сервер» и направляет последующие запросы того же клиента на тот же сервер. Идентификация клиента — по IP-адресу (проблема: за одним IP могут быть тысячи пользователей через NAT) или по cookie (LB добавляет `Set-Cookie: SERVERID=A`, потом читает его при следующих запросах).
