...
[[system-design/reliability-patterns#retry-с-backoff-повтор-с-нарастающей-задержкой|Retry]] решает проблему transient failures: запрос не дошёл — отправь ещё раз. Но retry отвечает на вопрос «дойдёт ли вызов до получателя?», а не на вопрос «сколько раз получатель его обработает?». Разница критична.
...
Это не exactly-once доставка — это exactly-once *обработка*, построенная поверх at-least-once доставки. Механизм подробно описан в [[system-design/reliability-patterns#idempotency-безопасность-повторных-запросов|idempotency]].
