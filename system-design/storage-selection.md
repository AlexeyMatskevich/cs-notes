...
<- [Message Queues: асинхронная коммуникация между сервисами](message-queues.md) | [API Design: проектирование границы между системами](api-design.md) ->
...
Интернет-магазин хранит в PostgreSQL всё: пользователей, заказы, каталог товаров, историю поисковых запросов, аналитические события, сессии. Архитектура уже зрелая — primary с [репликами](replication.md), [шардинг](sharding.md) по `user_id` для заказов, Redis для [кэша](caching.md), [message queue](message-queues.md) для фоновых задач. Система справляется. Но появляются боли, которые не решаются добавлением реплик или шардов, потому что их причина — не в объёме нагрузки, а в форме запросов.
...
Граница между ними размыта. MongoDB даёт ACID-транзакции на нескольких документах с версии 4.0. Redis с AOF в режиме `always` даёт durability. PostgreSQL с асинхронной [репликацией](replication.md) на реплике даёт [eventual consistency](consistency-models.md). «ACID = SQL, BASE = NoSQL» — упрощение, которое ломается при ближайшем рассмотрении. На практике гарантии — это фильтр, сужающий выбор: если бизнес требует многозаписных транзакций (списать с одного счёта и зачислить на другой в одной операции), хранилище без ACID не подходит, какая бы у него ни была модель данных.
