...
<- [Микросервисы](microservices.md) | [[system-design/cases/hotel-booking|Система бронирования отелей]] ->
...
Ключевое различие: команда подразумевает знание получателя и ожидаемого действия; событие — нет. Команда — «сделай вот это». Событие — «вот что произошло». В Sidekiq-варианте Orders отправлял команды ([[rails/sidekiq/job-design#command-vs-event-модель-интеграции|command vs event в Sidekiq]]). В pub/sub — публикует события.
...
**Write model (command side):** нормализованная схема в PostgreSQL, оптимизированная под checkout. Минимум индексов. Транзакция: `INSERT order` → `UPDATE inventory` → вызов Payment. После commit — публикация события в [message queue](message-queues.md).
...
### Цена: [eventual consistency](consistency-models.md)
...
[Eventual consistency](consistency-models.md) между write и read сторонами одинакова что с event sourcing, что без: задержка определяется очередью, а не источником данных на write-стороне.
...
Event sourcing добавляет стоимость не в виде [eventual consistency](consistency-models.md) (она определяется очередью, одинакова что с ES, что без), а в виде: версионирования событий (upcasting при изменении контракта), snapshot-ов (когда создавать, как хранить, когда инвалидировать), размера хранилища (10-20 событий на сущность vs одна мутабельная строка).
