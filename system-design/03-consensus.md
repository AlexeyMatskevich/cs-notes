...
Все три проблемы ведут к одному: узлы расходятся в представлении о текущем состоянии. Самый опасный вариант — [split brain](replication.md#split-brain-два-primary-одновременно): два узла одновременно считают себя лидером и принимают записи, данные расходятся необратимо. Алгоритм консенсуса должен гарантировать, что в любой момент максимум один узел считает себя лидером.

## Raft: понятный алгоритм консенсуса

Paxos (Лэмпорт, 1989) — первый алгоритм консенсуса. Корректный, но известен сложностью понимания. Raft (Ongaro, Ousterhout, 2014) разработан для понятности: те же гарантии, проще реализовать и объяснить. Название — акроним от Reliable, Replicated, Redundant, And Fault-Tolerant, и одновременно метафора: плот (raft) собирается из брёвен (logs), как алгоритм строится вокруг реплицированного журнала команд (replicated log), а сам плот позволяет «уплыть с острова Paxos». Большинство современных систем используют Raft или его вариации.
...
[Кворум](replication.md#кворум-как-узлы-определяют-кто-изолирован) — правило большинства, описанное в репликации: узел работает, только если связан с большинством кластера. Raft добавляет к нему два механизма.
