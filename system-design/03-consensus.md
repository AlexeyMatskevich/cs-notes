...
Raft разделяет консенсус на три задачи: выбор лидера (leader election), репликация журнала (log replication) и гарантии корректности (safety). В нормальном режиме лидер принимает запросы клиентов и реплицирует их на followers. Когда лидер падает — кластер выбирает нового. Safety гарантирует, что при смене лидера ни одна подтверждённая запись не теряется.
