...
Ключевое наблюдение: сервис B не упал — он просто стал медленным. Но этого достаточно, чтобы обрушить цепочку. Медленный ответ часто опаснее, чем быстрая ошибка. [[system-design/caching#cache-stampede|Cache stampede]] — частный случай: истечение TTL популярного ключа создаёт внезапную нагрузку на origin.
...
**Отдельные очереди в [Sidekiq](../rails/sidekiq/concurrency-and-scaling.md):**
...
Практический пример идемпотентности в фоновых задачах — [гарантии Sidekiq](../rails/sidekiq/guarantees.md): at-least-once означает повторное выполнение, и каждый job должен быть идемпотентным.
