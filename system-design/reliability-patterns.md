...
Ключевое наблюдение: сервис B не упал — он просто стал медленным. Но этого достаточно, чтобы обрушить цепочку. Медленный ответ часто опаснее, чем быстрая ошибка. [Cache stampede](caching.md#cache-stampede) — частный случай: истечение TTL популярного ключа создаёт внезапную нагрузку на origin.
