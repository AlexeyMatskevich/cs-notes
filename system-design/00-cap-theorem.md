...
**Предпосылки:** базовое понимание [репликации](replication.md) (leader/follower, синхронная и асинхронная), понятие network partition, [ACID](../databases/postgresql/storage/00-acid.md) (достаточно знать, что Consistency в ACID — это про constraints).
...
CAP Consistency (она же [linearizability](01-consistency-models.md)) — требование к распределённой системе: после завершения записи любое последующее чтение с любого узла вернёт это значение или более новое. Система ведёт себя так, будто копия данных одна. Именно это свойство нарушается, когда New York отдаёт `balance = 100` вместо актуальных 50.
...
Кворум определяет, кто работает, а кто нет. Но CP-поведение проявляется и на уровне каждой отдельной записи: при синхронной репликации primary не подтверждает COMMIT, пока синхронная реплика не подтвердила получение [WAL](../databases/postgresql/durability/00-wal.md) (Write-Ahead Log).
