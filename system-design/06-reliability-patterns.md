...
**Предпосылки:** [HTTP](../networking/application/01-http.md) (запрос/ответ, статус-коды), [TCP](../networking/transport/01-tcp.md) (соединение, таймаут), клиент-серверная архитектура, понятие потока/треда и пула соединений.
...
**Отдельные очереди в [Sidekiq](../rails/sidekiq/06-concurrency-and-scaling.md):**
...
Практический пример идемпотентности в фоновых задачах — [гарантии Sidekiq](../rails/sidekiq/02-guarantees.md): at-least-once означает повторное выполнение, и каждый job должен быть идемпотентным.
