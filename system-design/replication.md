...
Этот механизм не специфичен для баз данных. Kafka реплицирует партиции между broker'ами ([ISR](../messaging/kafka/architecture/replication.md) — In-Sync Replicas), etcd и Consul хранят несколько копий конфигурации и метаданных кластера, CDN реплицирует контент на edge-серверы ближе к пользователям. Везде задача одна: не потерять данные при отказе и продолжить обслуживание. Конкретные реализации: [streaming replication в PostgreSQL](../databases/postgresql/distribution/replication.md) (WAL-поток, hot standby, Patroni) и [асинхронная репликация в Redis](../databases/redis/distribution/replication.md) (full resync, replication backlog, WAIT).
