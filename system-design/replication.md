...
**Асинхронная репликация.** Primary подтверждает запись клиенту сразу, не дожидаясь реплик. Это даёт низкую задержку записи, но создаёт два эффекта: чтения с реплик могут быть устаревшими (replication lag), а при отказе primary последние подтверждённые записи, не успевшие попасть на реплику, теряются — фактически [at-most-once](08-delivery-guarantees.md) на уровне репликации.
...
Решение для таких случаев — read-your-writes consistency: после записи направлять чтение на primary (или на реплику, которая гарантированно получила эти данные). Это частный случай из спектра [моделей консистентности](01-consistency-models.md), где каждая модель ослабляет или усиливает гарантии в зависимости от требований.
...
Кворум предотвращает split brain, но не определяет порядок действий: кто инициирует выборы, как кандидат собирает голоса, что происходит при одновременных кандидатах. Эти вопросы решают [алгоритмы консенсуса](03-consensus.md) — в частности, Raft использует рандомизированные таймауты и монотонные термы поверх кворума.
