...
**Предпосылки:** [Паттерны надёжности](06-reliability-patterns.md) (idempotency, circuit breaker, bulkhead), [Message Queues](09-message-queues.md) (temporal decoupling, pub/sub, consumer groups), [API Design](11-api-design.md) (REST, gRPC, контракты), базовое понимание SQL-транзакций (ACID, COMMIT/ROLLBACK).
...
Команда Referral хочет реагировать на заказы? Она пишет своего consumer и подписывается на событие через [pub/sub](09-message-queues.md). Команда Orders даже не знает об этом.
