...
Обработчик заказа отправляет HTTP-запрос к платёжному сервису — прямой вызов, который мы уже разобрали. Retry без idempotency key даёт at-least-once с риском двойного списания, с idempotency key — exactly-once обработку, без retry — at-most-once. Подробнее о retry и idempotency в контексте HTTP — в [API Design](11-api-design.md).

Платёжный сервис записывает транзакцию в базу данных. Эта запись должна попасть на реплику: leader отправляет follower'у [WAL-записи](../databases/postgresql/durability/00-wal.md) (журнал изменений — последовательный лог всех модификаций данных). При асинхронной репликации падение leader'а до отправки записи означает at-most-once для этой транзакции — реплика её не получит. При синхронной репликации follower подтверждает получение, что даёт at-least-once; при failover возможен replay уже применённых записей, но [LSN](../databases/postgresql/durability/00-wal.md) (позиция записи в WAL-потоке) обеспечивает идемпотентность — запись с уже применённым LSN не применяется повторно. Подробнее — в [репликации](replication.md).

Одновременно событие «платёж завершён» публикуется в очередь сообщений для аналитики, нотификаций и сверки. Общий механизм подтверждения в очередях — [ACK](09-message-queues.md#acknowledgment): пока consumer не подтвердил обработку, сообщение считается недоставленным. В Kafka consumer читает [партицию](../messaging/kafka/architecture/00-what-is-kafka.md) (append-only лог на диске брокера) с определённого [offset'а](../messaging/kafka/architecture/00-what-is-kafka.md) (позиция в партиции). Если consumer коммитит offset до обработки — at-most-once: при crash событие пропущено, и запись о платеже не появится в аналитике. Если коммитит после — at-least-once: при crash событие обработается повторно. Для аналитики дубликат некритичен; для триггера нотификации клиенту нужна дедупликация. Подробнее — в [разделе о Kafka](../messaging/kafka/index.md).
