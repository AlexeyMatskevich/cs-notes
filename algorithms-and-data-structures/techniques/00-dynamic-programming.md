...
Пример реального применения: оптимизация порядка JOIN в PostgreSQL использует перебор подмножеств для небольшого числа таблиц, а дальше переключается на эвристический режим (см. [порядок соединения](../../databases/postgresql/query-processing/01-join-order.md)).
