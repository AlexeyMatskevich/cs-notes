...
Обычно top-down проще написать и отладить, а bottom-up проще контролировать по памяти и порядку вычислений (особенно если важна [[computer/data-path/memory-hierarchy#локальность-данных-почему-кеш-вообще-помогает|локальность данных]] в кеше CPU).
...
Пример реального применения: оптимизация порядка JOIN в PostgreSQL использует перебор подмножеств для небольшого числа таблиц, а дальше переключается на эвристический режим (см. [порядок соединения](../../databases/postgresql/query-processing/join-order.md)).
