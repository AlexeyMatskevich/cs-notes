...
На случайных данных Timsort делает примерно столько же сравнений, что и обычный merge sort, — те же O(n log n) (на почти отсортированных он замечает уже готовые упорядоченные участки и сравнений делает заметно меньше, но здесь интересно не это). Для cache-aware взгляда важно другое: даже при равном числе сравнений Timsort экономит походы в RAM — фаза сортировки блоков целиком умещается в L1, и на современных процессорах это срезает реальное время в 1.5–3 раза.
