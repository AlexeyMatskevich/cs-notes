...
Процессор читает память [кэш-линиями](../../computer/data-path/00-memory-hierarchy.md#кеш-линия-минимальная-единица-обмена) — блоками фиксированного размера (часто 64 байта; зависит от CPU). Когда читаешь один байт, загружается вся линия вокруг него.

В «честном» массиве (элементы подряд) обход — последовательное чтение, следующий элемент почти наверняка уже в кэше. В массиве указателей каждый прыжок ведёт в произвольное место — потенциальный [cache miss](../../computer/data-path/00-memory-hierarchy.md#три-типа-промахов). Попадание в кэш стоит единицы циклов, промах до оперативной памяти — сотни. Поэтому «прыжки по указателям» часто доминируют над асимптотикой.
...
Массив реализует ADT Array, но размер фиксирован при создании. Если заранее неизвестно сколько элементов будет — выделять «с запасом» значит тратить память, выделять «впритык» — не влезет следующий элемент. [Динамический массив](02-dynamic-array.md) решает эту проблему, сохраняя O(1) доступ по индексу.
