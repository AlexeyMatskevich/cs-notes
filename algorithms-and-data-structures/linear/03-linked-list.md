...
Односвязный список добавляет минимум один указатель на узел — обычно 8 байт на 64-bit, плюс возможный padding из-за [выравнивания](../../computer/programmer-model/01-abi-and-data-layout.md). Двусвязный — два указателя, обычно 16 байт.
...
Вставка в начало — O(1), но доступ по индексу — O(n). Ещё хуже — узлы разбросаны по куче, каждый переход по указателю — потенциальный [cache miss](../../computer/data-path/00-memory-hierarchy.md#три-типа-промахов). Последовательный обход массива ([кэш-линии](../../computer/data-path/00-memory-hierarchy.md#кеш-линия-минимальная-единица-обмена) загружаются заранее) на практике часто в несколько раз быстрее, чем обход списка (случайные прыжки по памяти).
