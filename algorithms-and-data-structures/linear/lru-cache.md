...
В однопоточной программе это не проблема. Но сервер базы данных с тысячами параллельных запросов к кэшу — это постоянное ожидание на блокировке. [Clock-Sweep](clock-sweep.md) решает эту проблему, заменяя манипуляции со списком [атомарным инкрементом](../../linux/concurrency/synchronization.md).
