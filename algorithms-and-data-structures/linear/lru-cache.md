...
Только [хеш-таблица](hash-table.md) даёт O(1) доступ по ключу, но не отслеживает порядок использования. Только двусвязный [список](linked-list.md) даёт O(1) перемещение в начало, но не находит элемент по ключу за O(1). Решение — комбинация обеих структур.
...
Ключ хранится в узле, потому что при вытеснении берётся tail — самый старый элемент. Чтобы удалить его из [хеш-таблицы](hash-table.md), нужен ключ. Если ключ не хранится в узле, придётся искать его перебором всей [хеш-таблицы](hash-table.md) — O(n) вместо O(1).
...
`remove_node` обрабатывает три случая. В общем (узел в середине): соседи перелинковываются друг на друга — `prev.next` на `next`, `next.prev` на `prev`. Если узел — tail: предыдущий становится новым tail — та же проблема инвалидации [двух указателей на одну структуру](linked-list.md), что и в связном списке. Если узел единственный (он и head, и tail): оба обнуляются.
...
`add_to_head` вставляет узел в начало, делая его "свежим". Проверка `@tail = node` при пустом [списке](linked-list.md) — симметричная задача: если head обновился, tail может потребовать обновления.

### Критичный баг с порядком операций

При вытеснении нужно удалить tail из [хеш-таблицы](hash-table.md) и из [списка](linked-list.md). Порядок этих двух действий критичен: `remove_node(@tail)` обновляет `@tail` на предпоследний узел — после этого `@tail.key` указывает уже не на вытесняемый элемент.
...
[LRU](lru-cache.md) хорошо работает, когда паттерн доступа меняется: недавнее обращение — хороший предиктор будущего. Но что если паттерн стабилен и есть чёткое разделение на "горячие" и "холодные" данные? Тогда частота обращений — лучший сигнал, чем время последнего.

LFU (Least Frequently Used) вытесняет элемент с наименьшим количеством обращений за всё время. [LRU](lru-cache.md) смотрит на время последнего обращения, LFU — на частоту.
...
При стабильном паттерне доступа LFU лучше [LRU](lru-cache.md). При меняющемся — наоборот.

## Проблема: contention в многопоточной среде

[LRU](lru-cache.md) работает идеально по сложности, но при каждом обращении нужно переместить элемент в начало [списка](linked-list.md). Это 4 записи в указатели:
...
Эти изменения должны быть согласованы — иначе [список](linked-list.md) развалится. На практике это означает блокировку, пускающую только один поток за раз. Альтернатива — более сложные реализации без блокировок.
...
В однопоточной программе это не проблема. Но сервер базы данных с тысячами параллельных запросов к кэшу — это постоянное ожидание на блокировке. [Clock-Sweep](clock-sweep.md) решает эту проблему, заменяя манипуляции со [списком](linked-list.md) [атомарным инкрементом](../../linux/concurrency/synchronization.md).
