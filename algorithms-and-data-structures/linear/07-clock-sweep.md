...
LRU при каждом обращении перестраивает двусвязный список, что требует блокировки и создаёт bottleneck в многопоточной среде. Clock-sweep жертвует точностью вытеснения ради масштабируемости: вместо точного порядка по давности использования каждый элемент хранит `usage_count`, который инкрементируется атомарно при обращении — без блокировок общих структур.
