...
LRU-кэш перемещает элемент при каждом обращении: четыре записи в указатели плюс глобальная блокировка. При тысячах параллельных потоков это становится узким местом. Clock-sweep жертвует точностью вытеснения ради масштабируемости: вместо точного порядка по давности использования каждый элемент хранит `usage_count`, который инкрементируется при обращении [атомарной операцией](../../linux/concurrency/00-synchronization.md) — операцией, которую другие потоки наблюдают как единый шаг, без промежуточных значений. Атомарный инкремент обновляет только локальный счётчик элемента, не затрагивая общие структуры.
...
PostgreSQL использует clock-sweep для [shared buffers](../../databases/postgresql/durability/01-buffer-cache.md#алгоритм-вытеснения) — кэша страниц диска в разделяемой памяти, где тысячи backend-процессов конкурируют за доступ. Операционные системы применяют аналогичный алгоритм (CLOCK) для [page replacement](../../linux/foundations/05-virtual-memory.md) — вытеснения страниц виртуальной памяти на диск.
