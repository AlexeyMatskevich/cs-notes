...
Прямолинейная реализация — [односвязный список](linked-list.md), который помнит и голову, и хвост: добавление идёт в хвост, выдача — из головы, обе операции O(1). Но узлы списка разбросаны по памяти, и при большом потоке элементов процессор постоянно промахивается мимо кеша (см. [типы промахов](../../computer/data-path/memory-hierarchy.md#Три%20типа%20промахов)) — расплата за непрерывные прыжки по ссылкам. Кольцевой буфер (ring buffer) убирает эту расплату, держа элементы подряд в одном [[algorithms-and-data-structures/linear/array|массиве]].
