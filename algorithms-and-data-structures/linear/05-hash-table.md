...
При удалении в open addressing нельзя просто поставить nil — это сломает поиск элементов, которые "перепрыгнули" эту ячейку. Поэтому ставят специальный маркер — tombstone (надгробие на месте "умершего" элемента). При поиске tombstone означает "продолжай искать", при вставке — "можно занять".
