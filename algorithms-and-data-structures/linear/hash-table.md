...
Все рассмотренные структуры — [массив](array.md), [список](linked-list.md), [стек, очередь, дек](stack-queue-deque.md) — работают с позицией: индекс, начало, конец. Ключ доступа всегда число. Но данные часто адресуются иначе: веб-сервер хранит активные сессии по уникальному идентификатору, роутер маппит URL на обработчик, DNS-резолвер кэширует домены. Искать строковый ключ перебором [массива](array.md) — O(n). [Хеш-таблица](hash-table.md) даёт O(1) доступ по произвольному ключу.
...
На практике реализации часто комбинируют обе стратегии. Ruby `Hash` использует адаптивный подход: маленькие хеши (≤8 пар) хранятся в компактном [массиве](array.md) с линейным поиском, а при росте конвертируются в полноценную [хеш-таблицу](hash-table.md) с open addressing — подробности в [заметке о внутренностях Ruby Hash](../../ruby/internal/collections/hash.md).

### Проблема: нет порядка для вытеснения

O(1) доступ по ключу — есть. Но [хеш-таблица](hash-table.md) сама по себе не даёт порядка «по давности использования». Даже если конкретная реализация сохраняет порядок вставки, [LRU](lru-cache.md) зависит от обращений, а не от вставок.

Для кэша с ограниченным размером нужно вытеснять элементы. Какие? Нужна политика вытеснения — и структура, которая её поддерживает. [LRU-кэш](lru-cache.md) комбинирует [хеш-таблицу](hash-table.md) с двусвязным [списком](linked-list.md) для решения этой задачи.
