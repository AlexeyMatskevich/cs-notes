...
**Предпосылки:** [[algorithms-and-data-structures/linear/array|массив]] (индексация, формула адреса), [связный список](linked-list.md) (для стратегии цепочек), [указатели/ссылки](../../programming/memory.md), оценка сложности O(…), базовое чтение Ruby (методы, блоки, массивы и хеши).

<- [Стек, очередь, дек](stack-queue-deque.md) | [LRU-кэш](lru-cache.md) ->

[[algorithms-and-data-structures/linear/array|Массив]] и [список](linked-list.md) работают с позицией: индекс, начало, конец. Ключ доступа всегда число. Но искать данные часто нужно по другому ключу — по строке или другому значению, а не по номеру места. Активные сессии веб-сервера хранятся по строковому идентификатору; кэш доменных имён ищет запись по самому имени. Чтобы найти такой ключ в [[algorithms-and-data-structures/linear/array|массиве]], приходится перебирать элементы по одному — O(n). Нужна структура, которая по произвольному ключу сразу попадает в нужное место, за одну-две операции вместо тысячи. Такая структура называется хеш-таблицей.
