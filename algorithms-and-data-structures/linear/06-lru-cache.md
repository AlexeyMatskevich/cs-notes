...
В однопоточной программе это не проблема. Но сервер базы данных с тысячами параллельных запросов к кэшу — это постоянное ожидание на блокировке. [Clock-Sweep](07-clock-sweep.md) решает эту проблему, заменяя манипуляции со списком [атомарным инкрементом](../../linux/concurrency/00-synchronization.md).
