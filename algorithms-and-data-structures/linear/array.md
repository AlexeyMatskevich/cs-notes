...
[Массив](array.md) определяется тремя параметрами: адресом начала `base`, размером одного элемента `size` и количеством элементов `n`.
...
Процессор читает память [[computer/data-path/memory-hierarchy#кеш-линия-минимальная-единица-обмена|кэш-линиями]] — блоками фиксированного размера (часто 64 байта; зависит от CPU). Именно поэтому [[computer/data-path/memory-hierarchy#локальность-данных-почему-кеш-вообще-помогает|локальность данных]] становится практическим преимуществом массива: когда читаешь один байт, загружается вся линия вокруг него.

В «честном» массиве (элементы подряд) обход — последовательное чтение, следующий элемент почти наверняка уже в кэше. В массиве указателей каждый прыжок ведёт в произвольное место — потенциальный [[computer/data-path/memory-hierarchy#три-типа-промахов|cache miss]]. Попадание в кэш стоит единицы циклов, промах до оперативной памяти — сотни. Поэтому «прыжки по указателям» часто доминируют над асимптотикой.
...
[Массив](array.md) реализует ADT Array, но размер фиксирован при создании. Если заранее неизвестно сколько элементов будет — выделять «с запасом» значит тратить память, выделять «впритык» — не влезет следующий элемент. [Динамический массив](dynamic-array.md) решает эту проблему, сохраняя O(1) доступ по индексу.
