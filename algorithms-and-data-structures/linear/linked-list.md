...
Вставка в начало — O(1), но доступ по индексу — O(n). Ещё хуже — узлы разбросаны по куче, каждый переход по указателю — потенциальный [cache miss](../../computer/data-path/memory-hierarchy.md#три-типа-промахов). Последовательный обход массива ([кэш-линии](../../computer/data-path/memory-hierarchy.md#кеш-линия-минимальная-единица-обмена) загружаются заранее) на практике часто в несколько раз быстрее, чем обход списка (случайные прыжки по памяти).
