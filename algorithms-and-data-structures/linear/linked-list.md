...
Вставка в начало — O(1), но доступ по индексу — O(n). Ещё хуже — узлы разбросаны по куче, и на каждом шаге теряется [[computer/data-path/memory-hierarchy#локальность-данных-почему-кеш-вообще-помогает|локальность данных]]: каждый переход по указателю — потенциальный [[computer/data-path/memory-hierarchy#три-типа-промахов|cache miss]]. Последовательный обход массива ([[computer/data-path/memory-hierarchy#кеш-линия-минимальная-единица-обмена|кэш-линии]] загружаются заранее) на практике часто в несколько раз быстрее, чем обход списка (случайные прыжки по памяти).
...
На практике [динамический массив](dynamic-array.md) выигрывает в большинстве случаев из-за лучшей [[computer/data-path/memory-hierarchy#локальность-данных-почему-кеш-вообще-помогает|локальности данных]]. [Связный список](linked-list.md) — для задач, где ключевая операция — O(1) удаление/перемещение узла по указателю: [LRU-кэш](lru-cache.md), undo-history, некоторые реализации [очередей](stack-queue-deque.md). Полная реализация операций `remove_node` и `add_to_head` для двусвязного [списка](linked-list.md) показана в контексте [LRU-кэша](lru-cache.md), где эти операции составляют ядро структуры.
