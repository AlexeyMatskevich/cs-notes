...
Стек работает с одного конца. Но задания на принтере обрабатываются в порядке поступления, BFS обходит граф по слоям — здесь нужен доступ с противоположного конца: добавляем в хвост, забираем из головы. Это принцип FIFO — First In, First Out: первым пришёл, первым вышел.
