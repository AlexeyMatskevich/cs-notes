...
Стек реализуется на динамическом массиве (лучшая локальность, амортизированный O(1) для push) или на односвязном списке (O(1) без resize). На практике почти всегда массив. Call stack работает по LIFO: последняя вызванная функция завершается первой.
...
Работаем с head, а не tail: в односвязном списке `pop` с tail требует предыдущий узел — O(n). С head — O(1).

## Очередь

Стек работает с одним концом. Но задания на принтере обрабатываются в порядке поступления, BFS обходит граф по слоям — здесь нужен доступ с противоположного конца: добавляем в хвост, забираем из головы. Это принцип FIFO — First In, First Out: первым пришёл, первым вышел. Как очередь в магазине: кто раньше встал, тот раньше обслужен.
...
Очередь реализуется на ring buffer (локальность) или на односвязном списке с tail (проще код). Ring buffer обеспечивает амортизированный O(1), если буфер растёт при переполнении.
