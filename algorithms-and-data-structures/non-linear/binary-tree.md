...
<- [Дерево](tree.md) | далее: [Двоичное дерево поиска](binary-search-tree.md) · [[algorithms-and-data-structures/non-linear/heap|Куча]] ->
...
**Завершённое (complete)** — все уровни заполнены полностью, кроме, возможно, последнего, и последний заполняется слева направо без пропусков. Такая форма гарантирует минимальную высоту: дерево из n узлов имеет высоту ⌊log₂(n)⌋ (⌊ ⌋ — округление вниз). У завершённого дерева есть бонус: раз пропусков нет, узлы можно уложить подряд в [массив](../linear/array.md) по уровням и обойтись без [ссылок](../../programming/memory.md) на детей — позицию ребёнка вычисляют из позиции родителя арифметикой. На этом свойстве построена [[algorithms-and-data-structures/non-linear/heap|куча]].
