...
BST даёт O(log n) в среднем, но вырожденное дерево превращается в список с O(n). Для гарантированного O(log n) на произвольный поиск используют самобалансирующиеся деревья (AVL, red-black). Но если задача уже — не произвольный поиск по ключу, а только быстро добавить элемент и извлечь максимальный (или минимальный), есть более простая структура с **гарантированным** O(log n) и без риска вырождения: [куча](heap.md).
