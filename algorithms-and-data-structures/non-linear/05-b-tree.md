...
**Предпосылки:** оценка сложности в O(…), [двоичное дерево поиска (BST)](03-binary-search-tree.md) — инвариант порядка, поиск за O(h); понятие страницы дисковой памяти (блок фиксированного размера, обычно 4–16 KB, который диск читает за одну операцию).

BST хранит один ключ в узле. В оперативной памяти это работает хорошо, но на диске каждый узел — потенциальная отдельная операция чтения. При миллионе ключей BST имеет высоту ~20 (log₂ 10⁶ ≈ 20), значит один поиск может потребовать порядка 20 чтений. Для случайного доступа это быстро упирается в I/O (особенно на HDD).
