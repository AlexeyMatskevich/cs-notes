...
Для параллельного доступа [без блокировок (lock-free)](../../linux/concurrency/02-lock-free.md#определение-lock-free-vs-wait-free) skip list проще: каждый forward-указатель обновляется независимо атомарной операцией [CAS](../../linux/concurrency/00-synchronization.md) (compare-and-swap — «сравнить текущее значение с ожидаемым и заменить, если совпадает»). Самобалансирующееся дерево требует атомарного обновления нескольких связей одновременно, что значительно сложнее. Поэтому `ConcurrentSkipListMap` в Java — стандартная lock-free упорядоченная коллекция.
