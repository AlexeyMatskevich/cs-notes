...
Преимущество skip list — простота реализации и параллельного доступа. Вставка и удаление затрагивают только соседние узлы на затронутых уровнях; в сбалансированных деревьях ротация может захватить цепочку предков до корня, что требует более широкой блокировки или сложных CAS-цепочек. Поэтому lock-free реализации skip list (как `ConcurrentSkipListMap` в Java) значительно проще аналогичных для red-black деревьев.
