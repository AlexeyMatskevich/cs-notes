...
**Предпосылки:** [связный список](../linear/03-linked-list.md) (узел, указатели next), [двоичное дерево поиска](03-binary-search-tree.md) (инвариант, O(h) поиск, вырождение).

## Проблема: поиск в отсортированном списке

Отсортированный связный список позволяет вставить элемент за O(1), если известна позиция. Но найти эту позицию — O(n): приходится пройти все узлы последовательно. BST решает проблему поиска за O(log n), но в худшем случае вырождается в тот же список. Для гарантированного O(log n) применяют самобалансирующиеся деревья (AVL, red-black). Они после каждой вставки или удаления перестраивают часть дерева — перевешивают поддеревья (такие операции называют ротациями), чтобы высота оставалась логарифмической. Это работает, но ротации усложняют реализацию и делают параллельный доступ затратнее: одна ротация затрагивает несколько узлов одновременно, и все они должны быть заблокированы.

Skip list — альтернативный подход: вместо балансировки дерева — добавление «экспресс-уровней» поверх обычного отсортированного списка. При вставке элемента количество уровней, на которых он появится, определяется случайно. В среднем это даёт логарифмическое деление пространства поиска — O(log n) без ротаций и перебалансировки.
...
| Аспект | Skip list | BST (несбалансированное) | AVL / Red-black |
|--------|-----------|--------------------------|-----------------|
| Поиск (среднее) | O(log n) | O(log n) | O(log n) |
| Поиск (худшее) | O(n)* | O(n) | O(log n) |
| Вставка | O(log n) | O(log n) | O(log n) + ротации |
| Реализация | Проще | Проще | Сложнее |
| Параллельный доступ | Проще (локальные изменения) | — | Сложнее (ротации) |
...
Преимущество skip list — простота реализации и параллельного доступа. Вставка в skip list затрагивает только соседние узлы на затронутых уровнях — обычно 1–2 узла. В red-black дереве одна ротация перевешивает поддерево и затрагивает до трёх узлов одновременно (узел, его родитель и дядя); при каскадных ротациях блокировка может распространиться до корня. Для lock-free реализации skip list достаточно CAS на forward-указатели отдельных узлов — каждый уровень обновляется независимо. Lock-free red-black дерево требует атомарного обновления нескольких связей одновременно, что значительно сложнее. Поэтому `ConcurrentSkipListMap` в Java — стандартная lock-free упорядоченная коллекция, а lock-free red-black дерево в стандартных библиотеках практически не встречается.
