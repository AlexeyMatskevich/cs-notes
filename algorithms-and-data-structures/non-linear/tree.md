...
[Граф](graph.md) допускает произвольные связи, в том числе циклы и несколько путей между вершинами. Но представим файловую систему: папки содержат файлы и другие папки, образуя иерархию. У каждого файла ровно одна родительская папка, циклов нет (папка не может содержать саму себя), и из корня `/` можно добраться до любого файла ровно одним путём. [Граф](graph.md) для этого избыточен — лишние связи и циклы только мешают. Если ограничить [граф](graph.md) до связного ациклического, получится **дерево**.
...
Если ограничить число детей до двух, у каждого узла появляются фиксированные роли: левый и правый ребёнок. Это **бинарное дерево**: [Бинарное дерево](binary-tree.md).
