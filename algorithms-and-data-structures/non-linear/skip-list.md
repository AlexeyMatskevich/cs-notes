...
Для параллельного доступа [[linux/concurrency/lock-free#определение-lock-free-vs-wait-free|без блокировок (lock-free)]] [skip list](skip-list.md) проще: каждый forward-указатель обновляется независимо атомарной операцией [CAS](../../linux/concurrency/synchronization.md) (compare-and-swap — «сравнить текущее значение с ожидаемым и заменить, если совпадает»). Самобалансирующееся [дерево](tree.md) требует атомарного обновления нескольких связей одновременно, что значительно сложнее. Поэтому `ConcurrentSkipListMap` в Java — стандартная lock-free упорядоченная коллекция.
