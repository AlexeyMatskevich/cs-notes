...
Отсортированный [связный список](../linear/linked-list.md) позволяет вставить элемент за O(1), если известна позиция. Но найти эту позицию — O(n): приходится пройти все узлы последовательно. [BST](binary-search-tree.md) решает проблему поиска за O(log n) в среднем случае, но при неудачном порядке вставок вырождается в [список](../linear/linked-list.md) и даёт O(n). Чтобы гарантировать логарифмическую высоту, самобалансирующиеся [деревья](tree.md) после каждой вставки или удаления перестраивают часть структуры. Это работает, но усложняет реализацию и затрудняет параллельный доступ: перестройка затрагивает несколько узлов одновременно, и все они должны быть заблокированы.

[Skip list](skip-list.md) — альтернативный подход: вместо перестройки [дерева](tree.md) — добавление «экспресс-уровней» поверх обычного отсортированного [списка](../linear/linked-list.md). При вставке элемента количество уровней, на которых он появится, определяется случайно. В среднем это даёт логарифмическое деление пространства поиска — O(log n) без перестройки структуры.
...
### Сравнение с [деревьями](tree.md)

| Аспект | [Skip list](skip-list.md) | [BST](binary-search-tree.md) (несбалансированное) | Самобалансирующиеся [деревья](tree.md) |
|--------|-----------|--------------------------|-----------------|
...
Преимущество [skip list](skip-list.md) — простота реализации и параллельного доступа. Вставка в [skip list](skip-list.md) затрагивает только соседние узлы на затронутых уровнях — обычно 1–2 узла. В самобалансирующихся [деревьях](tree.md) перестройка может затронуть несколько узлов одновременно; при каскадной перестройке блокировка может распространиться до корня.

Для параллельного доступа [без блокировок (lock-free)](../../linux/concurrency/lock-free.md#определение-lock-free-vs-wait-free) [skip list](skip-list.md) проще: каждый forward-указатель обновляется независимо атомарной операцией [CAS](../../linux/concurrency/synchronization.md) (compare-and-swap — «сравнить текущее значение с ожидаемым и заменить, если совпадает»). Самобалансирующееся [дерево](tree.md) требует атомарного обновления нескольких связей одновременно, что значительно сложнее. Поэтому `ConcurrentSkipListMap` в Java — стандартная lock-free упорядоченная коллекция.
