...
Словарь инвертированного индекса, как и B-дерево, хранит ключи в отсортированном порядке и требует поддержки вставки, удаления и поиска за O(log n). Для такой упорядоченной структуры в памяти обычно используют самобалансирующиеся деревья — но есть более простая альтернатива, где баланс достигается не ротациями, а случайностью: [skip list](09-skip-list.md).
