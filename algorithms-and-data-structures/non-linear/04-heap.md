...
BST даёт O(log n) на поиск, вставку и удаление, но только в среднем случае — вырожденное дерево превращается в список с O(n). Если задача уже — не произвольный поиск по ключу, а быстро добавить элемент и извлечь максимальный (или минимальный), есть более простая структура с гарантированным O(log n) на вставку и извлечение: **куча**.
