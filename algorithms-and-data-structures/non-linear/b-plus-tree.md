...
В [B-дереве](b-tree.md) при split медиана **перемещается** в родителя — в листьях её больше нет. В B+ дереве при split листа медиана **копируется** в родителя, оставаясь в листе, потому что все данные должны быть доступны через листья.
...
При split внутренних узлов — как в обычном [B-дереве](b-tree.md): медиана перемещается, а не копируется. Копирование нужно было только потому, что лист обязан сохранять все данные у себя; внутренний узел данных не хранит, его ключи и так лишь разделители, поэтому медиану можно просто поднять.
...
При merge листьев в B+ дереве разделитель из родителя **просто удаляется** — все данные уже есть в листьях. В [B-дереве](b-tree.md) разделитель спускается в объединённый узел, потому что он содержит данные, которых нет в детях.
...
При merge внутренних узлов — как в обычном [B-дереве](b-tree.md) (разделитель спускается).
