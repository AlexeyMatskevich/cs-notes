...
**Предпосылки:** оценка сложности в O(…), [B-дерево](b-tree.md) — split/merge, параметр t, инварианты заполнения; [связный список](../linear/linked-list.md) — указатели prev/next.

В [B-дереве](b-tree.md) внутренние узлы хранят и ключи-разделители, и данные (или указатели на данные). Данные занимают место на странице — на неё помещается меньше ключей, [дерево](tree.md) становится выше, и для поиска нужно больше чтений с диска. [B+ дерево](b-plus-tree.md) убирает данные из внутренних узлов: они хранятся только в листьях.

## Ключевое отличие от B-дерева

| [B-дерево](b-tree.md) | [B+ дерево](b-plus-tree.md) |
|----------|-----------|
...
Минимально достаточен [односвязный список](../linear/linked-list.md) (только next) для обхода слева направо. Но двусвязный (prev + next) позволяет обходить листья в обоих направлениях, что нужно для запросов с обратной сортировкой.

Связанные листья делают эффективным **range scan** — сканирование диапазона. Запрос `SELECT * FROM employees WHERE salary BETWEEN 40000 AND 60000` в [B-дереве](b-tree.md) потребовал бы in-order обхода по внутренним узлам, поднимаясь и спускаясь для каждого следующего ключа. В [B+ дереве](b-plus-tree.md) достаточно одного спуска до листа с первым ключом ≥ 40000, а затем линейного прохода по связному [списку](../linear/linked-list.md) вправо до ключа, превышающего 60000. Каждый следующий лист — последовательное чтение соседней страницы, а не случайный доступ по [дереву](tree.md).
...
В [B-дереве](b-tree.md) при split медиана **перемещается** в родителя — в листьях её больше нет. В [B+ дереве](b-plus-tree.md) при split листа медиана **копируется** в родителя, оставаясь в листе, потому что все данные должны быть доступны через листья.
...
При split внутренних узлов — как в обычном [B-дереве](b-tree.md): медиана перемещается, а не копируется.
...
При merge листьев в [B+ дереве](b-plus-tree.md) разделитель из родителя **просто удаляется** — все данные уже есть в листьях. В [B-дереве](b-tree.md) разделитель спускается в объединённый узел, потому что он содержит данные, которых нет в детях.
...
[B+ дерево](b-plus-tree.md) — стандарт для баз данных (PostgreSQL, MySQL, SQLite) и файловых систем. Его split создаёт два узла, заполненных примерно наполовину. В худшем случае [дерево](tree.md) может быть заполнено всего на ~50%, а половина дискового пространства под индекс — пустой. [B* дерево](b-star-tree.md) решает эту проблему, повышая минимальное заполнение до 2/3.
