...
**Предпосылки:** [Исполнение](02-execution.md) — фреймы (METHOD, CFUNC, BLOCK), push/pop, PC/SP/EP. [Объекты и классы](04-objects-and-classes.md) — m_tbl, klass, super, метакласс. [Модули](05-modules.md) — iclass, include/prepend в цепочке super, поиск методов по цепочке. [Формы](06-shapes.md) — shape_id, инлайн-кеш для переменных.

В [заметке о модулях](05-modules.md) мы видели, как Ruby находит метод: начинает с класса получателя, проверяет таблицу методов, не нашёл — переходит по `super` к следующему. Но нашёл — и что дальше? `full_name` определён через `def`, `first_name` — через `attr_reader`, `times` — встроенный C-метод. Все три найдены по одному алгоритму, но вызываются совершенно по-разному. И ещё вопрос: неужели Ruby проходит по цепочке на *каждый* вызов?
...
Метод нашёлся сразу. Если бы его не было в `Mathematician`, VM перешёл бы к следующему узлу — модулю или суперклассу — и проверил бы его таблицу. Алгоритм — цикл по связному списку, уже знакомый из [заметки о модулях](05-modules.md):
...
Сравнение с кешем переменных (из [заметки о формах](06-shapes.md)): кеш методов ключится на класс получателя и инвалидируется явно (флаг на method entry). Кеш переменных ключится на `shape_id` объекта и инвалидируется неявно (`shape_id` изменился — сравнение не прошло). Кеш методов затрагивает все объекты класса при `include`; кеш переменных — только конкретный объект при добавлении новой переменной.
