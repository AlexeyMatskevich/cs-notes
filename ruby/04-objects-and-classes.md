...
- **Указатель на суперкласс** (`super`) — один родитель. Если суперкласс не указан, Ruby назначает `Object`. Когда Ruby ищет метод, он идёт по цепочке `super` вверх — подробности в [заметке о диспетчеризации](method-dispatch-wip.md).
