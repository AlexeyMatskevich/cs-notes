...
**Предпосылки:** [Компиляция](01-compilation.md) — каждый scope (метод, блок) = отдельный ISeq. [Исполнение](02-execution.md) — фреймы, EP, динамический доступ через `getlocal idx, level`. [Управление потоком](03-control-flow.md) — `throw` + catch tables для break/return.

В [заметке об исполнении](02-execution.md) мы видели, что блок `{ puts name }` обращается к переменной `name` из окружающего метода через цепочку EP. Но мы рассматривали это как техническую деталь — «блок видит переменные метода». На самом деле за этим стоит фундаментальная идея, которая объединяет блоки, `Proc` и `lambda` в одну концепцию.
...
Блок `{ |i| puts ... }` делает две вещи одновременно: принимает параметр `i` (как мини-метод) и читает `name` из метода `greet` (как часть метода). Эта «двойная природа» — не случайность синтаксиса. Это реализация *замыкания* (closure) — идеи, которую Сассман и Стил сформулировали в 1975 году для Scheme: замыкание — это сочетание функции и окружения, которое используется при её вызове.

В терминах Ruby: блок хранит две вещи — **код** (свой ISeq) и **ссылку на окружение** (EP метода `greet`). Дизассемблер показывает обе:
...
Два фрагмента — метод и блок. В методе `send :times` ссылается на «block in greet» — это ISeq блока, код замыкания. Внутри блока `getlocal_WC_1 name` читает переменную на один уровень вверх по EP — это ссылка на окружение. Эти два элемента мы уже разбирали по отдельности в [заметке о компиляции](01-compilation.md) и [заметке об исполнении](02-execution.md). Здесь они соединяются.
...
`invokeblock` не ищет метод по имени — блок уже передан при вызове. VM находит его в специальном слоте текущего фрейма: при вызове `greet` → `3.times` блок был записан как «block handler» в EP вызывающего метода (`ep[-1]`, тот самый specval из [заметки об исполнении](02-execution.md)).
...
Ruby избегает этого трюком: структура, описывающая блок (`rb_captured_block` в `vm_core.h`), состоит из трёх полей — `self`, `ep`, `code`. Эти три поля лежат в точно тех же позициях внутри фрейма (`rb_control_frame_t`). Поэтому при передаче блока-литерала VM не аллоцирует новый объект — он просто записывает ISeq блока в поле `block_code` текущего фрейма и передаёт указатель на часть фрейма как «описание блока». Это каст указателя, без malloc.
...
При создании `Proc` или `lambda` Ruby копирует фрейм стека в кучу (heap). Функция `vm_make_env_each()` в `vm.c` выполняет три шага:
...
Механизм — тот же `throw`, что мы видели в [заметке об управлении потоком](03-control-flow.md). `return` компилируется в `throw TAG_RETURN`. Когда VM обрабатывает throw, она ищет целевой фрейм по цепочке EP. Для lambda: VM видит флаг `VM_FRAME_FLAG_LAMBDA` на фрейме блока и останавливается — return выходит из lambda. Для proc: флага нет, VM продолжает искать — доходит до METHOD-фрейма окружающего метода и выходит из него.
...
- Исходники Ruby: `vm_core.h` (rb_captured_block — строка 878, rb_proc_t — строка 1285, rb_env_t — строка 1298, VM_FRAME_FLAG_LAMBDA — строка 1390), `vm.c` (VM_CFP_TO_CAPTURED_BLOCK — строка 282, vm_make_env_each — строка 1077), `vm_insnhelper.c` (vm_invoke_iseq_block — строка 5370, vm_invoke_block — строка 5488, return/lambda detection — строка 1837), `vm_args.c` (arg_setup_type — строка 31), `insns.def` (invokeblock — строка 1134, send с блоком — строка 847).
