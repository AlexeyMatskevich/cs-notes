...
Кеш инвалидируется при структурных изменениях: определение нового метода, `include`, `prepend`, изменение видимости. Ruby не использует глобальный счётчик для инвалидации — вместо этого каждая запись метода помечается как невалидная, и при следующем обращении кеш обновляется.
