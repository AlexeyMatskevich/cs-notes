...
Когда метод найден, VM определяет, как его вызвать: создать фрейм и исполнить ISeq (Ruby-метод), вызвать C-функцию напрямую (встроенный метод) или прочитать переменную без фрейма (`attr_reader`). Подробности — в [заметке о диспетчеризации](07-method-dispatch.md).
