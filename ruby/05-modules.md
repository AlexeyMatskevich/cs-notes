...
**Предпосылки:** [Объекты и классы](04-objects-and-classes.md) — RClass, таблица методов, указатель super, метакласс.

В [предыдущей заметке](04-objects-and-classes.md) мы видели, что класс — это объект с таблицей методов, таблицей констант и указателем `super` на суперкласс. Наследование через `super` позволяет подклассу использовать методы родителя. Но что если нужно разделить поведение между классами, которые не связаны наследованием?
...
Что делает `include`? Ruby создаёт **included class** — обёртку (внутренний тип `T_ICLASS`), которая **разделяет** таблицу методов с оригинальным модулем. Не копирует — именно разделяет. Эта обёртка вставляется в цепочку `super` между классом и его прежним суперклассом:
...
Когда метод найден, VM определяет, как его вызвать: создать фрейм и исполнить ISeq (Ruby-метод), вызвать C-функцию напрямую (встроенный метод) или прочитать переменную без фрейма (`attr_reader`). Подробности — в [заметке о диспетчеризации](method-dispatch-wip.md).

### Кеширование

Проходить по цепочке на каждый вызов было бы дорого. Ruby кеширует результат: каждый call site в скомпилированных инструкциях хранит **inline cache** — запоминает пару «класс получателя → найденный метод». При повторном вызове с тем же классом — поиск не нужен, метод вызывается напрямую.

Кеш инвалидируется при структурных изменениях: определение нового метода, `include`, `prepend`, изменение видимости. Ruby не использует глобальный счётчик для инвалидации — вместо этого каждая запись метода помечается как невалидная, и при следующем обращении кеш обновляется. Сравнение с кешем для инстанс-переменных (другой механизм, другие триггеры) — в [заметке о формах](06-shapes.md).
...
Допустим, мы хотим, чтобы методы `Professor` были доступны не экземплярам `Mathematician`, а самому классу — как методы класса. В [предыдущей заметке](04-objects-and-classes.md) мы видели, что методы класса хранятся в метаклассе. `extend` делает именно это — включает модуль в метакласс объекта:
...
Методы Ruby ищет по цепочке `super` — дереву наследования. Константы ищутся иначе.
...
Внутри VM эта цепочка хранится как связный список **CREF** (`rb_cref_t`): каждый узел содержит текущий класс или модуль и ссылку на внешнюю область. При каждом `class`/`module` Ruby добавляет новый узел в начало списка. В скомпилированных инструкциях обращение к константе — инструкция `opt_getconstant_path`, которая использует CREF текущего фрейма.
...
- Исходники Ruby: `class.c` (rb_include_class_new — создание included class; ensure_origin — prepend и origin class; rb_include_module — пропагация к подклассам), `method.h` (rb_cref_t — лексическая область), `vm_insnhelper.c` (vm_get_ev_const — поиск константы), `vm_callinfo.h` (rb_callcache — inline cache).
