...
**Предпосылки:** [Объекты и классы](04-objects-and-classes.md) — RObject, массив значений ivar, RBasic, shape_id (кратко). [Модули](05-modules.md) — инлайн-кеш для методов (кратко).
...
`attr_reader :first_name` работает по тому же пути. Разница — кеш хранится не в кеш-слоте ISeq, а в `rb_callcache`, том самом инлайн-кеше для вызова методов. Dispatch типа IVAR (из [заметки о диспетчеризации](method-dispatch-wip.md)) вызывает ту же `vm_getivar()`, но берёт кешированную пару из другого места.
...
## Кеш переменных и кеш методов

В [заметке о модулях](05-modules.md) мы видели инлайн-кеш для методов: call site запоминает пару «класс получателя → метод». Теперь мы видели инлайн-кеш для переменных: инструкция запоминает пару «`shape_id` → индекс». Оба избегают повторного поиска. Но работают по-разному.

| | Кеш методов | Кеш переменных |
|---|---|---|
| Ключ | класс получателя | `shape_id` объекта |
| Промах при | `include`, `prepend`, `define_method` | объект получает новую переменную |
| Инвалидация | явная: Ruby помечает записи невалидными | неявная: `shape_id` изменился, сравнение не прошло |
| Область влияния | все объекты класса | конкретный объект |

Метод-кеш инвалидируется при изменениях *иерархии* — `include` модуля или определение метода затрагивает все экземпляры класса. Кеш переменных «инвалидируется» при изменении *структуры конкретного объекта* — добавление новой переменной затрагивает только этот объект. Остальные объекты с тем же `shape_id` продолжают попадать в кеш.
