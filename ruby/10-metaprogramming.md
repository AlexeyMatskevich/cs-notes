...
**Предпосылки:** [Определение методов](08-method-definition.md) — CREF, `definemethod`, `class << obj`, self vs CREF. [Блоки](09-blocks.md) — замыкания, EP, stack-to-heap, Proc.

В [заметке об определении методов](08-method-definition.md) мы видели, что `def` всегда работает через CREF — лексическую область. Но `def` не создаёт замыкания: тело метода не видит локальных переменных окружающего scope. В [заметке о блоках](09-blocks.md) — обратная ситуация: блок является замыканием, он видит окружение через EP. Метапрограммирование в Ruby — это способы совместить оба механизма: менять, куда попадает метод (CREF), и что видит его код (EP).
...
Но `eval`, `instance_eval` и `define_method` используют блоки или строки — и блоки, как мы знаем из [заметки о блоках](09-blocks.md), являются замыканиями. Это открывает возможности, которых нет у `def`.
...
- Исходники Ruby: `method.h` (CREF_SINGLETON — IMEMO_FL_USER3), `eval_intern.h` (CREF_CLASS — строка 180, CREF_CLASS_FOR_DEFINITION — строка 191), `vm_insnhelper.c` (refined dispatch — строка 4617), `vm_eval.c` (specific_eval — строка 2267, yield_under — строка 2186, rb_obj_instance_eval — строка 2336), `vm_method.c` (resolve_refined_method — строка 2025, make_method_entry_refined — строка 1227), `vm.c` (invoke_block — строка 1736, invoke_iseq_block_from_c — строка 1774), `eval.c` (add_activated_refinement — строка 1512).
