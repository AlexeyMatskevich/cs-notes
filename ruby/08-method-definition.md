...
Первые три стадии разобраны в [заметке о компиляции](01-compilation.md), в этой заметке и в [заметке о диспетчеризации](07-method-dispatch.md). Четвёртая стадия — удаление — бывает двух видов, и разница между ними принципиальна.
...
`undef_method` действует иначе: вместо удаления записи Ruby заменяет её на заглушку типа `VM_METHOD_TYPE_UNDEF` (мы видели этот тип в [заметке о диспетчеризации](07-method-dispatch.md)). Поиск *находит* запись в `m_tbl`, но dispatch вызывает `method_missing`. Это блокирует наследование — даже если суперкласс имеет метод, подкласс с `undef` его не увидит:
