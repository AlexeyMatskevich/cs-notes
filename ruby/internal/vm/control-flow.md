...
Переходы бывают двух масштабов. Простой — прыжок внутри одного [ISeq](compilation.md): `if`, `while`, `until`. Сложный — прыжок через границу scope: `break` из [блока](../blocks.md) должен покинуть текущий фрейм и вернуть управление в родительский. Проследим оба на конкретной программе:
