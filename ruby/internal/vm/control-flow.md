...
← [Исполнение](execution.md) | [Объекты и классы](../object-model/objects-and-classes.md) →

В [предыдущей заметке](execution.md) VM шагала по инструкциям линейно: PC двигался вперёд, инструкция за инструкцией. Но `if`, `while`, `break` требуют менять направление. Как YARV реализует переходы?

Переходы бывают двух масштабов. Простой — прыжок внутри одного [ISeq](compilation.md) (плоского массива инструкций, в который компилируется один кусок кода): `if`, `while`, `until`. Сложный — прыжок через границу scope: `break` из [блока](../blocks.md) должен покинуть текущий фрейм и вернуть управление в родительский. Scope здесь — граница одного ISeq и его фрейма: метод, блок — каждый отдельный scope со своим ISeq. Проследим оба на конкретной программе:
...
[Блок](../blocks.md) — отдельный [ISeq](compilation.md), отдельный фрейм. Инструкция `jump` здесь не поможет: она умеет прыгать только внутри одного [ISeq](compilation.md). А `break` должен покинуть и [блок](../blocks.md), и C-метод `Integer#times`, и вернуть управление в основную программу — к `puts "done"`.

Для таких кросс-scope переходов Ruby использует два механизма: инструкцию **`throw`** и **таблицу перехвата** (catch table). Эта `throw` — внутренняя VM-инструкция, в которую компилируются `break`, `return`, `next`; в коде её не пишут, и с пользовательской парой `Kernel#throw`/`catch` она не связана.
