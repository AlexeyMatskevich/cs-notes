...
Это механизм [замыканий](../blocks.md): [блок](../blocks.md) «помнит» окружение, в котором был создан, через ссылку на родительский EP. Поэтому `{ puts name }` видит переменную `name` из [метода](../methods/method-definition.md) `greet`, даже когда между ними в стеке вызовов стоит C-фрейм `Integer#times`.
