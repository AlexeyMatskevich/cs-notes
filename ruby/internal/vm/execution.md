...
[ISeq](compilation.md) готов: плоский массив инструкций YARV плюс таблица локальных переменных. Но инструкции — это данные. Сами по себе `putobject 2` и `opt_plus` ничего не делают. Нужен механизм, который читает их одну за другой и выполняет, — виртуальная машина YARV.
...
setlocal_WC_0 str            стек: []           ← "hello" ушло в слот EP-1
                             (суффикс _WC_0 — WC = walk count,
                              сколько уровней вверх по цепочке EP пройти;
                              0 = текущий фрейм)
putself                      стек: [self]
...
Выше EP — рабочая область: значения, которые инструкции кладут и забирают. SP двигается на каждом шаге — значения приходят и уходят. EP остаётся на месте — это фиксированная «база», относительно которой адресуются переменные. EP — «полка» для переменных: фиксированная, на ней всегда лежит одно и то же. SP — «рабочий стол» для промежуточных вычислений: значения приходят и уходят на каждом шаге.
...
Так на уровне VM устроено [замыкание](../../../programming/closures.md) — функция, которая помнит окружение, где была создана: блок держит переменную `name` живой через ссылку на родительский EP. Поэтому `{ puts name }` видит `name` из [метода](../methods/method-definition.md) `greet`, даже когда между ними в стеке вызовов стоит C-фрейм `Integer#times`.
