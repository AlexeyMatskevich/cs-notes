...
`putobject 2` кладёт число `2` на стек. Второй `putobject 2` кладёт ещё одну двойку. `opt_plus` забирает два верхних значения, складывает их и кладёт результат `4` обратно. На стеке остаётся одно число — результат. YARV — стековая машина: операнды кладутся на стек, инструкция забирает с вершины. В отличие от [физических процессоров](../../../computer/00-cpu.md), где операнды хранятся в регистрах.
