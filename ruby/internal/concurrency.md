...
GVL защищает внутренние структуры [VM](vm/execution.md): таблицы [методов](methods/method-dispatch.md), constant tables, аллокатор [объектов](object-model/objects-and-classes.md). Без глобальной блокировки каждую из этих структур пришлось бы защищать отдельным локом (как делают JRuby и TruffleRuby). Исторически MRI выбрал простоту: один лок на всё, потому что Ruby ориентирован на I/O-bound задачи, а C-расширения напрямую манипулируют Ruby-объектами без синхронизации.
...
Timer thread периодически ставит флаг прерывания. Поток, держащий GVL, проверяет этот флаг между [байткод](vm/compilation.md)-инструкциями и при необходимости уступает GVL другому потоку. С Ruby 3.4 квант можно настраивать через `RUBY_THREAD_TIMESLICE` (в миллисекундах).
...
GVL гарантирует: в каждый момент только один поток исполняет [байткод](vm/compilation.md). Переключение между потоками происходит между [байткод](vm/compilation.md)-инструкциями, а не между Ruby-выражениями. Одна строка Ruby — несколько инструкций:
