...
Метод нашёлся сразу. Если бы его не было в `Mathematician`, VM перешёл бы к следующему узлу — модулю или суперклассу — и проверил бы его таблицу. Алгоритм — цикл по связному списку, уже знакомый из [заметки о модулях](../object-model/01-modules.md):
...
Сравнение с кешем переменных (из [заметки о формах](../object-model/02-shapes.md)): кеш методов ключится на класс получателя и инвалидируется явно (флаг на method entry). Кеш переменных ключится на `shape_id` объекта и инвалидируется неявно (`shape_id` изменился — сравнение не прошло). Кеш методов затрагивает все объекты класса при `include`; кеш переменных — только конкретный объект при добавлении новой переменной.
