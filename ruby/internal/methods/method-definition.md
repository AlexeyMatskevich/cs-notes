...
> [!info]- Предпосылки
> [Компиляция](../vm/compilation.md) — ISeq. [Исполнение](../vm/execution.md) — фреймы, `self`, EP. [Объекты и классы](../object-model/objects-and-classes.md) — метакласс, singleton-класс, m_tbl. [Модули](../object-model/modules.md) — поиск методов по цепочке super, лексическая область (CREF).

← [Диспетчеризация методов](method-dispatch.md) | [Блоки](../blocks.md) →

`def` компилируется в отдельный [ISeq](../vm/compilation.md) для тела метода; поиск по цепочке `super` потом находит этот метод и вызывает его ([диспетчеризация](method-dispatch.md)). Но между этими двумя шагами есть пропущенный: как скомпилированный [ISeq](../vm/compilation.md) попадает в таблицу методов класса?
...
Мы уже знаем из [заметки о компиляции](../vm/compilation.md), что при парсинге Ruby создаёт отдельный ISeq для `display`. Но куда он попадает при выполнении? В таблицу методов `Quote`. Вопрос — как Ruby узнаёт, что целевой класс именно `Quote`.

Ответ: из **лексической области**. Когда Ruby встречает `class Quote`, он создаёт структуру [`CREF`](../object-model/modules.md) (`rb_cref_t` в исходниках), которая содержит два ключевых поля — `klass_or_self` (указатель на класс `Quote`) и `next` (ссылка на предыдущую область). CREF-ы образуют цепочку: каждый вложенный `class`/`module` добавляет звено.

Эта цепочка хранится в служебном слоте рядом с базой фрейма. Локальные переменные адресуются относительно EP как `EP - idx` (из [заметки об исполнении](../vm/execution.md)); CREF лежит не среди них, а в отдельной зарезервированной ячейке того же окружения (на индексе `-2`). В ней VM хранит CREF текущей области — или, для фреймов другого типа, описание текущего метода. Функция `vm_env_cref()` извлекает CREF, проходя по цепочке EP.
...
3. Вызывает `rb_add_method_iseq()`, которая вставляет в `m_tbl` этого класса **method entry** — запись, описывающую метод (тип, тело, видимость); в [заметке о диспетчеризации](method-dispatch.md) мы видели её как «описание метода».
...
Здесь `self` — это класс `Quote` (внутри области `class Quote` значение `self` равно самому классу). Но `def self.display` работает иначе, чем обычный `def`. Компилятор генерирует другую инструкцию — `definesmethod`. Она не использует CREF. Вместо этого:

1. Вычисляет выражение перед точкой (`self` → `Quote`).
2. Берёт `CLASS_OF` этого объекта. Для класса `Quote` это его **метакласс** (из [заметки об объектах](../object-model/objects-and-classes.md)).
...
Внутри это инструкция `defineclass` с типом `VM_DEFINECLASS_TYPE_SINGLETON_CLASS`.
...
Именно поэтому `Module.nesting` (возвращает стек CREF) не меняется при вызове метода: какой CREF принадлежит методу, определяется его лексическим положением на компиляции и записывается вместе с ISeq. При вызове это значение лишь переносится в окружение фрейма, откуда `vm_env_cref()` его и читает — оно не пересчитывается. `self` же, наоборот, вычисляется заново на каждом вызове.
...
Первые три стадии разобраны в [заметке о компиляции](../vm/compilation.md), в этой заметке и в [заметке о диспетчеризации](method-dispatch.md). Четвёртая стадия — удаление — бывает двух видов, и разница между ними принципиальна.
...
`undef_method` действует иначе: вместо удаления записи Ruby заменяет её на заглушку типа `VM_METHOD_TYPE_UNDEF` (мы видели этот тип в [заметке о диспетчеризации](method-dispatch.md)). Поиск *находит* запись в `m_tbl`, но dispatch вызывает `method_missing`. Это блокирует наследование — даже если суперкласс имеет метод, подкласс с `undef` его не увидит:
