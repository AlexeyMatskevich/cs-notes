...
Поиск [метода](method-definition.md) идёт по цепочке `super`: от класса получателя, проверяя таблицу методов на каждом узле, пока не найдёт ([модули](../object-model/modules.md)). Но нашёл — и что дальше? `full_name` определён через `def`, `first_name` — через `attr_reader`, `times` — встроенный C-метод. Все три найдены по одному алгоритму, но вызываются совершенно по-разному. И ещё вопрос: неужели Ruby проходит по цепочке на *каждый* вызов?
...
Метод нашёлся сразу. Если бы его не было в `Mathematician`, VM перешёл бы к следующему узлу — модулю или суперклассу — и проверил бы его таблицу. Это всё тот же обход связного списка по `super`, прожитый в [заметке о модулях](../object-model/modules.md): каждый узел — обычный `RClass` с таблицей методов (`m_tbl`) и указателем `super`, а include-обёртки (iclass) и оригинальные классы от prepend для цикла неразличимы.

Новое здесь — не сам обход, а его результат.
...
При нарушении видимости VM не бросает ошибку напрямую — вместо этого направляет вызов в `method_missing`, единый запасной путь для всех неуспешных вызовов (полностью разбирается ниже). Туда передаётся код причины — `MISSING_PRIVATE` или `MISSING_PROTECTED`, — а стандартная реализация `method_missing` превращает его в `NoMethodError` с соответствующим сообщением.
...
Двенадцать типов покрывают все способы определения методов в Ruby. Четыре основных — ISEQ, CFUNC, IVAR, ATTRSET — разобраны выше. Остальные встречаются реже:
