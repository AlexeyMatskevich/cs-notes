...
← [Метапрограммирование](metaprogramming.md) | [[ruby/internal/collections/string|String]]

В [[ruby/internal/vm/execution|заметке об исполнении]] мы видели цикл интерпретатора: прочитать инструкцию YARV, декодировать её, выполнить, передвинуть PC. Этот цикл универсален, но у него есть цена: непрямые ветвления, проверки типов, обслуживание стека значений. Пока код вызывается редко, эта цена почти не видна. Но на горячем пути она начинает конкурировать с полезной работой самого метода.
