...
В [заметке об исполнении](vm/execution.md) мы видели цикл интерпретатора: прочитать инструкцию YARV, декодировать её, выполнить, передвинуть PC. Этот цикл универсален, но у него есть цена: непрямые ветвления, проверки типов, обслуживание стека значений. Пока код вызывается редко, эта цена почти не видна. Но на горячем пути она начинает конкурировать с полезной работой самого метода.

## Что такое JIT

На уровне общей [базы по программированию](../../programming/programming.md) JIT уже знаком как дополнительная [компиляция](vm/compilation.md) поверх виртуальной машины. **JIT** расшифровывается как **just-in-time**: программа начинает работу через [VM](vm/execution.md), а горячие участки потом переводятся в машинный код прямо во время выполнения.

Для [CRuby](../../ruby.md) это не альтернатива [YARV](../../ruby.md)-[байткоду](vm/compilation.md), а дополнительный слой поверх него. Когда некоторый [ISeq](vm/compilation.md) исполняется часто, [CRuby](../../ruby.md) переводит его в машинный код [процессора](../../computer/cpu.md). Следующий вызов идёт уже не через общий цикл интерпретатора, а напрямую в сгенерированный код. Компилятор по-прежнему создаёт [ISeq](vm/compilation.md), а JIT ускоряет только самые горячие участки.
...
- [формы объектов](object-model/shapes.md) дают дешёвый ключ для guards на ivar-доступ;
- [диспетчеризация методов](methods/method-dispatch.md) уже имеет кеши и точки инвалидации;
...
Общий принцип JIT один и тот же, но в [CRuby](../../ruby.md) он реализован двумя разными стратегиями. YJIT старается рано дать ускорение за счёт быстрой [компиляции](vm/compilation.md) небольших участков. ZJIT идёт в сторону более тяжёлой [компиляции](vm/compilation.md) [метода](methods/method-dispatch.md) целиком после сбора профиля.

### YJIT: быстрый старт и постепенное покрытие

YJIT появился в Ruby 3.1. Его идея — **Basic Block Versioning (BBV)**: компилировать не весь [метод](methods/method-dispatch.md) сразу, а маленькие прямолинейные участки [байткода](vm/compilation.md) по мере исполнения.
...
- объект получает другую [форму](object-model/shapes.md), потому что часть экземпляров инициализируется иначе;
- [метод](methods/method-dispatch.md) или константа переопределяются во время работы программы;
