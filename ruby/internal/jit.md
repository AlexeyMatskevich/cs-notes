...
Чтобы увидеть, как этот лимит расходуется на конкретной нагрузке, YJIT отдаёт runtime-статистику через `RubyVM::YJIT.runtime_stats`: сколько кода скомпилировано, сколько случилось side exit'ов, какая доля инструкций исполняется внутри YJIT, а не в интерпретаторе. По этим числам видно, упирается ли горячий путь в потолок памяти или «плывёт» из-за слишком частых выходов обратно.
