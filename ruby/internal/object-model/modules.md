...
← [Объекты и классы](objects-and-classes.md) | [Формы объектов](shapes.md) →

Класс — это объект с таблицей [методов](../methods/method-definition.md), таблицей констант и указателем `super` на суперкласс ([объекты и классы](objects-and-classes.md)); наследование через `super` отдаёт подклассу методы родителя. Но наследование выстраивает одну линию предков, а поведение часто нужно разделить между классами из разных линий.
...
Что делает `include`? Ruby создаёт **included class** (`T_ICLASS`) — класс-обёртку для включённого модуля, которая **разделяет** таблицу методов с оригинальным модулем. Не копирует — именно разделяет. Эта обёртка вставляется в цепочку `super` между классом и его прежним суперклассом:
...
Модули встроены в цепочку `super` — теперь проследим, как Ruby проходит по ней при вызове метода. Создадим математика и зададим имя:
...
Когда метод найден, его остаётся вызвать — как именно VM это делает, разбирает [заметка о диспетчеризации](../methods/method-dispatch.md).
...
Теперь `Professor#name` найдётся первым — он ближе всех к началу цепочки. А `super` внутри него продолжает ту же прогулку по `super` с узла, следующего за `Professor`; следующий узел — origin, где и лежит исходный `name` из `attr_accessor`. Поэтому `"Prof. #{super}"` сначала зовёт метод модуля, а через `super` дотягивается до оригинала. Снаружи origin невидим: `ancestors` показывает `[Professor, Mathematician, ...]`, скрывая внутреннюю механику.

### Extend: include в метакласс

Допустим, методы `Professor` нужны не экземплярам `Mathematician`, а самому классу — как методы класса. Методы класса хранятся в его [метаклассе](objects-and-classes.md). `extend` включает модуль именно туда — в метакласс объекта:
