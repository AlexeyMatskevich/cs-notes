...
В [предыдущей заметке](objects-and-classes.md) мы видели, что класс — это объект с таблицей методов, таблицей констант и указателем `super` на суперкласс. Наследование через `super` позволяет подклассу использовать методы родителя. Но что если нужно разделить поведение между классами, которые не связаны наследованием?
