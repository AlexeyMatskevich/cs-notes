...
Immediate values — полноценные объекты: у них есть класс, они отвечают на сообщения. Но у них нет структуры в памяти, а значит, нет и массива, куда писать значения инстанс-переменных. Хранить ivar буквально негде: одно и то же число `5` — это всегда один и тот же VALUE `11`, у него нет персонального участка памяти, который можно было бы пометить переменной. Поэтому Ruby делает все такие значения замороженными (frozen) — попытка записи падает сразу, по frozen-пути:
...
Класс содержит метод `full_name`, константу `FIELD` и знает о переменных `@first_name`, `@last_name`. Класс также может наследоваться от другого класса. Всю эту информацию Ruby хранит в структуре `RClass`:

- **Таблица методов** (`m_tbl`) — [хеш-таблица](../../../algorithms-and-data-structures/linear/hash-table.md): имя метода → определение (включая скомпилированный ISeq для Ruby-методов — [компиляция](../vm/compilation.md)).
...
Классы — тоже объекты. `Mathematician.class` возвращает `Class`. У класса есть собственный `klass`, собственные флаги, собственные инстанс-переменные. Отсюда видно, чем класс отличается от обычного объекта: объекту достаточно `klass` и значений ivar, но классу нужно где-то держать методы, константы и связь с родителем — под каждую из этих ролей в `RClass` заведено своё хранилище. То, что для обычного объекта живёт в его классе, для класса хранится в нём самом.
