...
Кодирование числа прямо в VALUE напоминает принцип [tagged pointers на уровне ABI](../../../computer/programmer-model/01-abi-and-data-layout.md) — младшие биты отличают тип данных от указателя.
...
- **Таблица методов** (`m_tbl`) — [хеш-табл��ца](../../../algorithms-and-data-structures/linear/05-hash-table.md): имя метода → определение (включая скомпилированный ISeq для Ruby-методов, о котором мы говорили в [заметке о компиляции](../vm/01-compilation.md)).
- **Таблица констант** (`const_tbl`) — хеш: имя константы → значение.
- **Указатель на суперкласс** (`super`) — один родитель. Если суперкласс не указан, Ruby назначает `Object`. Когда Ruby ищет метод, он идёт по цепочке `super` вверх — подробности в [заметке о модулях](01-modules.md).
