...
`attr_reader :first_name` работает по тому же пути. Разница — кеш хранится не в кеш-слоте ISeq, а в `rb_callcache` — структуре, связанной с вызовом метода (подробнее в [заметке о диспетчеризации](../methods/00-method-dispatch.md)). При диспетчеризации типа IVAR вызывается та же `vm_getivar()`, но кешированная пара берётся из другого места.
