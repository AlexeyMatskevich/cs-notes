...
> [!info]- Предпосылки
> [Объекты и классы](objects-and-classes.md) — RObject, массив значений ivar, RBasic, shape_id (кратко); [Компиляция](../vm/compilation.md) — ISeq, дизассемблер `RubyVM::InstructionSequence`; [Исполнение](../vm/execution.md) — VM-цикл, инструкции байткода.

← [Модули](modules.md) | дальше — ветка «методы»: [Диспетчеризация методов](../methods/method-dispatch.md), ветка «память»: [GC](../gc.md)

`RObject` хранит массив *значений* инстанс-переменных — без имён ([объекты и классы](objects-and-classes.md)): `"Leonhard"` в ячейке 0, `"Euler"` — в ячейке 1. Но как Ruby узнаёт, что `@first_name` — это ячейка 0, а `@last_name` — ячейка 1? Когда [VM](../vm/execution.md) выполняет `getinstancevariable @first_name`, ему нужна таблица, которая преобразует имя переменной в индекс.
...
Дерево форм глобально и общее для всего процесса: один объект породил форму — она остаётся доступной любому другому объекту с такой же структурой. Поэтому формы не освобождаются вместе с объектами и не собираются мусором — дерево только растёт за время жизни процесса. Пока вариаций структуры немного, это дёшево, и переиспользование форм окупается. Но неограниченный рост — реальная цена: чем больше различных путей присвоения переменных, тем больше веток в дереве.
...
Принцип тот же, что у [аппаратных кешей процессора](../../../computer/data-path/memory-hierarchy.md): запомнить результат дорогого поиска, чтобы не повторять его. Разница в уровне — здесь кеш живёт в структурах самой VM, в байткоде метода, а не в кремнии.
...
В исходниках VM этот путь — функция `vm_getivar()` (горячий путь чтения ivar): она выполняет `if (LIKELY(cached_id == shape_id))` и при совпадении сразу возвращает `ivar_list[index]`. При промахе вызывается `fill_ivar_cache()`, которая ищет индекс и записывает новый результат в кеш-слот.
