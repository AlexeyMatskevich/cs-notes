...
В [заметке об объектах](objects-and-classes.md) мы видели, что `RObject` хранит массив *значений* инстанс-переменных — без имён. `"Leonhard"` лежит в ячейке 0, `"Euler"` — в ячейке 1. Но как Ruby узнаёт, что `@first_name` — это ячейка 0, а `@last_name` — ячейка 1? Когда [VM](../vm/execution.md) выполняет `getinstancevariable @first_name`, ему нужна таблица, которая преобразует имя переменной в индекс.
