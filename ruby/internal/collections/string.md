...
Эти значения не задаются заранее — строка проходит через них по мере работы с ней. Только что созданная строка несёт UNKNOWN: содержимое ещё не сканировалось. Первая операция, которой важно содержимое (`length`, сравнение, поиск), проходит байты один раз и кеширует итог — 7BIT, VALID или BROKEN. Дальше операции читают готовый ответ, не пересканируя. Запись в строку (`<<`, `gsub!`) сбрасывает coderange обратно в UNKNOWN: дописанные байты могли изменить картину, и кеш больше не действителен — следующая операция пересчитает его.
