...
> [!info]- Предпосылки
> [хеш-таблица](../../../algorithms-and-data-structures/linear/hash-table.md) (хеш-функция, коллизии, open addressing, load factor, рехеширование), [Array](array.md) (embedded/heap-паттерн), [объекты и классы](../object-model/objects-and-classes.md) (VALUE, RBasic), [GC](../gc.md) (VWA, слоты), [[computer/data-path/memory-hierarchy#Локальность данных: почему кеш вообще помогает|иерархия памяти]] (cache lines, стоимость cache miss).

← [Array](array.md) | [String](string.md) →

[Array](array.md) даёт доступ по числовому индексу за O(1). Но HTTP-заголовки, опции [метода](../methods/method-definition.md), конфигурация — это пары ключ-значение, где ключ не число, а строка или символ. Поиск по ключу в массиве — перебор всех элементов, O(n). [Хеш-таблица](../../../algorithms-and-data-structures/linear/hash-table.md) решает это за O(1) через хеш-функцию и массив бакетов. Но в Ruby-программе большинство хешей маленькие — 3–7 ключей: `params`, `options`, заголовки отдельного HTTP-ответа. Поддерживать полноценную хеш-таблицу с массивом бакетов ради пяти пар расточительно: вычисление хеша, деление по модулю, косвенное обращение через массив индексов дороже, чем пройти по пяти парам последовательно.
...
AR table размещается прямо в слоте объекта `RHash`, сразу после заголовка — без отдельной аллокации. Вся структура компактна и укладывается в несколько соседних cache lines процессора (по 64 байта). Последовательный обход 8 hints выигрывает за счёт [[computer/data-path/memory-hierarchy#Локальность данных: почему кеш вообще помогает|пространственной локальности кеша]] — все 8 байт hints лежат рядом в памяти.

### Поиск: hints вместо полного хеширования

Когда Ruby ищет ключ `:age` в `params`, он не вычисляет хеш по модулю и не идёт в массив бакетов. Вместо этого:
...
Hint из 8 бит даёт 256 вариантов, поэтому два разных ключа совпадают по hint редко — вероятность ложного совпадения 1/256. Полное сравнение ключа запускается только при совпадении hint, а оно почти всегда одно: на 8 элементах при такой вероятности до полного сравнения дело доходит обычно не больше одного раза. А сравнение 8 однобайтовых hints — это несколько тактов процессора, всё в кеше.

Для сравнения, полноценная хеш-таблица на тех же 3 элементах потребовала бы: вычисление хеша по модулю (деление — одна из самых дорогих арифметических операций), обращение к массиву бинов (возможный cache miss), затем обращение к массиву записей (ещё один cache miss). На маленьких данных непрямые обращения к памяти стоят дороже, чем линейный проход по компактной структуре.

### Удаление из AR table

При удалении элемента из AR table Ruby сдвигает оставшиеся пары и hints, заполняя пробел — как при удалении из обычного массива. Tombstones не нужны: нет цепочек поиска, которые могли бы разорваться. Стоимость — O(n) сдвиг, но при n ≤ 8 он укладывается в пределах одной компактной структуры в кеше.
...
Ruby извлекает ключи из AR table и заново вычисляет их полные хеши — в AR table хранятся только 8-битные hints, не полные хеши. Затем создаёт ST table нужного размера и переносит записи. Важный момент: вычисление хеша ключа — это вызов метода `#hash`, то есть Ruby-кода. Пользовательский `#hash` может модифицировать сам хеш (reentrancy). Если бы Ruby вычислял хеш и сразу вставлял в ST table, модификация хеша внутри `#hash` могла бы повредить недостроенную таблицу. Поэтому код перехода сначала собирает все хеши в локальный массив, а потом строит ST table из уже вычисленных значений.
