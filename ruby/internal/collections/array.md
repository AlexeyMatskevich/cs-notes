...
Массив из 1000 элементов занимает 8 КБ данных (1000 × 8 байт VALUE). Если каждый вызов `ary[100..200]` копирует 100 элементов — это 800 байт побайтового копирования буфера на каждый срез. В коде, активно работающем со срезами (парсинг, обработка данных), эти копии складываются в заметный расход CPU и памяти.
...
Единственный механизм возврата памяти — переход heap → embedded. Когда длина массива после `pop`, `shift`, `clear` или `slice!` снова помещается в слот, Ruby копирует элементы из heap-буфера обратно в слот, освобождает буфер через `free` и выставляет `RARRAY_EMBED_FLAG`. Условия: массив не shared и не frozen (замороженный — объект, помеченный неизменяемым; его буфер трогать нельзя). Для массива в слоте 80 байт это означает ≤8 элементов — типичный порог для небольших коллекций. Но массив из 100 элементов в heap-буфере на 10 000 не получит ни уменьшения буфера, ни перехода в embedded.
