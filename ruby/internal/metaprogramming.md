...
`def` направляет метод через [[ruby/internal/methods/method-definition|CREF]] — лексическую область, — но не создаёт замыкания: тело метода не видит локальных переменных окружающего scope. [Блок](blocks.md) — обратная ситуация: он является замыканием и видит окружение через [[ruby/internal/vm/execution|EP]]. Метапрограммирование в Ruby — это способы совместить оба механизма: менять, куда попадает метод (CREF), и что видит его код (EP).

## Как обойти границу def

Границу `def` обходят `eval`, `instance_eval` и `define_method`: они исполняют код из блоков или строк, а [блок](blocks.md) — замыкание. Так тело метода получает доступ к окружающему scope, которого у `def` нет.
...
**1. Замыкание сохраняется.** `yield_under()` копирует `rb_captured_block` (описание блока без отдельной аллокации, знакомое по [заметке о блоках](blocks.md)), оставляя оригинальный EP. Локальные переменные доступны через цепочку EP — как у обычного блока.
...
**3. Лексическая область переключается — лениво.** Ruby создаёт новый CREF с флагом `CREF_SINGLETON` и сохраняет в `klass_or_self` сам объект-receiver (не его класс). Если внутри `instance_eval` выполнить `def`, Ruby вызовет `CREF_CLASS_FOR_DEFINITION()` — функцию, которая решает, в какой класс попадёт определяемый метод, — и только в этот момент создаст singleton-класс через `rb_singleton_class()`. Если `def` нет — singleton-класс не создаётся вообще.
...
Внутри Ruby создаёт method entry типа `VM_METHOD_TYPE_BMETHOD` (block-method — method entry, чьё тело хранит Proc-замыкание, а не ISeq), который хранит ссылку на `rb_proc_t`. Когда метод вызывается, Ruby перезаписывает `self` на receiver'а (как у `instance_eval`), но EP замыкания сохраняется — блок по-прежнему видит `level`.

Существенное следствие: методы, определённые через `define_method`, **медленнее** обычных `def`-методов. Для ISEQ-метода dispatch может использовать оптимизированный путь без создания Proc. Для BMETHOD — всегда вызов тела блока из C-кода через `invoke_iseq_block_from_c()`, лишний слой косвенности.
...
**2. Оригинальный метод помечается.** Если в целевом классе уже есть метод с таким именем, `make_method_entry_refined()` оборачивает его: тип меняется на `VM_METHOD_TYPE_REFINED`, а оригинальный method entry сохраняется внутри обёртки — чтобы было к чему вернуться, когда refinement неактивен.

**3. `using` активирует.** `using Shout` сохраняет в поле `refinements` текущего CREF хеш, который сопоставляет уточняемому классу (`String`) refinement-модуль, активный в этой лексической области. При диспетчеризации, когда Ruby встречает метод типа REFINED, он проверяет CREF текущей лексической области. Если активный refinement найден — вызывает уточнённый метод; если нет — вызывает оригинальный.
...
Шесть способов определить метод различаются по двум осям: куда попадает метод (в текущий класс или в singleton class) и что видит его тело (замыкание текущего scope или нет).
...
`eval` в этой таблице нет: он не добавляет method entry в класс, а исполняет код в чужом окружении — это вторая ось (что видит код), которую он показывает в чистом виде.
