...
`invokeblock` не ищет метод по имени — блок уже передан при вызове. VM находит его в специальном слоте текущего фрейма: при вызове `greet` → `3.times` блок был записан как «block handler» в EP вызывающего метода (`ep[-1]`, тот самый specval из [[ruby/internal/vm/execution|заметки об исполнении]]).
