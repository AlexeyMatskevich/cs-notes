...
В [заметке об исполнении](vm/02-execution.md) мы видели, что блок `{ puts name }` обращается к переменной `name` из окружающего метода через цепочку [EP](vm/02-execution.md). Но мы рассматривали это как техническую деталь — «блок видит переменные метода». На самом деле за этим стоит фундаментальная идея, которая объединяет блоки, `Proc` и `lambda` в одну концепцию.
...
В терминах Ruby: блок хранит две вещи — **код** (свой [ISeq](vm/01-compilation.md)) и **ссылку на окружение** ([EP](vm/02-execution.md) метода `greet`). Дизассемблер показывает обе:
...
При создании `Proc` или `lambda` Ruby копирует фрейм стека в кучу (heap). Копирование окружения из стека в кучу — вызов malloc ([управление памятью](../../linux/programming/05-memory-management.md)). Функция `vm_make_env_each()` в `vm.c` выполняет три шага:
