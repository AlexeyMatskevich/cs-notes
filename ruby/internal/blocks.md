...
`invokeblock` не ищет метод по имени — блок уже передан при вызове. VM находит его в специальном слоте фрейма: у каждого фрейма ниже локальных переменных есть служебный слот (block handler — ссылка на переданный блок), и при вызове `greet` → `3.times` блок был записан именно туда — в фрейм вызывающего метода `greet`.
