...
- **SP** (Stack Pointer) — вершина стека значений. Двигается при каждом push/pop.
- **EP** (Environment Pointer) — база локальных переменных. Фиксирована в рамках фрейма.
...
SP двигается на каждом шаге — значения приходят и уходят. EP остаётся на месте — это фиксированная «база», относительно которой адресуются переменные. Можно думать об EP как о «полке» для переменных, а об SP — как о «рабочем столе» для промежуточных вычислений.
