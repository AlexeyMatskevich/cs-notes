...
**Предпосылки:** [Компиляция](01-compilation.md) — ISeq, local table, стековая машина, два стека (значений и фреймов).
...
Чтобы исполнять инструкции, VM нужно помнить несколько вещей одновременно: какую инструкцию выполнять следующей, где вершина стека значений, кто текущий `self`, где лежат локальные переменные. В любом языке с вызовами функций эту роль играет **фрейм** (frame — «кадр»: один вызов = один кадр со всем состоянием) — снимок состояния одного вызова. В YARV фрейм — структура `rb_control_frame_t`.

Пять ключевых полей фрейма:
...
Каждый уровень — отдельный фрейм со своим типом:

- **TOP** — технический начальный фрейм, Ruby создаёт его при старте.
- **EVAL** — верхний уровень программы (ваш скрипт). Именно EVAL-фрейм исполняет инструкции `putobject 10`, `send :times`.
- **CFUNC** — метод, написанный на C. `Integer#times` — встроенный метод без собственного ISeq. VM вызывает C-код напрямую, а не исполняет YARV-инструкции. В фрейме CFUNC поле `iseq` пустое.
- **BLOCK** — блок `do |n| puts n end`. У него свой ISeq (второй фрагмент, который компилятор создал для блока), свой PC, свой EP с параметром `n`.
...
Local table связывает имена переменных с числовыми индексами. Теперь — как VM использует эти индексы при исполнении.

Локальные переменные лежат на стеке значений, в фиксированной части порции текущего фрейма. EP (Environment Pointer) указывает на базу этой области. Инструкция `setlocal idx` берёт значение с вершины стека и записывает в слот `EP - idx`. Инструкция `getlocal idx` читает значение из слота `EP - idx` и кладёт на вершину стека.
...
Выше EP — рабочая область: значения, которые инструкции кладут и забирают. Ниже EP (включительно) — слоты локальных переменных, неподвижные на протяжении всего фрейма. SP двигается на каждом шаге — значения приходят и уходят. EP остаётся на месте — это фиксированная «база», относительно которой адресуются переменные. Можно думать об EP как о «полке» для переменных, а об SP — как о «рабочем столе» для промежуточных вычислений.
...
Стек фреймов можно увидеть через `caller`:
...
Два фрагмента — метод `greet` и блок. В методе: `setlocal_WC_0 name` — запись переменной в текущий фрейм (level=0). В блоке: `getlocal_WC_1 name` — чтение из родительского фрейма (level=1). Блок не имеет своей local table — переменная `name` принадлежит методу `greet`.
