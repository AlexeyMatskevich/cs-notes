...
<- [UDP](udp.md) | [TCP Tuning](tcp-tuning.md) ->

[UDP](udp.md) передаёт данные без гарантий — быстро, но ненадёжно. Для загрузки файла, [HTTP](../application/http.md)-запроса или подключения к базе данных потеря или перестановка байтов недопустима. Нужен протокол, который гарантирует: данные дойдут, дойдут в правильном порядке, не продублируются, и отправитель не перегрузит получателя.
...
В Linux эта абстракция доступна через системные вызовы `socket()`, `connect()`, `listen()` и `accept()` — подробнее в [сокетах](../../linux/programming/sockets.md).
...
| Применение         | [HTTP](../application/http.md), файлы, БД        | Видео, игры, [DNS](../application/dns.md)     |

TCP обеспечивает надёжный канал между приложениями. Но приложениям нужно ещё понимать друг друга — преобразовать имя сервера в [IP](../foundations/ip-and-routing.md)-адрес ([DNS](../application/dns.md)), договориться о формате обмена ([HTTP](../application/http.md)), защитить данные от перехвата ([TLS](../application/tls.md)).
