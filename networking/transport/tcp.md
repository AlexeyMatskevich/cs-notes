...
TCP обеспечивает надёжный канал между приложениями. Но приложениям нужно ещё понимать друг друга — преобразовать имя сервера в IP-адрес ([DNS](../application/dns.md)), договориться о формате обмена ([HTTP](../application/http.md)), защитить данные от перехвата ([TLS](../application/tls.md)).
