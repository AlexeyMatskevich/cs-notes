...
В Linux эта абстракция проявляется на двух уровнях: userspace видит её через `socket()`, `connect()`, `listen()` и `accept()` ([сокеты](../../linux/programming/03-sockets.md)), а путь сегмента внутри ядра — через `tcp_v4_rcv()`, receive queues и ACK-логику — разобран в [сетевом стеке Linux](../../linux/kernel/03-network-stack.md).
