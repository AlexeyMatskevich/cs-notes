...
**TCP_NODELAY** — опция [[linux/programming/sockets|сокета]], отключающая алгоритм Nagle. Данные отправляются немедленно, без накопления. Redis, SSH, игровые серверы, биржевые терминалы — все устанавливают `TCP_NODELAY`.
...
На Linux параметр `backlog` в `listen(fd, backlog)` ограничивает прежде всего accept queue и сверху ещё обрезается значением `somaxconn`. Для незавершённых [TCP](tcp.md)-соединений есть отдельный лимит `tcp_max_syn_backlog`. Это важное различие: "увеличить backlog" и "увеличить память под незавершённые SYN" — не одно и то же.
