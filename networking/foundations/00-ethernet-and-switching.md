...
[IP и маршрутизация](01-ip-and-routing.md) ->
...
MAC-адрес получателя указывает, кому предназначен кадр — сетевая карта принимает кадр только если адрес совпадает с её собственным. <a id="broadcast"></a>Специальный адрес `FF:FF:FF:FF:FF:FF` — **широковещательный** (broadcast): кадр с таким адресом получателя принимают все устройства в сети. Это нужно, когда отправитель хочет обратиться ко всем сразу или не знает конкретного адресата. MAC-адрес отправителя нужен для ответа. **Payload** (полезная нагрузка) — собственно передаваемые данные.
