...
[NAT](nat.md) и [CGNAT](nat.md) — костыли, позволяющие [IPv4](ip-and-routing.md) работать при нехватке адресов. Но у них есть цена: если сервер в интернете хочет связаться с вашим телефоном напрямую, это уже не вопрос адресации, а вопрос обхода [NAT](nat.md). P2P-приложения, звонки и игры вынуждены использовать дополнительные механизмы согласования. IPv6 решает именно корневую проблему: адресов становится достаточно, чтобы [NAT](nat.md) перестал быть обязательным способом экономить публичные адреса.

## Адресное пространство

[IPv4](ip-and-routing.md): 32 бита = ~4.3 млрд адресов. IPv6: 128 бит = ~3.4 × 10³⁸ адресов. Для сравнения: на каждый квадратный метр поверхности Земли приходится ~6.7 × 10²³ IPv6-адресов.
...
Полный переход на IPv6 невозможен одномоментно — слишком много инфраструктуры работает на [IPv4](ip-and-routing.md). Стратегия перехода — **dual-stack**: устройства и сети поддерживают оба протокола одновременно.

В dual-stack каждый интерфейс имеет и [IPv4](ip-and-routing.md), и IPv6 адрес. [DNS](../application/dns.md) возвращает оба типа записей: A ([IPv4](ip-and-routing.md)-адрес) и AAAA (IPv6-адрес). Приложение выбирает протокол: современные реализации используют **Happy Eyeballs** — параллельно пробуют IPv6 и [IPv4](ip-and-routing.md), выбирая тот, который отвечает быстрее.
...
Переход идёт неравномерно. По статистике Google, IPv6 уже занимает заметную долю пользовательского трафика, но картина сильно зависит от региона и типа сети. Мобильные сети и крупные провайдеры продвинулись дальше; корпоративные сети, часть дата-центров и старые устройства часто по-прежнему живут в [IPv4](ip-and-routing.md)-мире.

Практический вывод для разработчика: приложения должны нормально работать в dual-stack среде. Библиотеки и фреймворки часто скрывают разницу, но при работе с сокетами, [DNS](../application/dns.md), правилами доступа и сетевой конфигурацией нужно помнить, что у хоста может быть несколько [IPv4](ip-and-routing.md)- и IPv6-адресов одновременно.
