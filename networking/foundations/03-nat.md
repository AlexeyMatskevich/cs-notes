...
**Предпосылки:** [IP и маршрутизация](01-ip-and-routing.md) (IP-адрес, маска, роутер, шлюз по умолчанию), [UDP](../transport/00-udp.md) (порт, сокет), [TCP](../transport/01-tcp.md) (соединение).
...
В Linux-контейнерах те же SNAT/DNAT используются для публикации портов (`docker -p`) и связи bridge-сети контейнера с хостом. Практическая сборка этого пути разобрана в [контейнерах](../../linux/containers/01-containers.md).
...
NAT нарушает сквозной принцип (end-to-end): устройства за NAT не могут быть адресованы напрямую из интернета. Протоколы, которые передают IP-адреса внутри данных (SIP для VoIP, FTP в активном режиме), ломаются, если NAT не знает об их специфике.
...
CGNAT и исчерпание IPv4-адресов — главная мотивация для перехода на [IPv6](04-ipv6.md), который полностью устраняет потребность в NAT за счёт огромного адресного пространства.
