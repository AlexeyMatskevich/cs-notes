...
Компьютерные сети — инфраструктура, на которой работает всё остальное: базы данных с репликацией, распределённые системы, веб-приложения, [CDN](infrastructure/cdn.md), брокеры сообщений. Заметки организованы снизу вверх: от передачи битов по проводу до [HTTP](application/http.md)-запросов и инфраструктуры интернета.
...
2. **[DNS](application/dns.md)** — www.example.com → IP-адрес (возможно через [CDN](infrastructure/cdn.md))
3. **[TCP](transport/tcp.md)** — трёхстороннее рукопожатие с сервером
4. **[TLS](application/tls.md)** — рукопожатие, установка шифрования
5. **[HTTP](application/http.md)-запрос** — `GET /page` через зашифрованное соединение
6. **Обработка на сервере** — роутинг, бизнес-логика, база данных
7. **[HTTP](application/http.md)-ответ** — HTML-страница
8. **Рендеринг** — парсинг HTML, загрузка CSS/JS/изображений (параллельные запросы), отрисовка

На каждом шаге работает свой уровень стека: [DNS](application/dns.md) использует [UDP](transport/udp.md), [TCP](transport/tcp.md) обеспечивает надёжность, [TLS](application/tls.md) — шифрование, [HTTP](application/http.md) — семантику запроса. Пакеты инкапсулируются при отправке и декапсулируются при получении.

## Как всё связано

**Надёжность vs скорость.** [TCP](transport/tcp.md) гарантирует доставку ценой задержек (handshake, retransmission, congestion control). [UDP](transport/udp.md) жертвует надёжностью ради скорости. HTTP/3 (QUIC) — попытка получить надёжность [TCP](transport/tcp.md) без его проблем (HOL blocking), работая поверх [UDP](transport/udp.md).

**Безопасность vs латентность.** [TLS](application/tls.md) добавляет RTT на рукопожатие. TLS 1.3 снижает это до 1 RTT, 0-RTT при повторном подключении. [VPN](infrastructure/vpn.md) добавляет ещё один уровень шифрования и маршрутизации.

**Централизация vs распределённость.** [DNS](application/dns.md) — иерархическая распределённая система. [CDN](infrastructure/cdn.md) — распределённый кэш. BGP — децентрализованный обмен маршрутами. Каждый решает проблему масштаба по-своему.
