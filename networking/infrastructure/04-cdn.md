...
**Предпосылки:** [DNS](../application/00-dns.md) (резолвинг, CNAME, TTL), [HTTP](../application/01-http.md) (заголовки, Cache-Control, коды статуса), [TCP](../transport/01-tcp.md) (RTT).

<- [VPN](03-vpn.md)

Пользователь в Токио запрашивает картинку с сервера в Нью-Йорке. Свет в оптоволокне проходит ~200 ms в одну сторону. Каждый HTTP-запрос требует минимум один RTT — 400+ ms только на ожидание. Если на странице 50 ресурсов, задержки накапливаются. Вторая проблема — нагрузка: популярный сайт получает миллионы запросов, один сервер не справляется с отдачей статики всему миру.
