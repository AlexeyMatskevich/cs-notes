...
**Предпосылки:** [IP и маршрутизация](../foundations/01-ip-and-routing.md) (IP-адрес, пакет), [TCP](../transport/01-tcp.md) (соединение, SYN, ACK, порт), [UDP](../transport/00-udp.md) (порт, датаграмма).

<- [Протоколы маршрутизации](01-routing-protocols.md) | [VPN](03-vpn.md) ->
...
На Linux файрвол реализован в ядре. **iptables** — классический интерфейс, использует цепочки (chains) и таблицы. **nftables** — более новая замена с унифицированным синтаксисом. Оба используют [Netfilter](../../linux/kernel/03-network-stack.md) — фреймворк обработки пакетов в ядре Linux. Connection tracking реализован модулем `nf_conntrack`.
