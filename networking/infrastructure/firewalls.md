...
На Linux файрвол реализован в ядре. **iptables** — классический интерфейс, использует цепочки (chains) и таблицы. **nftables** — более новая замена с унифицированным синтаксисом. Оба используют [Netfilter](../../linux/kernel/network-stack.md) — фреймворк обработки пакетов в ядре Linux. Connection tracking реализован модулем `nf_conntrack`.
