...
Как это работает. Каждый роутер рассылает **LSA** (Link-State Advertisement) — информацию о своих соседях и стоимости каналов до них. Все роутеры получают LSA от всех и строят одинаковую карту сети — **базу данных link-state** (LSDB). На основе этой карты каждый роутер вычисляет кратчайший путь до каждой сети с помощью алгоритма Дейкстры.
