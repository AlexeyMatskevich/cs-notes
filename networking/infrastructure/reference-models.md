...
<- [WebSocket](../application/websockets.md) | [Протоколы маршрутизации](routing-protocols.md) ->

Вы уже изучили четыре протокола: [Ethernet](../foundations/ethernet-and-switching.md) передаёт кадры между соседними устройствами, [IP](../foundations/ip-and-routing.md) маршрутизирует пакеты между сетями, [TCP](../transport/tcp.md) обеспечивает надёжную доставку, [HTTP](../application/http.md) задаёт структуру запросов и ответов. Каждый решает свою задачу и не знает деталей остальных — [Ethernet](../foundations/ethernet-and-switching.md) не понимает [IP](../foundations/ip-and-routing.md)-адресов, [TCP](../transport/tcp.md) не разбирает [HTTP](../application/http.md)-заголовки. Это не случайность, а архитектурный принцип: сетевое взаимодействие разбито на **уровни** (layers), каждый из которых решает узкую задачу и предоставляет сервис уровню выше.
...
**Уровень 2: Канальный (Data Link)** — надёжная передача между непосредственно соединёнными устройствами. Единица данных: кадр (frame). [Ethernet](../foundations/ethernet-and-switching.md), Wi-Fi, MAC-адреса, коммутаторы.

**Уровень 3: Сетевой (Network)** — маршрутизация между разными сетями. Единица данных: пакет (packet). [IP](../foundations/ip-and-routing.md), роутеры, ICMP.

**Уровень 4: Транспортный (Transport)** — доставка данных между приложениями, надёжность. Единица данных: сегмент ([TCP](../transport/tcp.md)) / датаграмма ([UDP](../transport/udp.md)).
...
**Уровень 7: Прикладной (Application)** — протоколы для конечных приложений. [HTTP](../application/http.md), [DNS](../application/dns.md), SMTP, SSH.
...
OSI — теоретическая модель для обучения и терминологии ("проблема на L2", "L7 балансировщик"). [TCP](../transport/tcp.md)/[IP](../foundations/ip-and-routing.md) — практическая, описывающая реальный интернет.
...
**L7 load balancer** понимает [HTTP](../application/http.md) и принимает решения на основе URL, заголовков, cookies. Подробнее о балансировке см. в материалах по system design.

## Принцип песочных часов

В середине стека — только [IP](../foundations/ip-and-routing.md). Это обеспечивает универсальность:
...
Любое приложение работает поверх [IP](../foundations/ip-and-routing.md). [IP](../foundations/ip-and-routing.md) работает поверх любой физической сети. Добавление нового приложения (HTTP/3, gRPC) не требует изменений в сетевой инфраструктуре. Добавление новой физической среды (5G, Starlink) не требует изменений в приложениях.
