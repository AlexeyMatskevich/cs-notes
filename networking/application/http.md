...
<- [DNS](dns.md) | [TLS](tls.md) ->

Браузер знает [IP](../foundations/ip-and-routing.md)-адрес сервера ([DNS](dns.md)) и может установить надёжное соединение ([TCP](../transport/tcp.md)). Теперь клиенту и серверу нужен общий язык: как сформулировать "дай мне страницу /about", как ответить "вот она", как сообщить "такой страницы нет". Этот язык — HTTP.
...
**REST** (Representational State Transfer) — архитектурный стиль для API. Не протокол, а набор принципов: ресурсы идентифицируются через URI (`/users/123`), операции выражаются через HTTP-методы, сервер не хранит состояние клиента между запросами. Подробнее о проектировании API: [API Design](../../system-design/api-design.md).

[HTTP](http.md) передаёт данные открытым текстом — любой промежуточный узел может прочитать содержимое запроса. Для защиты данных [HTTP](http.md) оборачивается в [TLS](tls.md).
