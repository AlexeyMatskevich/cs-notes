...
[HTTP](http.md) передаёт данные открытым текстом. Когда пользователь вводит пароль на сайте, этот пароль проходит через десятки роутеров, каждый из которых может прочитать содержимое. Провайдер, оператор Wi-Fi в кафе, любой узел на пути — все видят данные как есть.

## HTTPS = HTTP + TLS

**HTTPS** — [HTTP](http.md) поверх **TLS** (Transport Layer Security — безопасность транспортного уровня). TLS — криптографический протокол, который оборачивает [TCP](../transport/tcp.md)-соединение в защитный слой, обеспечивая три свойства.
...
[TLS](tls.md) решает проблему безопасности на уровне одного соединения. Эволюция самого [HTTP](http.md) — от одного запроса на соединение до мультиплексирования — описана в [эволюции HTTP](http-evolution.md).
