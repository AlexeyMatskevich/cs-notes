...
Атомарность записи страницы. Современные диски гарантируют: запись сектора либо происходит целиком, либо не происходит. Но страница PostgreSQL — 8 КБ, это 16 секторов по 512 байт, и атомарность между секторами не гарантирована. Сбой посередине записи может оставить «разорванную» страницу (torn page). Для решения этой проблемы PostgreSQL использует журнал записи, описанный отдельно ([WAL](../durability/00-wal.md)), — здесь важно лишь то, что сбой посередине записи страницы не приводит к потере данных.
