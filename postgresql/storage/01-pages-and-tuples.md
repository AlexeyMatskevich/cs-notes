...
Атомарность записи страницы. Современные диски гарантируют атомарность на уровне сектора, но не на уровне 8 КБ страницы: сбой посередине записи может оставить «разорванную» страницу (torn page). Чтобы восстановление было корректным, PostgreSQL опирается на [WAL](../durability/00-wal.md): сначала на диск должен попасть журнал (WAL before data), а после checkpoint при первом изменении страницы в WAL пишется полный образ страницы (Full Page Writes; подробнее — [буферный кеш](../durability/01-buffer-cache.md)). Поэтому crash посередине записи страницы не превращает данные в необратимый мусор: при старте сервер может восстановить согласованное состояние страниц.
