...
Запрос может быть логически простым (`WHERE`, `JOIN`, `ORDER BY`), но выполняться очень по‑разному: полный перебор таблицы, точечный доступ через индекс, чтение по битмапу, сортировка, построение хеш‑таблицы, многократные проходы по одной и той же таблице. EXPLAIN нужен, чтобы увидеть выбранный способ выполнения и понять, что именно делает запрос дорогим: CPU, дисковый I/O, ошибочные оценки кардинальностей или «не тот» порядок операций.
...
2. **`rows=...`** — сколько строк планировщик ожидает от этого узла.
3. **`actual ... rows=... loops=...`** (только в `ANALYZE`) — сколько строк получилось на самом деле и сколько раз узел запускался.

Если `rows` сильно расходится с фактом, почти любая оптимизация дальше превращается в угадайку: неверная кардинальность ломает выбор join алгоритма, порядок соединения и объём памяти под сортировки/хеши.
