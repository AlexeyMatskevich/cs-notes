...
**Почему "Generalized" (обобщённый)?** Потому что работает не только с текстом, а с любыми типами данных, которые можно разбить на элементы: массивы, JSONB, полнотекстовые векторы, и другие. GIN — это не конкретная структура для конкретного типа, а фреймворк, адаптируемый под разные типы через operator class.
