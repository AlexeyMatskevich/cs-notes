...
>
> Нюанс store forwarding: поток может видеть свою собственную запись до её глобальной видимости (через [[computer/data-path/cache-coherency#store-buffer-запись-без-ожидания|store buffer]]). Store forwarding включён в формальную модель x86-TSO как явная часть — это не отклонение от неё, а намеренно смоделированное поведение. Формализация — статья «x86-TSO: A Rigorous and Usable Programmer's Model for x86 Multiprocessors» (CACM 2010).
>
