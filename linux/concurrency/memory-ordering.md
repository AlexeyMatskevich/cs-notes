...
Мьютекс и [[linux/concurrency/synchronization#Compare-and-swap|CAS (Compare-And-Swap)]] дают корректный параллельный доступ к данным. Но даже без конкуренции [[linux/concurrency/synchronization#Мьютекс|мьютекс]] стоит 15-20 нс на захват и освобождение. На горячем пути с миллионами операций в секунду эта цена заметна. Иногда задача проще: один поток записывает данные и выставляет флаг, второй поток крутится в цикле на флаге и читает данные. Две обычных записи, два обычных чтения — казалось бы, мьютекс здесь избыточен.
...
За этим выводом нуля на ARM стоит не один сбой, а два совершенно независимых — и чинятся они разными средствами.
...
В сценарии с флагом поток A должен гарантировать: если B увидит `READY == true`, то `DATA` уже содержит 42. Это роль записывающей стороны — зафиксировать порядок публикации до того, как другой поток увидит сигнал. Такая гарантия называется **Release** (освобождение).
...
На x86 SeqCst-store требует полного барьера, запрещающего store-load reorder — единственное переупорядочивание, которое TSO допускает. Компиляторы генерируют `xchg` (атомарный обмен, неявно содержащий full barrier). Стоимость: 10-40 нс на операцию. На AArch64 (ARMv8) глобальный порядок обеспечивают те же `stlr`/`ldar`, что и для Acquire/Release: пара store-release + load-acquire сама даёт согласованный порядок без дополнительного барьера, поэтому SeqCst здесь почти не дороже Acquire/Release. На более старом ARMv7 таких инструкций нет, и SeqCst требует полного барьера `dmb ish` — заметно дороже.
