...
[планировщик](../foundations/07-scheduler.md) (вытеснение между любыми двумя инструкциями), [потоки](../foundations/03-threads.md) (общее адресное пространство, CLONE_VM), [когерентность кешей](../../computer/02-cache-coherency.md) (MESI, invalidate, cache line ping-pong).

</details>

← [Права доступа и capabilities](../foundations/08-permissions-and-capabilities.md) | [Модель памяти](01-memory-ordering.md) →

Потоки разделяют адресное пространство — любой из них может прочитать и записать любой адрес в heap и в секции данных процесса. [Планировщик](../foundations/07-scheduler.md) при этом может прервать поток между любыми двумя машинными инструкциями, передав ядро другому потоку. Сочетание этих двух фактов — общая память и вытеснение — порождает гонки (race conditions): результат программы начинает зависеть от порядка, в котором планировщик чередует потоки.
...
Оба потока прочитали 0, оба записали 1. Два инкремента — а счётчик вырос на единицу. Это lost update: запись одного потока затирает запись другого, потому что между чтением и записью вклинился чужой поток.
...
Разница между двумя понятиями: гонка данных — свойство конкретных обращений к памяти, её можно обнаружить инструментами вроде ThreadSanitizer. Гонка — ошибка на уровне логики, где корректность зависит от тайминга. Классический пример — **TOCTOU** (time-of-check-to-time-of-use): программа проверяет, существует ли файл (`access()`), затем открывает его (`open()`). Между проверкой и открытием другой процесс может удалить файл или заменить его символической ссылкой. Каждая из двух операций по отдельности потокобезопасна — гонки данных нет, но логическая гонка есть.
...
```
              ...
              ┌──────────────┐
              │     CAS      │  одна атомарная
              │  (аппаратно) │  read-modify-write
              └──────────────┘
```

## Compare-and-swap

**CAS (compare-and-swap)** — аппаратная атомарная инструкция. Она принимает три аргумента: адрес ячейки, ожидаемое значение и новое значение. Если текущее содержимое ячейки совпадает с ожидаемым, CAS записывает новое и возвращает успех. Если содержимое изменилось (другой поток успел записать своё), CAS не трогает ячейку и возвращает неудачу. Вся операция — чтение, сравнение и условная запись — выполняется процессором как единое целое: никакой другой поток или ядро не может вклиниться посередине.

На x86 CAS реализована инструкцией `LOCK CMPXCHG`. Префикс `LOCK` блокирует кеш-линию на время выполнения: [протокол MESI](../../computer/02-cache-coherency.md) переводит линию в состояние Modified, и ни одно другое ядро не может прочитать или записать эту линию до завершения инструкции. На ARM аналогичный результат достигается парой инструкций LL/SC (load-linked / store-conditional): `LDXR` загружает значение и помечает кеш-линию как отслеживаемую, `STXR` записывает новое значение, только если с момента `LDXR` линию никто не трогал.
...
void atomic_increment(atomic_int *ptr) {
    ...
    /* если за время между load и CAS кто-то изменил *ptr, */
    /* CAS вернёт false, обновит old — и цикл повторится  */
}
```

Цикл работает так: поток читает текущее значение, вычисляет новое, затем пытается записать. Если между чтением и записью другой поток успел изменить значение, CAS обнаруживает расхождение и отказывает — цикл повторяется с обновлённым значением. При низкой конкуренции цикл срабатывает с первой попытки. При высокой конкуренции потоки повторяют попытки, но ни один инкремент не теряется.

На практике для простого инкремента не нужно писать CAS-цикл вручную. В C — `atomic_fetch_add(&counter, 1)`, в Rust — `counter.fetch_add(1, Ordering::Relaxed)`. Обе функции компилируются в единственную инструкцию `LOCK XADD` на x86.

## Стоимость атомарных операций

Атомарная инструкция без конкуренции (uncontended) стоит 5-20 нс — кеш-линия уже принадлежит текущему ядру, `LOCK` просто запрещает перехват на время инструкции. При конкуренции (contended) цена вырастает до 50-200 нс: каждая атомарная операция требует получения кеш-линии от другого ядра через [протокол когерентности](../../computer/02-cache-coherency.md). Два ядра перебрасывают одну линию туда-обратно — ping-pong, 20-70 нс на каждую передачу.
...
<details>
<summary>Задача: futex — это примитив или реализация?</summary>
...
Альтернативный подход — **try-lock с откатом**: `pthread_mutex_trylock()` пытается захватить мьютекс без блокировки. Если захват не удался, поток освобождает все удерживаемые мьютексы, ждёт случайное время (backoff) и пытается заново. Этот подход разрывает условие «hold and wait», но при высокой конкуренции приводит к livelock — потоки бесконечно отпускают и перезахватывают, не продвигаясь.
...
**PTHREAD_MUTEX_ERRORCHECK** — при повторном захвате тем же потоком `lock()` возвращает `EDEADLK` вместо зависания. При `unlock()` чужого мьютекса — возвращает `EPERM`. Дороже на ~5–10 нс (ядро проверяет владельца), но ловит баги при разработке.
...
Мьютекс и CAS решили задачу корректного доступа к общим данным. CAS стоит 5-20 нс без конкуренции, мьютекс через futex — 10-20 нс. Для флага «данные готовы» кажется достаточным простой записи в переменную из одного потока и чтения из другого — два обращения к памяти, никаких блокировок. На x86 это работает. На ARM — нет: процессор может переупорядочить инструкции, и читающий поток увидит флаг «готово» раньше, чем данные, которые этот флаг сигнализирует. Почему так происходит и как это контролировать — тема [модели памяти](01-memory-ordering.md).

## Sources

- Michael Kerrisk, 2010, *The Linux Programming Interface* — Chapters 30-31: Threads: Synchronization
- Ulrich Drepper, 2011, *Futexes Are Tricky* — https://www.akkadia.org/drepper/futex.pdf
- `man 2 futex`, `man 7 pthreads`
