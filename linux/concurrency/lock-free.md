...
**Lock-free** (неблокирующая) структура данных гарантирует: хотя бы один поток завершает операцию за конечное число шагов, независимо от того, что делают остальные потоки. Ни один поток не способен заблокировать всю систему. Если поток A приостановлен планировщиком посреди операции — остальные продолжают работать. У мьютекса наоборот: приостановленный держатель блокировки замораживает всех остальных.
...
На 64-битных машинах указатель реально занимает не все 64 бита. На типичном x86-64 виртуальный адрес занимает 48 бит из 64, оставляя 16 старших бит свободными под счётчик. Сколько именно бит свободно, зависит от конфигурации памяти системы, поэтому приём работает надёжно на большинстве текущих машин, но не является архитектурной гарантией — это распространённый трюк, а не часть контракта процессора.
...
Если процессор умеет атомарно выполнять вдвое более широкое — 128-битное — сравнение-и-обмен (на x86-64 это `CMPXCHG16B`), счётчик и указатель хранятся каждый в своих полных разрядах, и оборачивание исключается полностью. Ограничение tagged pointer в том, что он борется с симптомом: счётчик делает ABA крайне маловероятным, но не невозможным — и ничего не делает с самой проблемой переиспользования освобождённой памяти.
...
Как именно определяется grace period, зависит от того, может ли планировщик вытеснить поток внутри читательской секции. В простейшем варианте вытеснение внутри секции запрещено: `rcu_read_lock()` сводится к одной инструкции без барьера памяти, а раз читатель не может быть вытеснен посреди чтения, то переключение контекста на ядре означает, что секция уже завершена — поэтому grace period наступает, когда каждый CPU хотя бы раз переключил контекст. Когда же вытеснение внутри секции разрешено, читателя приходится отслеживать явно, и read-side стоит чуть дороже (атомарный инкремент-декремент счётчика), но всё ещё на порядки дешевле мьютекса.
...
В простейшем варианте стоимость `rcu_read_lock()` — ноль атомарных операций, ноль инвалидированных [[computer/data-path/cache-coherency#Протокол MESI: четыре состояния кеш-линии|кеш-линий]]. Для таблицы маршрутизации, к которой обращается каждый входящий пакет на каждом CPU, это критически важно.

RCU широко используется в ядре для таблицы маршрутизации, списка [[linux/foundations/file-descriptors#Файловые дескрипторы|файловых дескрипторов]], dentry cache (кеша записей каталогов файловой системы), модульной системы, сетевых фильтров — это один из самых нагруженных механизмов синхронизации в ядре.
...
Очередь Майкла-Скотта лежит в основе `java.util.concurrent.ConcurrentLinkedQueue`. В планировщиках Go и Tokio используются другие неблокирующие структуры — work-stealing deque (двусторонняя очередь с воровством работы: каждый воркер берёт задачи со своего конца, а простаивающий ворует с чужого), — но общий принцип, CAS-цикл вместо мьютекса, тот же.
