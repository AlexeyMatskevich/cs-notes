...
У ABA на стеке Трайбера два вредных следствия. Первое — use-after-free: освобождённый узел переиспользован, CAS проходит, но `top` или `top->next` указывают на освобождённую память. Второе — логическое повреждение даже без освобождения: если узел сняли и тут же вернули в стек (тот же объект, без `free`), успешный CAS потока A воскрешает узел, уже выданный потребителю на шаге pop, — он оказывается в стеке повторно, владение раздваивается. Hazard pointers (Майкл, 2004) закрывают первое следствие: они откладывают освобождение узла, пока на него ссылается хоть один поток, и use-after-free исчезает. Но логическое ABA от переиспользования узла они не предотвращают — для него нужен счётчик версий, заставляющий CAS заметить, что состояние менялось.
