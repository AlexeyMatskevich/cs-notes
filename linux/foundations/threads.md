...
Третья — скорость создания и переключения. fork на современном Linux занимает 50–100 мкс для лёгкого процесса: ядру нужно скопировать таблицу страниц, дескрипторы файлов, структуры сигналов. Это нижняя оценка — главный драйвер стоимости fork — копирование таблицы страниц, и с ростом рабочего набора цена растёт: процесс с десятками гигабайт mmap-памяти форкается за миллисекунды, а не микросекунды. Переключение контекста между процессами стоит 3–10 мкс, потому что ядро перезагружает регистр CR3 (базовый адрес таблицы страниц), и трансляции адресов предыдущего процесса в [TLB](virtual-memory.md#tlb-кеш-трансляций) (Translation Lookaside Buffer — кэш трансляций виртуальных адресов в физические) становятся неактуальными. После этого каждое первое обращение к памяти в новом процессе проходит через полную аппаратную трансляцию, пока TLB не прогреется заново.
