...
На одной машине одновременно работают десятки и сотни запущенных программ — и каждой нужны своя память, свои открытые файлы, свой счётчик инструкций. Ядро ведёт их по отдельности: помнит, кому принадлежит каждая страница памяти и каждый открытый файл, и в каком состоянии остановилась каждая программа, чтобы продолжить её с того же места. Единицу учёта, которую ядро заводит на каждую запущенную программу, называют **процессом** (process).
...
Метаданные ядро хранит в структуре `task_struct` — это всё, что ядру нужно знать о процессе, чтобы им управлять. Она содержит PID (идентификатор процесса), PPID (Parent PID — PID родителя), UID/GID (User ID/Group ID — владелец), текущее состояние (running/sleeping/zombie), приоритет, указатели на таблицу страниц, [сигнальные](../programming/signals.md) маски и многое другое. `task_struct` — центральная структура ядра: [планировщик](scheduler.md), подсистема памяти, [файловая система](filesystems.md) — все работают через неё.

Эта структура не возникает из ничего: в Unix у каждого процесса есть процесс-родитель, который его создал.
...
Процесс веб-сервера для конвертации изображений занимает 2 ГБ оперативной памяти. Наивное копирование 2 ГБ при каждом `fork()` заняло бы десятки миллисекунд и удвоило бы расход памяти. На практике `fork()` занимает порядка сотен микросекунд даже для такого процесса.

Ядро не копирует 2 ГБ. Оно копирует только таблицу страниц — компактную структуру, которая описывает, какие виртуальные адреса указывают на какие физические страницы памяти. После `fork()` обе копии — родитель и потомок — ссылаются на одни и те же физические страницы, но все эти страницы помечаются как read-only. Когда один из процессов пытается записать данные, процессор обнаруживает обращение на запись в страницу, помеченную read-only, и генерирует page fault (прерывание о нарушении доступа) — ядро перехватывает его и копирует ровно одну затронутую страницу (4 КБ). Только она становится независимой копией — остальные продолжают разделяться.

Если потомок сразу вызывает `exec()` и загружает новую программу, большинство страниц родителя вообще никогда не будут скопированы — адресное пространство потомка полностью заменяется. Это основной сценарий: `fork()` + `exec()` создаёт новый процесс за микросекунды, независимо от размера родителя. Полный механизм Copy-on-Write — в [[linux/foundations/virtual-memory#Copy-on-write: fork() без копирования|виртуальной памяти]].
...
По отдельности fork и exec почти бесполезны: клон без новой программы — лишняя копия, новая программа без отдельного процесса затёрла бы вызвавшего. Вместе они дают запуск чужой программы рядом с собой. Веб-сервер получает запрос на конвертацию изображения, и ему нужно запустить `/usr/bin/convert` (ImageMagick) как отдельный процесс:
...
Процесс создан через `fork()`, программа загружена через `exec()`. Но между запуском и завершением процесс не всё время считает: он ждёт данных с диска, обрабатывает запрос, получает сигнал. Ядро постоянно отслеживает, в каком из этих состояний он находится.
...
**Zombie (Z)** — процесс завершился, но его запись в таблице процессов (`task_struct`) ещё существует, потому что родитель пока не забрал код завершения.
...
У веб-сервера из примера это правило выходит боком. Если он порождает процессы для конвертации, но не вызывает `waitpid()`:
...
Каждый завершившийся `convert` станет зомби. 1000 запросов — 1000 зомби. Каждый зомби удерживает один PID и небольшой объём ядерной памяти — большая часть ресурсов уже освобождена, осталась лишь запись с кодом завершения. Лимит PID по умолчанию — 32768 (`/proc/sys/kernel/pid_max`), на 64-битных системах systemd поднимает его до 4194304. Тысяча зомби — тысяча занятых PID. Зомби невозможно убить: `kill -9` не действует, потому что процесс уже мёртв. Единственный способ очистить — заставить родителя вызвать `wait()`, или завершить самого родителя.
...
Fork, exec, состояния, зомби, сироты, дерево — каждая часть отвечает за свой отрезок жизни процесса. В одной обработке запроса они срабатывают по очереди.

## Сценарий целиком: жизнь процесса конвертации

Веб-сервер (PID 900) получает запрос на конвертацию изображения:
...
Процесс — единица изоляции: собственная память, собственные дескрипторы, собственный идентификатор. Но у полной изоляции есть цена. Если веб-серверу нужны параллельные обработчики, которые разделяют кеш изображений или пул соединений к базе данных, создавать отдельный процесс для каждого обработчика расточительно: каждый fork дублирует таблицу страниц, каждый процесс имеет собственный `task_struct`, а обмен данными между процессами требует IPC (Inter-Process Communication, межпроцессное взаимодействие — пайпы, разделяемая память, [сокеты](../programming/sockets.md)). Когда нужен параллелизм с общими данными — нужна более лёгкая единица выполнения: [поток](threads.md).
