...
Процесс — единица изоляции: собственная память, собственные дескрипторы, собственный идентификатор. Но у полной изоляции есть цена. Если веб-серверу нужны параллельные обработчики, которые разделяют кеш изображений или пул соединений к базе данных, создавать отдельный процесс для каждого обработчика расточительно: каждый fork дублирует таблицу страниц, каждый процесс имеет собственный `task_struct`, а обмен данными между процессами требует IPC (Inter-Process Communication, межпроцессное взаимодействие — пайпы, разделяемая память, сокеты). Когда нужен параллелизм с общими данными — нужна более лёгкая единица выполнения: [поток](threads.md).
