...
Каждый завершившийся `convert` станет зомби. 1000 запросов — 1000 зомби. Каждый зомби удерживает один PID и небольшой объём ядерной памяти — большая часть ресурсов уже освобождена, осталась лишь запись с кодом завершения. Лимит PID по умолчанию — 32768 (`/proc/sys/kernel/pid_max`); его можно поднять, но он конечен. Тысяча зомби — тысяча занятых PID. Зомби невозможно убить: `kill -9` не действует, потому что процесс уже мёртв. Единственный способ очистить — заставить родителя вызвать `wait()`, или завершить самого родителя.
