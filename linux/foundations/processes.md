...
**Регистры процессора** — контекст выполнения (полный список и соглашения о вызовах — в [программной модели CPU](../../computer/programmer-model/isa.md)). Указатель инструкции (`rip` на x86-64) говорит, какую инструкцию выполнять следующей. Указатель стека (`rsp`) указывает на вершину стека. Базовый указатель (`rbp`) обозначает начало текущего фрейма. Когда ядро переключает процессор с одного процесса на другой, оно сохраняет все регистры первого процесса и восстанавливает регистры второго — это **контекстное переключение** (context switch). Для процессов оно стоит порядка 3-10 мкс, потому что помимо сохранения регистров ядро перезагружает регистр CR3 (базовый адрес [[linux/foundations/virtual-memory/translation#Page table: карта трансляций|таблицы страниц]]), что сбрасывает TLB (Translation Lookaside Buffer — кеш трансляций адресов); последующие обращения к памяти проходят через полную аппаратную трансляцию, пока TLB не прогреется заново. Переключение между потоками одного процесса обходится дешевле: они делят таблицу страниц, поэтому CR3 не перезагружается и TLB не сбрасывается ([потоки](threads.md)).

**Таблица [[linux/foundations/file-descriptors#Трёхуровневая таблица|файловых дескрипторов]]** — массив указателей на открытые файлы, [сокеты](../programming/sockets.md), пайпы. Дескриптор 0 — stdin, 1 — stdout, 2 — stderr. Лимит по умолчанию — 1024 дескриптора на процесс (`ulimit -n`), можно поднять до сотен тысяч.

### Дескриптор процесса: task_struct

Метаданные ядро хранит в структуре `task_struct` (определена в `include/linux/sched.h`). Она занимает порядка 6-8 КБ и содержит PID (идентификатор процесса), PPID (Parent PID — PID родителя), UID/GID (User ID/Group ID — владелец), текущее состояние (running/sleeping/zombie), приоритет, указатели на таблицу страниц, [сигнальные](../programming/signals.md) маски и многое другое. `task_struct` — центральная структура ядра: [планировщик](scheduler.md), подсистема памяти, [файловая система](filesystems.md) — все работают через неё.

Интуитивно кажется, что каждая программа стартует с нуля. В Unix это не так: новый процесс всегда возникает как копия существующего. Единственное исключение — самый первый, init (PID 1), которого запускает ядро при загрузке. Всё остальное — разветвление от него.
...
Процесс веб-сервера для конвертации изображений занимает 2 ГБ оперативной памяти. Наивное копирование 2 ГБ при каждом `fork()` заняло бы десятки миллисекунд и удвоило бы расход памяти. На практике `fork()` занимает порядка сотен микросекунд даже для такого процесса: ядро копирует только таблицу страниц (~2 МБ при 2 ГБ RAM), а сами страницы разделяются с пометкой read-only. Первая запись в общую страницу одним из процессов вызывает page fault, и ядро копирует только эту страницу (4 КБ). Если потомок сразу вызывает `exec()`, большинство страниц родителя так и не будут скопированы — адресное пространство полностью заменяется. Полный механизм — в [[linux/foundations/virtual-memory/page-faults#Copy-on-write: fork() без копирования|виртуальной памяти]].


## exec(): замена программы

`fork()` создал копию текущего процесса. Но нам нужен не клон — нам нужно запустить другую программу. Системный вызов `execve()` (и его обёртки `execl`, `execvp`, `execlp`) заменяет адресное пространство процесса новым: ядро уничтожает сегменты кода, данных, кучу, стек и загружает на их место содержимое указанного [[linux/infrastructure/elf-and-linking#exec() и загрузка ELF|исполняемого файла]]. Регистры сбрасываются. Указатель инструкции устанавливается на точку входа новой программы.
...
void handle_request(const char *input, const char *output) {
    ...
    if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
        ...
        printf("ошибка конвертации: убит сигналом %d\n", WTERMSIG(status));
    }
}
```

Макросы `WIFEXITED`/`WEXITSTATUS` из `<sys/wait.h>` разбирают упакованный статус завершения для нормального выхода с кодом; `WIFSIGNALED`/`WTERMSIG` — для случая, когда процесс убит сигналом.
...
Потомок вызывает `_exit()`, а не `exit()`, если `exec()` не сработал. Разница важна: `exit()` сбрасывает буферы stdio, которые потомок унаследовал от родителя — это может привести к дублированию вывода. `_exit()` завершает процесс немедленно, не вызывая atexit-обработчики и не сбрасывая stdio-буферы.
...
Процесс создан через fork и exec. Но между запуском и завершением он проходит ряд состояний — выполняется, ждёт диск, принимает сигнал — и каждое из них ядро отслеживает явно, чтобы планировщик понимал, кому можно давать CPU.
...
**Sleeping, interruptible (S)** — процесс ждёт события: данных от [сокета](../programming/sockets.md), завершения таймера, нажатия клавиши. [Сигнал](../programming/signals.md) (например, SIGTERM) разбудит его. Это самое частое состояние: запустите `ps aux` и большинство процессов будут в состоянии S.
...
**Stopped (T)** — процесс приостановлен [сигналом](../programming/signals.md) SIGSTOP или SIGTSTP (Ctrl+Z в терминале). Продолжить можно сигналом SIGCONT (команда `fg` в shell). Отладчики (gdb, strace) используют SIGSTOP, чтобы остановить процесс и исследовать его состояние.

**Zombie (Z)** — процесс завершился, но его запись в таблице процессов ещё существует: ядро хранит код возврата, пока родитель не заберёт его через `wait()`.
...
> [!info]- Дополнительные буквы STAT
> `s` — лидер сессии, `l` — имеет потоки, `+` — в группе переднего плана (foreground) терминала.
...
Когда процесс вызывает `exit()`, ядро освобождает практически все его ресурсы: адресное пространство, открытые файлы, сокеты, стек ядра. Но в `task_struct` остаётся маленькая запись — PID и код завершения (exit status). Процесс в этом состоянии называется **зомби** (zombie).

Зачем хранить запись мёртвого процесса? Потому что родитель может захотеть узнать, как завершился потомок. Код возврата 0 означает успех, 127 — программа не найдена; если процесс убит сигналом, оболочка кодирует это как 128 + номер сигнала (SIGKILL = 9, итого 137). Пока родитель не вызовет `waitpid()` и не прочитает этот код, ядро обязано хранить запись.

Веб-сервер порождает процессы для конвертации, но не вызывает `waitpid()`:
...
Каждый завершившийся `convert` станет зомби. 1000 запросов — 1000 зомби. Каждый зомби удерживает один PID и небольшой объём ядерной памяти — большая часть ресурсов уже освобождена, осталась лишь запись с кодом завершения. Число PID конечно: `/proc/sys/kernel/pid_max` — от 32768 в базовой конфигурации ядра до 4194304 на современных серверах с systemd. Тысяча зомби — тысяча занятых PID. Зомби невозможно убить: `kill -9` не действует, потому что процесс уже мёртв. Единственный способ очистить — заставить родителя вызвать `wait()`, или завершить самого родителя.
...
fork, exec, состояния, зомби, сироты и дерево процессов складываются в один жизненный цикл — он виден целиком на сценарии конвертации изображения.
...
1. Сервер вызывает `fork()`. Ядро копирует `task_struct`, создаёт новую таблицу страниц (Copy-on-Write), назначает PID 1500. Занимает порядка сотен мкс.
...
Весь цикл — от fork до очистки зомби — занял, скажем, 2 секунды. Из них сотни мкс — fork, несколько микросекунд — exec, остальное — работа convert. Главное: ни на каком этапе ядро не копировало 2 ГБ памяти веб-сервера.

Процесс — единица изоляции: собственная память, собственные дескрипторы, собственный идентификатор. Но у полной изоляции есть цена. Если веб-серверу нужны параллельные обработчики, которые разделяют кеш изображений или пул соединений к базе данных, создавать отдельный процесс для каждого обработчика расточительно: каждый fork дублирует таблицу страниц, каждый процесс имеет собственный `task_struct`, а обмен данными между процессами требует IPC ([Inter-Process Communication](../programming/ipc.md) — пайпы, разделяемая память, [сокеты](../programming/sockets.md)). Когда нужен параллелизм с общими данными — нужна более лёгкая единица выполнения: [поток](threads.md).

## См. также

- [PostgreSQL — backend-процесс на соединение](../../databases/postgresql/postgresql.md) — классический preforking: `postmaster` на запросе делает fork, дочерний процесс обслуживает соединение и умирает.
- [Redis — BGSAVE через fork()+CoW](../../databases/redis/persistence/rdb.md) — снимок 10 ГБ без блокировки: дочерний процесс сериализует свою копию страниц, пока родитель продолжает принимать команды.
- [Ruby VM — процесс и потоки](../../ruby/internal/concurrency.md) — Unicorn/Puma используют тот же паттерн fork+CoW для пула воркеров.
