...
**Stopped (T)** — процесс приостановлен [сигналом](../programming/signals.md) SIGSTOP или SIGTSTP (Ctrl+Z в терминале). Продолжить можно [сигналом](../programming/signals.md) SIGCONT (команда `fg` в shell). Отладчики (gdb, strace) используют SIGSTOP, чтобы остановить процесс и исследовать его состояние.
...
fork, exec, состояния, зомби, сироты и дерево процессов складываются в один жизненный цикл. Проследим его по сценарию конвертации изображения.
