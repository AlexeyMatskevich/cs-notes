...
[виртуальная память](05-virtual-memory.md) (страницы, page fault, demand paging), [хранилище](../../computer/04-storage.md) (HDD seek/rotation, SSD IOPS, NAND write asymmetry), [файловые дескрипторы](04-file-descriptors.md) (fd, inode -- первое упоминание).

</details>

← [Виртуальная память](05-virtual-memory.md) | [Планировщик](07-scheduler.md) →
...
С точки зрения ядра, [диск](../../computer/04-storage.md) -- массив блоков фиксированного размера. HDD отдаёт секторы по 512 байт или 4 КБ; SSD -- страницы по 4-16 КБ. Ядро Linux работает с блоками (blocks) -- логическими единицами, обычно по 4 КБ, выровненными с размером страницы виртуальной памяти.
...
<details>
<summary>Задача: безопасный временный файл</summary>
...
Некоторые приложения управляют кэшированием самостоятельно. PostgreSQL поддерживает `shared_buffers` -- собственный кэш страниц в разделяемой памяти. Двойное кэширование (в shared_buffers и в page cache) расходует RAM впустую. Флаг `O_DIRECT` при `open()` говорит ядру: не пропускать ввод-вывод через page cache, писать и читать напрямую с устройства.

`O_DIRECT` требует, чтобы буфер в памяти и смещение в файле были выровнены по размеру блока (обычно 512 байт или 4 КБ). Невыровненный вызов вернёт ошибку `EINVAL`. PostgreSQL по умолчанию не использует `O_DIRECT` (двойное кэширование -- осознанный компромисс: page cache помогает при `fsync()`, упрощает read-ahead). Но некоторые базы данных (MySQL с InnoDB, RocksDB) активно используют `O_DIRECT`, потому что их внутренний кэш лучше знает приоритеты страниц, чем универсальный LRU ядра.
...
Идея та же, что и [WAL в PostgreSQL](../../databases/postgresql/durability/00-wal.md): прежде чем менять основные структуры на диске, записать описание всех изменений в отдельный журнал (journal). Журнал -- кольцевой буфер фиксированного размера (128 МБ по умолчанию в ext4).
...
**writeback** -- в журнал записываются только метаданные, данные могут быть записаны в любой момент -- до или после commit-маркера. Самый быстрый режим, но при сбое возможно чтение стейла: inode обновлён, а блок данных содержит старое (или чужое) содержимое.
...
Системный вызов `fsync(fd)` заставляет ядро записать все грязные страницы файла на диск и дождаться подтверждения от контроллера. После возврата `fsync()` данные гарантированно на устройстве хранения (при условии, что контроллер не врёт -- серверные SSD с конденсаторами не врут, десктопные иногда врут).

Стоимость `fsync()` определяется устройством. На HDD -- 5-15 мс: нужно дождаться, пока головка допишет данные и пластина провернётся. На серверном SSD -- 50-200 мкс: контроллер переносит данные из DRAM-буфера в NAND и подтверждает. На NVMe SSD с конденсаторным резервированием -- от 10 мкс: данные считаются надёжными, как только попали в DRAM контроллера с батарейкой.
...
- Michael Kerrisk, 2010, *The Linux Programming Interface* -- Chapters 14-18: File Systems
- Robert Love, 2010, *Linux Kernel Development* -- Chapter 13: The Virtual Filesystem
