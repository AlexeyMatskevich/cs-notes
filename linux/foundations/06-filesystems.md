...
[виртуальная память](05-virtual-memory.md) (страницы, page fault, demand paging), [хранилище](../../computer/data-path/03-storage.md) (HDD seek/rotation, SSD IOPS, NAND write asymmetry), [файловые дескрипторы](04-file-descriptors.md) (fd, inode).
...
С точки зрения ядра, [диск](../../computer/data-path/03-storage.md) -- массив блоков фиксированного размера. HDD отдаёт секторы по 512 байт или 4 КБ; SSD -- страницы по 4-16 КБ. Ядро Linux работает с блоками (blocks) -- логическими единицами, обычно по 4 КБ, выровненными с размером страницы виртуальной памяти.
...
Идея та же, что и [WAL (Write-Ahead Log) в PostgreSQL](../../databases/postgresql/durability/00-wal.md): прежде чем менять основные структуры на диске, записать описание всех изменений в отдельный журнал (journal). Журнал -- кольцевой буфер фиксированного размера (128 МБ по умолчанию в ext4).
...
Стоимость `fsync()` определяется устройством. На HDD -- 5-15 мс: нужно дождаться, пока головка допишет данные и пластина провернётся. На серверном SSD -- 50-200 мкс: контроллер переносит данные из DRAM-буфера (Dynamic RAM) в NAND-флеш и подтверждает. На NVMe SSD с конденсаторным резервированием -- от 10 мкс: данные считаются надёжными, как только попали в DRAM контроллера с батарейкой.
