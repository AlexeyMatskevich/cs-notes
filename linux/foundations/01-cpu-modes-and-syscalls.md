...
**Предпосылки:** [что такое операционная система](00-what-is-os.md) (абстракция, изоляция, разделение ресурсов).

← [Что такое операционная система](00-what-is-os.md) | [Процессы](02-processes.md) →
...
Кольца 1 и 2 были задуманы для драйверов и системных сервисов, но на практике ни Linux, ни Windows их не используют — только 0 и 3. Виртуализация добавила ещё один уровень: гипервизор работает в специальном режиме VMX root (иногда неформально называемом «ring -1»), а гостевые ОС получают аппаратное кольцо 0, но под контролем гипервизора.
...
Обработчик `entry_SYSCALL_64` сохраняет регистры пользовательского процесса, переключает стек на стек ядра и по номеру из `rax` находит нужную функцию в таблице системных вызовов (sys_call_table). В Linux ~450 системных вызовов: `read` (номер 0), `write` (1), `open` (2), `close` (3), `mmap` (9) и так далее.
       ...
       jump LSTAR]
         ...
         +------>  entry_SYSCALL_64
                   ...
                   sys_call_table[1]
                     |
                     v
                   ksys_write(fd=1, buf, 80)
                     |
                     v
                   rax = 80 (написано байт)
                     |
         <------+  sysret
         |       [CPU: CPL 0->3]
         v
...
Обычный вызов функции внутри программы — инструкция `call` — стоит 1-5 наносекунд. Системный вызов обходится в 100-300 наносекунд: примерно 50-100 раз дороже. Разница объясняется не самим переключением колец (оно занимает десятки наносекунд), а сопутствующими операциями: сохранение и восстановление регистров, переключение стека, сброс участков конвейера процессора (pipeline flush), проверки безопасности, возможная обработка сигналов при возврате. На процессорах с защитой от Spectre и Meltdown добавляется ещё 50-100 наносекунд на переключение таблиц страниц (KPTI — Kernel Page Table Isolation).
...
File descriptor (файловый дескриптор) — это просто индекс в таблице открытых файлов процесса. Каждый процесс при запуске получает три стандартных дескриптора: 0 — stdin (ввод), 1 — stdout (вывод), 2 — stderr (ошибки). Открытие файла через `open()` возвращает следующий свободный номер — обычно 3.
...
Вызов `gettimeofday()` через vDSO — это обычное чтение из памяти: 1-2 наносекунды вместо ~200. Разница в 100 раз. При 10 000 вызовов в секунду экономия составляет 10000 * 198 нс = ~2 мс — заметно для latency-sensitive приложений.
...
Альтернатива — накопить данные в буфере на стороне программы и отправить одним вызовом. Стандартная библиотека C (libc) делает именно это. Функции `fprintf()`, `fwrite()`, `fputs()` не вызывают `write()` напрямую. Вместо этого они копируют данные в буфер размером 4096 байт (размер по умолчанию для файлов в большинстве реализаций libc). Когда буфер заполняется — одним вызовом `write()` сбрасываются все 4096 байт.

```
fprintf() -> буфер (4096 байт)     <- без syscall, копирование в памяти
fprintf() -> буфер                  <- без syscall
fprintf() -> буфер                  <- без syscall
...
после ~51 строки (51 * 80 = 4080):
  write(fd, buf, 4080)              <- один syscall на 51 строку
```

Для 1000 строк по 80 байт: 80 000 байт / 4096 = ~20 системных вызовов вместо 1000. Накладные расходы: 20 * 200 нс = 4 микросекунды вместо 200. Разница в 50 раз.
...
Буферизация работает и в обратную сторону. Функция `fread()` при первом вызове читает не запрошенное количество байт, а целый блок — те же 4096 байт — одним вызовом `read()`. Последующие вызовы `fread()` берут данные из этого буфера, пока он не опустеет.
...
Вариант с `fread(&c, 1, 1, fp)`: первый вызов делает `read(fd, internal_buf, 4096)` — один системный вызов, 4096 байт. Следующие 4095 вызовов `fread()` — копирование из буфера в памяти программы, без системных вызовов. На весь файл: 1 048 576 / 4096 = 256 системных вызовов. Время на переключения: 256 * 200 нс ≈ 51 микросекунда. Плюс ~1 048 576 операций копирования одного байта из буфера — порядка 1 миллисекунды. Итого ~1 мс против ~210 мс. Разница — примерно 200 раз.

```
побайтовый read():                 fread() с буфером 4096:

read(fd, &c, 1) -- syscall         fread() -- read(fd, buf, 4096) syscall
read(fd, &c, 1) -- syscall         fread() -- копия из буфера
read(fd, &c, 1) -- syscall         fread() -- копия из буфера
...                                 ...
read(fd, &c, 1) -- syscall         fread() -- копия из буфера (x4095)
                                    fread() -- read(fd, buf, 4096) syscall
                                    ...

1 048 576 syscalls                  256 syscalls
~210 мс на переключения            ~0.05 мс на переключения + ~1 мс копирование
```

Буферизация — не магия, а замена дорогой операции (системный вызов, переключение колец) на дешёвую (копирование в памяти пользовательского процесса). Принцип тот же, что у [буферного кеша PostgreSQL](../../databases/postgresql/durability/01-buffer-cache.md): удерживать данные на более быстром уровне иерархии, чтобы реже обращаться к медленному.
...
<details>
<summary>Задача: почему tail -f показывает логи, а ./program | grep error — нет?</summary>
...
системный вызов (user->kernel)     ~100-300 нс     300-1000x
...
Системный вызов даёт доступ к ядру. Но ядро должно управлять множеством одновременно работающих программ — выделять каждой память, процессорное время, файловые дескрипторы. Единица такого управления — процесс: экземпляр запущенной программы со своим адресным пространством и состоянием.

## Sources

- Michael Kerrisk, 2010, *The Linux Programming Interface* — Chapter 3: System Call Interface
- `man 2 syscall` — описание механизма системных вызовов
- Intel, 2024, *Intel 64 and IA-32 Architectures Software Developer's Manual, Volume 3* — Chapter 5: Protection (privilege levels, CPL, rings)
