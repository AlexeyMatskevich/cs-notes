...
[процессы](02-processes.md) (состояния RUNNING/SLEEPING, context switch), [потоки](03-threads.md) (CLONE_VM, общее адресное пространство), [виртуальная память](05-virtual-memory.md) (CR3, TLB flush при переключении).

</details>

← [Файловые системы](06-filesystems.md) | [Права доступа и capabilities](08-permissions-and-capabilities.md) →
...
В первой заметке о [задачах ОС](00-what-is-os.md) мы сказали, что планировщик «принудительно забирает процессор у одной программы и передаёт другой — даже если та не хочет отдавать». Пришло время разобраться, как именно он это делает.
...
**Латентность (latency):** интерактивные потоки — терминал, GUI, веб-сервер, ожидающий ввод пользователя — должны получать процессор быстро после пробуждения. Пользователь нажал клавишу — терминал обязан ответить за миллисекунды, а не ждать, пока 399 других потоков доработают свои кванты.
...
Первый шаг — сохранение состояния текущего потока. Процессор помещает значения регистров общего назначения (rax, rbx, rcx, ..., r15), указатель инструкций (rip), указатель стека (rsp), регистр флагов (rflags) и регистры FPU/SSE/AVX в структуру `task_struct` текущего потока. Это примерно 200 байт на x86-64 — без AVX-512, где объём регистров достигает 2 КБ.
...
С версии ядра 2.6.23 (2007, автор Ingo Molnar) Linux использует CFS (Completely Fair Scheduler, полностью справедливый планировщик). Идея CFS — отказаться от фиксированных квантов и вместо этого отслеживать, сколько процессорного времени каждый поток уже получил.
...
CFS хранит все готовые к исполнению (runnable) потоки в красно-чёрном дереве ([BST](../../algorithms-and-data-structures/non-linear/03-binary-search-tree.md), самобалансирующийся вариант), упорядоченном по vruntime. Поток с минимальным vruntime находится в самом левом узле дерева. Выбор следующего потока — обход до крайнего левого узла — занимает O(1), потому что ядро кеширует указатель на этот узел. Добавление и удаление потока стоят O(log n).
...
Текущие значения этих параметров на конкретной машине можно посмотреть в `/proc/sys/kernel/sched_latency_ns` и `/proc/sys/kernel/sched_min_granularity_ns`.
...
CFS управляет обычными потоками с политикой `SCHED_NORMAL` (она же `SCHED_OTHER`). Но некоторые задачи требуют гарантий, которые CFS не может дать: аудио-обработка, промышленные контроллеры, телекоммуникационное оборудование. Для них Linux предоставляет политики реального времени (real-time scheduling policies).

**SCHED_FIFO** (First In, First Out): поток получает процессор и работает, пока не заблокируется или пока его не вытеснит поток с более высоким RT-приоритетом. Временных квантов нет — поток SCHED_FIFO может занимать ядро неограниченно долго. RT-приоритеты: от 1 (низший) до 99 (высший).

**SCHED_RR** (Round Robin): как SCHED_FIFO, но потоки с одинаковым RT-приоритетом чередуются по кругу с квантом ~100 мс.

Критическое правило: RT-потоки всегда вытесняют CFS-потоки, независимо от nice. Поток SCHED_FIFO с приоритетом 1 (минимальным) вытеснит любой поток SCHED_NORMAL, даже с nice -20. Это два разных класса планирования с абсолютным порядком: RT > CFS.
...
приоритет (выше = больше CPU)
  ...
  | (всегда вытесняют CFS)    |
  +---------------------------+
  |                           |
  +---------------------------+
  | SCHED_NORMAL (CFS)        |  nice -20..+19
  | (vruntime, weights)       |
  +---------------------------+
```
...
На многоядерной машине CFS работает не с одной глобальной очередью, а с отдельной очередью (runqueue) для каждого логического ядра. Каждая очередь содержит своё красно-чёрное дерево и свой `min_vruntime`. Поток принадлежит очереди одного ядра — не всех сразу.
...
Ядро само старается сохранять потоки на одних и тех же ядрах (cache affinity heuristic), но под нагрузкой балансировщик может мигрировать потоки ради равномерного распределения. Явная привязка через `taskset` или `sched_setaffinity()` переопределяет эвристику.

<details>
<summary>Задача: на 8-ядерном сервере с NUMA (2 узла по 4 ядра) PostgreSQL работает медленнее, чем ожидалось. perf stat показывает высокий процент remote NUMA accesses. Как исправить?</summary>
...
- Robert Love, 2010, *Linux Kernel Development* — Chapter 4: Process Scheduling
- Ingo Molnar, 2007, *CFS Scheduler Design* — Documentation/scheduler/sched-design-CFS.rst in Linux kernel
- Michael Kerrisk, 2010, *The Linux Programming Interface* — Chapter 35: Process Priorities and Scheduling
- `man 7 sched` — описание политик планирования Linux
