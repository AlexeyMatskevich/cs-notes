...
Начиная с ядра 6.6 (октябрь 2023, автор Peter Zijlstra) планировщик обычных задач (`SCHED_NORMAL`) — EEVDF (Earliest Eligible Virtual Deadline First, выбор по ближайшему допустимому виртуальному дедлайну). EEVDF сохраняет ключевую идею CFS — виртуальное время выполнения и красно-чёрное дерево, — но добавляет к vruntime второе измерение: **виртуальный дедлайн** (virtual deadline).
