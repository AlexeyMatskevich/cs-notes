...
Файловая система решила задачу хранения данных: программа вызывает `write(fd, buf, size)`, а ядро превращает это в блоки на диске. Но остаётся вопрос, не закрытый предыдущими подсистемами: если на машине 400 потоков и 4 ядра, кто решает, какой поток выполняется прямо сейчас?

Среди [задач ОС](what-is-os.md) планировщик отвечает за одну: «принудительно забрать процессор у одной программы и передать другой — даже если та не хочет отдавать». Как именно? Какой поток из 400 выбрать? По какому принципу отмерять время? Эти вопросы и есть суть планировщика.
...
Планировщик передаёт процессор от одного потока к другому через [переключение контекста](processes.md) (context switch). Эта операция не бесплатна: переключение между потоками одного процесса занимает 1-3 мкс, между потоками разных процессов — 3-10 мкс. Разница в том, что у разных процессов разные таблицы страниц, и их тоже приходится переключать. Отдельно от этого защита от Meltdown ([[linux/kernel/syscall-internals#KPTI: двойные таблицы страниц|KPTI]]) добавляет переключение таблиц страниц на каждом входе в ядро, удорожая syscall-тяжёлые нагрузки; на процессорах с PCID этот overhead частично снимается.

Из чего складывается эта стоимость? Переключение — три шага.

Первый шаг — сохранение состояния текущего потока. Ядро помещает значения процессорных регистров — общего назначения, указатели инструкций и стека, регистр флагов, а также широкие векторные регистры для вещественной арифметики и обработки нескольких чисел одной командой — в структуру [[linux/foundations/processes#Дескриптор процесса: task_struct|task_struct]] текущего потока. Векторных регистров много и они широкие, поэтому одно только их состояние доходит до пары килобайт — и всё это нужно скопировать на каждом переключении.
...
Долгое время штатным планировщиком обычных задач в Linux был CFS (Completely Fair Scheduler, полностью справедливый планировщик): он появился в ядре 2.6.23 (2007, автор Ingo Molnar) и работал до версии 6.5 включительно. Его преемник EEVDF развивает ту же идею виртуального времени, добавляя к ней дедлайны, поэтому понимание EEVDF опирается на устройство CFS.
...
Порог определяется двумя параметрами: `sched_latency` и `sched_min_granularity` (эти параметры действуют в ядрах до 6.5; EEVDF заменяет их на единый `base_slice`). Параметр `sched_latency` (типичное значение 6-24 мс в зависимости от дистрибутива и конфигурации) задаёт целевой период, за который каждый runnable-поток должен получить хотя бы один квант процессора. Идеальный квант: `sched_latency / число_runnable_потоков`.

4 runnable-потока + sched_latency 6 мс = квант ~1.5 мс на каждый. 8 потоков — ~0.75 мс. Но при 100 runnable-потоках квант стал бы 0.06 мс (60 мкс) — слишком мало, накладные расходы на переключение сожрут полезное время. Поэтому существует `sched_min_granularity` (типичное значение 1-4 мс в зависимости от конфигурации) — минимальный квант, ниже которого CFS не переключает. При 100 runnable-потоках каждый получит этот минимум, а полный «круг» растянется соответственно.
...
На сервере параллельно идёт компиляция ядра Linux (`make -j4`, 4 потока, каждый непрерывно потребляет CPU) и открыт терминал, в котором пользователь набирает команды.
...
При пробуждении после длительного сна vruntime потока может оказаться настолько низким, что поток получит непропорционально большой квант. Чтобы этого избежать, CFS ограничивает vruntime при пробуждении. Ориентир для ограничения — `min_vruntime`: наименьший vruntime среди потоков очереди, точка отсчёта, относительно которой меряют «обделённость» новых и проснувшихся. Проснувшемуся потоку CFS ставит vruntime не ниже, чем `min_vruntime` минус `sched_latency`: поток получает приоритет, но не бесконечный.
...
Начиная с ядра 6.6 (октябрь 2023, автор Peter Zijlstra) планировщик обычных задач (`SCHED_NORMAL`) — EEVDF (Earliest Eligible Virtual Deadline First, выбор по ближайшему допустимому виртуальному дедлайну). Модель "nice -> weight -> vruntime" остаётся основой и в EEVDF. Меняется не базовая идея справедливости, а способ выбора следующего runnable-потока и настройки латентности, которыми эта справедливость регулируется. EEVDF сохраняет ключевую идею CFS — виртуальное время выполнения и красно-чёрное дерево, — но добавляет к vruntime второе измерение: **виртуальный дедлайн** (virtual deadline).

В CFS единственным критерием выбора было vruntime: запускается поток с наименьшим vruntime. Это создавало проблему: коротким интерактивным задачам и длинным вычислительным задачам назначался квант одинаковой природы, а настройки справедливости и латентности задавались эвристиками (`sched_latency`, `sched_min_granularity`). EEVDF убирает эти эвристики и вычисляет для каждого потока виртуальный дедлайн: `VD = eligible_time + slice` (оба слагаемых — в единицах виртуального времени, где вес уже учтён через vruntime). Планировщик выбирает поток с наименьшим VD среди допустимых (eligible) — тех, чей vruntime не опережает справедливое значение.
...
Планировщик EEVDF (до версии 6.6 — CFS) управляет обычными потоками с политикой `SCHED_NORMAL` (она же `SCHED_OTHER`). Но некоторые задачи требуют гарантий, которые EEVDF не может дать. Аудиопоток должен заполнить аппаратный буфер каждые 5 мс — если планировщик даст джиттер (jitter, разброс задержки пробуждения) в 1-2 мс, буфер опустеет и в звуке появится глич. Промышленный контроллер, управляющий мотором, обязан реагировать на датчик в течение сотен микросекунд. EEVDF оптимизирует среднее, не максимальное время ожидания — а для RT-задач важен именно наихудший случай. Для них Linux предоставляет политики реального времени (real-time scheduling policies).
