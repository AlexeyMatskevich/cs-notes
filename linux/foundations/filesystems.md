...
> [!info]- Предпосылки
> [виртуальная память](virtual-memory.md) (страницы, page fault), [хранилище](../../computer/data-path/storage.md) (HDD seek/rotation, SSD latency), [файловые дескрипторы](file-descriptors.md) (fd, inode), [процессы](processes.md) (изолированное адресное пространство, единица выполнения).
...
Чтобы эта прослойка работала, ей нужно решить три задачи. Первая -- дать данным имена: диск хранит безымянные блоки, а программа открывает файлы по путям, поэтому что-то должно переводить путь в расположение блоков. Вторая -- скорость: каждое чтение и запись напрямую с диска стоили бы миллисекунды, поэтому что-то должно держать горячие данные в RAM. Третья -- durability: данные в RAM исчезают при сбое питания, поэтому что-то должно довести их до диска и пережить обрыв на полпути. Имена, скорость, durability -- три части, на которые распадается дальнейшее устройство.

## Диск как плоский массив блоков

Программе нужны файлы с именами, правами и путями. Диск ничего об этом не знает. С точки зрения ядра, [[computer/data-path/storage#HDD: механика вращающихся пластин|диск]] -- массив блоков фиксированного размера. HDD отдаёт секторы по 512 байт или 4 КБ; SSD -- страницы по 4-16 КБ. Ядро Linux работает с блоками (blocks) -- логическими единицами, обычно по 4 КБ, выровненными с размером страницы виртуальной памяти.
...
Inode знает о файле всё, кроме одного: где именно лежат его данные на диске. Inode должен как-то указать, в каких блоках лежит содержимое файла. Для маленького файла -- тривиально: записать номера блоков прямо в inode. Для файла на 4 ТБ -- номера не поместятся в 256 байт. Классическая схема ext2/ext3 решает это через уровни косвенной адресации (в ext4 она заменена extent-деревьями, о которых ниже).
...
В inode помещается 4 экстента. Когда файл фрагментирован и экстентов больше четырёх, ext4 строит [B-дерево](../../algorithms-and-data-structures/non-linear/b-tree.md) (сбалансированное дерево поиска) экстентов, корень которого лежит в inode. Максимальный размер файла в ext4 -- 16 ТБ (против ~4 ТБ при классической схеме ext2/ext3). На практике большинство файлов укладываются в 1-2 экстента, если файловая система не сильно фрагментирована. Посмотреть экстенты файла можно через `filefrag -v /path/to/file` -- команда покажет количество экстентов и их расположение на диске.
...
Когда процесс вызывает `read()`, ядро не обращается к диску сразу. Сначала проверяет: есть ли нужная страница файла в page cache. Если да -- данные копируются из RAM в буфер пользователя. Обращения к диску нет. Это аналог [minor page fault](virtual-memory.md) в виртуальной памяти: страница уже в RAM, просто не в том адресном пространстве.
...
Некоторые приложения управляют кэшированием самостоятельно. PostgreSQL поддерживает [shared_buffers](../../databases/postgresql/durability/buffer-cache.md) -- собственный кэш страниц в разделяемой памяти со своей политикой вытеснения. Двойное кэширование (в shared_buffers и в page cache) расходует RAM впустую. Флаг `O_DIRECT` при `open()` говорит ядру: не пропускать ввод-вывод через page cache, писать и читать напрямую с устройства.
...
**writeback** (не путать с writeback daemon из раздела о page cache выше — здесь режим журналирования, там фоновый сброс страниц) -- в журнал записываются только метаданные, данные могут быть записаны в любой момент -- до или после commit-маркера. Самый быстрый режим, но при сбое возможно чтение мусора: inode обновлён, а блок данных содержит старое (или чужое) содержимое.

Для типичного сервера приложений ordered -- правильный выбор: он гарантирует, что inode никогда не будет указывать на блок с данными другого файла, и не требует двойной записи, как journal. Переключаться на journal имеет смысл только если нужна защита самих данных файлов (а не только метаданных) при сбое без явного `fsync()` — это редкий сценарий. Writeback оправдан, если приложение само вызывает `fsync()` в нужных точках и хочет максимальную скорость операций с метаданными.
