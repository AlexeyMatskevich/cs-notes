...
> [!info]- Предпосылки
> [виртуальная память](virtual-memory/virtual-memory.md) (страницы, page fault), [хранилище](../../computer/data-path/storage.md) (HDD seek/rotation, SSD latency), [файловые дескрипторы](file-descriptors.md) (fd, inode), [процессы](processes.md) (изолированное адресное пространство, единица выполнения).

← [Page fault и отложенные механизмы](virtual-memory/page-faults.md) | [Планировщик](scheduler.md) →
...
Программе нужны файлы с именами, правами и путями. Диск ничего об этом не знает. С точки зрения ядра, [[computer/data-path/storage#HDD: механика вращающихся пластин|диск]] -- массив блоков фиксированного размера. HDD отдаёт секторы по 512 байт или 4 КБ; SSD -- страницы по 4-16 КБ. Ядро Linux работает с блоками (blocks) -- логическими единицами, обычно по 4 КБ, выровненными с размером страницы виртуальной памяти. Ext4 — файловая система по умолчанию в большинстве дистрибутивов Linux; XFS распространена на серверах RHEL и там, где важен параллелизм метаданных. Большинство принципов ниже общие; ext4-специфика отмечена отдельно.
...
Inode знает о файле всё, кроме одного: где именно лежат его данные на диске. Для маленького файла -- тривиально: записать номера блоков прямо в inode. Для файла на терабайты -- номера не поместятся в 256 байт. Ext4 решает это через **extent-деревья**.

Extent (экстент) -- запись вида «начиная с блока N, следующие M блоков принадлежат файлу». Один extent описывает непрерывный диапазон: вместо миллиона отдельных указателей -- одна запись «блок 500000, длина 1000000». В inode помещается 4 экстента. Когда файл сильно фрагментирован и экстентов больше четырёх, ext4 строит [B-дерево](../../algorithms-and-data-structures/non-linear/b-tree.md) (сбалансированное дерево поиска) экстентов, корень которого лежит в inode. Максимальный размер файла в ext4 -- 16 ТБ. На практике большинство файлов укладываются в 1-2 экстента, если ФС не сильно фрагментирована. Посмотреть экстенты файла можно через `filefrag -v /path/to/file`.

Исторически ext2/ext3 использовали другую схему: 12 прямых указателей на блоки в inode плюс одинарная/двойная/тройная косвенная адресация через блоки-указатели. Для больших файлов это означало три-четыре обращения к диску на каждый блок — ext4 заменила эту схему экстентами в 2008 году.
...
Когда [процесс](processes.md) вызывает `open("/var/log/nginx/access.log", O_RDONLY)`, ядро разбирает путь по компонентам:
...
Inode и имя файла хранятся в разных местах: inode -- на диске в таблице inode, имя -- в блоках данных директории. Из этого разделения следует неочевидное свойство: один inode может иметь несколько имён в разных директориях.
...
Symlink может указывать на несуществующий путь (dangling link), может пересекать границы файловых систем, может ссылаться на директорию. Цена -- дополнительное разрешение пути при каждом доступе (на практике дешёво благодаря dentry cache). Ядро ограничивает глубину разрешения symlink до 40 переходов (константа `MAXSYMLINKS`) -- защита от циклических ссылок, где `a -> b` и `b -> a`. Короткие пути (до ~60 байт в ext4) хранятся прямо в inode и не требуют отдельного обращения к блоку данных (fast symlink).

## Удаление файла: link count и open count

У файла два счётчика, управляющих его временем жизни. Link count -- количество имён в директориях, указывающих на inode. Open count -- количество [файловых дескрипторов](file-descriptors.md), открытых на этот inode.
...
Nginx вызывает `read(fd, buf, 4096)` -- он не знает и не должен знать, читает ли с NVMe или из структуры ядра вроде `/proc`. Как ядро обеспечивает единый вызов для принципиально разных источников?
...
Каждая файловая система регистрирует в VFS набор callback-функций: как прочитать inode, как найти имя в директории, как записать блок. Ext4 заполняет эти слоты своими реализациями, которые работают с блоками на диске. NFS заполняет те же слоты функциями, которые отправляют RPC-запросы (Remote Procedure Call) на удалённый сервер. Для программы ничего не меняется: тот же `read()`, те же fd.
...
Когда процесс вызывает `read()`, ядро не обращается к диску сразу. Сначала проверяет: есть ли нужная страница файла в page cache. Если да -- данные копируются из RAM в буфер пользователя. Обращения к диску нет. Это аналог [[linux/foundations/virtual-memory/page-faults#Page fault: когда трансляция не удалась|minor page fault]] в виртуальной памяти: страница уже в RAM, просто не в том адресном пространстве.
...
Если сбой между шагами 4 и 5 -- данные записаны и inode на них ссылается, но записи `access.log -> inode N` в директории ещё нет. Inode и его блоки помечены занятыми в bitmap, но ни одно имя к ним не ведёт -- файл недостижим по пути. С точки зрения программы он просто не появился, хотя дисковое пространство уже израсходовано.
...
Ext4 по умолчанию журналирует в режиме **ordered**: метаданные идут в журнал, данные пишутся на диск *до* commit-маркера журнала — при сбое inode никогда не будет ссылаться на блок с чужими данными. Альтернативы — опциональные компромиссы: **journal** журналирует и метаданные, и данные (удвоенный I/O, используется редко); **writeback** журналирует только метаданные, данные пишутся в произвольный момент (быстрее, но при сбое inode может ссылаться на блок со старыми или чужими данными).

> [!info]- Когда стоит уйти от ordered
> Переключаться на `journal` имеет смысл, только если нужна защита самих данных файлов (а не только метаданных) при сбое без явного `fsync()` — редкий сценарий. `writeback` оправдан, если приложение само вызывает `fsync()` в нужных точках и хочет максимум скорости операций с метаданными. Не путать writeback-режим журналирования с writeback daemon из раздела о page cache — первое про журнал, второе про фоновый сброс грязных страниц.
...
Системный вызов `fsync(fd)` заставляет ядро записать все грязные страницы файла на диск и дождаться подтверждения от контроллера. После возврата `fsync()` данные гарантированно на устройстве хранения -- при условии, что контроллер не врёт. Серверные SSD с PLP (Power Loss Protection) гарантируют сброс буфера в NAND при потере питания; о том, как это работает на уровне железа, подробнее в [хранилище](../../computer/data-path/storage.md).

Стоимость `fsync()` определяется устройством. На HDD -- 5-15 мс: нужно дождаться, пока головка допишет данные и пластина провернётся. На серверном SSD -- 50-200 мкс: контроллер переносит данные из DRAM-буфера (Dynamic RAM) в NAND-флеш и подтверждает. На NVMe SSD с PLP -- от 10 мкс.
...
Вся эта инфраструктура работает, пока ядро решает, какой [процесс](processes.md) выполнять в данный момент. `write()` возвращается мгновенно, а writeback daemon сбрасывает данные позже -- но кто решает, когда запустить writeback, когда вернуть управление Nginx, когда переключить CPU на PostgreSQL? [Планировщик](scheduler.md) (scheduler) -- механизм, который распределяет процессорное время между процессами.
