...
Каждый процесс в Linux имеет владельца. В [заметке о процессах](02-processes.md) мы видели, что `task_struct` хранит UID и GID. На самом деле идентичность процесса устроена сложнее -- у него не одна пара UID/GID, а несколько.
...
В [заметке о файловых системах](06-filesystems.md) мы видели, что inode хранит три набора rwx-битов: для владельца (owner), группы (group) и остальных (others). Когда процесс вызывает `open()`, ядро выполняет проверку, сравнивая идентичность процесса с метаданными inode.
...
**`CAP_NET_RAW`** -- создание raw-сокетов. Нужна утилите `ping` для отправки ICMP-пакетов.
...
Атакующий пробует `kill(950, SIGTERM)` для завершения PostgreSQL (PID 950, UID 999). Ядро проверяет: EUID отправителя (33) не равен EUID получателя (999) и не равен 0. Сигнал не доставлен -- `EPERM`.
