...
[файловые дескрипторы](04-file-descriptors.md) (fd, inode), [процессы](02-processes.md) (fork, copy-on-write на уровне эффекта), [иерархия памяти](../../computer/01-memory-hierarchy.md) (cache line, TLB), [когерентность кешей](../../computer/02-cache-coherency.md) (MESI).

</details>

← [Файловые дескрипторы](04-file-descriptors.md) | [Файловые системы](06-filesystems.md) →
...
TLB (Translation Lookaside Buffer, буфер трансляции) кеширует недавние трансляции, чтобы не обращаться к page table на каждой инструкции. Если страницы нет в RAM — процессор генерирует page fault, и ядро загружает её. Если страница помечена как copy-on-write — ядро скопирует её при первой записи.
...
Пять компонентов — страницы, page table, MMU, TLB, page fault — работают вместе. Разберём каждый.
...
CR3 ---> [ PGD: 512 записей ]
         ...
         [ PTE: 512 записей ]
              ...
              | 9 бит -> номер фрейма
              v
         физический фрейм + offset (12 бит) = физический адрес
```
...
Стоимость TLB shootdown — 1–10 мкс на каждое ядро-получатель: IPI пересекает interconnect, прерывает исполнение, процессор выполняет инвалидацию и возвращается к работе. На 128-ядерном сервере shootdown может затронуть десятки ядер одновременно. Для программ с частым `mmap()`/`munmap()` — JIT-компиляторы (JVM, V8), которые постоянно выделяют и освобождают регионы под скомпилированный код — shootdown становится заметной нагрузкой. Аналогия с [когерентностью кешей](../../computer/02-cache-coherency.md): протокол MESI инвалидирует кеш-линии при изменении данных, shootdown инвалидирует TLB-записи при изменении трансляций. PCID смягчает проблему при переключении контекста (старые записи помечены чужим тегом), но не при изменении page table текущего процесса — shootdown неизбежен.
...
Обработчик page fault в ядре различает эти три случая по состоянию VMA и PTE. Алгоритм:

Сначала ядро проверяет, есть ли VMA, покрывающий адрес fault (через red-black дерево ([BST](../../algorithms-and-data-structures/non-linear/03-binary-search-tree.md), самобалансирующийся вариант) VMA процесса — поиск за O(log n)). Если VMA нет — illegal access, SIGSEGV.
...
В [процессах](02-processes.md) мы видели, что `fork()` создаёт дочерний процесс — копию родительского. Эффект: дочерний процесс видит те же данные, но изменения в одном не затрагивают другой.

Наивная реализация — скопировать всю физическую память родителя — потребовала бы для процесса с 1 ГБ RSS около 250 мс и 1 ГБ свободной RAM. Это неприемлемо: типичный веб-сервер вызывает fork() при обработке запросов, и задержка в четверть секунды на каждый fork убила бы производительность.
...
6. Уменьшает refcount старого фрейма. Если refcount стал 1 — оставшийся процесс получает R/W обратно: копировать больше не нужно.
