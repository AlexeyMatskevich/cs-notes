...
**Предпосылки:** [процессы](02-processes.md) (fork, exec, адресное пространство, task_struct).

← [Процессы](02-processes.md) | [Файловые дескрипторы](04-file-descriptors.md) →
...
Веб-сервер обслуживает 10 000 одновременных клиентов. Классическая архитектура — по процессу на клиента: master принимает соединение, вызывает fork, дочерний процесс обрабатывает запрос. Такую модель использовали ранние версии Apache (prefork MPM).
...
Вторая — кэш. Веб-серверу нужен разделяемый кэш ответов, пул подключений к базе данных, таблица активных сессий. Процессы не видят память друг друга. Чтобы разделить данные, приходится использовать IPC: shared memory через mmap или System V, каналы, сокеты. Каждый из этих механизмов требует явной синхронизации — семафоров, блокировок, сериализации данных. Общий кэш, который в одном адресном пространстве был бы простым обращением к хеш-таблице, превращается в архитектурную задачу.

Третья — скорость создания и переключения. fork на современном Linux занимает 50–100 мкс: ядру нужно скопировать таблицу страниц, дескрипторы файлов, структуры сигналов. Переключение контекста между процессами стоит 3–10 мкс, потому что ядро перезагружает регистр CR3 (базовый адрес таблицы страниц), и это сбрасывает TLB — кэш трансляций виртуальных адресов в физические. После сброса TLB каждое первое обращение к памяти в новом процессе проходит через полную аппаратную трансляцию.
...
Всё остальное — общее: адресное пространство (код, глобальные переменные, heap), таблица файловых дескрипторов, обработчики сигналов, PID. Два потока одного процесса читают и пишут одни и те же адреса без какого-либо IPC.
...
CLONE_THREAD   — быть частью одной thread group (общий PID для getpid())
```
...
Приложения работают с потоками не через clone напрямую, а через POSIX Threads (pthreads) — стандартизированный интерфейс, реализованный в glibc как NPTL (Native POSIX Threads Library). NPTL создаёт каждый поток как отдельный task_struct через clone, поэтому каждый поток виден ядру и планируется независимо.
...
Это **состояние гонки** (race condition) — результат зависит от порядка чередования инструкций, который не детерминирован. Процессы защищены от этого по умолчанию: каждый работает со своей копией данных. Потоки вынуждены использовать механизмы синхронизации — мьютексы, атомарные операции, условные переменные — чтобы координировать доступ к общим данным.

Синхронизация вносит свою стоимость. Мьютекс (mutex, от mutual exclusion) гарантирует, что критическая секция выполняется только одним потоком. Остальные потоки, пытающиеся захватить тот же мьютекс, блокируются — они не выполняют полезную работу, а ждут. При высокой конкуренции за один мьютекс (hot lock) параллелизм вырождается в последовательное выполнение, и накладные расходы на переключение контекста и синхронизацию делают многопоточную программу медленнее однопоточной.
...
Решение — **thread-local storage** (TLS, не путать с Transport Layer Security). Каждый поток получает собственную копию переменной. В POSIX это реализуется через `pthread_key_create` / `pthread_getspecific` / `pthread_setspecific`. В GCC и Clang есть более удобный синтаксис — ключевое слово `__thread` (или `_Thread_local` в C11):
...
До сих пор речь шла о потоках, каждый из которых представлен task_struct в ядре. Ядро управляет их планированием, может вытеснить один поток и поставить на CPU другой. Это потоки в модели **1:1** — каждый пользовательский поток соответствует одному task_struct в ядре. Ядро знает о каждом из них, планирует и может вытеснить. Термин «kernel thread» в Linux обычно означает другое — потоки, работающие целиком в ядре, без пользовательского адресного пространства (kworker, ksoftirqd, migration). Их видно в `ps` по квадратным скобкам: `[kworker/0:1]`. Потоки, созданные через pthread_create — пользовательские потоки, управляемые ядром, но не kernel threads в этом смысле.

Существует альтернатива — **потоки пользовательского пространства** (user-space threads). Библиотека в пространстве процесса сама управляет переключением между несколькими контекстами исполнения, не обращаясь к ядру. Ядро видит один task_struct — один процесс с одним потоком. Внутри этого потока библиотека переключает контексты вручную: сохраняет регистры и указатель стека одного контекста, восстанавливает другой.

Переключение обходится в 100–200 нс — на порядок дешевле, чем 1–5 мкс для переключения через ядро. Причина: нет перехода из user mode в kernel mode и обратно (каждый такой переход — сотни наносекунд на сохранение/восстановление состояния, проверки безопасности). Переключение происходит кооперативно — поток добровольно уступает управление (yield). Это упрощает синхронизацию: если поток не делает yield внутри критической секции, гонки невозможны. Но если один поток зацикливается и не уступает — остальные навсегда останавливаются.

Но у модели 1:1 (один user-space thread на один kernel thread — то, что делает pthreads) есть преимущество, которого лишены чисто пользовательские потоки. Когда пользовательский поток выполняет блокирующий системный вызов — read из сокета, write на диск — блокируется весь kernel thread, а значит, все пользовательские потоки, привязанные к нему. Один медленный read на диск останавливает 1000 корутин.

Это привело к модели **M:N** — M пользовательских потоков мультиплексируются на N потоков ядра. Если один пользовательский поток блокируется в системном вызове, его kernel thread занят, но остальные N−1 kernel threads продолжают исполнять другие пользовательские потоки.

```
Модель 1:1 (pthreads)         Модель M:N (Go, Erlang)
─────────────────────          ─────────────────────────
User      Kernel               User         Kernel
Thread 1 -> KThread 1          Goroutine 1 -+
Thread 2 -> KThread 2          Goroutine 2 -+-> KThread 1
Thread 3 -> KThread 3          Goroutine 3 -+
                               Goroutine 4 -+-> KThread 2
                               Goroutine 5 -+
                               Goroutine 6 -+-> KThread 3
```

Горутины в Go — пример M:N модели. Go runtime содержит собственный планировщик (GMP: Goroutine, Machine, Processor), который распределяет горутины по OS-потокам. Стек горутины начинается с 2–8 КБ (вместо 8 МБ у OS-потока) и растёт по мере необходимости — при вызове функции runtime проверяет, хватает ли места в стеке, и при необходимости выделяет новый, большего размера, копируя содержимое. Это позволяет создавать сотни тысяч горутин в одном процессе: 100 000 горутин по 4 КБ = 400 МБ стеков, тогда как 100 000 OS-потоков по 8 МБ = 800 ГБ виртуальной памяти (что превышает лимиты большинства систем).

Green threads в ранних версиях Java (до 1.3) использовали модель N:1 — все потоки на одном kernel thread, что делало их непригодными для SMP-систем. Файберы (fibers) в Ruby используют кооперативное переключение: fiber явно передаёт управление через `Fiber.yield`, без вытеснения. С Ruby 3.0 появился Fiber Scheduler, который позволяет автоматически переключать файберы при блокирующем I/O, приближаясь к M:N модели.

Каждая модель — компромисс:

```
Модель       Стоимость переключения  Блокирующие вызовы    Количество
──────────────────────────────────────────────────────────────────────
...
Модель 1:1 проста и предсказуема — один поток, один task_struct, вытеснение ядром. Модель M:N даёт массовый параллелизм, но усложняет runtime и отладку: gdb видит OS-потоки, а не горутины, и stack trace горутины требует поддержки со стороны runtime. Модель N:1 (все пользовательские потоки на одном kernel thread) дешёвая, но непригодна для I/O-нагруженных задач — один блокирующий вызов останавливает всё.

Большинство современных языков и серверов выбирают одну из двух стратегий: 1:1 с пулом потоков фиксированного размера (Java, C++, Rust) или M:N с runtime-планировщиком (Go, Erlang/BEAM). Веб-сервер из начала заметки с 10 000 клиентами на практике не создаёт 10 000 OS-потоков — вместо этого используется пул из сотен потоков в комбинации с неблокирующим I/O (epoll), либо M:N модель с горутинами. Чистая модель «по потоку на клиента» остаётся полезной иллюстрацией компромиссов, но промышленные решения комбинируют подходы.

Nginx — пример радикально другого решения: вместо потоков на клиента он использует event loop с одним рабочим процессом на ядро CPU. Каждый процесс обслуживает тысячи соединений через epoll, не создавая ни потоков, ни дочерних процессов для отдельных запросов. Это устраняет и стоимость контекстных переключений между потоками, и сложность синхронизации — ценой невозможности использовать блокирующие операции внутри event loop.
...
Потоки и процессы — это контексты исполнения: они определяют, кто выполняет код и в каком адресном пространстве. Но любая полезная программа взаимодействует с внешним миром — читает файлы, отправляет данные по сети, пишет в терминал. Для этого нужен механизм, через который процесс обращается к ресурсам ядра. Этот механизм — [файловые дескрипторы](04-file-descriptors.md).

## Sources

- Michael Kerrisk, 2010, *The Linux Programming Interface* — Chapters 28–33: Threads
