...
Диагностика: заглянуть в `/proc/<pid>/fd`. `/proc` — виртуальная файловая система, через которую ядро показывает информацию о процессах ([подробнее](../kernel/02-devices-and-drivers.md)). Каждый открытый дескриптор отображается как символическая ссылка (файл-указатель, хранящий путь к другому файлу) на ресурс:
