...
**Предпосылки:** [потоки](03-threads.md) (общее адресное пространство, разделение ресурсов).

← [Потоки](03-threads.md) | [Виртуальная память](05-virtual-memory.md) →
...
Эти три fd — не магия ядра. Их создаёт и передаёт процесс-родитель (обычно shell) перед запуском программы. Если закрыть fd=0 и вызвать `open()`, новый файл получит номер 0 — ядро всегда выбирает наименьший свободный.
...
Закрытие неиспользуемых концов pipe критично. Если родитель не закроет `pipefd[1]`, grep никогда не получит EOF при чтении: ядро считает pipe открытым, пока существует хоть один fd на запись. `grep` повиснет навсегда, ожидая данных, которые никто не отправит.

Буфер pipe ограничен: на Linux это 65536 байт (16 страниц по 4 КБ). Когда буфер заполнен, `write()` блокируется, пока читатель не заберёт данные. Когда буфер пуст, `read()` блокируется, пока писатель не добавит. Это встроенная синхронизация без явных мьютексов — producer-consumer через механизм ядра.
...
Каждый процесс имеет лимит на количество открытых дескрипторов. Мягкий лимит (soft limit) по умолчанию — 1024, жёсткий (hard limit) — обычно до 1048576 (~1 млн). Мягкий лимит процесс может поднять сам через `setrlimit()` до значения жёсткого. Жёсткий может поднять только root.
...
Но процессы работают не только с дескрипторами — они работают с памятью. Каждый процесс «видит» адреса начиная с 0x400000, и два процесса могут использовать один и тот же адрес без конфликта. Как это возможно — вопрос [виртуальной памяти](05-virtual-memory.md).

## Sources

- Michael Kerrisk, 2010, *The Linux Programming Interface* — Chapters 4-5: File I/O
- `man 2 open`, `man 2 dup2`, `man 2 pipe`, `man 2 fork`
