...
Проверить легко: открыть три терминала и выполнить `ps aux | grep bash`. В выводе будет три строки с разными PID, но одинаковым путём к исполняемому файлу.
...
**Регистры процессора** — контекст выполнения. Указатель инструкции (`rip` на x86-64) говорит, какую инструкцию выполнять следующей. Указатель стека (`rsp`) указывает на вершину стека. Базовый указатель (`rbp`) обозначает начало текущего фрейма. Когда ядро переключает процессор с одного процесса на другой, оно сохраняет все регистры первого процесса и восстанавливает регистры второго — это **контекстное переключение** (context switch), которое стоит порядка 1-5 мкс.
...
При этом PID не меняется — это тот же процесс, просто с другим содержимым. Файловые дескрипторы по умолчанию сохраняются (если на них не стоит флаг `FD_CLOEXEC`). Благодаря этому родитель может до `exec()` перенаправить stdin/stdout потомка на файл или пайп — и новая программа унаследует эти перенаправления, ничего о них не зная.
...
**Sleeping, uninterruptible (D)** — процесс ждёт завершения операции ввода-вывода, которую нельзя прервать: обычно это дисковый I/O или обращение к NFS. Сигналы не действуют — даже `kill -9` не завершит процесс в состоянии D. Он выйдет из этого состояния только когда I/O завершится. Массовое появление процессов в D — признак проблем с диском или сетевой файловой системой.
...
Дополнительные буквы после основного состояния: `s` — лидер сессии, `l` — имеет потоки, `+` — в foreground-группе терминала.
...
Обратная ситуация: родитель завершился раньше потомка. Потомок стал **сиротой** (orphan). Осиротевшие процессы не остаются без присмотра — ядро немедленно переназначает им нового родителя. По умолчанию это init (PID 1, systemd), но если один из предков процесса установил себя subreaper'ом через `prctl(PR_SET_CHILD_SUBREAPER, 1)` — сироту усыновит он. Контейнерные рантаймы (Docker, systemd-nspawn) используют subreaper, чтобы забирать зомби внутри контейнера, не доводя их до PID 1 хоста.
...
Процесс — единица изоляции: собственная память, собственные дескрипторы, собственный идентификатор. Но у полной изоляции есть цена. Если веб-серверу нужны параллельные обработчики, которые разделяют кеш изображений или пул соединений к базе данных, создавать отдельный процесс для каждого обработчика расточительно: каждый fork дублирует таблицу страниц, каждый процесс имеет собственный `task_struct`, а обмен данными между процессами требует IPC (пайпы, shared memory, сокеты). Когда нужен параллелизм с общими данными — нужна более лёгкая единица выполнения: [поток](03-threads.md).

## Sources

- Michael Kerrisk, 2010, *The Linux Programming Interface* — Chapters 24-26: Process Creation, Termination, Monitoring
- Robert Love, 2010, *Linux Kernel Development*, 3rd ed. — Chapter 3: Process Management
- `man 2 fork`, `man 2 execve`, `man 2 waitpid`, `man 7 signal`
