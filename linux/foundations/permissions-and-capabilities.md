...
> [!info]- Предпосылки
> [процессы](processes.md) (UID/GID, fork/exec), [файловые дескрипторы](file-descriptors.md) (open(), проверка прав), [файловые системы](filesystems.md) (inode, rwx-биты), [сигналы](../programming/signals.md) (kill/SIGTERM/SIGHUP — уведомление, посланное процессу, с проверкой прав отправителя).

← [Планировщик](scheduler.md) | [Синхронизация](../concurrency/synchronization.md) →

[Процесс](processes.md) пытается прочитать `/etc/shadow` с хешами паролей, привязаться к порту 80, отправить [сигнал](../programming/signals.md) завершения чужому процессу. Каждое такое действие ядро может разрешить или отклонить -- и решает это не [планировщик](scheduler.md) (его дело -- кому дать CPU), а отдельная подсистема: контроль доступа (access control). Вопрос здесь не «когда дать процессор», а «вправе ли процесс это сделать».

Nginx обслуживает HTTP-трафик на порту 80. Для привязки к порту ниже 1024 требуются привилегии. Конфигурационные файлы в `/etc/nginx/` должны быть доступны только администратору и самому Nginx. Логи в `/var/log/nginx/` принадлежат определённому пользователю. Статические файлы сайта в `/var/www/` читаются Nginx, но не модифицируются. Если на той же машине работает PostgreSQL, его [процессы](processes.md) не должны иметь доступа к конфигурации Nginx, а Nginx -- к файлам базы данных. Как ядро обеспечивает все эти границы?

## Идентичность процесса

Каждый процесс в Linux имеет владельца. Дескриптор процесса [[linux/foundations/processes#Дескриптор процесса: task_struct|task_struct]] хранит UID и GID. Но идентичность процесса устроена сложнее -- у него не одна пара UID/GID (User ID / Group ID), а несколько.
...
**Supplementary groups** (дополнительные группы) -- пользователь может входить в несколько групп одновременно. У каждого процесса есть одна основная группа (primary GID -- та, что хранится в GID процесса), но пользователь `deploy` входит ещё и в группы `www-data` и `docker`. Процесс, запущенный от `deploy`, наследует все эти группы. При проверке доступа ядро сравнивает GID файла со всеми группами процесса, а не только с primary GID.
...
[Inode](filesystems.md) хранит три набора rwx-битов: для владельца (owner), группы (group) и остальных (others). Когда процесс вызывает [`open()`](file-descriptors.md), ядро выполняет проверку, сравнивая идентичность процесса с метаданными inode.
...
**`CAP_NET_RAW`** -- создание raw-сокетов (прямое формирование сетевых пакетов в обход обычного TCP/UDP-стека). Нужна утилите `ping` для отправки ICMP-пакетов (Internet Control Message Protocol).
...
Одного набора capabilities недостаточно: процесс может иметь разрешённые capabilities, но не активировать их все сразу -- это снижает поверхность атаки. Три набора дают точный контроль: что разрешено в принципе, что активно прямо сейчас, и что можно передать дочернему процессу. У каждого потока эти три набора хранятся в `task_struct`:
...
[[linux/containers/containers#Capabilities: дробление root-привилегий|Контейнеры]] активно используют capabilities для ограничения привилегий -- Docker по умолчанию даёт контейнеру лишь часть из ~40 capabilities, остальные сбрасывает.

## Сброс привилегий: паттерн безопасного демона

Capabilities появились относительно поздно. До них -- и во многих системах до сих пор -- использовался другой паттерн: запуск с полными привилегиями и последующий сброс до минимума. Nginx применяет именно его.
...
Почему нельзя просто вернуть EUID 0 обратно? Для этого нужно понять, что у процесса не два UID (RUID и EUID), а три. Saved UID (SUID) -- третий идентификатор, который ядро сохраняет при смене EUID. Он нужен именно для того, чтобы процесс мог временно понизить привилегии и вернуться. Вызов `seteuid()` меняет только effective UID, не трогая saved UID: процесс с правами root вызывает `seteuid(unprivileged)`, выполняет работу, затем вызывает `seteuid(saved_uid)` и восстанавливает прежний EUID из сохранённого значения. Но это работает только когда SUID содержит привилегированное значение.
...
В работающей системе все механизмы действуют одновременно, и каждый запрос проходит через них как через цепочку границ.
