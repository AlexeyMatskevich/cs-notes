...
Каждый процесс в Linux имеет владельца. В [заметке о процессах](processes.md) мы видели, что [[linux/foundations/processes#дескриптор-процесса-task_struct|task_struct]] хранит UID и GID. На самом деле идентичность процесса устроена сложнее -- у него не одна пара UID/GID (User ID / Group ID), а несколько.
...
В [заметке о файловых системах](filesystems.md) мы видели, что inode хранит три набора rwx-битов: для владельца (owner), группы (group) и остальных (others). Когда процесс вызывает [`open()`](file-descriptors.md), ядро выполняет проверку, сравнивая идентичность процесса с метаданными inode.
