...
> [!info]- Предпосылки
> [файловые дескрипторы](file-descriptors.md) (fd, inode), [процессы](processes.md) (fork, copy-on-write на уровне эффекта), [иерархия памяти](../../computer/data-path/memory-hierarchy.md) (cache line, TLB), [когерентность кешей](../../computer/data-path/cache-coherency.md) (MESI).
...
Сначала ядро проверяет, есть ли VMA, покрывающий адрес fault (через red-black дерево ([BST](../../algorithms-and-data-structures/non-linear/binary-search-tree.md), самобалансирующийся вариант) VMA процесса — поиск за O(log n)). Если VMA нет — illegal access, SIGSEGV.
