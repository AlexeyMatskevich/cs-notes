...
Диагностика: заглянуть в `/proc/<pid>/fd`. `/proc` — виртуальная файловая система, через которую ядро показывает информацию о процессах (устройство `/proc` описано в разделе [Наблюдаемость: /proc и /sys](../kernel/devices-and-drivers.md#наблюдаемость-proc-и-sys)). Каждый открытый дескриптор отображается как символическая ссылка (файл-указатель, хранящий путь к другому файлу) на ресурс:
