...
← [Потоки](threads.md) | [Виртуальная память](virtual-memory/virtual-memory.md) →
...
> [!info]- Задача: куда пишут оба процесса?
> Родитель открыл лог-файл, сделал `fork()`, после чего ребёнок записал 15 байт через `write(fd, ...)`. С какой позиции начнётся следующий `write()` родителя — с 0 или с 15?
>
> **Интуитивная догадка:** fd — это просто номер, у каждого процесса своя таблица дескрипторов, значит offset тоже свой; родитель запишет с позиции 0 и затрёт сообщение ребёнка.
>
> **Что происходит на самом деле:** offset лежит не в fd, а в open file description — structure, **разделяемой** между родителем и потомком после fork. Запись ребёнка сдвинула общий offset на 15, и родитель запишет с позиции 15. Строки чередуются, но не затирают друг друга.
...
`close(fd)` удаляет запись из таблицы дескрипторов процесса и уменьшает refcount в open file description. Когда refcount падает до нуля, ядро освобождает description. У inode две независимые группы ссылок: открытые fd через open file description и записи директории — имена файла в директориях (отдельная ссылка на inode, не связанная с тем, открыт ли файл хоть кем-то). Пока есть хоть одна ссылка любой из групп, ресурс жив; когда обе обнуляются — освобождается и сам ресурс (двойной счётчик и жизненный цикл файла подробнее — в [[linux/foundations/filesystems#Удаление файла: link count и open count|файловых системах]]). Для pipe это означает, что другой конец получает EOF при следующем `read()`; для TCP-[сокета](../programming/sockets.md) — ядро отправляет FIN, сигнализируя об окончании передачи.
...
Но процессы работают не только с дескрипторами — они работают с памятью. Каждый процесс «видит» адреса начиная с 0x400000, и два процесса могут использовать один и тот же адрес без конфликта. Как это возможно — вопрос [виртуальной памяти](virtual-memory/virtual-memory.md).
