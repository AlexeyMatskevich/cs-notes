...
Программа пишет лог в файл, принимает сетевое соединение, читает нажатия клавиш из терминала — и для всех этих несхожих задач вызывает одни и те же `read()` и `write()`, передавая в них одно целое число. За этим числом ядро прячет, что стоит по ту сторону: диск, сетевая карта или клавиатура. Это число — файловый дескриптор (file descriptor, fd): через него процесс обращается к любому внешнему ресурсу одинаково.
...
**(b) Open file description (открытое файловое описание)** — системная структура, общая для всего ядра. Хранит текущее смещение (offset), режим открытия (чтение/запись), флаги (`O_APPEND` — дозапись в конец) и счётчик ссылок. Одно open file description может быть разделено между несколькими fd — и это ключ к пониманию fork() и dup().
...
Номер 1 остался прежним — изменилась запись в таблице дескрипторов, на которую он указывает: теперь это открытое описание файла `output.txt`, а не терминала. Программа адресует свои данные числу 1, и пока за этим числом стоит файл, байты идут в файл. Любая программа, пишущая в stdout, теперь пишет в `output.txt`, не подозревая об этом.
...
if (pid == 0) {
    ...
    dup2(fd, STDOUT_FILENO);  // STDOUT_FILENO — именованная константа для fd=1; теперь fd=1 -> files.txt
