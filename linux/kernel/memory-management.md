...
Одиннадцать чисел — количество свободных блоков каждого порядка от 0 до 10. На нашем сервере после нескольких часов работы блоков order 9-10 (2048 и 4096 страниц, нужных для непрерывных аллокаций) может не быть вовсе — физическая память фрагментирована. Это одна из причин, по которой huge pages (2 МБ = order 9) [резервируют заранее](../programming/memory-management.md), при загрузке системы, пока RAM не фрагментирована.

Buddy allocator быстр — выделение и освобождение за O(1) в типичном случае. Но у него есть фундаментальная проблема: минимальная единица выдачи — одна страница (4 КБ). Ядро постоянно создаёт объекты значительно меньшего размера: [[linux/foundations/processes#дескриптор-процесса-task_struct|`struct task_struct`]] (дескриптор процесса, ~8 КБ), [[linux/foundations/filesystems#inode-метаданные-файла|`struct inode`]] (метаданные файла, ~600 байт), `struct sk_buff` (~240 байт), `struct dentry` (~192 байта). Выделять 4 КБ страницу под 192-байтовый dentry — расход в 20 раз.
...
На нашем сервере ZONE_NORMAL содержит ~60 ГБ — это рабочая лошадка. ZONE_DMA32 — ~4 ГБ, резерв для устройств с 32-битной адресацией. ZONE_DMA — 16 МБ, наследие [ISA-шины](../../computer/data-path/buses-and-dma.md), практически не используется на современном оборудовании.
...
Ядро поддерживает два [LRU](../../algorithms-and-data-structures/linear/lru-cache.md)-списка (Least Recently Used) для каждой зоны: **active** и **inactive**. Страницы, к которым часто обращаются, попадают в active list. Страницы, к которым давно не обращались, — в inactive list. Кандидаты на вытеснение выбираются из хвоста inactive list.
...
Пока свободная память выше `pages_high`, процессы получают фреймы мгновенно из buddy allocator. Когда свободные страницы в зоне падают ниже `pages_low`, ядро будит `kswapd` — один фоновый поток на каждый [NUMA](../../computer/data-path/ram.md)-узел (NUMA, Non-Uniform Memory Access) (`kswapd0`, `kswapd1`, ...).
...
Запись грязных страниц выполняется flusher threads, которые работают как обычные потоки ядра и могут быть спланированы на любой CPU. Запрос на I/O уходит через слой блочного ввода-вывода: [softirq](interrupts.md) `BLOCK_SOFTIRQ` обрабатывает завершение дисковых операций. Когда диск подтверждает запись, страница помечается как чистая и становится кандидатом на освобождение.
...
OOM killer вызывается из контекста direct reclaim: функция `out_of_memory()` в `mm/oom_kill.c`. Ядро перебирает все [процессы](../foundations/processes.md) и для каждого вычисляет `oom_score` — эвристику от 0 до ~1000, где основной вклад делает RSS (Resident Set Size): процесс, занимающий 50% RAM, получает score ~500. Администратор корректирует через `oom_score_adj` (от -1000 до +1000) в `/proc/<pid>/oom_score_adj`. Процесс с максимальным итоговым score получает `SIGKILL`. Логика проста: убить того, кто освободит больше всего памяти одним действием.
...
В контейнерной среде OOM изолирован через memory cgroups: когда pod в Kubernetes превышает `memory.max`, OOM kill срабатывает *внутри* cgroup — убивается [процесс](../foundations/processes.md) в этом pod, а не произвольный [процесс](../foundations/processes.md) на ноде. Подробнее — в [управлении памятью (userspace)](../programming/memory-management.md).
