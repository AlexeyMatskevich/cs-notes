...
В контейнерной среде OOM изолирован через memory cgroups: когда pod в Kubernetes превышает `memory.max`, OOM kill срабатывает *внутри* cgroup — убивается процесс в этом pod, а не произвольный процесс на ноде. Подробнее — в [управлении памятью (userspace)](../programming/memory-management.md).
