...
**Предпосылки:** [режимы CPU и системные вызовы](../foundations/01-cpu-modes-and-syscalls.md) (ring 0/3, syscall снаружи, LSTAR, vDSO), [процессы](../foundations/02-processes.md) (task_struct, kernel stack).

← [Межпроцессное взаимодействие](../programming/06-ipc.md) | [Прерывания](01-interrupts.md) →

В предыдущей заметке мы видели системный вызов снаружи: программа кладёт номер в `rax`, выполняет `syscall`, получает результат. Процессор прыгает на адрес из `LSTAR`, а дальше — «ядро что-то делает». Разберём, что именно происходит внутри ядра на пути от инструкции `syscall` до возврата в пользовательский код. Сценарий — вызов `read(fd=3, buf, 4096)`: номер 0 в `rax`, три аргумента в регистрах, один проход через весь механизм.
...
Весь путь — от инструкции `syscall` до `sysret` — без учёта времени работы самого обработчика занимает 50-100 наносекунд на современных процессорах. Из чего складывается эта цена: `swapgs` и сохранение/восстановление ~15 регистров (запись/чтение на стеке ядра), переключение стека (две записи в память), проверка номера вызова и индексация в таблице, проверка флагов при выходе, pipeline flush при смене уровня привилегий. Для сравнения: обычный вызов функции через `call`/`ret` стоит 1-5 наносекунд — системный вызов в 20-50 раз дороже даже без учёта самой работы.

На машинах с включённой защитой KPTI (Kernel Page Table Isolation, заплатка от Meltdown) стоимость возрастает. Уязвимость Meltdown (2018) позволяла пользовательскому коду читать память ядра через спекулятивное выполнение: процессор начинал выполнять инструкцию чтения привилегированной памяти, откатывал её при проверке прав, но данные оставались в кеше — и через side-channel атаку их можно было извлечь.

KPTI решает проблему радикально: вместо одной таблицы страниц на процесс ядро поддерживает две. Полная таблица содержит маппинги и пользовательской памяти, и памяти ядра — она используется, когда процессор выполняет код ядра. Урезанная таблица содержит только маппинги пользовательской памяти и минимальный набор trampoline-страниц для обработки входа в ядро — она используется, когда процессор выполняет пользовательский код.
...
- Robert Love, 2010, *Linux Kernel Development* — Chapter 5: System Calls
- Linux kernel source: `arch/x86/entry/entry_64.S`, `arch/x86/entry/syscall_64.c`
