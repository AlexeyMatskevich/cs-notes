...
> [!info]- Предпосылки
> [прерывания](interrupts.md) (NAPI, softirq, top/bottom half), [устройства и драйверы](devices-and-drivers.md) (file_operations, bus model, DMA), [сокеты](../programming/sockets.md) (TCP server, receive buffer, send buffer).
...
Когда кадр приходит на NIC, карта берёт дескриптор по указателю head, записывает данные кадра по физическому адресу из дескриптора через DMA, обновляет поле length и выставляет флаг «готово» в status. Затем сдвигает head на следующий дескриптор. Если head догоняет tail — кольцо заполнено, и NIC начинает отбрасывать пакеты (rx_dropped в `/proc/net/dev`). На загруженном сервере это сигнал, что либо буфер слишком мал, либо ядро не успевает забирать пакеты. Размер буфера настраивается через `ethtool -G eth0 rx 4096`.
...
Каждый сетевой пакет внутри ядра представлен структурой `sk_buff` (socket buffer, `struct sk_buff` в `include/linux/skbuff.h`). Это не просто блок данных — это метаданные пакета плюс указатели на его содержимое. Структура весит около 240 байт и выделяется из [slab-кеша](memory-management.md) (пул заранее нарезанных объектов одного размера — ядро держит такие объекты горячими) для каждого входящего пакета.
...
L2, L3, L4 — канальный, сетевой и транспортный уровни [сетевой модели](../../networking/infrastructure/reference-models.md): Ethernet-заголовок, IP-заголовок, TCP/UDP-заголовок соответственно.
...
Пакет, пришедший извне и предназначенный локальному [процессу](../foundations/processes.md), проходит цепочки PREROUTING и INPUT. Пакет, проходящий транзитом через машину (routing), проходит PREROUTING, FORWARD, POSTROUTING. Пакет, отправленный локальным [процессом](../foundations/processes.md), — OUTPUT и POSTROUTING.
...
[Процесс](../foundations/processes.md), вызвавший `read()` или `epoll_wait()` на этом сокете, спит в очереди ожидания сокета (socket wait queue). Когда TCP помещает данные в receive buffer, он вызывает `sk_data_ready()` — callback, который будит [процесс](../foundations/processes.md).

Если [процесс](../foundations/processes.md) использует `epoll`, механизм работает так: при регистрации fd через `epoll_ctl()` ядро установило callback в wait queue сокета. Когда `sk_data_ready()` срабатывает, callback перемещает этот fd в ready list структуры epoll. [Процесс](../foundations/processes.md), спящий в `epoll_wait()`, просыпается и получает список готовых fd. Далее процесс вызывает `read()` или `recv()` — системный вызов копирует данные из receive buffer ядра в пользовательский буфер (`copy_to_user()`). Это единственное копирование на всём пути (не считая DMA-записи): от ring buffer через IP, netfilter и TCP данные передаются через указатели в `sk_buff`, без копирования. Копирование в userspace обходится в ~200-500 нс для типичного пакета 1500 байт и ограничивается пропускной способностью кеша L1/L2.
