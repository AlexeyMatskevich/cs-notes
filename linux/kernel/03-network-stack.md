...
Каждый сетевой пакет внутри ядра представлен структурой `sk_buff` (socket buffer, `struct sk_buff` в `include/linux/skbuff.h`). Это не просто блок данных — это метаданные пакета плюс указатели на его содержимое. Структура весит около 240 байт и выделяется из [slab-кеша](04-memory-management.md) (пул заранее нарезанных объектов одного размера — ядро держит такие объекты горячими) для каждого входящего пакета.
...
L2, L3, L4 — канальный, сетевой и транспортный уровни [сетевой модели](../../networking/infrastructure/00-reference-models.md): Ethernet-заголовок, IP-заголовок, TCP/UDP-заголовок соответственно.
