...
После помещения данных в receive buffer TCP отправляет ACK-сегмент. Для экономии ядро откладывает ACK (delayed ACK), надеясь совместить его с данными ответа — один сегмент вместо двух. Минимальная задержка определяется константой `TCP_DELACK_MIN` = `HZ/25`: при HZ=250 (типичная конфигурация) это 10 мс, при HZ=1000 — 40 мс. Максимальная задержка `TCP_DELACK_MAX` = `HZ/5` = 200 мс. Фактическая задержка зависит от внутренней логики TCP: quickack mode после получения данных вне окна, число принятых сегментов, наличие данных для отправки. Опция сокета `TCP_QUICKACK` отключает delayed ACK, заставляя ядро отправлять подтверждения немедленно. Если за время таймера процесс вызвал `write()` на том же сокете, ACK уходит вместе с данными (piggybacking — совмещение подтверждения с полезной нагрузкой). Если нет — отправляется чистый ACK без данных.
