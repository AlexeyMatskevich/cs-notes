...
Маршрут работает, пока обработчик укладывается в единицы микросекунд. Стоит обработчику затянуться — и устройство начинает терять данные.
...
Статистику обработки softirq по каждому CPU показывает `/proc/softirqs`. Значения `NET_RX` порядка миллионов на загруженном сервере — нормальная картина. Резкий перекос между CPU обычно означает неверно настроенный RSS (Receive Side Scaling — распределение входящих пакетов картой по своим очередям и через них по ядрам): все очереди карты упираются в одно ядро.
...
Полный путь сетевого пакета — от железа до `recv()`:
...
## Где смотреть прерывания и потери пакетов
