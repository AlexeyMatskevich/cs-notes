...
Прерывания дали способ реагировать на внешние события: NIC (Network Interface Card, сетевая карта) сообщает о пакете, таймер --- об истечении кванта, отмеренного [планировщиком](../foundations/scheduler.md). Но прерывание --- это сигнал, а не интерфейс. Когда [процесс](../foundations/processes.md) вызывает `open("/dev/sda", O_RDONLY)` и затем `read(fd, buf, 4096)`, ядро должно знать, какой код отвечает за это конкретное устройство и какие операции оно поддерживает. За этим стоит цепочка: файловый дескриптор -> inode -> таблица функций драйвера -> аппаратура. Драйвер --- код, который умеет разговаривать с конкретным оборудованием, и эта цепочка связывает пользовательский `read()` с байтами, возвращёнными устройством.
...
Механизм диспетчеризации устроен так же, как вызов виртуального метода через vtable в C++ --- таблицу указателей на функции: одно и то же имя операции ведёт к разному коду в зависимости от того, на каком объекте её вызвали. Вызов `read(fd, buf, 4096)` из пространства пользователя превращается в системный вызов `sys_read()`. Тот вызывает `vfs_read()` --- функцию [[linux/foundations/filesystems#VFS: единый интерфейс для разных файловых систем|VFS]] (Virtual File System, слой ядра, направляющий файловые вызовы к нужной реализации), которая по fd находит `struct file` и извлекает `file_operations`. Вызов `f->f_op->read(f, buf, 4096, &pos)` передаёт управление конкретному драйверу. Если fd указывает на `/dev/sda`, вызовется `read()` SCSI-драйвера; если на `/dev/tty` --- `read()` драйвера терминала. Пользовательский код не знает и не должен знать, какой драйвер стоит за дескриптором.
...
> [!info]- udevd как часть systemd
> На большинстве дистрибутивов (Debian, Ubuntu, Fedora) `udevd` --- это `systemd-udevd`, часть systemd. Правила пользователь кладёт в `/etc/udev/rules.d/`, правила, поставляемые пакетами, --- в `/lib/udev/rules.d/`. После добавления или изменения файла правила перечитываются командой `udevadm control --reload`, а применить их к уже подключённым устройствам можно через `udevadm trigger`.
...
Когда совпадение найдено, ядро вызывает функцию `probe()` драйвера, передавая ей ссылку на устройство. В `probe()` драйвер инициализирует оборудование: настраивает [[computer/data-path/buses-and-dma#Четыре шага DMA-передачи|DMA-буферы]] (DMA, Direct Memory Access), регистрирует обработчики прерываний, создаёт очереди ввода-вывода. Регистрируя устройство, драйвер связывает его с таблицей операций (для символьных --- `file_operations`, для блочных --- `block_device_operations`) --- та самая регистрация, по которой позже `open()` находит код драйвера. Если инициализация успешна, устройство готово к работе. При отключении устройства (или выгрузке драйвера) ядро вызывает `remove()`.
...
> [!info]- Где живут модули и как они грузятся при старте
> Скомпилированные модули лежат в `/lib/modules/<версия ядра>/` --- по поддиректории на каждую установленную версию ядра; туда же `depmod` кладёт `modules.dep`. Чтобы модуль загружался при каждой загрузке системы, его имя вписывают в файл в `/etc/modules-load.d/` (например, `/etc/modules-load.d/storage.conf` со строками `nvme` и `ext4`); параметры модуля задаются в `/etc/modprobe.d/`.
>
> Автозагрузка по событию udev работает через `modalias` --- строку, которую ядро строит из идентификаторов устройства (vendor/device ID) и кладёт в uevent. `systemd-udevd` передаёт эту строку `modprobe`, а тот по таблице алиасов в `modules.alias` находит модуль, объявивший поддержку такого устройства. Поэтому драйвер для только что воткнутого USB-устройства подгружается сам, без участия пользователя.
...
Современный блочный уровень построен на архитектуре **blk-mq (multi-queue block layer)**. Каждому [[computer/cpu|CPU]] назначается отдельная очередь отправки (software queue), а устройство имеет одну или несколько аппаратных очередей (hardware queues). Раньше все ядра проталкивали запросы через одну общую очередь и боролись за единственную блокировку, защищающую её от одновременного изменения; на быстром устройстве эта борьба сама становилась узким местом. NVMe SSD может поддерживать десятки аппаратных очередей --- например, по одной на каждое ядро CPU, --- и каждое ядро работает со своей очередью, не пересекаясь с остальными.
...
**mq-deadline** разделяет запросы на две очереди --- чтения и записи --- и гарантирует, что ни один запрос не ждёт дольше установленного дедлайна: 500 мс для чтения, 5 секунд для записи по умолчанию. Чтения приоритетнее: пользователь ждёт данных синхронно, а запись часто оседает в [[linux/foundations/filesystems#Page cache: файловые данные в оперативной памяти|page cache]] и попадает на диск позже, не блокируя процесс. Внутри каждой очереди запросы сортируются по номеру сектора (elevator), что минимизирует перемещение головки HDD. Хороший выбор для серверов с HDD, где важна предсказуемость задержек.
...
`/proc` появился в ранних версиях Unix для информации о процессах, и со временем в него добавляли всё подряд --- от состояния памяти до параметров SCSI-устройств, пока `/proc` не превратился в неструктурированную свалку. В ответ на это появилась **sysfs** --- виртуальная файловая система, смонтированная в `/sys/`, с чёткой иерархией, отражающей модель устройств ядра.
