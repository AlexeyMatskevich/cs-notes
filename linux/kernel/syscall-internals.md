...
**Предпосылки:** [режимы CPU и системные вызовы](../foundations/cpu-modes-and-syscalls.md) (ring 0/3, syscall снаружи, LSTAR, vDSO), [процессы](../foundations/processes.md) (task_struct, kernel stack), [планировщик](../foundations/scheduler.md) (квант времени, вытеснение).

← [Межпроцессное взаимодействие](../programming/ipc.md) | [Прерывания](interrupts.md) →

IPC, сокеты, mmap — всё это API пользовательского пространства. Каждый из них в итоге вызывает syscall: `pipe()`, `sendto()`, `mmap()` кладут номер в `rax` и передают управление ядру. Снаружи виден только контур: программа кладёт номер вызова в `rax`, выполняет инструкцию `syscall`, получает результат. Что происходит между инструкцией `syscall` и `sysret`, остаётся за аппаратной границей.

А за этой границей управление сменило привилегии, ядро переключило стек, а на машинах с защитой от Meltdown — ещё и таблицу страниц ([[#KPTI: двойные таблицы страниц|KPTI]]). Базовый путь стоит 50–100 нс без учёта работы самого обработчика, KPTI добавляет ещё столько же. Этот же путь перехватывают инструменты трассировки (strace, ptrace), и через него проходит каждый запрос программы к ядру — поэтому его внутреннее устройство определяет и цену любого ввода-вывода, и то, что́ вообще можно проконтролировать на входе в ядро. Дальше — разбор одного вызова `read(fd=3, buf, 4096)` от инструкции `syscall` до возврата в пользовательский код.
...
После переключения стека `entry_SYSCALL_64` сохраняет все пользовательские регистры на стеке ядра в структуру `pt_regs` (определена в `arch/x86/include/asm/ptrace.h`). Это снимок состояния процессора (snapshot) на момент входа в ядро: `rax` (номер вызова), `rdi`, `rsi`, `rdx`, `r10`, `r8`, `r9` (аргументы — [[computer/programmer-model/abi-and-data-layout#Соглашение о вызовах: System V AMD64 ABI|Linux syscall convention]], отличается от обычной конвенции вызовов тем, что 4-й аргумент передаётся в `r10` вместо `rcx` — потому что `syscall` использует `rcx` для адреса возврата), `rcx` (адрес возврата, сохранён процессором), `r11` (флаги, сохранены процессором), `rsp` (пользовательский стек) и оставшиеся регистры общего назначения.
...
`ksys_read(fd=3, buf, count=4096)` начинает работу с проверки файлового дескриптора. Ядро обращается к таблице файловых дескрипторов текущего процесса (`task_struct->files`), берёт элемент с индексом 3 и получает структуру [[linux/foundations/file-descriptors#Трёхуровневая таблица|struct file]] — она описывает открытый ресурс: текущую позицию чтения (`f_pos`), флаги (`f_flags`), права доступа и, главное, указатель на `struct file_operations`.
...
Этот механизм — полиморфизм через таблицу функций. Один и тот же `read()` обрабатывает файлы, сокеты, пайпы, устройства — код `ksys_read` не меняется, меняется только `fops`, привязанная к конкретному дескриптору. Это тот же паттерн, что и vtable в C++ (таблица функций объекта, через которую вызов попадает в реализацию конкретного типа), только собранный вручную через указатели на функции в структуре.

Для `read(fd=3)` на обычном файле ext4 путь продолжается: `ext4_file_read_iter()` проверяет, есть ли запрошенные данные в [[linux/foundations/filesystems#Page cache: файловые данные в оперативной памяти|page cache]] (кеш страниц файла в оперативной памяти). Если да — данные копируются из page cache в пользовательский буфер через `copy_to_user()`. Если нет — ядро планирует чтение с диска, [процесс](../foundations/processes.md) засыпает (`TASK_UNINTERRUPTIBLE` — нельзя прервать, пока I/O не завершится), и [планировщик](../foundations/scheduler.md) переключает процессор на другую задачу. Когда данные поступят с диска, процесс проснётся и завершит копирование.
...
Перед возвратом в пользовательский код ядро проверяет два флага в `thread_info` текущего процесса. `thread_info` — служебная часть `task_struct`, хранящая флаги состояния текущего потока; проверка одного слова в памяти дешевле, чем обращение ко всей структуре процесса.

Первый — `TIF_SIGPENDING`: есть ли необработанные [[linux/programming/signals#Доставка сигнала: что происходит внутри|сигналы]]. Если процессу отправили `SIGTERM`, пока он был внутри `read()`, обработка произойдёт именно сейчас, на выходе из системного вызова. Ядро не прерывает обработчик системного вызова для доставки сигнала — оно дожидается естественной точки выхода. Вот почему `kill -9` не действует мгновенно на процесс в uninterruptible sleep (`D` state): в этом состоянии процесс не пробуждается сигналами вообще — ядро намеренно не доставляет их до завершения I/O.

Второй — `TIF_NEED_RESCHED`: нужно ли переключить процессор на другую задачу. [Планировщик](../foundations/scheduler.md) устанавливает этот флаг, когда [процесс](../foundations/processes.md) исчерпал свой квант времени или когда проснулся процесс с более высоким приоритетом. Аппаратные прерывания (например, таймерный тик) прерывают пользовательский код между любыми двумя инструкциями, но решение о переключении задачи принимается не в момент прерывания, а в точках вытеснения: возврат из syscall и возврат из прерывания в userspace. Проверка `TIF_NEED_RESCHED` на выходе из системного вызова — одна из таких точек; другая — `ret_from_intr`, выполняемая при возврате из аппаратного прерывания.
...
После проверок ядро восстанавливает регистры из `pt_regs`, загружает пользовательский `rsp` и выполняет `sysret`. Процессор устанавливает CPL = 3, загружает `rip` из `rcx` (адрес инструкции после `syscall` в пользовательском коде — именно его процессор сохранил туда при входе) и восстанавливает `RFLAGS` из `r11` (флаги состояния процессора, тоже сохранённые при входе). Программа продолжает выполнение, видя результат в `rax`.

Инструкция `sysret` быстрее альтернативы `iret` (которая используется для возврата из прерываний), но требует корректных значений: `rcx` должен содержать канонический пользовательский адрес возврата, `r11` — флаги. Если пользовательский `rip` оказывается в неканоническом адресном пространстве (адреса, не принадлежащие ни пользователю, ни ядру), `sysret` генерирует General Protection Fault уже в кольце 0, но с пользовательским `rsp` — это создавало уязвимость на ранних ядрах. Современное ядро проверяет `rcx` перед `sysret` и в сомнительных случаях использует более безопасный, но медленный путь через `iret`.
...
### KPTI: двойные таблицы страниц

На машинах с включённой защитой KPTI (Kernel Page Table Isolation) стоимость возрастает. KPTI — заплатка от Meltdown: без неё процессор мог начать выполнять инструкцию чтения памяти ядра, откатить её при проверке прав, но данные при этом оседали в кеше и были извлекаемы через измерение времени доступа. KPTI решает проблему радикально: вместо одной таблицы страниц на процесс ядро поддерживает две. Полная таблица содержит маппинги и пользовательской памяти, и памяти ядра — она используется, когда процессор выполняет код ядра. Урезанная таблица содержит только маппинги пользовательской памяти и минимальный набор trampoline-страниц (страниц-трамплинов для обработки входа в ядро) — она используется, когда процессор выполняет пользовательский код.
...
[[linux/foundations/cpu-modes-and-syscalls#vDSO: системный вызов без системного вызова|vDSO]] полностью исключает переход в ядро для таких случаев: ядро отображает в адресное пространство каждого процесса страницу с готовым кодом и timekeeping-данными, которые userspace-реализация `clock_gettime()` читает напрямую — без `syscall`, без переключения привилегий, без переключения стека. Вызов занимает ~20-50 нс вместо 100-200.
