...
Прерывания дали способ реагировать на внешние события: NIC сообщает о пакете, таймер — о кванте планировщика. Но прерывание — это сигнал, а не интерфейс. Когда процесс вызывает `open("/dev/sda", O_RDONLY)` и затем `read(fd, buf, 4096)`, ядро должно знать, какой код отвечает за это конкретное устройство и какие операции оно поддерживает. За этим стоит цепочка: файловый дескриптор -> inode -> таблица функций драйвера -> аппаратура. Драйвер — код, который умеет разговаривать с конкретным оборудованием. Далее — механизмы, которые связывают пользовательский `read()` с байтами, возвращёнными устройством.

## Устройство — это таблица функций

Семь ключевых компонентов работают вместе. Устройство предоставляет таблицу функций (`file_operations`). Системные вызовы `open()`, `read()`, `write()` диспетчеризуются через неё — как через vtable в C++. Демон `udev` автоматически создаёт файлы в `/dev/`. Модули ядра загружаются на лету. `/proc` и `/sys` — виртуальные файловые системы для наблюдения за ядром и оборудованием. I/O scheduler (планировщик ввода-вывода) переупорядочивает запросы к диску.

Сначала — два фундаментально разных типа устройств, потом — как ядро маршрутизирует вызовы к нужному драйверу.
...
`/dev/` смонтирован как `devtmpfs` — файловая система в RAM. При загрузке системы ядро само создаёт минимальный набор устройств в devtmpfs (`/dev/null`, `/dev/zero`, `/dev/console`), а `udevd` подхватывает управление после старта и обрабатывает все остальные устройства.
...
Устройства не существуют сами по себе — они подключены к шинам. Модель устройств ядра Linux (Linux Device Model) отражает эту физическую реальность тремя абстракциями: **шина (bus)** — канал связи (PCI, USB, I2C), **устройство (device)** — оборудование на шине, **драйвер (driver)** — код, управляющий устройством.

При загрузке системы ядро перечисляет (enumerate) устройства на каждой шине. PCI-шина — наиболее наглядный пример. Пространство конфигурации PCI (PCI configuration space) — 256 байт на каждое устройство — содержит vendor ID и device ID. Ядро читает эти идентификаторы и ищет драйвер, который объявил поддержку данной пары. NVMe SSD Samsung 980 PRO имеет vendor ID `0x144d` (Samsung), device ID `0xa80a`. Драйвер NVMe в ядре регистрирует таблицу поддерживаемых ID, и ядро находит совпадение.
...
Результат перечисления виден в `/sys/bus/pci/devices/`. Каждое устройство представлено директорией с именем в формате `домен:шина:слот.функция` — например, `0000:03:00.0`. Внутри — файлы с атрибутами: `vendor`, `device`, `driver` (символическая ссылка на драйвер), `resource` (MMIO-регионы).
...
Модуль — это скомпилированный объектный файл в формате ELF с двумя обязательными функциями. `module_init()` вызывается при загрузке модуля: здесь драйвер регистрирует себя в подсистеме (PCI, USB, блочных устройств). `module_exit()` вызывается при выгрузке: драйвер снимает регистрацию и освобождает ресурсы. Между этими двумя вызовами драйвер живёт в адресном пространстве ядра и имеет полный доступ ко всем структурам ядра — никакой изоляции между модулями нет. Сбой в модуле роняет всю систему (kernel panic).
...
`/proc/interrupts` — счётчики прерываний по каждому CPU и каждому источнику. Помогает диагностировать аппаратные проблемы: если один CPU обрабатывает в 10 раз больше прерываний от NIC, чем остальные, — interrupt affinity настроена неравномерно.
...
Когда процесс вызывает `read()` на блочном устройстве, запрос не уходит к диску немедленно. Он попадает в очередь, и **I/O scheduler (планировщик ввода-вывода)** решает, в каком порядке запросы будут отправлены на устройство. Для HDD это критично: перемещение головки между дорожками стоит 5-10 мс, и переупорядочивание запросов по номеру сектора (elevator sorting) может сократить суммарное время обслуживания в разы. Для NVMe SSD, где произвольный доступ стоит столько же, сколько последовательный (~70 мкс), переупорядочивание бессмысленно и создаёт лишнюю задержку.
...
Итог всей цепочки: устройства зарегистрированы в модели шина-устройство-драйвер, драйверы предоставляют `file_operations`, udev создаёт файлы в `/dev/`, `/proc` и `/sys` дают наблюдаемость. Когда процесс Nginx вызывает `read(fd, buf, 4096)` для чтения файла с диска, запрос проходит через VFS, файловую систему (ext4), блочный уровень с I/O scheduler, драйвер NVMe, DMA-передачу — и данные оказываются в буфере процесса.

Но что происходит, когда данные приходят не с диска, а из сети? NIC получает Ethernet-фрейм, генерирует прерывание, драйвер копирует данные из DMA-буфера — дальше эти байты должны пройти через стек сетевых протоколов и попасть в сокет приложения. Как именно — в [следующей заметке](03-network-stack.md).

## Sources

- Robert Love, 2010, *Linux Kernel Development* — Chapters 17-18: Devices and Modules
- Jonathan Corbet, Alessandro Rubini, Greg Kroah-Hartman, 2005, *Linux Device Drivers* — 3rd edition
- `cat /proc/devices`, `lsmod`, `cat /sys/block/*/queue/scheduler`
