...
Устройство предоставляет таблицу функций (`file_operations`). Системные вызовы `open()`, `read()`, `write()` диспетчеризуются через неё --- как через vtable в C++. Демон `udev` автоматически создаёт файлы в `/dev/`. Модели шина-устройство-драйвер описывают обнаружение оборудования, а модули ядра загружаются на лету. I/O scheduler переупорядочивает запросы к диску. В конце --- наблюдаемость: виртуальные файловые системы `/proc` и `/sys` для диагностики и настройки.
