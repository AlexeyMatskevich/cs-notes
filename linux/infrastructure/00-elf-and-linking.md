...
Когда процесс вызывает `printf("hello\n")`, код `printf` находится не в самом исполняемом файле, а в разделяемой библиотеке libc.so, которую загрузил динамический линкер. Первый вызов `printf` проходит через два уровня косвенности — PLT и GOT — прежде чем попасть в код libc. Второй вызов обходит линкер и прыгает по адресу напрямую. За этой механикой стоит формат ELF, статическая и динамическая линковка, ленивое связывание и рандомизация адресов.
...
Программа `hello.c` вызывает `printf()`. Код `printf` живёт в libc — библиотеке стандартных функций C. Компилятор создаёт объектный файл `hello.o`, где вызов `printf` — просто незаполненная ссылка (relocation entry). Кто-то должен подставить реальный адрес.
...
Переменная окружения `LD_BIND_NOW=1` (или флаг `-z now` при компиляции) заставляет ld-linux разрешить все символы при загрузке. Все записи GOT заполняются реальными адресами до вызова `main()`. Время старта увеличивается — программа с 500 импортированными функциями потратит ~5-10 мс на разрешение — но после старта каждый вызов стабильно быстр.

Есть ещё одно преимущество: если GOT заполнен при старте, его можно пометить как read-only (`-z relro -z now`, full RELRO). Это закрывает вектор атаки GOT overwrite — атакующий, нашедший уязвимость записи в произвольную память, не сможет подменить адрес в GOT, потому что страница помечена read-only на уровне MMU.
...
- Michael Kerrisk, 2010, *The Linux Programming Interface* — Chapter 41-42: Shared Libraries
- John R. Levine, 1999, *Linkers and Loaders* — Chapter 10: Dynamic Linking and Loading
- `readelf -h`, `readelf -l`, `readelf -S`, `ldd`, `man 8 ld-linux`, `man 3 dlopen`
