...
`execve()` заменяет адресное пространство процесса новой программой ([[linux/foundations/processes#exec(): замена программы|процессы]]). Внутри этой замены [[linux/foundations/what-is-os#Ядро: привилегированная часть ОС|ядро]] разбирает ELF-файл по шагам.
...
Линкер сам находит символы и подставляет адреса. Но что если нужно заменить функцию без перекомпиляции программы — например, подсунуть свою версию `malloc` (подробнее об [[linux/programming/memory-management|аллокаторах памяти]]), чтобы отследить утечки памяти?
