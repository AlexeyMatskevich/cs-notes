...
В [[linux/foundations/processes#exec(): замена программы|процессах]] мы видели, что `execve()` заменяет адресное пространство процесса новой программой. Теперь можно разобрать, что именно делает ядро с ELF-файлом.
...
16 КБ вместо 880 КБ. `ldd` показывает три зависимости: `linux-vdso.so.1` ([[linux/foundations/cpu-modes-and-syscalls#vdso-системный-вызов-без-системного-вызова|vDSO]] — виртуальная библиотека ядра для быстрых syscall), `libc.so.6` (стандартная библиотека C) и `/lib64/ld-linux-x86-64.so.2` — динамический линкер.

Код `.so` отображается через `mmap()` с флагами `MAP_PRIVATE` и `PROT_READ|PROT_EXEC` — механизм тот же, что уже описан в [[linux/foundations/virtual-memory#Demand paging: ленивая загрузка|виртуальной памяти]]. Все [процессы](../foundations/processes.md), отобразившие один и тот же файл libc.so, читают его через одни и те же физические фреймы кода: 400 процессов с `printf` — одна копия libc в RAM (~2 МБ), а не 400 копий (800 МБ).
...
До сих пор мы видели, как линкер сам находит символы и подставляет адреса. Но что если нужно заменить функцию без перекомпиляции программы — например, подсунуть свою версию `malloc`, чтобы отследить утечки памяти?
