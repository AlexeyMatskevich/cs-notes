...
При загрузке ядро отображает код `.so` через `mmap()` с флагами MAP_PRIVATE и PROT_READ|PROT_EXEC. Как описано в [виртуальной памяти](../foundations/virtual-memory.md), несколько процессов, использующих одну библиотеку, разделяют одни и те же физические фреймы кода. Те же 400 процессов, использующих `printf` — одна копия libc.so в RAM (~2 МБ), а не 400 копий (800 МБ).
