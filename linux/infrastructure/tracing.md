...
**Предпосылки:** [процессы](../foundations/processes.md) (адресное пространство, PID, потоки внутри процесса), [потоки](../foundations/threads.md) (поток как единица исполнения с разделяемой памятью процесса), [сигналы](../programming/signals.md) (доставка сигнала процессу, SIGTRAP), [прерывания](../kernel/interrupts.md) (прерывание останавливает исполнение и передаёт управление ядру), [режимы CPU и системные вызовы](../foundations/cpu-modes-and-syscalls.md) (syscall), [ELF и линковка](elf-and-linking.md) (PLT/GOT, символы), [процессор](../../computer/cpu.md) (конвейер, IPC, суперскалярное исполнение), [иерархия памяти](../../computer/data-path/memory-hierarchy.md) (кеш-иерархия, промахи кеша, задержки).

← [Терминалы](terminals.md) | [Загрузка системы](boot.md) →

Production API отвечает за 50 мс, но после очередного деплоя p99 скакнул до 500 мс. Логи молчат — код не изменился, ошибки не выросли. Проблема где-то между приложением и ядром, но где именно? Нужен способ наблюдать за работающим [процессом](../foundations/processes.md), не модифицируя ни байта его кода.
...
Каждая остановка — ptrace-stop: ядро замораживает целевой [[linux/foundations/processes|процесс]] и уведомляет strace через `waitpid()`. Статус остановки обозначается как [[linux/programming/signals|SIGTRAP]] — сигнал-ловушка, который ядро доставляет трассировщику при каждом syscall-stop и breakpoint. На каждом вызове strace читает аргументы (`PTRACE_GETREGS`), форматирует вывод, копирует строковые буферы из памяти процесса — всё это сверх самих переключений контекста. В итоге накладные расходы составляют порядка 10-100x замедления (Brendan Gregg в реальных тестах наблюдал свыше 100x). Кроме того, strace также не видит библиотечные вызовы — только границу с ядром.

Замедление делает strace непригодным для production-диагностики высоконагруженных систем. Снизить оверхед при выборочной трассировке помогает фильтрация вызовов прямо в ядре: strace отдаёт ядру список интересующих syscall, и для остальных целевой процесс не останавливается — это вдвое сокращает количество переходов в ядро.
...
[[computer/cpu#Суперскалярное исполнение|IPC]] = 0.3 — процессор почти всё время простаивает в ожидании данных. 40% промахов кеша подтверждают гипотезу: код ходит по памяти непредсказуемо, промах в [[computer/data-path/memory-hierarchy#Иерархия: от регистров до оперативной памяти|оперативную память]] останавливает конвейер на 80–120 наносекунд, и процессор простаивает.
...
perf нашёл горячие функции, парсинг JSON оптимизирован — но часть запросов всё ещё медленная. perf показывает агрегированную картину — средние значения за период. А нужно понять, почему конкретные запросы медленные. Нужен инструмент, который может измерять задержку каждого отдельного события и строить распределение в реальном времени.
...
Верификатор работает как статический анализатор. Он проходит по каждому возможному пути выполнения программы и проверяет: все циклы имеют гарантированный верхний предел итераций, каждое обращение к памяти проверено на границы, программа завершается за конечное число шагов. Если хотя бы одна проверка не пройдена, ядро отказывается загружать программу.
