...
С этого момента `/` -- настоящая файловая система на диске. Все модули ядра доступны в `/lib/modules/$(uname -r)/`, конфигурация системы -- в `/etc/`. Ядро готово запустить первый [процесс](../foundations/processes.md) userspace.

`pivot_root` используется в другом контексте -- [контейнерах](../containers/containers.md), где корневая ФС контейнера -- не rootfs, а overlay-монтирование, и pivot_root атомарно переключает корень процесса на неё. `chroot` для этого не подходит: он меняет только корневой каталог процесса, не затрагивая точки монтирования -- старая ФС остаётся доступной через `fchdir()` к сохранённому fd.

## systemd: первый процесс

Ядро запускает [процесс](../foundations/processes.md), указанный в `init=` командной строки (по умолчанию `/sbin/init`, который на большинстве дистрибутивов -- символическая ссылка на `/usr/lib/systemd/systemd`). Этот [процесс](../foundations/processes.md) получает PID 1 и становится предком всех остальных [процессов](../foundations/processes.md). Если PID 1 завершается -- ядро вызывает kernel panic: система без init существовать не может.
