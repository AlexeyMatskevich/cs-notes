...
Порт — номер, по которому входящее соединение попадает в нужную программу: на одном адресе номер уникален, и заявить его может только один процесс. Интерфейс — точка подключения к сети, через которую процесс отправляет и принимает пакеты. Клиент A и клиент B оба хотят принимать соединения на порту 80, но порт один на адрес — второму запрос на тот же номер отклонят с ошибкой «адрес занят».

Network namespace создаёт изолированный сетевой стек: собственный набор интерфейсов, собственная таблица маршрутизации, собственные правила фильтрации и NAT (iptables), собственный пул портов. Внутри своего пространства каждый клиент свободно занимает порт 80 — номера больше не сталкиваются, потому что счёт идёт отдельно. Новый network namespace при создании содержит единственный интерфейс — `lo` (loopback, внутренняя петля для связи процесса с самим собой), и тот выключен.

Изолированный стек никуда не подключён, поэтому для связи с внешним миром ядро создаёт **veth pair** (virtual Ethernet pair — пару виртуальных Ethernet-интерфейсов): два конца, между которыми пакет проходит мгновенно, как по виртуальному кабелю. Один конец помещается в network namespace контейнера, другой — в хостовой namespace, обычно подключённый к bridge-интерфейсу (программный коммутатор, который соединяет несколько виртуальных интерфейсов в одну общую сеть).
...
Оба клиента слушают порт 80 внутри своего namespace, но снаружи у сервера один публичный адрес. Развести входящий поток по нужному пространству помогает подмена адреса в пакете на лету (NAT, network address translation). На хосте [[networking/foundations/nat#SNAT и DNAT|правило подмены адреса назначения]] (DNAT, destination NAT) переписывает, куда идёт входящее соединение: `203.0.113.10:8080` → `10.0.1.2:80` (клиент A), `203.0.113.10:8081` → `10.0.1.3:80` (клиент B). Обратный путь симметричен: на ответных пакетах хост переписывает адрес источника (SNAT, source NAT) с внутреннего `10.0.1.x` на публичный `203.0.113.10`, чтобы ответ вернулся отправителю от того адреса, к которому тот обращался.
...
**IPC (Inter-Process Communication) namespace** изолирует каналы, через которые процессы обмениваются данными напрямую в памяти — очереди сообщений, семафоры, разделяемые области памяти. Каждый такой канал адресуется числовым ключом; без изоляции ключ, выбранный одним клиентом, может совпасть с ключом другого, и они влезут в чужой канал.
...
**Time namespace** позволяет [процессу](../foundations/processes.md) видеть собственное значение часов, отсчитывающих время с момента загрузки машины (`CLOCK_MONOTONIC`, `CLOCK_BOOTTIME`). Это нужно при миграции [контейнеров](containers.md) между серверами: контейнер запустился на машине, проработавшей месяц, и переезжает на только что загруженный сервер — без time namespace эти часы скачком отмотались бы назад, ломая таймеры внутри приложения, которые ждут монотонного хода времени.
...
Когда клиент B жалуется, что его сервис не отвечает, проверить причину снаружи не получится: интерфейсы и порты B спрятаны в его network namespace. `nsenter` — аналог `setns()` для командной строки — входит в пространства имён указанного процесса и даёт посмотреть на сеть глазами B:
...
Управляют cgroup не отдельным системным вызовом, а через каталог в специальной [файловой системе](../foundations/filesystems.md) `/sys/fs/cgroup/`. Файлы в нём не лежат на диске: ядро отдаёт их «на лету», и запись в такой файл — это команда ядру. Создать каталог означает создать группу, записать число в файл `memory.max` — выставить лимит, дописать PID в `cgroup.procs` — перенести процесс в группу.

### Контроллер памяти: memory.max и OOM внутри группы

Клиент A запускает конвертацию видео, которая пожирает память. Жёсткий лимит 2 ГБ для его cgroup ограничит аппетит:
...
Клиент A записывает на диск результаты конвертации — непрерывный поток записей. Дисковая подсистема перегружена, запросы клиента B на чтение базы данных ждут в очереди. `io.max` ограничивает I/O по числу IOPS (I/O Operations Per Second) и полосе пропускания, привязывая лимит к конкретному устройству. Устройство адресуется парой чисел `major:minor` — это идентификатор, по которому ядро отличает один диск от другого (`8:0` — первый диск `sda`):
...
В cgroups v2 появился механизм PSI — pressure stall information (информация о простоях из-за нехватки ресурсов). Лимит говорит, сколько ресурса группа может взять, но не показывает, страдает ли она от нехватки уже сейчас — а клиент B может тормозить задолго до того, как сработает OOM. Это и видно через PSI: файлы `cpu.pressure`, `memory.pressure`, `io.pressure` сообщают, какую долю времени [процессы](../foundations/processes.md) в cgroup ожидают ресурс. Значение `some avg10=25.00` в `memory.pressure` означает: за последние 10 секунд 25% времени хотя бы один процесс в cgroup ожидал память. Проверить, какая версия используется:
...
Сама cgroup живёт, пока в ней есть процессы: процесс выбывает из учёта автоматически при завершении, а пустую группу удаляют как каталог — `rmdir`. Удалить группу с процессами внутри ядро не даст, поэтому systemd сносит cgroup сервиса только после того, как опустошит её.
