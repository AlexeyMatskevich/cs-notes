...
io_uring даёт выигрыш, когда узким местом становятся системные вызовы: NVMe-хранилища с задержкой 10-20 мкс на операцию (200 нс syscall overhead = 1-2% потерь при каждой операции), серверы с > 100 000 IOPS, приложения, чувствительные к хвостовым задержкам (tail latency). В бенчмарках (fio, ядро 5.10) io_uring с SQPOLL достигает 400 000 IOPS на NVMe, epoll в аналогичном сценарии — около 200 000 IOPS. Разница объясняется именно устранением syscall overhead и batch-обработкой.
