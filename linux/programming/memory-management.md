...
OOM killer уровня системы — грубый инструмент: он выбирает жертву среди всех [процессов](../foundations/processes.md), и критический сервис может пострадать, даже если проблему создал другой [процесс](../foundations/processes.md). Memory cgroups (control groups) позволяют ограничить потребление памяти группой [процессов](../foundations/processes.md) и изолировать OOM.
...
`memory.max` — жёсткий лимит. Когда суммарный RSS процессов в cgroup достигает лимита, ядро пытается освободить память внутри группы (вытесняет page cache процессов группы, swaps). Если невозможно — OOM kill срабатывает внутри cgroup: убивается [процесс](../foundations/processes.md) из этой группы, а не произвольный системный [процесс](../foundations/processes.md). [Процессы](../foundations/processes.md) вне cgroup не затронуты.
