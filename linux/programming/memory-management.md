...
> [!info]- Предпосылки
> [виртуальная память](../foundations/virtual-memory.md) (страницы, TLB, page fault, demand paging, overcommit, OOM killer), [отображение памяти](memory-mapping.md) (mmap, MAP_ANONYMOUS, madvise), [оперативная память](../../computer/data-path/ram.md) (NUMA).
...
Для буферного пула 128 ГБ с huge pages на 2 МБ: 128 ГБ / 2 МБ = 65 536 страниц вместо 32 миллионов. L1 DTLB обычно содержит отдельные записи для huge pages — порядка нескольких десятков. Тот же L2 TLB (в спецификациях процессоров — STLB, второй уровень TLB) общий для 4 КБ и 2 МБ страниц: порядка полутора тысяч записей, конкретное число зависит от модели процессора. Если все записи L2 TLB отданы под 2 МБ страницы: ~1500 x 2 МБ ≈ 3 ГБ покрытия; на практике часть записей занята 4 КБ страницами, и реальное покрытие — порядка 1–2 ГБ. Даже 1 ГБ покрытия — это 1 ГБ / 128 ГБ = 0.8% буферного пула, что в 170 раз больше, чем 0.0046% при 4 КБ страницах.
...
Поэтому huge pages резервируют заранее, при загрузке системы, пока RAM ещё не фрагментирована. Размер пула задаётся через `/proc` — каталог, где ядро отдаёт свои параметры и счётчики в виде файлов: запись числа в такой файл меняет настройку ядра, чтение возвращает текущее состояние.
...
Запись меньшего числа (`echo 0 > /proc/sys/vm/nr_hugepages`) уменьшает пул и возвращает свободные страницы в общую RAM; занятые процессом страницы освобождаются, когда процесс завершается. Размер пула меняют во время работы, но без фрагментации заново выделить непрерывные блоки уже не гарантируется — поэтому пул задают при загрузке.
...
Три режима: `always` (THP для всех процессов), `madvise` (только для регионов, помеченных `madvise(MADV_HUGEPAGE)`), `never` (отключена обычная автоматическая политика). `never` убирает fault-time и фоновый коллапс в рамках стандартной THP-политики. Это не абсолютный запрет: приложение по-прежнему может явным флагом `madvise(MADV_COLLAPSE)` потребовать собрать huge page для конкретного региона.
...
**Раздувание памяти.** Процесс выделяет 8 КБ — получает 2 МБ huge page. Внутренняя фрагментация: 2 МБ - 8 КБ = 2 040 КБ потрачены впустую. Для базы данных, которая нарезает память собственным аллокатором (компонентом, выдающим мелкие куски под объекты приложения) на множество мелких объектов (Redis — in-memory key-value хранилище — хранит тысячи мелких ключей), THP может увеличить потребление памяти в 2–3 раза.
...
На production-сервере базы данных администратор выставляет `oom_score_adj = -1000` для postmaster, чтобы при нехватке памяти ядро убило менее важный процесс (агент мониторинга, фоновый скрипт) вместо базы. Systemd (менеджер сервисов Linux, запускающий и контролирующий фоновые процессы) поддерживает это через директиву `OOMScoreAdjust=-1000` в описании сервиса.
...
OOM killer уровня системы — грубый инструмент: он выбирает жертву среди всех [процессов](../foundations/processes.md), и критический сервис может пострадать, даже если проблему создал другой. cgroup (control group) — это группа процессов, которой ядро назначает общие лимиты ресурсов. Memory cgroups изолируют OOM внутри группы: `memory.max` задаёт жёсткий лимит, и при его превышении OOM kill срабатывает только внутри этой cgroup. `memory.high` — мягкий лимит: при его превышении ядро замедляет аллокации группы (throttling), не убивая процессы, давая мониторингу время среагировать. Детали механизма — в [namespaces-and-cgroups](../containers/namespaces-and-cgroups.md).
...
`MemFree` и `MemAvailable` — разные числа. `MemFree` — фреймы, вообще не используемые. `MemAvailable` — оценка памяти, которую ядро может освободить без swap: MemFree + reclaimable page cache + reclaimable slab (slab — кеш внутренних объектов ядра, часть которого тоже можно сбросить). На загруженном сервере `MemFree` может быть 500 МБ, а `MemAvailable` — 80 ГБ, потому что 79.5 ГБ page cache можно сбросить.
