...
[[linux/foundations/virtual-memory/tlb#TLB: кеш трансляций|TLB]] типичного серверного процессора (Intel Xeon, AMD EPYC) хранит около 1600 записей для 4 КБ страниц (64 в L1 DTLB (Data TLB) + 1536 в L2 TLB). Каждая запись покрывает 4 КБ — суммарное покрытие TLB составляет ~6 МБ. Из 128 ГБ буферного пула TLB одновременно покрывает 6 МБ / 128 ГБ = 0.000046, то есть 0.0046%. База обращается к буферному пулу почти случайно — индексный поиск и hash join прыгают по всему пулу, а не идут по соседним страницам, поэтому очередное обращение почти всегда попадает на адрес вне покрытых 0.0046%. При таком разбросанном доступе оставшиеся 99.995% обращений к буферному пулу приводят к TLB miss.
...
`MemFree` и `MemAvailable` — разные числа. `MemFree` — фреймы, вообще не используемые. `MemAvailable` — оценка памяти, которую ядро может освободить без swap: MemFree + reclaimable page cache + reclaimable [[linux/kernel/memory-management#Slab/SLUB: аллокатор для мелких объектов|slab]] (кеши ядерных объектов). На загруженном сервере `MemFree` может быть 500 МБ, а `MemAvailable` — 80 ГБ, потому что 79.5 ГБ page cache можно сбросить.
