...
**Предпосылки:** [отображение памяти](01-memory-mapping.md) (mmap, page cache), [файловые системы](../foundations/06-filesystems.md) (page cache, fsync, dirty pages).

← [Отображение памяти](01-memory-mapping.md) | [Сокеты](03-sockets.md) →

Некоторые конфигурации баз данных записывают WAL (write-ahead log) с флагами `O_DIRECT|O_DSYNC` — данные минуют page cache и ложатся прямо на диск. Веб-серверы собирают HTTP-ответ из заголовков и тела вызовом `writev()` — два буфера уходят в сокет за один системный вызов. Оба примера используют расширенные возможности файлового ввода-вывода, которые прячутся за флагами `open()` и специализированными системными вызовами.

Простейший цикл `open() → read()/write() → close()` достаточен для утилит вроде `cat`. Но серверное приложение с тысячами одновременных клиентов, база данных с требованием durability, или логгер, принимающий записи от нескольких процессов, нуждаются в точном контроле: когда данные достигнут диска, как избежать гонок при параллельной записи, как уменьшить число переключений в ядро.
...
Сам по себе `O_NONBLOCK` бесполезен — приложению пришлось бы опрашивать (poll) дескриптор в цикле, тратя CPU впустую. Сила `O_NONBLOCK` раскрывается в связке с мультиплексорами: `epoll` (Linux), `kqueue` (BSD). Приложение регистрирует тысячи неблокирующих дескрипторов в epoll и вызывает `epoll_wait()` — ядро разбудит процесс, только когда на одном из них появятся данные. Так работают Nginx, Redis, Node.js — один поток обслуживает тысячи соединений без создания тысяч потоков.
...
Scatter-gather I/O особенно полезен, когда формат данных содержит заголовок фиксированной длины и тело переменной длины. Вместо чтения в один большой буфер с последующим `memcpy()` для разделения заголовка и тела — `readv()` раскладывает данные по местам сразу.

WAL-запись базы данных — ещё один типичный случай. Каждая запись состоит из заголовка (длина, LSN, тип операции) и тела (изменённые данные). Заголовок формируется в стеке, тело указывает на область в shared buffers. `writev()` позволяет записать и то, и другое одним вызовом без промежуточного копирования в единый буфер:
...
Оба механизма — рекомендательные (advisory): ядро не запрещает процессу читать или писать в заблокированный файл, если процесс не проверяет блокировку. Рекомендательная блокировка — это конвенция между корректно написанными программами, а не принудительная защита. Linux поддерживает и обязательные блокировки (mandatory locking) через монтирование с флагом `mand` и установку setgid-бита без execute, но этот механизм считается устаревшим и удалён из ядра начиная с версии 5.15.
...
Файловый ввод-вывод работает с локальным хранилищем: диски, pipe-каналы, устройства. Все флаги `open()` и вызовы `read()`/`write()`/`pread()`/`writev()` предполагают, что ресурс доступен через файловую систему или создан внутри ядра. Но сервер общается с внешним миром через сеть, а процессы на одной машине могут обмениваться данными эффективнее, чем через pipe. Для этого используется другая абстракция — [сокеты](03-sockets.md), которые тоже возвращают файловый дескриптор и совместимы с `read()`/`write()`, но добавляют сетевую адресацию, протоколы и собственные флаги.

## Sources

- Michael Kerrisk, 2010, *The Linux Programming Interface* — Chapters 4-5, 13: File I/O
- `man 2 open`, `man 2 readv`, `man 2 pread`
