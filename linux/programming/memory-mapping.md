...
struct stat st;          // метаданные файла
fstat(fd, &st);          // st.st_size — размер файла в байтах
...
`MAP_SHARED` связывает запись в память с записью в файл. Изменённые страницы помечаются как «грязные» (dirty) в page cache, и фоновый поток ядра периодически сбрасывает их на диск (writeback, отложенная запись). Момент этого сброса не определён; явный вызов `msync()` записывает грязные страницы немедленно и не возвращает управление, пока они не дойдут до диска.
...
`malloc()` в [[linux/infrastructure/elf-and-linking#Динамическая линковка: разделяемые библиотеки|glibc]] использует `mmap(MAP_ANONYMOUS | MAP_PRIVATE)` для аллокаций свыше 128 КБ (порог `MMAP_THRESHOLD` — начальное значение; glibc динамически поднимает его при обработке освобождаемых блоков). Маленькие аллокации обслуживает `brk()`/`sbrk()` (сдвиг верхней границы сегмента данных), но крупные блоки выгоднее выделять через `mmap`: при `munmap()` память сразу возвращается ядру, без фрагментации кучи.
...
Обработка ошибок различается принципиально. `read()` возвращает `-1` и устанавливает `errno` — стандартный путь. С `mmap` ошибка чтения с диска (bad sector, NFS timeout) приходит как [[linux/programming/signals#Установка обработчика: sigaction()|сигнал `SIGBUS`]] — сигнал аппаратной ошибки шины, отличный от `SIGSEGV` (нарушение защиты памяти). Типичный перехват: обработчик через `sigaction()`, внутри — `siglongjmp()` — нелокальный переход в заранее сохранённую точку восстановления. Путь ошибки заметно сложнее, чем проверка `errno` после `read()`.
...
Без защиты стека переполнение молча перезаписывает соседнюю область памяти — ошибку крайне трудно отладить: данные оказываются повреждены задолго до того, как программа начинает вести себя неправильно. Guard page (страница-охранник) — страница с правами `PROT_NONE`, размещённая на границе стека потока. Стек растёт в сторону младших адресов, поэтому guard page ставят сразу под его нижней границей: переполнение, шагнув вниз за пределы стека, попадает прямо в `PROT_NONE`-страницу. Любое обращение к ней вызывает `SIGSEGV`, немедленно указывая на источник проблемы.
...
**MADV_SEQUENTIAL** сообщает ядру: доступ будет последовательным. Ядро увеличивает окно read-ahead и освобождает уже прочитанные страницы. Каждое касание новой страницы по-прежнему вызывает page fault, но расширенный read-ahead успевает подгрузить её в page cache заранее: fault разрешается из памяти, без ожидания диска. Для нашего аналитического сервиса, если обработка идёт от начала к концу файла, `MADV_SEQUENTIAL` снижает стоимость page fault'ов — дорогой major fault с чтением диска превращается в дешёвый minor fault из page cache.
...
[[linux/concurrency/synchronization#Мьютекс|Мьютекс]] с атрибутом `PTHREAD_PROCESS_SHARED` размещается в самой разделяемой памяти — оба процесса обращаются к одному и тому же `pthread_mutex_t`. Без него одновременные доступы не упорядочены: два процесса, выполняя «прочитал-изменил-записал» над одной ячейкой, затирают записи друг друга (потерянное обновление), а связанные поля структуры видны в несогласованном промежуточном состоянии. Выровненный `int` при этом сам по себе не читается полузаписанным — проблема не в разорванной ячейке, а в отсутствии порядка между операциями.
