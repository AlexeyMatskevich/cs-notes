# Rails

**Предпосылки:** Ruby, базовое понимание Rails (MVC, ActiveRecord, маршрутизация).

Материалы об инструментах и паттернах, специфичных для Rails-экосистемы.

## Содержание

### Redis в Rails

Как Rails-приложение использует Redis: подключения, практические сценарии, подводные камни.

- [Redis в Rails-приложении](redis/index.md) — клиенты, соединения, структуры данных на практике

### Фоновые задачи

- [Sidekiq](sidekiq/index.md) — архитектура, жизненный цикл задачи, гарантии, retry, deploy, масштабирование, тестирование
