...
<- [Concurrency и масштабирование](concurrency-and-scaling.md)

RSpec-тест вызывает `SendEmailJob.perform_async(42)`. [Задача](job-lifecycle.md) уходит в Redis. Тест проходит — но что он проверил? Факт постановки, не результат. Чтобы проверить результат, нужен запущенный [Sidekiq](sidekiq.md)-процесс, Redis, ожидание выполнения. Для unit-теста это слишком медленно и хрупко.

## Три режима тестирования

[Sidekiq](sidekiq.md) предоставляет тестовую обвязку, которая подменяет поведение `perform_async`. В новых версиях [Sidekiq](sidekiq.md) её можно включать новым API:
...
**Fake** (`Sidekiq::Testing.fake!`) — [задачи](job-lifecycle.md) не отправляются в Redis. Вместо этого `perform_async` складывает JSON-хеш в массив класса: `SendEmailJob.jobs`. Самый частый режим для unit-тестов.
...
**Inline** (`Sidekiq::Testing.inline!`) — `perform_async` немедленно выполняет [задачу](job-lifecycle.md) в текущем потоке. Без очереди, без задержки, без Redis. Полезно для integration-тестов, где важен конечный результат, а не механика очереди.
...
**Disable** (`Sidekiq::Testing.disable!`) — никакой подмены, [задачи](job-lifecycle.md) уходят в реальный Redis. Для end-to-end тестов с запущенным [Sidekiq](sidekiq.md)-процессом.
...
**ActiveJob** даёт абстракцию (переключение backend без изменения кода), GlobalID (передача ActiveRecord-объектов вместо ID), Rails-конвенции (`retry_on`, `discard_on`). Цена: ~30% overhead на сериализацию/десериализацию, несовместимость с Pro-фичами (Batches, rate limiting), ограниченный контроль над [retry](retry-and-errors.md)-логикой.

**Sidekiq::Job** даёт прямой доступ ко всем возможностям [Sidekiq](sidekiq.md), лучшую производительность, полный контроль над [retry](retry-and-errors.md) (формула, per-job настройки, death handlers). Цена: привязка к Sidekiq.

Для большинства Rails-приложений, которые не планируют менять backend, `Sidekiq::Job` — более практичный выбор: меньше overhead, больше контроля, совместимость с [Sidekiq](sidekiq.md) Pro/Enterprise.
...
В актуальных версиях [Sidekiq](sidekiq.md) строгая проверка совместимых с JSON аргументов включена по умолчанию: попытка передать Symbol, Time или ActiveRecord-объект в `perform_async` вызывает исключение сразу на стороне client, в момент постановки [задачи](job-lifecycle.md). Ошибка обнаруживается немедленно, а не через минуты в логах [Sidekiq](sidekiq.md)-процесса.
...
**Не тестируй retry-логику через [Sidekiq](sidekiq.md).** [Retry](retry-and-errors.md) — ответственность фреймворка, она покрыта тестами самого [Sidekiq](sidekiq.md). Тестируй бизнес-логику внутри `perform`: вызови `job.perform(args)` напрямую и проверь результат. Исключение — кастомная `sidekiq_retry_in` или `sidekiq_retries_exhausted`, где поведение специфично для приложения.
