...
**Redis connections.** Sidekiq использует примерно `concurrency + 5` соединений к Redis (по одному на Processor + внутренние потоки). Точное число зависит от версии и включённых фич (Capsules, Pro-механизмы добавляют свои соединения).
