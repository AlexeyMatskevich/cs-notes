...
Для массовых операций (обработать 10 000 записей) — тот же паттерн, но с `perform_bulk` (метод класса, который группирует [задачи](job-lifecycle.md) в чанки и отправляет за несколько round-trip к Redis вместо отдельного вызова на каждую задачу):
...
Задачи спроектированы: маленькие, атомарные, идемпотентные, каждая со своим [retry](retry-and-errors.md). Их стало десять тысяч в час. Одного процесса с пятью потоками не хватает — как масштабировать?
