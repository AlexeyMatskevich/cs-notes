...
Принцип: каждый job делает одну операцию, которая либо полностью выполняется, либо нет. Job должен быть [идемпотентным](../../system-design/reliability-patterns.md#idempotency-безопасность-повторных-запросов) — безопасным для повторного выполнения.
