...
Sidekiq предоставляет `sidekiq/testing` — библиотеку, которая подменяет поведение `perform_async`:
...
Начиная с Sidekiq 7, строгая проверка аргументов включена по умолчанию: попытка передать Symbol, Time или ActiveRecord-объект в `perform_async` вызовет исключение немедленно — на стороне client, в момент постановки задачи. Ошибка обнаруживается сразу, а не через минуты в логах Sidekiq-процесса.

Если нужно ослабить проверку (например, в переходный период миграции на Sidekiq 7):
