...
Sidekiq-процесс обрабатывает десять задач одновременно. [[linux/programming/memory-management#OOM killer: последняя линия защиты|OOM-killer]] убивает процесс. `BRPOP` извлёк все десять задач из Redis — они уже не в очереди. Процесс мёртв — они не в работе. Десять задач потеряны.
