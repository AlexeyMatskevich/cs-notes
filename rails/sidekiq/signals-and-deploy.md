...
**TSTP** — «тихий режим» (quiet). Процесс перестаёт забирать новые задачи из Redis, но дорабатывает текущие. Это первый шаг [graceful shutdown](../../linux/programming/signals.md#практические-паттерны): прекратить приём, завершить начатое.
