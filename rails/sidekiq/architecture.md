...
**Poller** — фоновый поток, который периодически проверяет sorted sets `schedule` и `retry`. Если score задачи ≤ текущему времени, Poller перемещает её в рабочую очередь (`LPUSH`). Это тот же паттерн [отложенной очереди](../../databases/redis/patterns/queues.md#отложенные-задачи-delayed-queue), но реализованный как отдельный поток внутри процесса.
