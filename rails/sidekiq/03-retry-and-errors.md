...
**Предпосылки:** [архитектура Sidekiq](00-architecture.md), [жизненный цикл задачи](01-job-lifecycle.md), [гарантии и идемпотентность](02-guarantees.md), [reliability patterns](../../system-design/06-reliability-patterns.md).

<- [Гарантии и идемпотентность](02-guarantees.md) | [Сигналы и deploy](04-signals-and-deploy.md) ->
...
Sorted set `retry` работает как [delayed queue](../../databases/redis/patterns/03-queues.md#отложенные-задачи-delayed-queue): score = timestamp следующей попытки. Poller периодически выполняет `ZRANGEBYSCORE retry -inf <now>` и перемещает «созревшие» задачи в рабочие очереди.
...
После 25 попыток (по умолчанию) задача переносится в sorted set `dead` — это [Dead Letter Queue](../../system-design/09-message-queues.md#dead-letter-queue). Dead set ограничен: максимум 10 000 задач, хранение 6 месяцев. Задачи из dead set можно вручную запустить заново через Web UI.

Не каждая ошибка заслуживает 25 попыток. [Transient failure](../../system-design/06-reliability-patterns.md#transient-vs-permanent-failure) (API down на час) — retry поможет. Permanent failure (невалидный email, несуществующий order_id) — 25 попыток бессмысленны, задача займёт место в retry set, потратит ресурсы и всё равно окажется в dead.
