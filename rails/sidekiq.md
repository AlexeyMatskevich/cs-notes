...
└─────────────────────────────────────────────────────────────┘
                    ...
                    │              Redis                │
                    │  ┌─────────────────────────────┐  │
                    │  │ queue:default, queue:critical│  │  (List) — рабочие очереди
                    │  │ queues                       │  │  (Set) — индекс имён очередей
                    │  │ retry                        │  │  (Sorted Set) — retry backlog
                    │  │ schedule                     │  │  (Sorted Set) — отложенные jobs
                    │  │ dead                         │  │  (Sorted Set) — dead letter queue
                    │  └─────────────────────────────┘  │
                    └───────────────────────────────────┘
```
...
Проблема OSS-подхода: BRPOP **удаляет** job из Redis до начала выполнения. Если процесс упадёт во время работы — job потерян навсегда. Pro решает это через атомарное перемещение вместо удаления:

```ruby
# OSS: BRPOP queue:default → job удалён, если crash — потерян

# Pro (super_fetch):
# 1. LMOVE queue:default queue:default:#{hostname}:#{pid}
#    Job атомарно перемещается в приватную очередь процесса,
#    но остаётся в Redis

# 2. Job выполняется

# 3. LREM queue:default:#{hostname}:#{pid} 1 <job_json>
#    Удаление только после успешного завершения

# Если процесс упал между шагами 1 и 3:
# - Job остаётся в приватной очереди
# - При старте любого Sidekiq Pro процесса: orphan recovery
#   (сканирование приватных очередей мёртвых процессов)
```

Приватная очередь — обычный Redis List с именем `queue:{name}:{hostname}:{pid}`. Heartbeat обновляет статус процесса каждые ~5 секунд; если процесс не отвечает — его приватная очередь считается orphaned и jobs возвращаются в основную очередь.
