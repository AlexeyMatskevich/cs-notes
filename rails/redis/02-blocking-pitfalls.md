...
Redis обрабатывает все команды в одном потоке — [event loop на базе мультиплексирования I/O](../../linux/programming/04-io-multiplexing.md). Пока одна команда выполняется, остальные клиенты ждут. Для типичных команд (`GET`, `SET`, `INCR`) это незаметно — они занимают микросекунды. Проблемы начинаются, когда команда за один вызов обходит большой объём данных: все Puma-процессы, Sidekiq-воркеры и другие клиенты Redis встают в очередь.
