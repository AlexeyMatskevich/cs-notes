...
Redis обрабатывает все команды в одном потоке — [event loop на базе мультиплексирования I/O](../../linux/programming/04-io-multiplexing.md). Пока одна команда выполняется, все Puma-процессы, Sidekiq-воркеры и другие клиенты ждут. Для типичных команд (`GET`, `SET`, `INCR`) это незаметно — они занимают микросекунды. Проблемы начинаются, когда команда за один вызов обходит большой объём данных.
...
`KEYS` обходит каждый ключ в Redis и проверяет его на соответствие маске. При миллионе ключей это занимает десятки миллисекунд. В это время ни один другой клиент не может выполнить даже простой `GET` — rate limiting, кеш, Sidekiq, ActionCable ждут.
