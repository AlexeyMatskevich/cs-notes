...
Один инстанс Redis обслуживает и кеш, и сессии, и Sidekiq — но у этих задач разные требования к сохранности данных и поведению при нехватке памяти. Если Redis для кеша вытеснит ключ Sidekiq-очереди, задача потеряется. Разделение на инстансы — это ещё один [bulkhead](../../system-design/reliability-patterns.md#bulkhead-изоляция-ресурсов): кеш, сессии и очереди получают разные лимиты памяти, разные политики eviction и не делят одну общую область отказа.
