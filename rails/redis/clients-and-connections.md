...
**Предпосылки:** [Redis: архитектура](../../databases/redis/redis.md), [TCP](../../networking/transport/tcp.md) (соединение), [[ruby/internal/concurrency#puma-reactor--thread-pool|Puma]] (воркеры + потоки).
...
Один инстанс Redis обслуживает и кеш, и сессии, и [Sidekiq](../sidekiq/sidekiq.md) — но у этих задач разные требования к сохранности данных и поведению при нехватке памяти. Если Redis для кеша вытеснит ключ Sidekiq-очереди, задача потеряется. Разделение на инстансы — это ещё один [[system-design/reliability-patterns#bulkhead-изоляция-ресурсов|bulkhead]]: кеш, сессии и очереди получают разные лимиты памяти, разные политики eviction и не делят одну общую область отказа.
