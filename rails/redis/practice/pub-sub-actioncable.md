...
Это тоже задача выбора между [[system-design/message-queues#point-to-point-и-pubsub|point-to-point и Pub/Sub]]: сообщение должно попасть во все Puma-процессы, а уже каждый процесс локально решает, есть ли у него нужный WebSocket.
...
Итого на каждый Puma-процесс: 5 потоков Puma (HTTP + WebSocket I/O) + 1 поток Redis-подписки. Этот дополнительный поток держит одно выделенное TCP-соединение к Redis, которое не входит в [[rails/redis/clients-and-connections#connection_pool|connection_pool]] приложения.
