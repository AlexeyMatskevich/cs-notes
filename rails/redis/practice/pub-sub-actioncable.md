...
Итого на каждый Puma-процесс: 5 потоков Puma (HTTP + WebSocket I/O) + 1 поток Redis subscriber. Этот дополнительный поток держит одно выделенное TCP-соединение к Redis, которое не входит в [connection_pool](../00-clients-and-connections.md#connection_pool) приложения.
