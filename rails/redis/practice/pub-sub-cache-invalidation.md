...
Это типичная задача [когерентности локального vs внешнего кэша](../../../system-design/07-caching.md#когерентность-локальный-vs-внешний-кэш): данные уже обновились в основном хранилище, теперь нужно быстро донести факт изменения до всех локальных копий.

Альтернативы не подходят: структура с персистентным хранением сообщений избыточна, когда при перезапуске процесс загрузит свежие данные. Очередь с `BRPOP` доставляет сообщение только одному получателю, а нужно всем. [Pub/Sub](../../../databases/redis/data-structures/08-pub-sub.md) рассылает сообщение каждому подписчику одновременно:
...
По модели обмена это именно [Pub/Sub](../../../system-design/09-message-queues.md#point-to-point-и-pubsub), а не очередь: одно событие должно дойти до всех живых процессов сразу, без `acknowledgment` и без хранения истории.
