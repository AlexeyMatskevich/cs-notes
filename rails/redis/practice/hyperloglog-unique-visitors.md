...
class UniqueVisitorTracker
  ...
  def site_wide_uniques(date)
    ...
    page_keys = @redis.with { |r| r.keys("uv:*:#{date.iso8601}") }
...
3 миллиона HLL-счётчиков × 12 КБ = 36 ГБ. Те же данные в SET — сотни ГБ. Стандартная ошибка HLL — 0.81%. Для аналитических дашбордов, где «≈1 024 000» и «1 024 000» неразличимы, это приемлемо. Для точного ответа «был ли конкретный пользователь на странице?» HLL не подходит — для этого нужен SET или Bitmap.

Подробнее: [HyperLogLog](../../../databases/redis/data-structures/06-hyperloglog.md).
