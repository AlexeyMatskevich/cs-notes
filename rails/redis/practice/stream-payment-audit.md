...
В терминах [[system-design/message-queues#две-модели-очередь-сообщений-и-лог|очередей сообщений]] Stream здесь ближе к логу, чем к простой очереди: события сохраняются, читаются по позиции и могут быть перечитаны после сбоя.
