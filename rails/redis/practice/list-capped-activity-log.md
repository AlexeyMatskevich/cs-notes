...
[`LRANGE(key, 0, limit - 1)`](../../../databases/redis/data-structures/list.md) возвращает элементы с начала списка (самые свежие) без удаления. `LPUSH` и `LTRIM` выполняются за O(1) (LTRIM удаляет элементы с конца, и при обрезке на 1 элемент это константа). `LRANGE` — O(limit).
